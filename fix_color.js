const fs = require('fs');
const file = 'C:/Users/sangmesh/Desktop/global-drill-system-nextjs - Copy/app/drills/memory/short-term-memory/color-sequence/ColorSequenceClient.js';
let content = fs.readFileSync(file, 'utf8');
let lines = content.split('\n');

const startIdx = lines.findIndex(l => l.includes('{/* Instructions */}'));
const endFooterIdx = lines.findIndex(l => l.includes('</footer>'));
const uiHelperIdx = lines.findIndex(l => l.includes('// UI HELPER COMPONENTS'));

if (startIdx !== -1 && endFooterIdx !== -1 && uiHelperIdx !== -1) {
            ...
            <div className="grid md:grid-cols-2 gap-8">
              ...
              <div>
                <p className="text-sm text-gray-400 leading-relaxed mb-6">
                  Color Sequence trains your visual-spatial short-term memory by presenting a sequence of colors that you must remember and reproduce. The sequence grows longer as you succeed, testing your working memory capacity limits.
                </p>
                ...
                <div className="mb-6 p-5 rounded-xl border border-gray-800 bg-black/40">
                  ...
                  <ul className="space-y-3 text-sm text-gray-400">
                    <li><strong className="text-green-400">Perfect Sequence:</strong> +5s Time | Level Up</li>
                    <li><strong className="text-cyan-400">Correct Tap:</strong> +3 Points | Per Match</li>
                    <li><strong className="text-red-400">Wrong Tap:</strong> -2s Time | Game Over on 0</li>
                    <li><strong className="text-orange-400">Mastery:</strong> Sequence of 15 | Peak Capacity</li>
                  </ul>
                </div>
              </div>
              ...
              <div>
                <div className="p-5 rounded-xl border border-gray-800 bg-black/40 mb-6">
                  ...
                  <ol className="space-y-3 list-decimal pl-5 text-sm text-gray-400">
                    <li><strong className="text-white">Observe:</strong> Watch the sequence of colors carefully as they light up.</li>
                    <li><strong className="text-white">Chunking:</strong> Try to group colors into chunks (e.g., Red-Blue, Green-Yellow) to remember them easier.</li>
                    <li><strong className="text-white">Reproduce:</strong> Tap the colors in the exact same order they appeared to level up.</li>
                  </ol>
                </div>
                ...
                <div className="p-5 rounded-xl border border-gray-800 bg-black/40">
                  ...
                  <div className="space-y-4">
                    <div>
                      ...
                      <p className="text-xs text-gray-400 mt-1">Anyone looking to improve visual-spatial working memory, especially helpful for tasks requiring sequential memory retention.</p>
                    </div>
                    <div>
                      <h4 className="text-sm font-bold text-gray-200">What skills are improved?</h4>
                      <p className="text-xs text-gray-400 mt-1">Short-term memory span, sequential processing, visual chunking, and sustained attention.</p>
                    </div>
                  </div>
                </div>
              </div>
    ...
    lines.splice(startIdx, (endFooterIdx - startIdx) + 2, newSection);
    
    // Also remove RuleItem from UI helpers
    const ruleItemStart = lines.findIndex(l => l.includes('function RuleItem'));
    if (ruleItemStart !== -1) {
        let ruleItemEnd = ruleItemStart;
        while (!lines[ruleItemEnd].startsWith('}')) {
            ruleItemEnd++;
        }
        lines.splice(ruleItemStart, ruleItemEnd - ruleItemStart + 1);
    }
    
    fs.writeFileSync(file, lines.join('\n'));
    console.log('Fixed ColorSequenceClient.js successfully.');
} else {
