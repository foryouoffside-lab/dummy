...
import { 
  ...
  Sun, Moon, Eye, Timer, Trophy, Crosshair, Activity, Award, Info, Check, X
} from 'lucide-react';
...
export default function FlickShotPage() {
  ...
  const targetRef = useRef({ x: 0, y: 0, r: 32, active: true, timer: 0 });
  ...
  useEffect(() => {
    const savedBestScore = localStorage.getItem('flickDrillBestScore');
    if (savedBestScore) {
  ...
  const updateBestScore = (finalScore) => {
    const currentBestScore = parseInt(localStorage.getItem('flickDrillBestScore') || '0', 10);
    if (finalScore > currentBestScore) {
      localStorage.setItem('flickDrillBestScore', finalScore.toString());
    ...
    timerIntervalRef.current = setInterval(() => {
      if (gameStateRef.current === 'playing' && isActiveRef.current) {
        ...
        if (timeLeftRef.current <= 0) {
          ...
          const total = flickCount + missedCount;
          const finalAccuracy = total === 0 ? 100 : Math.round((flickCount / total) * 100);
          setAccuracy(finalAccuracy);
          updateBestScore(scoreRef.current);
        }
      }
    }, 1000);
  }, [flickCount, missedCount]);
  ...
  const spawnTarget = () => {
    ...
    const pad = 0.12;
    targetRef.current.x = cvs.width * (pad + Math.random() * (1 - pad * 2));
    targetRef.current.y = cvs.height * (pad + Math.random() * (1 - pad * 2));
  ...
  const handlePenalty = (reason) => {
    ...
    currentLifespanRef.current = Math.min(MAX_LIFESPAN, currentLifespanRef.current + 0.08);
  ...
  useEffect(() => {
    const handleMouseDown = () => {
      ...
      if (dist < target.r + 10) {
        ...
        const timeSaved = currentLifespanRef.current - target.timer;
        const speedIncrease = timeSaved * 0.15;
        currentLifespanRef.current = Math.max(MIN_LIFESPAN, currentLifespanRef.current - speedIncrease);
        setLifespan(Math.round(currentLifespanRef.current * 1000));
        ...
        if (currentLifespanRef.current === MIN_LIFESPAN) {
          playSound('speed_up');
          showFeedback(`âš¡ Elite Window!`, 'success');
        }
        
        spawnTarget();
        
        const total = flickCount + 1 + missedCount;
        setAccuracy(Math.round(((flickCount + 1) / total) * 100));
      } else {
        handlePenalty('Miss');
      }
    };
  ...
  }, [gameState, bestReaction, flickCount, missedCount]);
  ...
  useEffect(() => {
    ...
    function drawTarget() {
      ...
      const color = remainingPercent > 0.5 ? '#00ff88' : remainingPercent > 0.25 ? '#ffaa00' : '#ff3366';
      ...
      ctx.arc(target.x, target.y, target.r + 10, -Math.PI / 2, endAngle);
      ctx.strokeStyle = color;
      ctx.lineWidth = 4;
      ctx.stroke();
    }
  ...
  const startGame = () => {
    ...
    scoreRef.current = 0;
    comboRef.current = 0;
    currentLifespanRef.current = 0.8;
    setLifespan(800);
    ...
    <div className={`min-h-screen select-none ${isDarkMode ? 'bg-gray-900' : 'bg-gray-50'}`}>
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        <div className="mb-6">
          ...
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-3">
              ...
              <div>
                <h1 className={`text-3xl font-bold ${isDarkMode ? 'text-white' : 'text-gray-900'}`}>Flick Shot Drill</h1>
                <p className={`${isDarkMode ? 'text-gray-400' : 'text-gray-500'}`}>Elite flick shots â€¢ Penalty = 100 â€¢ Window shrinks</p>
              </div>
            </div>
        ...
        <div className="grid grid-cols-7 gap-3 mb-4 h-[88px]">
          ...
          <StatCard icon={<Check className="text-green-500" />} value={flickCount} label="Hits" isDark={isDarkMode} />
          ...
          {gameState === 'start' && (
            <div className={`absolute inset-0 flex items-center justify-center backdrop-blur-sm rounded-xl z-40 ${isBoxDarkMode ? 'bg-gray-900/95' : 'bg-white/95'}`}>
              <div className={`rounded-2xl p-8 text-center max-w-md mx-4 shadow-xl border ${isBoxDarkMode ? 'bg-gray-800 border-gray-700' : 'bg-white border-gray-200'}`}>
                ...
                <h3 className={`text-2xl font-bold mb-2 ${isBoxDarkMode ? 'text-white' : 'text-gray-900'}`}>Flick Shot Drill</h3>
                <p className={`mb-6 ${isBoxDarkMode ? 'text-gray-300' : 'text-gray-600'}`}>60-second challenge â€¢ Penalty = 100 â€¢ Window shrinks</p>
                <button 
                  onClick={startGame} 
            ...
            <div className={`absolute inset-0 flex items-center justify-center backdrop-blur-sm rounded-xl z-40 ${isBoxDarkMode ? 'bg-gray-900/95' : 'bg-white/95'}`}>
              <div className={`rounded-2xl p-8 shadow-xl border w-[480px] ${isBoxDarkMode ? 'bg-gray-800 border-gray-700' : 'bg-white border-gray-200'}`}>
                ...
                <div className="grid grid-cols-2 gap-4 mb-8">
                  ...
                  <ResultCard label="Flicks" value={flickCount} icon={<Check className="w-4 h-4" />} color="text-green-500" />
                  ...
                  <ResultCard label="Final Window" value={lifespan} unit="ms" icon={<Target className="w-4 h-4" />} color="text-blue-500" />
                  <ResultCard label="Misses" value={missedCount} icon={<X className="w-4 h-4" />} color="text-red-500" />
                </div>
            ...
            <div className={`rounded-xl border overflow-hidden ${isDarkMode ? 'bg-gray-800 border-gray-700' : 'bg-white border-gray-200'}`}>
              ...
              <div className="p-4">
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  <div className="space-y-2">
                    <div className="flex items-start gap-2">
                      ...
                      <p className={`text-sm ${isDarkMode ? 'text-gray-300' : 'text-gray-600'}`}>
                        <span className="font-semibold text-red-500">White targets appear randomly</span> â€¢ Click quickly
                      </p>
                    </div>
                    <div className="flex items-start gap-2">
                      <div className="w-5 h-5 rounded-full bg-green-500 flex items-center justify-center text-white text-xs font-bold mt-0.5">2</div>
                      <p className={`text-sm ${isDarkMode ? 'text-gray-300' : 'text-gray-600'}`}>
                        <span className="font-semibold text-green-500">Faster flicks = Higher scores</span> (&lt;150ms: +100, &lt;250ms: +80)
                      </p>
                    </div>
                    <div className="flex items-start gap-2">
                  ...
                  <div className="space-y-2">
                    <div className="flex items-start gap-2">
                      ...
                      <p className={`text-sm ${isDarkMode ? 'text-gray-300' : 'text-gray-600'}`}>
                        <span className="font-semibold text-blue-500">Miss/Timeout = -100 points</span> â€¢ Penalty equals max points
                      </p>
                    </div>
                    <div className="flex items-start gap-2">
                      ...
                      <p className={`text-sm ${isDarkMode ? 'text-gray-300' : 'text-gray-600'}`}>
                        <span className="font-semibold text-orange-500">Window shrinks with fast hits</span> â€¢ Down to 150ms elite level
                      </p>
                    </div>
                    <div className="flex items-start gap-2">
                ...
                <div className={`mt-3 pt-3 border-t text-xs ${isDarkMode ? 'border-gray-700 text-gray-400' : 'border-gray-200 text-gray-500'} flex items-center justify-between`}>
                  <span>ðŸŽ¯ Timer ring: Green â†’ Yellow â†’ Red</span>
                  <span>âš¡ Click the white target to score</span>
                </div>
              </div>
            </div>
          </div>
