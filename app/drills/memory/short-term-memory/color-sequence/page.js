// app/drills/memory/short-term-memory/color-sequence/page.js
"use client";
...
import { 
  ...
  Eye, Brain, BarChart3, Timer, Trophy, Info, Heart, Repeat
} from "lucide-react";
...
export default function ColorSequenceDrill() {
  ...
  // Effect to start round when phase is "ready" and game is active
  useEffect(() => {
  ...
  // Get points/penalty for current level
  ...
  const handleColorClick = (color) => {
    ...
    if (isWrong) {
      ...
      // Generate new sequence for same level
    ...
    if (newUserSeq.length === sequence.length) {
      const correct = newUserSeq.every((c, i) => c === sequence[i]);
      ...
      if (correct) {
        ...
        // Set phase to "result" first, then "ready" to trigger next round
        setPhase("result");
        setTimeout(() => {
          setPhase("ready");
        }, 1200);
  ...
  const getAccuracy = () => {
    const totalAttempts = attemptsAtLevel + (levelRef.current - 1);
    const successfulLevels = levelRef.current - 1;
        ...
        <div className="mb-6">
          ...
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-3">
              ...
              <div>
                <h1 className={`text-3xl font-bold ${isDarkMode ? 'text-white' : 'text-gray-900'}`}>Color Sequence</h1>
                <p className={`${isDarkMode ? 'text-gray-400' : 'text-gray-500'}`}>Memorize and repeat • Level = Points</p>
              </div>
            </div>
            ...
            {gameState === 'start' && (
              <div className={`absolute inset-0 flex items-center justify-center backdrop-blur-sm rounded-xl z-40 ${isBoxDarkMode ? 'bg-gray-900/95' : 'bg-white/95'}`}>
                <div className={`rounded-2xl p-8 text-center max-w-md mx-4 shadow-xl border ${isBoxDarkMode ? 'bg-gray-800 border-gray-700' : 'bg-white border-gray-200'}`}>
                  ...
                  <h3 className={`text-2xl font-bold mb-2 ${isBoxDarkMode ? 'text-white' : 'text-gray-900'}`}>Color Sequence</h3>
                  <p className={`mb-6 ${isBoxDarkMode ? 'text-gray-300' : 'text-gray-600'}`}>Memorize colors and repeat the sequence!</p>
                  <button 
                    onClick={startGame} 
              ...
              <div className="w-full max-w-md mx-auto text-center">
                {(phase === "showing" || phase === "input" || phase === "result") && (
                  <div className="space-y-6">
                    <div className={`w-32 h-32 mx-auto rounded-full transition-all duration-300 shadow-lg ${
                      showingColor ? colorMap[showingColor] : (isBoxDarkMode ? 'bg-gray-800' : 'bg-gray-200')
                    }`} />
                        ...
                        <div
                          ...
                          className={`w-8 h-8 rounded-full transition-all ${
                            i < userSequence.length 
                              ...
                              : isBoxDarkMode ? "bg-gray-700" : "bg-gray-300"
                          }`}
                        />
                      ))}
                    </div>
                        ...
                        <div className="grid grid-cols-3 gap-3 mt-6">
                          {colors.map(color => (
                            <button
                              ...
                              className={`${colorMap[color]} h-16 rounded-xl transition transform hover:scale-105 active:scale-95 shadow-lg`}
                            />
                          ))}
                        </div>
              ...
              <div className={`absolute inset-0 flex items-center justify-center backdrop-blur-sm rounded-xl z-40 ${isBoxDarkMode ? 'bg-gray-900/95' : 'bg-white/95'}`}>
                <div className={`rounded-2xl p-8 shadow-xl border w-[480px] ${isBoxDarkMode ? 'bg-gray-800 border-gray-700' : 'bg-white border-gray-200'}`}>
                  ...
                  <div className="grid grid-cols-2 gap-4 mb-8">
                    ...
                    <ResultCard label="Accuracy" value={getAccuracy()} unit="%" icon={<BarChart3 className="w-4 h-4" />} color="text-green-500" />
                    <ResultCard label="Points Earned" value={score} icon={<Target className="w-4 h-4" />} color="text-blue-500" />
                  </div>
                  ...
                  <div className="space-y-2">
                    <div className="flex items-start gap-2">
                      ...
                      <p className={`text-sm ${isDarkMode ? 'text-gray-300' : 'text-gray-600'}`}>
                        <span className="font-semibold text-purple-500">Level 1: 3 colors = +3 points</span>
                      </p>
                    </div>
                    <div className="flex items-start gap-2">
                      ...
                      <p className={`text-sm ${isDarkMode ? 'text-gray-300' : 'text-gray-600'}`}>
                        <span className="font-semibold text-blue-500">Level 2: 4 colors = +4 points</span>
                      </p>
                    </div>
                    <div className="flex items-start gap-2">
                      ...
                      <p className={`text-sm ${isDarkMode ? 'text-gray-300' : 'text-gray-600'}`}>
                        <span className="font-semibold text-green-500">Higher levels = More points</span>
                      </p>
                    </div>
                  </div>
                  <div className="space-y-2">
                    <div className="flex items-start gap-2">
                      ...
                      <p className={`text-sm ${isDarkMode ? 'text-gray-300' : 'text-gray-600'}`}>
                        <span className="font-semibold text-red-500">Wrong answer: -{level} points penalty</span> (matches level score)
                      </p>
                    </div>
                    <div className="flex items-start gap-2">
                      ...
                      <p className={`text-sm ${isDarkMode ? 'text-gray-300' : 'text-gray-600'}`}>
                        <span className="font-semibold text-orange-500">Complete level = Move to next level</span>
                      </p>
                    </div>
                    <div className="flex items-start gap-2">
                      <div className="w-5 h-5 rounded-full bg-yellow-500 flex items-center justify-center text-white text-xs font-bold mt-0.5">6</div>
                      <p className={`text-sm ${isDarkMode ? 'text-gray-300' : 'text-gray-600'}`}>
                        <span className="font-semibold text-yellow-500">60-second timer • Best Score saves locally</span>
                      </p>
                    </div>
                  </div>
                </div>
                <div className={`mt-3 pt-3 border-t text-xs ${isDarkMode ? 'border-gray-700 text-gray-400' : 'border-gray-200 text-gray-500'} flex items-center justify-between`}>
                  <span>🎨 Sequence length = level + 2 • Gets harder as you progress</span>
                  <span>⚡ Penalty equals level score • Streak resets on mistake!</span>
                </div>
              </div>
            </div>
          </div>
