...
export default function ReactiveStrafePage() {
  ...
  const [trackingZone, setTrackingZone] = useState('none');
  ...
  const fractionalScoreRef = useRef(0);
  ...
  useEffect(() => {
    ...
    trackingIntervalRef.current = setInterval(() => {
      ...
      if (distance <= pupilRadius) {
        ...
        newAccuracy = Math.round(100 - ((distance - pupilRadius) / (radius - pupilRadius)) * 40);
        zone = 'good';
      } else if (distance <= radius + 15) {
        newAccuracy = Math.round(60 - ((distance - radius) / 15) * 30);
        zone = 'edge';
      } else {
        newAccuracy = Math.max(0, Math.round(30 - ((distance - radius - 15) / 30) * 30));
        zone = 'off';
        comboRef.current = 0;
        setCombo(0);
      }
      ...
      setTrackingZone(zone);
      
      const now = performance.now();
      const timeSinceLastScore = now - lastScoreTimeRef.current;
      
      // Award points based on zone (every 300ms)
      if (timeSinceLastScore >= 300 && zone !== 'off') {
        if (zone === 'perfect') {
          fractionalScoreRef.current += 1;
          comboRef.current++;
          ...
          fractionalScoreRef.current += 0.5;
          comboRef.current++;
          setCombo(comboRef.current);
          if (comboRef.current > bestCombo) setBestCombo(comboRef.current);
          ...
          fractionalScoreRef.current += 0.25;
        }
        
        // Update score when we have at least 0.5 accumulated
        if (fractionalScoreRef.current >= 0.5) {
          const wholePoints = Math.floor(fractionalScoreRef.current);
          if (wholePoints > 0) {
            scoreRef.current += wholePoints;
            setScore(scoreRef.current);
            fractionalScoreRef.current -= wholePoints;
            
            if (zone === 'perfect') {
              playSound('perfect');
              showFeedback(`✓ +${wholePoints} | Perfect!`, 'success');
            } else if (zone === 'good') {
              playSound('good');
            }
          }
      ...
      // Adjust speed based on accuracy
  ...
  useEffect(() => {
    ...
    const updateCanvasSize = () => {
      ...
      if (targetRef.current.x === 0 && targetRef.current.y === 0) {
        ...
        targetRef.current.y = fixedYRef.current;
      }
    };
    ...
    function drawBackground() {
      ...
      ctx.beginPath();
      ctx.moveTo(0, fixedYRef.current);
      ctx.lineTo(cvs.width, fixedYRef.current);
    ...
    function drawTarget() {
      ...
      const distToMouse = Math.hypot(mouse.x - target.x, mouse.y - target.y);
      if (distToMouse <= radius && trackingZone !== 'off') {
        ctx.beginPath();
        ctx.arc(target.x, target.y, radius + 3, 0, Math.PI * 2);
        ctx.strokeStyle = distToMouse <= 5 ? '#00ff88' : '#ffaa00';
        ctx.lineWidth = 2;
        ctx.stroke();
      }
      
      if (mouse.x > 0 && mouse.x < cvs.width && mouse.y > 0 && mouse.y < cvs.height) {
        ctx.strokeStyle = trackingZone === 'perfect' ? '#00ff88' : 
                         trackingZone === 'good' ? '#ffaa00' : 
                         trackingZone === 'edge' ? '#ff6600' : '#888888';
        ctx.lineWidth = 2;
        ctx.beginPath();
        ...
        ctx.stroke();
        ctx.fillStyle = trackingZone === 'perfect' ? '#00ff88' : 
                       trackingZone === 'good' ? '#ffaa00' : 
                       trackingZone === 'edge' ? '#ff6600' : '#888888';
  ...
  }, [gameState, isBoxDarkMode, speed, trackingZone]);
  ...
  const startGame = () => {
    ...
    setSpeed(18);
    setFeedback('');
    setTrackingZone('none');
    
    isActiveRef.current = true;
    scoreRef.current = 0;
    comboRef.current = 0;
    fractionalScoreRef.current = 0;
    lastScoreTimeRef.current = performance.now();
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
          ...
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <p className={`${isDarkMode ? 'text-gray-400' : 'text-gray-500'}`}>Pupil: 1pt • White: 0.5pt • Edge: 0.25pt</p>
              </div>
            </div>
        ...
        <div className="h-8 mb-2 flex justify-center items-center">
          <div className={`px-4 py-1 rounded-lg text-white font-medium text-sm transition-opacity duration-150 ${feedback ? 'opacity-100' : 'opacity-0'} ${
            feedbackType === 'success' ? 'bg-green-500' : 'bg-gray-500'
          }`}>
          ...
          {gameState === 'start' && (
            <div className={`absolute inset-0 flex items-center justify-center backdrop-blur-sm rounded-xl z-40 ${isBoxDarkMode ? 'bg-gray-900/95' : 'bg-white/95'}`}>
              <div className={`rounded-2xl p-8 text-center max-w-md mx-4 shadow-xl border ${isBoxDarkMode ? 'bg-gray-800 border-gray-700' : 'bg-white border-gray-200'}`}>
                ...
                <p className={`mb-6 ${isBoxDarkMode ? 'text-gray-300' : 'text-gray-600'}`}>Pupil: 1pt • White: 0.5pt • Edge: 0.25pt</p>
                <button 
                  onClick={startGame} 
                  ...
                  <div className="space-y-2">
                    <div className="flex items-start gap-2">
                      ...
                      <p className={`text-sm ${isDarkMode ? 'text-gray-300' : 'text-gray-600'}`}>
                        <span className="font-semibold text-green-500">Black pupil = 1 point</span> • Perfect tracking zone
                      </p>
                    </div>
                    <div className="flex items-start gap-2">
                      ...
                      <p className={`text-sm ${isDarkMode ? 'text-gray-300' : 'text-gray-600'}`}>
                        <span className="font-semibold text-yellow-500">White area = 0.5 points</span> • Good tracking zone
                      </p>
                    </div>
                    <div className="flex items-start gap-2">
                      ...
                      <p className={`text-sm ${isDarkMode ? 'text-gray-300' : 'text-gray-600'}`}>
                        <span className="font-semibold text-orange-500">Edge area = 0.25 points</span> • Near the ball
                      </p>
                    </div>
                  </div>
                  <div className="space-y-2">
                    <div className="flex items-start gap-2">
                      ...
                      <p className={`text-sm ${isDarkMode ? 'text-gray-300' : 'text-gray-600'}`}>
                        <span className="font-semibold text-purple-500">Build combo streaks</span> • 3 and 5 combo bonus
                      </p>
                    </div>
                    <div className="flex items-start gap-2">
                      ...
                      <p className={`text-sm ${isDarkMode ? 'text-gray-300' : 'text-gray-600'}`}>
                        <span className="font-semibold text-cyan-500">Speed adapts to accuracy</span> • Faster when tracking well
                      </p>
                    </div>
                    <div className="flex items-start gap-2">
                      ...
                      <p className={`text-sm ${isDarkMode ? 'text-gray-300' : 'text-gray-600'}`}>
                        <span className="font-semibold text-blue-500">Ball moves horizontally only</span> • Follow the dashed line
                      </p>
                    </div>
                  </div>
                </div>
                <div className={`mt-3 pt-3 border-t text-xs ${isDarkMode ? 'border-gray-700 text-gray-400' : 'border-gray-200 text-gray-500'} flex items-center justify-between`}>
                  <span>🎲 No penalties • Pure score building</span>
                  <span>⚡ 60 second countdown</span>
                </div>
              </div>
            </div>
          </div>
