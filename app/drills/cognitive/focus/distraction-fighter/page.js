...
import { 
  ...
  BarChart3, Info, ShieldCheck, CheckCircle, XCircle, AlertCircle
} from 'lucide-react';
...
export default function DistractionFighterPage() {
  ...
  const handleWrongClick = () => {
    ...
    // If no lives left, apply penalty of 2 points
    if (livesRef.current === 0) {
  ...
  const generateTrial = useCallback(() => {
    ...
    timeoutRef.current = setTimeout(() => {
      if (gameStateRef.current === 'playing') {
        ...
        // No penalty for timeout - just move to next trial
        showFeedback(`⏱ Timeout - Next!`, 'info');
  ...
  const handleAnswer = (selectedName) => {
    ...
    if (selectedName === currentTarget.trueColor) {
      ...
      // Wrong answer - penalty only on wrong click
      handleWrongClick();
    }
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
          ...
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <p className={`${isDarkMode ? 'text-gray-400' : 'text-gray-500'}`}>Stroop Test • +2/-2 • 3 lives • No timeout penalty</p>
              </div>
            </div>
        ...
        <div className="h-8 mb-2 flex justify-center items-center">
          <div className={`px-4 py-1 rounded-lg text-white font-medium text-sm transition-opacity duration-150 ${feedback ? 'opacity-100' : 'opacity-0'} ${
            feedbackType === 'success' ? 'bg-green-500' : feedbackType === 'info' ? 'bg-blue-500' : 'bg-red-500'
          }`}>
            ...
            {gameState === 'start' && (
              <div className={`absolute inset-0 flex items-center justify-center backdrop-blur-sm rounded-xl z-40 ${isBoxDarkMode ? 'bg-gray-900/95' : 'bg-white/95'}`}>
                <div className={`rounded-2xl p-8 text-center max-w-md mx-4 shadow-xl border ${isBoxDarkMode ? 'bg-gray-800 border-gray-700' : 'bg-white border-gray-200'}`}>
                  ...
                  <p className={`mb-6 ${isBoxDarkMode ? 'text-gray-300' : 'text-gray-600'}`}>60-second challenge • +2 per correct • -2 penalty • 3 lives</p>
                  <button 
                    onClick={startDrill}
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  <div className="space-y-2">
                    ...
                    <div className="flex items-start gap-2">
                      ...
                      <p className={`text-sm ${isDarkMode ? 'text-gray-300' : 'text-gray-600'}`}>Correct: <span className="font-semibold text-green-500">+2 points</span> • Fixed reward</p>
                    </div>
                    <div className="flex items-start gap-2">
                  ...
                  <div className="space-y-2">
                    <div className="flex items-start gap-2">
                      ...
                      <p className={`text-sm ${isDarkMode ? 'text-gray-300' : 'text-gray-600'}`}>No lives left: <span className="font-semibold text-orange-500">-2 point penalty</span> per wrong click</p>
                    </div>
                    <div className="flex items-start gap-2">
                    ...
                    <div className="flex items-start gap-2">
                      ...
                      <p className={`text-sm ${isDarkMode ? 'text-gray-300' : 'text-gray-600'}`}>Timeout after <span className="font-semibold text-purple-500">1.5 seconds</span> • No penalty</p>
                    </div>
                  </div>
                </div>
                <div className={`mt-3 pt-3 border-t text-xs ${isDarkMode ? 'border-gray-700 text-gray-400' : 'border-gray-200 text-gray-500'} flex items-center justify-between`}>
                  <span>🎯 60-second challenge • Timeout just moves to next</span>
