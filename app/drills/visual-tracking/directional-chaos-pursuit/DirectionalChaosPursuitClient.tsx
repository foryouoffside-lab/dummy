...
export default function DirectionalChaosPursuitClient() {
  ...
  useEffect(() => {
    ...
    try {
      const stored = localStorage.getItem('visualTrackingDirectionalChaosPursuitClientTrialsCount');
        ...
        setTimeLeft(p => {
          if (p <= 1) {
            ...
            setTotalTrials(prev => {
              const next = prev + 1;
              try {
                localStorage.setItem('visualTrackingDirectionalChaosPursuitClientTrialsCount', next.toString());
              } catch (e) {}
              return next;
            });
  ...
  useEffect(() => {
    ...
    const updateSize = () => {
      ...
      trackingState.current.py = h * 0.5;
      trackingState.current.vx = 5;
      trackingState.current.vy = 5;
    };
    ...
    const draw = (ts: number) => {
      ...
      // Chaotic updates
        trackingState.current.vx += (Math.random() - 0.5) * 0.8 * (scaledDt / 0.016);
        trackingState.current.vy += (Math.random() - 0.5) * 0.8 * (scaledDt / 0.016);

        const maxSpeed = 12;
        trackingState.current.vx = Math.max(Math.min(trackingState.current.vx, maxSpeed), -maxSpeed);
        trackingState.current.vy = Math.max(Math.min(trackingState.current.vy, maxSpeed), -maxSpeed);
        ...
        // Trails
        if (trailEffect) {
          const trailArr = trackingState.current.trail;
          trailArr.push({ x: trackingState.current.px, y: trackingState.current.py });
          if (trailArr.length > 15) trailArr.shift();
          for (let i = 0; i < trailArr.length; i++) {
            ctx.fillStyle = targetColor;
            ctx.globalAlpha = (i / trailArr.length) * 0.15;
            ctx.beginPath();
            ctx.arc(trailArr[i].x, trailArr[i].y, targetSize * (i / trailArr.length), 0, Math.PI * 2);
            ctx.fill();
          }
          ctx.globalAlpha = 1.0;
        }
    ...
    if (typeof navigator !== 'undefined' && navigator.share) {
      try {
        await navigator.share({
          title: 'Free Directional Chaos Pursuit Drill | SkillDrills',
          text: 'Track a bouncing target subjected to continuous chaotic velocity nudges.',
          url: 'https://skilldrills.online/drills/visual-tracking/directional-chaos-pursuit'
        });
      } catch (e) {}
    } else if (typeof navigator !== 'undefined' && navigator.clipboard) {
      navigator.clipboard.writeText('https://skilldrills.online/drills/visual-tracking/directional-chaos-pursuit');
        ...
        {!isFullscreen && (
          <nav aria-label="Breadcrumb" className="mb-5">
            <ol className="flex items-center gap-2 text-[10px] text-slate-550 uppercase tracking-widest font-mono">
              ...
              <li><span className="text-slate-755">/</span></li>
              <li><span className="text-red-400 font-bold">Directional Chaos Pursuit</span></li>
            </ol>
          </nav>
        )}
                ...
                <div className="absolute inset-0 bg-[#05070e]/95 flex flex-col items-center justify-center p-6 z-30 select-none">
                  <div className="max-w-md text-center">
                    ...
                    <p className="text-xs text-slate-400 leading-relaxed mb-6">
                      Track a bouncing target subjected to continuous chaotic velocity nudges. Focus your eyes on the moving target. Do not use your head or mouse.
                    </p>
            ...
            <div className="md:col-span-2 bg-[#0b0f19]/40 border border-slate-900 p-6 rounded-2xl flex flex-col justify-between">
              <div>
                ...
                <p className="text-xs leading-relaxed text-slate-400 mb-4 font-sans">
                  The Directional Chaos Pursuit conditions tracking reflexes against highly unpredictable targets. Continual random adjustments to velocity components simulate human-like evasion patterns, training the eyes to make immediate tracking adjustments.
                </p>
              </div>
            </div>
            ...
            <div className="bg-[#0b0f19]/40 border border-slate-905 p-6 rounded-2xl">
              ...
              <ul className="space-y-3 text-xs text-slate-400 font-mono">
                <li className="flex items-start gap-2">
                  ...
                  <span><strong>Chaotic Tracking</strong>: Prepares visual muscles for completely erratic target behaviors.</span>
                </li>\n                <li className="flex items-start gap-2">
                  <Check className="w-4 h-4 text-green-500 flex-shrink-0 mt-0.5" />
                  <span><strong>Reflex Overclock</strong>: Optimizes rapid direction correction responses.</span>
                </li>\n                <li className="flex items-start gap-2">
                  <Check className="w-4 h-4 text-green-500 flex-shrink-0 mt-0.5" />
                  <span><strong>Pursuit Alignment</strong>: Maintains centering under continuous vector fluctuations.</span>
                </li>\n
              </ul>
            </div>
          </div>
            ...
            <div className="max-w-7xl mx-auto">
              ...
              <div className="border-t border-slate-900 pt-8 text-center">
                ...
                <p className="text-[10px] max-w-2xl mx-auto leading-relaxed mb-6 font-sans text-slate-700">
                  Free online Directional Chaos Pursuit drill for visual gaze tracking and foveal stability training. Train foveal pursuit and coordinate re-acquisition speeds under speed configurations. Perfect for gamers, athletes, and attention diagnostics. No registration required. More free drills at skilldrills.online.
                </p>
