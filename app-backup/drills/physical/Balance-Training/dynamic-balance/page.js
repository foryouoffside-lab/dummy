...
export default function DynamicBalanceElitePage() {
  ...
  const update = (dt, cvs) => {
    ...
    // BALANCED SCORE RATES
    const GAIN_RATE = 35;      // Points per second when on target
    const DECAY_RATE = 45;     // Points per second when off target
    
    if (IS_ON_TARGET) {
      // ON TARGET: Gain points slowly
      ...
      // Streak bonuses every 40 consecutive frames on target
      if (streakRef.current % 40 === 0 && streakRef.current > 0) {
        playSound('streak');
        showFeedback(`🔥 ${streakRef.current} Streak! +25 Bonus`, 'success');
        scoreRef.current += 25;
      ...
      // Apply decay
      scoreRef.current = Math.max(-100, scoreRef.current - (DECAY_RATE * dt));
      streakRef.current = Math.max(0, streakRef.current - 2);
      
      // Penalty when first losing target
      if (streakRef.current === 0 && wasTracking) {
        showFeedback('⚠️ Target lost! -15', 'warning');
        playSound('break');
        scoreRef.current = Math.max(-100, scoreRef.current - 15);
      }
    }
          ...
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <p className={`${isDarkMode ? 'text-gray-400' : 'text-gray-500'}`}>Track the moving target - 60 second challenge</p>
              </div>
            </div>
          ...
          {gameState === 'start' && (
            <div className={`absolute inset-0 flex items-center justify-center backdrop-blur-sm rounded-xl z-40 ${isBoxDarkMode ? 'bg-gray-900/95' : 'bg-white/95'}`}>
              <div className={`rounded-2xl p-8 text-center max-w-md mx-4 shadow-xl border ${isBoxDarkMode ? 'bg-gray-800 border-gray-700' : 'bg-white border-gray-200'}`}>
                ...
                <p className={`mb-6 ${isBoxDarkMode ? 'text-gray-300' : 'text-gray-600'}`}>60-second challenge • Track the moving target</p>
                <button 
                  onClick={startGame} 
            ...
            <div className={`absolute inset-0 flex items-center justify-center backdrop-blur-sm rounded-xl z-40 ${isBoxDarkMode ? 'bg-gray-900/95' : 'bg-white/95'}`}>
              <div className={`rounded-2xl p-8 shadow-xl border w-[520px] ${isBoxDarkMode ? 'bg-gray-800 border-gray-700' : 'bg-white border-gray-200'}`}>
                ...
                <div className="grid grid-cols-2 gap-4 mb-8">
                  ...
                  <ResultCard label="Tracking Accuracy" value={trackingAccuracy} unit="%" icon={<Activity className="w-4 h-4" />} color="text-green-500" />
                </div>
            ...
            <div className={`rounded-xl border overflow-hidden ${isDarkMode ? 'bg-gray-800 border-gray-700' : 'bg-white border-gray-200'}`}>
              ...
              <div className="p-4">
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  <div className="space-y-2">
                    <div className="flex items-start gap-2">
                      ...
                      <p className={`text-sm ${isDarkMode ? 'text-gray-300' : 'text-gray-600'}`}>Keep cursor <span className="font-semibold text-green-500">inside the green ring</span> to gain points</p>
                    </div>
                    <div className="flex items-start gap-2">
                      <div className="w-5 h-5 rounded-full bg-red-500 flex items-center justify-center text-white text-xs font-bold mt-0.5">2</div>
                      <p className={`text-sm ${isDarkMode ? 'text-gray-300' : 'text-gray-600'}`}>Losing the target causes <span className="font-semibold text-red-500">-15 point penalty</span> and score decay</p>
                    </div>
                    <div className="flex items-start gap-2">
                      ...
                      <p className={`text-sm ${isDarkMode ? 'text-gray-300' : 'text-gray-600'}`}>Every <span className="font-semibold text-orange-500">40 consecutive hits</span> gives +25 bonus points</p>
                    </div>
                  </div>
                  <div className="space-y-2">
                    <div className="flex items-start gap-2">
                      ...
                      <p className={`text-sm ${isDarkMode ? 'text-gray-300' : 'text-gray-600'}`}>Target moves in a <span className="font-semibold text-blue-500">Lissajous trajectory</span></p>
                    </div>
                    <div className="flex items-start gap-2">
                ...
                <div className={`mt-3 pt-3 border-t text-xs ${isDarkMode ? 'border-gray-700 text-gray-400' : 'border-gray-200 text-gray-500'} flex items-center justify-between`}>
                  <span>🎯 Score: +35/sec on target • -45/sec off target</span>
                  <span>⚡ Green ring = Gaining • Red ring = Losing points</span>
                </div>
              </div>
            </div>
          </div>
