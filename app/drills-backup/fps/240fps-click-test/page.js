...
import { 
  ...
  Eye, Timer, Gauge, Trophy, Target, Zap, Activity, X, Award, Info, Check
} from 'lucide-react';
...
export default function ReactionDrillPage() {
  ...
  useEffect(() => {
    const savedBestScore = localStorage.getItem('reactionDrillBestScore');
    if (savedBestScore) {
  ...
  const updateBestScore = (finalScore) => {
    const currentBestScore = parseInt(localStorage.getItem('reactionDrillBestScore') || '0', 10);
    if (finalScore > currentBestScore) {
      localStorage.setItem('reactionDrillBestScore', finalScore.toString());
  ...
  const calculateScoreFromSpeed = (reactionTimeMs) => {
    ...
    if (reactionTimeMs <= 200) return 80;
    if (reactionTimeMs <= 250) return 60;
    if (reactionTimeMs <= 300) return 40;
    if (reactionTimeMs <= 350) return 25;
    if (reactionTimeMs <= 400) return 15;
    return 10;
  };
  ...
  const handleSuccess = (reactionTime) => {
    const basePoints = calculateScoreFromSpeed(reactionTime);
    ...
    playSound('success');
    showFeedback(`âœ“ ${reactionTime}ms | +${totalPoints}`, 'success');
  ...
  const handleFail = (reason) => {
    // Penalty equals the maximum potential gain (100 + combo bonus)
    const maxBasePoints = 100;
    const comboBonus = Math.floor(comboRef.current / 3) * 10;
    const penaltyPoints = maxBasePoints + comboBonus;
    ...
    setMistakes(prev => {
      ...
      if (newMistakes >= 5) {
        setScore(0);
        scoreRef.current = 0;
        showFeedback('âš ï¸ Score reset! 5 mistakes!', 'warning');
        return 0;
      }
      return newMistakes;
    });
  ...
  useEffect(() => {
    const handleMouseDown = () => {
      ...
      if (dist < target.r) {
        if (isLitRef.current) {
          const reactionTime = Math.floor(performance.now() - reactionStartTimeRef.current);
          handleSuccess(reactionTime);
          isLitRef.current = false;
          flashTimerRef.current = -(Math.random() * 1.5);
        } else {
          handleFail('Early click');
        }
      }
    };
  ...
  useEffect(() => {
    ...
    function drawTarget(now) {
      ...
      ctx.beginPath();
      ctx.arc(target.x, target.y, currentR * 0.6, 0, Math.PI * 2);
      ctx.strokeStyle = isLitRef.current ? 'rgba(255,255,255,0.5)' : 'rgba(0,0,0,0.3)';
      ctx.lineWidth = 2;
      ctx.stroke();
    }
    ...
    function render(now) {
      ...
      if (isActiveRef.current) {
        ...
        if (!isLitRef.current && flashTimerRef.current > (Math.random() * 1.2 + 0.8)) {
          isLitRef.current = true;
          flashTimerRef.current = 0;
          reactionStartTimeRef.current = performance.now();
        }
        
        if (isLitRef.current && flashTimerRef.current > 0.20) {
          isLitRef.current = false;
          if (reactionStartTimeRef.current > 0) {
            handleFail('Missed');
          }
          flashTimerRef.current = -(Math.random() * 0.8);
        }
      }
  ...
  const getAccuracy = () => {
    ...
    return Math.round((reactionCount / total) * 100);
  };
        ...
        <div className="mb-6">
          ...
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-3">
              ...
              <div>
                <h1 className={`text-3xl font-bold ${isDarkMode ? 'text-white' : 'text-gray-900'}`}>240FPS Reaction Drill</h1>
                <p className={`${isDarkMode ? 'text-gray-400' : 'text-gray-500'}`}>Click when target lights up â€¢ 200ms window</p>
              </div>
            </div>
          ...
          {isFullscreen && gameState === 'playing' && (
            <>
              ...
              <div className="absolute top-4 left-4 z-20 bg-black/50 backdrop-blur-sm rounded-lg px-4 py-2 text-white text-sm">
                Score: <span className="text-yellow-400">{score}</span> | Combo: <span className="text-purple-400">{combo}x</span> | Best: <span className="text-green-400">{bestReaction || '-'}ms</span>
              </div>
            </>
          )}
          ...
          {gameState === 'start' && (
            <div className={`absolute inset-0 flex items-center justify-center backdrop-blur-sm rounded-xl z-40 ${isBoxDarkMode ? 'bg-gray-900/95' : 'bg-white/95'}`}>
              <div className={`rounded-2xl p-8 text-center max-w-md mx-4 shadow-xl border ${isBoxDarkMode ? 'bg-gray-800 border-gray-700' : 'bg-white border-gray-200'}`}>
                <Gauge className="w-16 h-16 text-red-500 mx-auto mb-4" />
                <h3 className={`text-2xl font-bold mb-2 ${isBoxDarkMode ? 'text-white' : 'text-gray-900'}`}>240FPS Reaction Drill</h3>
                <p className={`mb-6 ${isBoxDarkMode ? 'text-gray-300' : 'text-gray-600'}`}>60-second challenge â€¢ Click when target lights up</p>
                <button 
                  onClick={startGame} 
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  <div className="space-y-2">
                    <div className="flex items-start gap-2">
                      ...
                      <p className={`text-sm ${isDarkMode ? 'text-gray-300' : 'text-gray-600'}`}>
                        <span className="font-semibold text-red-500">Target lights up WHITE for 200ms</span> â€¢ Click during this window
                      </p>
                    </div>
                    <div className="flex items-start gap-2">
                      <div className="w-5 h-5 rounded-full bg-green-500 flex items-center justify-center text-white text-xs font-bold mt-0.5">2</div>
                      <p className={`text-sm ${isDarkMode ? 'text-gray-300' : 'text-gray-600'}`}>
                        <span className="font-semibold text-green-500">Faster reactions = Higher scores</span> (&lt;150ms: +100, &lt;200ms: +80)
                      </p>
                    </div>
                    <div className="flex items-start gap-2">
                  ...
                  <div className="space-y-2">
                    <div className="flex items-start gap-2">
                      ...
                      <p className={`text-sm ${isDarkMode ? 'text-gray-300' : 'text-gray-600'}`}>
                        <span className="font-semibold text-blue-500">Penalty = Maximum Potential Gain</span> â€¢ Early/Miss loses 100+combo bonus
                      </p>
                    </div>
                    <div className="flex items-start gap-2">
                      ...
                      <p className={`text-sm ${isDarkMode ? 'text-gray-300' : 'text-gray-600'}`}>
                        <span className="font-semibold text-orange-500">Symmetric scoring</span> â€¢ Penalty equals what perfect hit would earn
                      </p>
                    </div>
                    <div className="flex items-start gap-2">
                      <div className="w-5 h-5 rounded-full bg-yellow-500 flex items-center justify-center text-white text-xs font-bold mt-0.5">6</div>
                      <p className={`text-sm ${isDarkMode ? 'text-gray-300' : 'text-gray-600'}`}>
                        <span className="font-semibold text-yellow-500">5 mistakes reset score</span> â€¢ Best Score saves locally
                      </p>
                    </div>
                  </div>
                </div>
                <div className={`mt-3 pt-3 border-t text-xs ${isDarkMode ? 'border-gray-700 text-gray-400' : 'border-gray-200 text-gray-500'} flex items-center justify-between`}>
                  <span>ðŸŽ¯ Random delays 0.8-2.0s between flashes</span>
                  <span>âš¡ 60 second challenge</span>
                </div>
              </div>
            </div>
          </div>
