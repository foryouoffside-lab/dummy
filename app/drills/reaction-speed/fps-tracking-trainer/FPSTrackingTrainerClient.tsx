  ...
  useEffect(() => {
    ...
    const draw = (ts: number) => {
      ...
      // Draw horizontal target
      {
        ...
        ctx.beginPath();
        ctx.arc(px - r * 0.2, py - r * 0.2, r * 0.28, 0, Math.PI * 2);
        ctx.fill();
        // Bright center core
        ctx.globalAlpha = 1.0;
        ctx.fillStyle = '#ffffff';
...
animationRef.current = requestAnimationFrame(draw);
  ...
  const handlePointerDown = (e: React.PointerEvent<HTMLCanvasElement>) => {
    ...
    if (dist <= hitRadius) {
      ...
      const bounds = (cvs.width / dpr2) * 0.15;
      trackingState.current.px = bounds + Math.random() * ((cvs.width / dpr2) - bounds * 2);
      trackingState.current.timer = 0;
                ...
                <div className="absolute inset-0 bg-[#05070e]/98 overflow-y-auto p-6 z-30 select-none scrollbar-thin scroll-smooth backdrop-blur-sm">
                  <div className="min-h-full flex flex-col justify-center items-center py-4 w-full">
                    <div className="max-w-md w-full text-center">
                      ...
                      <button
                        onClick={startDrill}
                        className="w-full py-3 bg-red-650 hover:bg-red-600 text-white font-bold rounded-xl text-xs flex items-center justify-center gap-2 shadow-lg uppercase tracking-widest font-mono transition-all duration-200 active:scale-95"
                      >
                  ...
                  <div className="min-h-full flex flex-col justify-center items-center py-4 w-full">
                    <div className="max-w-md w-full text-center">
                      ...
                      <div className="flex gap-2">
                        <button
                          onClick={startDrill}
                          className="flex-1 py-3 bg-red-650 hover:bg-red-600 text-white font-bold rounded-xl text-xs flex items-center justify-center gap-2 shadow-lg uppercase tracking-widest font-mono transition-all duration-200 active:scale-95"
                        >
                          <RefreshCw className="w-3.5 h-3.5" />
                          Run another trial
                        </button>
