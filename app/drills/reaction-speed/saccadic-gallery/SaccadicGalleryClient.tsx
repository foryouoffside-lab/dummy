...
// Types for in-canvas particle system
...
export default function SaccadicGalleryClient() {
  ...
  // Physics engine states (Specific to Saccadic Gallery)
  ...
  // Mobile Device & Orientation Checks
  useEffect(() => {
  ...
  useEffect(() => {
    ...
    if (typeof window !== 'undefined') {
      ...
      try {
        const stored = localStorage.getItem('skilldrills_saccadic_gallery_best');
  ...
  // Intercept navigation for mobile users swiping wildly
  useEffect(() => {
  ...
  const handleHit = useCallback((x: number, y: number) => {
    ...
    // Add particle feedback directly to canvas loop
        ...
        if (timeLeftRef.current <= 0) {
          ...
          try {
            ...
            if (scoreRef.current > currentBest) {
              localStorage.setItem('skilldrills_saccadic_gallery_best', scoreRef.current.toString());
  ...
  useEffect(() => {
    ...
    const updateDimensions = () => {
      ...
      // Setup Zig-Zag Gallery Grid
      trackingState.current.spots = [];
      for (let i = 0; i < 5; i++) {
        trackingState.current.spots.push([w * 0.25 + i * (w * 0.125), h / 2 + (i % 2 ? -h * 0.15 : h * 0.15)]);
      }
    ...
    const drawLoop = (ts: number) => {
      ...
      {
        // 1. Draw all hollow port anchors (inactive spots)
        ...
        if (trackingState.current.spots.length === 5) {
          ...
          // Render Trail Effect (pulse ring)
          if (trailEffect) {
            ...
            ctx.lineWidth = 3;
            ctx.beginPath();
            ctx.arc(s[0], s[1], radius + 8, 0, Math.PI * 2);
            ctx.stroke();
          }
        }
      }
  ...
  const shareScore = useCallback(async () => {
    const text = `🎮 I scored ${score} PTS on Saccadic Gallery reaction test! Average reaction: ${avgReactionTime}ms. Practice free reflex drills at skilldrills.online! ⚡`;
  ...
  const copyPageLink = useCallback(() => {
    if (typeof navigator !== 'undefined' && navigator.clipboard) {
      navigator.clipboard.writeText('https://skilldrills.online/drills/reaction-speed/saccadic-gallery');
  ...
  let diagnostics = 'Solid attempt! Focus on visual scanning lines. Zig-zag patterns require horizontal and vertical coordination.';
  if (overallAccPercent < 75) {
    diagnostics = 'Accuracy is key! Try to slow down slightly to hit target cores. Speed will naturally follow precision.';
  } else if (avgReactionTime > 400 && hits > 0) {
    diagnostics = 'Focus on your fast-twitch visual reflexes. Try tapping as soon as the target spawns without over-aiming.';
  } else if (maxCombo < 15 && hits > 10) {
    diagnostics = 'Maintain consistency to build high combos. High combos prevent time drain and keep the session active longer.';
  }
          ...
          <div className="mb-6">
            <nav className="mb-4">
              <ol className="flex flex-wrap items-center gap-2 text-sm text-gray-500">
                ...
                <li><ChevronRight className="w-4 h-4 text-gray-600" /></li>
                <li className="text-red-400 font-medium">Saccadic Gallery</li>
              </ol>
            </nav>
            ...
            <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
              <div className="flex items-center gap-4">
                ...
                <div>
                  <h1 className="text-2xl sm:text-3xl font-bold tracking-tight">Saccadic Gallery</h1>
                  <p className="text-sm text-gray-400 mt-1 font-medium">Zig-Zag Visual Reflex Calibration • Device Adaptive</p>
                </div>
              </div>
                ...
                <div className="absolute inset-0 bg-[#05070e]/98 overflow-y-auto p-6 z-30 select-none scrollbar-thin scroll-smooth backdrop-blur-sm">
                  <div className="min-h-full flex flex-col justify-center items-center py-4 w-full">
                    ...
                    <p className="text-xs text-slate-550 uppercase tracking-widest mb-6">
                      Zig-Zag Visual Reflex Test • Mouse / Touch
                    </p>
          ...
          <article className="mt-12 text-gray-300">
            <div className="rounded-2xl border border-gray-800 overflow-hidden bg-gray-900 shadow-xl">
              <div className="px-6 py-5 border-b border-gray-800 bg-black/40 flex items-center gap-3">
                ...
                <h2 className="font-bold text-white text-lg tracking-wide font-mono">About Saccadic Gallery</h2>
              </div>
              
              <div className="p-8 space-y-8">
                <section>
                  <h2 className="text-xl font-bold text-white mb-3">What is Saccadic Gallery?</h2>
                  <p className="text-sm leading-relaxed mb-4">
                    The Saccadic Gallery Drill conditions rapid eye movements (saccades) across horizontal and vertical vectors. In tactical shooters, visual scanning efficiency allows you to sweep corners and clear angles quickly by shifting your foveal focus between multiple target entry points without head rotation latency.
                  </p>
                </section>

                <section className="border-t border-gray-800 pt-6">
                  <h2 className="text-xl font-bold text-white mb-3">Why Zig-Zag Tracking Matters</h2>
                  <p className="text-sm leading-relaxed mb-4">
                    Unlike standard linear tracking, targets in the real world (and in competitive gaming) shift depth, elevation, and position simultaneously. Zig-Zag tracking forces your visual cortex to recalculate X/Y coordinates instantly, strengthening your ability to re-acquire aim when targets jump or dash.
                  </p>
                </section>
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                    <div className="bg-black/30 p-4 border border-gray-800 rounded-lg">
                      <h4 className="font-bold text-white mb-2 font-mono uppercase text-xs flex items-center gap-2">
                        <Check className="w-3.5 h-3.5 text-green-400" /> Gaze Shifts
                      </h4>
                      <p className="text-xs text-gray-400 leading-relaxed">
                        Accelerates visual scanning speeds between horizontal and vertical vectors.
                      </p>
                    </div>
                    <div className="bg-black/30 p-4 border border-gray-800 rounded-lg">
                      <h4 className="font-bold text-white mb-2 font-mono uppercase text-xs flex items-center gap-2">
                        <Check className="w-3.5 h-3.5 text-green-400" /> Corner Clearance
                      </h4>
                      <p className="text-xs text-gray-400 leading-relaxed">
                        Calibrates ocular height tracking to standard sweeps for tactical shooters.
                      </p>
                    </div>
                    <div className="bg-black/30 p-4 border border-gray-800 rounded-lg">
                      <h4 className="font-bold text-white mb-2 font-mono uppercase text-xs flex items-center gap-2">
                        <Check className="w-3.5 h-3.5 text-green-400" /> Target Selection
                      </h4>
                      <p className="text-xs text-gray-400 leading-relaxed">
                        Reduces cognitive delay when identifying multiple sequential threats.
                      </p>
                    </div>
                  </div>
                </section>
              ...
              <div className="bg-[#0b0f19] border-t border-gray-800 p-8">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                  <FAQItem q="What is Saccadic tracking?" a="Saccades are rapid, ballistic movements of the eyes that abruptly change the point of fixation. This drill specifically trains how fast you can jump focus between targets." />
                  <FAQItem q="How does this improve reaction time?" a="By conditioning your eyes to snap directly to new stimuli points, you reduce the time it takes for visual information to reach the motor cortex to execute a click." />
                  <FAQItem q="Is this drill suitable for mobile?" a="Yes! The target sizes scale dynamically based on the device, ensuring the difficulty remains consistent whether using a mouse or touch inputs." />
                  <FAQItem q="Why do I get time penalties for misses?" a="To discourage aimless spam-clicking. You must visually confirm the target presence and location before committing to motor action." />
                </div>
              </div>
            </div>
          </article>
        ...
        {!isFullscreen && !isMobileLandscape && (
          <section className="mt-14" aria-label="Explore related aim and response drills">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
              <RelatedCard href="/drills/reaction-speed/saccadic-snap" title="Saccadic Snap" desc="Instant click reflex snaps targeting fast ocular re-acquisition." color="red" icon={<Target className="w-4 h-4" />} />
              <RelatedCard href="/drills/reaction-speed/reaction-simulator" title="Reaction Sim" desc="Catch falling stimuli at progressive velocities." color="orange" icon={<Compass className="w-4 h-4" />} />
              <RelatedCard href="/drills/cognitive/attention/divided-attention" title="Divided Attention" desc="Dual-task visual sweeps and numeric parity checks." color="indigo" icon={<Zap className="w-4 h-4" />} />
              <RelatedCard href="/drills/reaction-speed/stop-and-go-dash" title="Stop & Go Dash" desc="Calibrate tracking against sudden micro-strafing stops." color="blue" icon={<Brain className="w-4 h-4" />} />
            </div>
          </section>
        )}
