...
export default function ClickAccuracyClient() {
  ...
  const engine = useRef({
    ...
    targets: [], // { x, y, r, maxR, vx, vy, ax, ay, life, maxLife, born }
    
    // Adaptive Mechanics
    targetSpeedMulti: 1.0,
    maxTargets: 1, // STRICTLY CAP AT 1 TARGET AT A TIME
  ...
  useEffect(() => {
    try {
      const savedSens = localStorage.getItem('clickAccuracy_sens');
      if (savedSens) setUniversalSens(parseFloat(savedSens));
      const savedBest = localStorage.getItem('clickAccuracy_bestScore');
  ...
  useEffect(() => {
    if (gameState !== 'playing') {
      try { localStorage.setItem('clickAccuracy_sens', universalSens.toString()); } catch (e) {}
    }
  ...
  const endGame = useCallback(() => {
    ...
    setBestScore(prev => {
      if (e.score > prev) {
        setIsNewBest(true);
        try { localStorage.setItem('clickAccuracy_bestScore', e.score.toString()); } catch(err){}
        return e.score;
      }
      return prev;
    });
  ...
  const spawnTarget = useCallback((cvs) => {
    ...
    // Adaptive Sizing: Shrinks faster and smaller as streak increases
    const dynamicMaxR = Math.max(10, 45 - (e.streak * 1.5));
    const padding = 50; 
    
    const x = padding + Math.random() * (cvs.width - padding * 2); 
    const y = padding + Math.random() * (cvs.height - padding * 2);
    
    // Initial random velocity based on multiplier
    const initialV = 200 * e.targetSpeedMulti;
    const vx = (Math.random() - 0.5) * initialV;
    const vy = (Math.random() - 0.5) * initialV;
    
    // Lifespan shrinks significantly as difficulty increases
    const maxLife = Math.max(0.7, 3.0 - (e.streak * 0.15));
    
    e.targets.push({
      x, y, vx, vy, ax: 0, ay: 0,
      r: dynamicMaxR, maxR: dynamicMaxR,
  ...
  const applyPenalty = useCallback((reason) => {
    ...
    e.misses++;
    e.totalAttempts++;
    e.score = Math.max(0, e.score - 3); // -3 PTS Penalty
    e.timeLeft -= 1.0; // -1.0s Time Penalty
    
    e.streak = 0;
    e.screenShake = 15;
    
    // Forgiveness: Slow target slightly to recover
  ...
  const handleHit = useCallback((cvs, targetIndex) => {
    const e = engine.current;
    const hitTarget = e.targets[targetIndex];
    
    e.hits++;
    e.totalAttempts++;
    e.score += 5; // +5 PTS Reward
    
    // +2.0s Time Bonus, STRICTLY CAPPED at 60s
    e.timeLeft = Math.min(DRILL_DURATION, e.timeLeft + 2.0); 
    
    const reactTime = Math.floor(performance.now() - hitTarget.born);
    ...
    // Adaptive Difficulty: Target speeds up faster
    if (e.streak % 2 === 0) {
      e.targetSpeedMulti = Math.min(6.0, e.targetSpeedMulti + 0.2);
    }
    ...
    e.targets.splice(targetIndex, 1);
  ...
  const startGame = useCallback(async () => {
    ...
    e.maxTargets = 1; // Strict 1 Target
    e.targets = [];
    e.screenShake = 0;
    ...
    const handleMouseDown = (e) => {
      ...
      if (gameState === 'playing' && pointerLocked && isActiveRef.current) {
        ...
        for (let i = eRef.targets.length - 1; i >= 0; i--) {
          ...
          if (dist < t.r + 10) { // 10px hitbox forgiveness
            hitIndex = i;
            break;
          }
        }
        ...
        if (hitIndex !== -1) {
          handleHit(canvasRef.current, hitIndex);
        } else {
          // Missed click - penalize but leave target alive
  ...
  useEffect(() => {
    ...
    const loop = (time) => {
      ...
      if (gameState === 'playing' && pointerLocked && isActiveRef.current) {
        ...
        // Maintain Target Population (Strictly 1 Target)
        while (e.targets.length < e.maxTargets) {
          spawnTarget(cvs);
        }
        ...
        for (let i = e.targets.length - 1; i >= 0; i--) {
          ...
          // Random erratic acceleration changes
          if (Math.random() > 0.95) {
            t.ax = (Math.random() - 0.5) * 1000 * e.targetSpeedMulti;
            t.ay = (Math.random() - 0.5) * 1000 * e.targetSpeedMulti;
          }
          ...
          const maxSpeed = 400 * e.targetSpeedMulti;
          ...
          // Shrinking Mechanic (Life timer)
          t.life -= dt;
          t.r = Math.max(0, t.maxR * (t.life / t.maxLife));
          
          // Target Expired (Timeout)
          if (t.life <= 0) {
            applyPenalty("Timeout");
            e.targets.splice(i, 1); // Remove expired target to trigger respawn
          }
        }
      ...
      if (gameState === 'playing' || gameState === 'start') {
        ...
        // Render Active Target (Classic Bullseye Style)
        e.targets.forEach(t => {
          // Heat mapping color based on velocity/streak
          const hs = e.streak > 15;
          const ev = e.targetSpeedMulti > 3.0;
          let sc; 
          if (ev) sc = "#ef4444"; // Red
          else if (hs) sc = "#06b6d4"; // Cyan
          else if (e.targetSpeedMulti > 1.8) sc = "#f97316"; // Orange
          else sc = "#a855f7"; // Purple
          
          const radius = Math.max(0, t.r);

          // 1. Outer Ring (Colored)
          ctx.beginPath(); 
        ...
        if (ch.initialized) {
          const activeColor = pointerLocked ? '#00ff88' : '#ef4444'; // Emerald / Red
  ...
  }, [gameState, pointerLocked, applyPenalty, spawnTarget, endGame]);

  const shareDrillLink = useCallback(() => {
    const url = typeof window !== 'undefined' ? window.location.href : '';
    if (navigator.share) {
      navigator.share({ title: 'Click Accuracy Elite', url }).catch(() => {});
    } else {
    ...
    <div ref={pageRef} className="min-h-screen select-none bg-[#050508] text-white">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
          ...
          <div className="mb-6">
            <nav className="mb-4">
              <ol className="flex flex-wrap items-center gap-2 text-sm text-gray-500">
                ...
                <li><Link href="/drills/motor" className="hover:text-gray-300">Motor Drills</Link></li>
                <li><ChevronRight className="w-4 h-4 text-gray-600" /></li>
                <li className="text-purple-400 font-medium">Click Accuracy Elite</li>
              </ol>
            </nav>
            ...
            <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
              <div className="flex items-center gap-4">
                ...
                <div>
                  <h1 className="text-2xl sm:text-3xl font-bold tracking-tight">Click Accuracy Elite</h1>
        ...
        <div 
          ...
          style={{ backgroundColor: flashBg === 'red' ? '#450a0a' : flashBg === 'green' ? '#064e3b' : '#000000' }}
        >
          {/* Progress Bar */}
          {gameState === 'playing' && (
          ...
          {gameState === 'start' && (
            <div className="absolute inset-0 flex items-center justify-center z-50 bg-black/70 backdrop-blur-md p-4 overflow-y-auto">
              <div className="rounded-3xl p-8 text-center max-w-lg w-full border border-gray-700 bg-gray-900 shadow-2xl my-auto">
                ...
                <h2 className="text-3xl font-black mb-3 tracking-tight text-white uppercase">Click Accuracy Elite</h2>
                <p className="text-sm mb-6 text-gray-400 leading-relaxed">
                  Raw input dynamic aim tracking. Focus on the single moving, shrinking bullseye. Clicking empty space or letting the target expire violently deducts your score and drains your master clock.
                </p>
                ...
                >
                  <Play className="w-6 h-6 fill-white" /> BEGIN AIM TRAINING
                </button>
              </div>
            </div>
          )}
            ...
            <div className="absolute inset-0 flex items-center justify-center z-50 bg-black/90 backdrop-blur-md p-4 animate-in fade-in duration-300 overflow-y-auto">
              <div className="rounded-3xl max-w-2xl w-full shadow-2xl border border-gray-800 bg-gray-950 overflow-hidden my-auto">
                <div className="bg-gradient-to-br from-purple-900/40 to-pink-900/40 p-6 border-b border-gray-800 text-center relative">
                  ...
                  <h2 className="text-2xl font-black text-white tracking-tight mt-4">Aim Analysis Complete</h2>
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-6 text-xs leading-relaxed text-gray-300">
                      ...
                      <div className="space-y-3 sm:border-r border-gray-800 sm:pr-6">
                        ...
                        <ul className="space-y-2">
                          ...
                          <li className="flex justify-between items-center bg-gray-900/50 p-2 rounded border border-gray-800">
                            <span className="text-gray-400">Velocity Scale Beaten:</span>
                            <span className="font-bold text-orange-400">{analytics.peakSpeed}x</span>
                          </li>
                          <li className="flex justify-between items-center bg-gray-900/50 p-2 rounded border border-gray-800">
                      ...
                      <div className="space-y-3 flex flex-col justify-between">
                        <div>
                          ...
                          <p className="text-gray-400 leading-relaxed font-sans">
                            {analytics.misses > 5 ? (
                              <span className="text-red-300">You are missing too often. Clicking empty space or letting the single target expire violently drains your clock and deducts points. Focus purely on accuracy and smooth tracking of the moving target rather than panic-flicking.</span>
                            ) : analytics.peakSpeed < 2.5 ? (
            ...
            <div className="rounded-2xl border border-gray-800 overflow-hidden bg-gray-900 shadow-2xl pointer-events-none">
              ...
              <div className="p-6 grid grid-cols-1 md:grid-cols-2 gap-6">
                <div className="space-y-5">
                  <RuleItem num="1" color="green" text="Hit Dynamic Target" highlight="+5 PTS | +2.0s Time" result="Max Time Capped at 60s" />
                  <RuleItem num="2" color="indigo" text="Dynamic Scaling" highlight="Endless difficulty" result="Velocity and shrink rate increase" />
                </div>
                <div className="space-y-5">
                  <RuleItem num="3" color="red" text="Miss / Timeout" result="-3 PTS | -1.0s Time" />
          ...
          <section className="mt-12" aria-label="About this drill">
            <div className="rounded-2xl border border-gray-800 overflow-hidden bg-gray-900 shadow-xl">
              <div className="px-6 py-5 border-b border-gray-800 bg-black/40 flex items-center gap-3">
                ...
                <h2 className="font-bold text-white text-lg tracking-wide">About Click Accuracy Elite</h2>
              </div>
              <div className="p-8">
                <p className="text-sm leading-relaxed mb-6 text-gray-300">
                  This free dynamic click-timing aim trainer tests your reaction speed and clicking precision against a single moving, shrinking bullseye target. The engine utilizes random-acceleration vector physics to force the target to fly erratically around the screen. As you succeed, the target severely shrinks and speeds up, demanding high-level mouse control.
                </p>
                ...
                <div className="mb-8 bg-[#0b0f19]/40 border border-gray-800 rounded-xl p-6">
                  ...
                  <div className="grid sm:grid-cols-2 gap-6 text-sm text-gray-300">
                    <ol className="space-y-3 list-decimal pl-5">
                      ...
                      <li>Track the single dynamic moving target as it shrinks over time.</li>
                      <li>Click exactly inside the target before it collapses entirely.</li>
                    </ol>
                    <ul className="space-y-3">
                      <li className="flex items-center gap-2"><CheckCircle2 className="w-4 h-4 text-green-500" /> <span className="text-white font-bold">Valid Hit:</span> Clicking the target grants +5 PTS and +2.0s (capped at 60s total). The target speeds up.</li>
                      <li className="flex items-center gap-2"><XCircle className="w-4 h-4 text-red-500" /> <span className="text-white font-bold">Miss / Timeout:</span> Clicking empty space or letting the ring shrink to zero deducts -3 PTS and -1.0s Time.</li>
                    </ul>
                  </div>
                </div>
                ...
                <div className="p-5 rounded-xl border border-gray-800 bg-black/40">
                  ...
                  <div className="space-y-4">
                    ...
                    <div>
                      ...
                      <p className="text-xs text-gray-400 mt-1">This is the engine's adaptive speed radar. As you successfully hit the target and build your streak, the engine scales up the target's random velocity. The colors shift from Purple &rarr; Emerald &rarr; Orange &rarr; Cyan &rarr; Red to visually warn you of the intense speed.</p>
                    </div>
                  </div>
                </div>
              </div>
