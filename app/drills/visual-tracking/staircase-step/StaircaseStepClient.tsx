...
export default function StaircaseStepClient() {
  ...
  useEffect(() => {
    ...
    try {
      const stored = localStorage.getItem('visualTrackingStaircaseStepClientTrialsCount');
        ...
        setTimeLeft(p => {
          if (p <= 1) {
            ...
            setTotalTrials(prev => {
              const next = prev + 1;
              try {
                localStorage.setItem('visualTrackingStaircaseStepClientTrialsCount', next.toString());
              } catch (e) {}
              return next;
            });
  ...
  useEffect(() => {
    ...
    const updateSize = () => {
      ...
      trackingState.current.px = w * 0.1;
      trackingState.current.py = h - 100;
      trackingState.current.stepPhase = 0;
      trackingState.current.phaseTimer = 0;
    };
    ...
    const draw = (ts: number) => {
      ...
      trackingState.current.phaseTimer += scaledDt * 1000;
        
        if (trackingState.current.stepPhase === 0) {
          trackingState.current.px += 240 * scaledDt;
          if (trackingState.current.phaseTimer > 400) {
            trackingState.current.stepPhase = 1;
            trackingState.current.phaseTimer = 0;
            playSound('transition');
          }
        } else if (trackingState.current.stepPhase === 1) {
          trackingState.current.py -= 240 * scaledDt;
          if (trackingState.current.phaseTimer > 200) {
            trackingState.current.stepPhase = 0;
            trackingState.current.phaseTimer = 0;
            playSound('transition');
          }
        ...
        // Reset when boundary reached
        if (trackingState.current.px > W - 50 || trackingState.current.py < 50) {
          trackingState.current.px = W * 0.1;
          trackingState.current.py = H - 100;
          trackingState.current.stepPhase = 0;
          trackingState.current.phaseTimer = 0;
    ...
    if (typeof navigator !== 'undefined' && navigator.share) {
      try {
        await navigator.share({
          title: 'Free Staircase Step Drill | SkillDrills',
          text: 'Track stepped trajectories combining horizontal slides with rapid vertical snaps.',
          url: 'https://skilldrills.online/drills/visual-tracking/staircase-step'
        });
      } catch (e) {}
    } else if (typeof navigator !== 'undefined' && navigator.clipboard) {
      navigator.clipboard.writeText('https://skilldrills.online/drills/visual-tracking/staircase-step');
        ...
        {!isFullscreen && (
          <nav aria-label="Breadcrumb" className="mb-5">
            <ol className="flex items-center gap-2 text-[10px] text-slate-550 uppercase tracking-widest font-mono">
              ...
              <li><span className="text-slate-755">/</span></li>
              <li><span className="text-red-400 font-bold">Staircase Step</span></li>
            </ol>
          </nav>
        )}
                ...
                <div className="absolute inset-0 bg-[#05070e]/95 flex flex-col items-center justify-center p-6 z-30 select-none">
                  <div className="max-w-md text-center">
                    ...
                    <p className="text-xs text-slate-400 leading-relaxed mb-6">
                      Track stepped trajectories combining horizontal slides with rapid vertical snaps. Focus your eyes on the moving target. Do not use your head or mouse.
                    </p>
            ...
            <div className="md:col-span-2 bg-[#0b0f19]/40 border border-slate-900 p-6 rounded-2xl flex flex-col justify-between">
              <div>
                ...
                <p className="text-xs leading-relaxed text-slate-400 mb-4 font-sans">
                  The Staircase Step Drill trains mixed smooth pursuit and saccadic adjustments. Targets alternate between continuous horizontal movement and sharp vertical steps, challenging eye muscles to adapt instantly.
                </p>
              </div>
            </div>
            ...
            <div className="bg-[#0b0f19]/40 border border-slate-905 p-6 rounded-2xl">
              ...
              <ul className="space-y-3 text-xs text-slate-400 font-mono">
                <li className="flex items-start gap-2">
                  ...
                  <span><strong>Stepped Tracking</strong>: Integrates horizontal pursuit with vertical snapping.</span>
                </li>\n                <li className="flex items-start gap-2">
                  <Check className="w-4 h-4 text-green-500 flex-shrink-0 mt-0.5" />
                  <span><strong>Phase Adaptive</strong>: Adapts ocular speed scaling under sudden transition changes.</span>
                </li>\n                <li className="flex items-start gap-2">
                  <Check className="w-4 h-4 text-green-500 flex-shrink-0 mt-0.5" />
                  <span><strong>Refocus Speed</strong>: Maximizes eye coordinate sweep accuracy.</span>
                </li>\n
              </ul>
            </div>
          </div>
            ...
            <div className="max-w-7xl mx-auto">
              ...
              <div className="border-t border-slate-900 pt-8 text-center">
                ...
                <p className="text-[10px] max-w-2xl mx-auto leading-relaxed mb-6 font-sans text-slate-700">
                  Free online Staircase Step drill for visual gaze tracking and foveal stability training. Train foveal pursuit and coordinate re-acquisition speeds under speed configurations. Perfect for gamers, athletes, and attention diagnostics. No registration required. More free drills at skilldrills.online.
                </p>
