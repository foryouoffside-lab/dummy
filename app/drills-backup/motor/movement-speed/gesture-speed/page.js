...
import { 
  ...
  Eye, Brain, BarChart3, Timer, Trophy, Info, Move
} from 'lucide-react';
...
export default function VectorRecoilPage() {
  ...
  const gateRef = useRef({ active: false, x: 0, y: 0, angle: 0, timer: 1.0 });
  ...
  const limitRef = useRef(1.0); // 1 second timer
  ...
  useEffect(() => {
    if (totalAttemptsRef.current > 0) {
      const acc = (successfulRecoilsRef.current / totalAttemptsRef.current) * 100;
  ...
  const spawnGate = (cvs) => {
    ...
    gateRef.current = {
      ...
      angle: angle,
      timer: limitRef.current, // 1 second
      active: true
    };
  ...
  const addPenalty = (reason, penaltyPoints = 20) => {
  ...
  const addMistake = (reason) => {
    if (!isActiveRef.current) return;
    setMistakes(prev => prev + 1);
    addPenalty(reason, 30);
    playSound('fail');
    ...
    const handleMouseDown = (e) => {
      ...
      if (stateRef.current === 'FLICKING' && gateRef.current.active && !gateHitProcessedRef.current) {
        ...
        if (distToGate < 35) {
          ...
          // Award points for hitting the gate - speed bonus up to 30
          const timeBonus = Math.floor(gate.timer * 30);
          const gatePoints = 50 + timeBonus;
          scoreRef.current += gatePoints;
          setScore(scoreRef.current);
          
          playSound('gateHit');
          showFeedback(`✓ Gate hit! +${gatePoints}`, 'success');
      ...
      if (stateRef.current === 'FLICKING' && gateRef.current.active && !gateHitProcessedRef.current) {
        ...
        if (distToGate < 60 && distToGate >= 35) {
          // Near miss - small penalty
          addPenalty('Near miss', 10);
  ...
  useEffect(() => {
    ...
    function update(dt) {
      ...
      if (stateRef.current === 'CENTER') {
        ...
        if (onCenter) {
          // Success! Complete recoil
          totalAttemptsRef.current++;
          successfulRecoilsRef.current++;
          setSuccessfulRecoils(successfulRecoilsRef.current);
          
          // Award return points
          const returnPoints = 100;
          const streakBonus = streakRef.current * 20;
          const pointsEarned = returnPoints + streakBonus;
          ...
          let feedbackMsg = `✓ Return! +${pointsEarned}`;
          if (streakBonus > 0) feedbackMsg += ` 🔥${newStreak}x`;
          showFeedback(feedbackMsg, 'success');
          
          if (newStreak % 5 === 0) {
            playSound('streak');
            showFeedback(`🔥 ${newStreak} STREAK! +50 BONUS!`, 'success');
            scoreRef.current += 50;
            setScore(scoreRef.current);
          }
    ...
    function draw() {
      ...
      if (gate.active && stateRef.current === 'FLICKING') {
        ...
        if (isHoveringGate) {
          ...
          ctx.fillText(`+${50 + timeBonus}`, gate.x, gate.y - 35);
        }
      }
      ...
      if (stateRef.current === 'RETURNING') {
        ...
        ctx.font = "bold 10px monospace";
        ctx.textAlign = "center";
        ctx.fillText(`Return for +${100 + streakBonus}`, cx, cy - 40);
      }
      ...
      if (stateRef.current === 'FLICKING') {
        ...
        ctx.textAlign = "center";
        ctx.fillText('→ CLICK THE GATE (1s) →', cvs.width / 2, 35);
      } else if (stateRef.current === 'RETURNING') {
        ctx.fillStyle = "#00ff88";
        ctx.font = "bold 11px monospace";
        ctx.textAlign = "center";
        ctx.fillText('← RETURN TO CENTER ←', cvs.width / 2, 35);
  ...
  const startGame = () => {
    ...
    stateRef.current = 'CENTER';
    gateRef.current = { active: false, x: 0, y: 0, angle: 0, timer: 1.0 };
    gateHitProcessedRef.current = false;
          ...
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <p className={`${isDarkMode ? 'text-gray-400' : 'text-gray-500'}`}>Click gate & return - 1 sec window</p>
              </div>
            </div>
          ...
          {gameState === 'start' && (
            <div className={`absolute inset-0 flex items-center justify-center backdrop-blur-sm rounded-xl z-40 ${isBoxDarkMode ? 'bg-gray-900/95' : 'bg-white/95'}`}>
              <div className={`rounded-2xl p-8 text-center max-w-md mx-4 shadow-xl border ${isBoxDarkMode ? 'bg-gray-800 border-gray-700' : 'bg-white border-gray-200'}`}>
                ...
                <p className={`mb-6 ${isBoxDarkMode ? 'text-gray-300' : 'text-gray-600'}`}>60-second challenge • 1 sec window</p>
                <button 
                  onClick={startGame} 
            ...
            <div className={`absolute inset-0 flex items-center justify-center backdrop-blur-sm rounded-xl z-40 ${isBoxDarkMode ? 'bg-gray-900/95' : 'bg-white/95'}`}>
              <div className={`rounded-2xl p-8 shadow-xl border w-[480px] ${isBoxDarkMode ? 'bg-gray-800 border-gray-700' : 'bg-white border-gray-200'}`}>
                ...
                <div className="grid grid-cols-2 gap-4 mb-8">
                  ...
                  <ResultCard label="Successful Recoils" value={successfulRecoils} icon={<Move className="w-4 h-4" />} color="text-green-500" />
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  <div className="space-y-2">
                    ...
                    <div className="flex items-start gap-2">
                      ...
                      <p className={`text-sm ${isDarkMode ? 'text-gray-300' : 'text-gray-600'}`}><span className="font-semibold text-green-500">CLICK gate:</span> 50 + speed bonus (up to +30)</p>
                    </div>
                    <div className="flex items-start gap-2">
                      ...
                      <p className={`text-sm ${isDarkMode ? 'text-gray-300' : 'text-gray-600'}`}><span className="font-semibold text-emerald-500">Return to center:</span> 100 + streak bonus</p>
                    </div>
                  </div>
                  <div className="space-y-2">
                    <div className="flex items-start gap-2">
                      <div className="w-5 h-5 rounded-full bg-red-500 flex items-center justify-center text-white text-xs font-bold mt-0.5">4</div>
                      <p className={`text-sm ${isDarkMode ? 'text-gray-300' : 'text-gray-600'}`}>Penalties: Miss -15, Near miss -10, Timeout -30</p>
                    </div>
                    <div className="flex items-start gap-2">
                      ...
                      <p className={`text-sm ${isDarkMode ? 'text-gray-300' : 'text-gray-600'}`}>5x Streak: +50 bonus • <span className="font-semibold text-yellow-500">5 mistakes reset score</span></p>
                    </div>
                    <div className="flex items-start gap-2">
                      ...
                      <p className={`text-sm ${isDarkMode ? 'text-gray-300' : 'text-gray-600'}`}>Challenge: <span className="font-semibold text-purple-500">60 seconds</span> • 1 sec window per gate</p>
                    </div>
                  </div>
                </div>
                <div className={`mt-3 pt-3 border-t text-xs ${isDarkMode ? 'border-gray-700 text-gray-400' : 'border-gray-200 text-gray-500'} flex items-center justify-between`}>
                  <span>🎯 Gate: +50~80 • Return: +100~150 • Streak: +20 per level</span>
