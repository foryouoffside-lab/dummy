...
import { 
  ...
  TrendingUp, Trophy, Volume2, VolumeX, 
  Share2, Sparkles, Zap, Users, XSquare
} from 'lucide-react';
...
class AudioSynthesizer {
  ...
  playHitTier(tier) {
    ...
    try {
      ...
      const freqMap = { 'EXACT': 1318.51, 'PERFECT': 1046.50, 'EXCELLENT': 880, 'GOOD': 659.25, 'OK': 523.25, 'HIT': 440 };
...
export default function StopwatchClickClient() {
  ...
  useEffect(() => {
    try {
      const savedBest = localStorage.getItem('stopwatchEndless_bestScore');
  ...
  const endGame = useCallback(() => {
    ...
    setBestScore(prev => {
      ...
      if (finalScore > prev) {
        setIsNewBest(true);
        try { localStorage.setItem('stopwatchEndless_bestScore', finalScore.toString()); } catch(err){}
        return finalScore;
      }
      return prev;
    });
  ...
  const generateNewRound = useCallback(() => {
    ...
    // Max target expands by 500ms per level, up to 10 seconds max
    const maxTarget = Math.min(10000, 2000 + (e.level * 500)); 
    
    e.targetTime = minTarget + Math.floor(Math.random() * (maxTarget - minTarget));
    
    // Display phase gets shorter at high levels
    e.displayTimer = Math.max(0.4, 1.5 - (e.level * 0.05));
    // Result phase gets shorter at high levels for faster pacing
    e.resultTimer = Math.max(0.4, 1.2 - (e.level * 0.05));
  ...
  useEffect(() => {
    const handleMouseMove = (e) => {
      ...
      engine.current.mousePos.x = Math.max(0, Math.min(cvs.width, engine.current.mousePos.x + dx));
      engine.current.mousePos.y = Math.max(0, Math.min(cvs.height, engine.current.mousePos.y + dy));
    };
    ...
    const handleMouseDown = (e) => {
      ...
      if (gameState === 'playing') {
        ...
        if (eRef.state === 'TIMER') {
          ...
          // Base tolerance 50ms + 5% of target time.
          ...
          if (errorAbs <= tHit) { // Hit!
            ...
            // Miss! Drill continues indefinitely, but punishes combo and visual feedback.
            eRef.misses++;
            eRef.combo = 0;
          ...
          // If they double click fast during result, immediately start next round
          generateNewRound();
        }
      }
    };
  ...
  }, [gameState, pointerLocked, generateNewRound, createExplosion]);
  ...
  useEffect(() => {
    ...
    const loop = (time) => {
      ...
      if (e.state === 'TIMER') {
        ...
        // "FOCUS/ESTIMATING" Text Indicator
        ctx.fillStyle = "rgba(6, 182, 212, 0.4)";
        ctx.font = "bold 14px sans-serif";
        ctx.letterSpacing = "6px";
        ctx.textAlign = "center";
        ctx.fillText("ESTIMATING...", cx, cy - 80);
        ...
        ctx.save();
        ctx.translate(cx, cy);
        ctx.rotate(elapsed * 0.001); // Slow rotation
        ctx.beginPath();
        ...
        ctx.save();
        ctx.translate(cx, cy);
        ctx.rotate(-elapsed * 0.0005); // Slower counter-rotation
        ctx.beginPath();
        ...
        // Continuous outward pulsing rings
        const pulse = (elapsed % 1000) / 1000;
        ctx.beginPath();
        ctx.arc(cx, cy, 70 + (pulse * 40), 0, Math.PI * 2);
        ctx.strokeStyle = `rgba(6, 182, 212, ${0.4 * (1 - pulse)})`;
        ctx.lineWidth = 2;
        ctx.stroke();
        
        // Secondary outward ring (staggered)
        const pulse2 = ((elapsed - 500) % 1000) / 1000;
        if (elapsed > 500) {
            ctx.beginPath();
            ctx.arc(cx, cy, 70 + (pulse2 * 40), 0, Math.PI * 2);
            ctx.strokeStyle = `rgba(6, 182, 212, ${0.2 * (1 - pulse2)})`;
            ctx.lineWidth = 1;
            ctx.stroke();
        }
      }
      ...
      if (e.state === 'RESULT') {
        ...
        const maxResult = Math.max(0.4, 1.2 - (e.level * 0.05));
  ...
  }, [gameState, pointerLocked, generateNewRound]);

  const shareScore = useCallback(async () => {
    const text = `⏱️ I reached Level ${analytics.levelReached} and scored ${score} PTS in the Stopwatch Timing Test! Grade: ${analytics.gradeData.grade}, Avg Error: ±${analytics.avgError}ms. Test your internal clock at skilldrills.online!`;
    if (typeof navigator !== 'undefined' && navigator.share) {
      try {
        await navigator.share({ title: 'My Mental Chronometry Score', text, url: 'https://skilldrills.online/drills/motor/timing-accuracy/stopwatch-click' });
      } catch (e) {}
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
          ...
          <div className="mb-6">
            <nav className="mb-4">
              <ol className="flex flex-wrap items-center gap-2 text-sm text-gray-500">
                ...
                <li><ChevronRight className="w-4 h-4 text-gray-600" /></li>
                <li className="text-cyan-400 font-medium">Stopwatch Timing Test</li>
              </ol>
            </nav>
            ...
            <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
              <div className="flex items-center gap-4">
                ...
                <div>
                  <h1 className="text-2xl sm:text-3xl font-bold tracking-tight">Stopwatch Timing Test</h1>
                  <p className="text-sm text-gray-400 mt-1 font-medium">Mental Chronometry Trainer • Endless Practice</p>
                </div>
              </div>
              
              <div className="flex gap-2">
                {gameState === 'playing' && (
                  <button onClick={endGame} className="px-4 py-2 bg-red-600 hover:bg-red-500 text-white font-bold rounded-lg text-sm flex items-center justify-center gap-2 shadow-lg transition-all">
                     <XSquare className="w-4 h-4" /> End Drill
                  </button>
                )}
        ...
        >
          ...
          {gameState === 'playing' && !pointerLocked && (
            <div 
              className="absolute inset-0 z-40 bg-black/60 backdrop-blur-sm flex flex-col items-center justify-center cursor-pointer"
              onClick={(e) => { 
                e.stopPropagation(); 
                if (canvasRef.current) canvasRef.current.requestPointerLock(); 
              }}
            >
              <div className="text-center animate-pulse pointer-events-none mb-8">
              ...
              <button 
                ...
                className="px-6 py-3 bg-red-600/80 hover:bg-red-500 text-white font-bold rounded-xl text-sm uppercase tracking-widest transition-all shadow-lg z-50 pointer-events-auto flex items-center gap-2"
              >
                 <XSquare className="w-4 h-4" /> End Session & View Stats
              </button>
            </div>
          )}
              ...
              <div className="max-w-md w-full text-center">
                ...
                <div className="bg-[#0b0f19] border border-slate-850 p-4 rounded-xl mb-6 text-left text-xs text-slate-400">
                  ...
                  <ul className="list-disc pl-4 space-y-1 text-[10px] text-slate-500 leading-relaxed">
                    ...
                    <li>When the timer hides and begins estimating, click exactly when the time has elapsed.</li>
                    <li>The closer to 0ms error, the higher your score and combination modifier.</li>
                    <li>Play at your own pace indefinitely. Press `Esc` or the End Drill button when you are ready to view your analytics.</li>
                  </ul>
                </div>
          ...
          <section className="mt-10">
            <div className="rounded-2xl border border-gray-800 overflow-hidden bg-gray-900 shadow-2xl pointer-events-none">
              <div className="px-6 py-5 border-b border-gray-800 bg-black/40 flex items-center gap-3">
                <Info className="w-5 h-5 text-cyan-400" /><h2 className="font-bold text-white text-lg tracking-wide">Drill Rules & Scoring</h2>
              </div>
              <div className="p-6 grid grid-cols-1 md:grid-cols-2 gap-6">
                <div className="space-y-5">
                  <RuleItem num="1" color="yellow" text="Exact Timing" highlight="+25 PTS & Multiplier" result="Maximum reward" />
                ...
                <div className="space-y-5">
                  <RuleItem num="3" color="red" text="Miss Punishment" highlight="- Combo Reset" result="Visual indicator & momentum loss" />
                  <RuleItem num="4" color="cyan" text="Endless Scaling" highlight="Level up every 100pts" result="Longer times & faster pacing" />
                </div>
              </div>
            </div>
          </section>
          ...
          <section className="mt-12" aria-label="About this drill">
            <div className="rounded-2xl border border-gray-800 overflow-hidden bg-gray-900 shadow-xl">
              <div className="px-6 py-5 border-b border-gray-800 bg-black/40 flex items-center gap-3">
                ...
                <h2 className="font-bold text-white text-lg tracking-wide">About the Stopwatch Timing Test</h2>
              </div>
              
              <div className="p-8 space-y-8">
                <section>
                  <h2 className="text-xl font-bold text-white mb-3">Mastering Mental Chronometry</h2>
                  <p className="text-sm leading-relaxed mb-4">
                    This internal clock test pushes your mental chronometry and time estimation to the absolute limit. You must memorize a target time and click exactly when that duration elapses in your head. The timer is completely hidden. There is zero negative scoring—your goal is to maintain extreme millisecond accuracy to build massive combo multipliers over an endless practice session. Play as long as you want to calibrate your brain's temporal processing centers, and review your analytics when you manually end the drill.
                  </p>
                </section>
                ...
                <div className="grid grid-cols-1 sm:grid-cols-3 gap-5 mb-8">
                  ...
                  <div className="p-5 rounded-xl border bg-black/40 border-gray-800">
                    ...
                    <p className="text-xs leading-relaxed text-slate-400">Mental chronometry, internal clock calibration, time perception, sub-division of seconds, rhythm coordination, and sustained mental focus.</p>
                  </div>
                  <div className="p-5 rounded-xl border bg-black/40 border-gray-800">
                    <div className="flex items-center gap-3 mb-3">
              ...
              <div className="bg-[#0b0f19] border-t border-gray-800 p-8">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                  <FAQItem q="What is Mental Chronometry?" a="Mental chronometry is the scientific study of processing speed and internal time perception. It measures how accurately a human brain can estimate elapsed time without external visual or auditory cues." />
                  <FAQItem q="How accurate is the human internal clock?" a="For short durations (under 2 seconds), humans are generally accurate within 100-200 milliseconds. For longer durations (5+ seconds), the error variance increases significantly without rhythmic subdivisions." />
                  <FAQItem q="Can timing accuracy improve?" a="Yes. Like any cognitive or motor skill, your internal clock can be calibrated. By practicing rhythm games and time estimation drills, you train your brain to track seconds more consistently." />
                  <FAQItem q="Is this useful for musicians?" a="Absolutely. Musicians, especially drummers, rely heavily on internal tempo. This drill acts as a strict metronome test, forcing you to subdivide time mentally without an audible click track." />
                  <FAQItem q="How does the scoring work?" a="Accurate clicks reward you with points and combo multipliers. The closer you are to the exact millisecond, the higher the reward. A bad miss simply resets your combo modifier." />
                  <FAQItem q="How does the game end?" a="The drill is completely endless, allowing you to enter a flow state and practice indefinitely. Simply press Esc to pause, or hit the End Drill button to stop and view your comprehensive statistics." />
                  <FAQItem q="How does the difficulty increase?" a="As your score increases, the game levels up. Target times become longer and more unpredictable, the memorization phase shortens, and transitions become significantly faster, creating intense mental pressure." />
                  <FAQItem q="How often should I practice this?" a="For optimal internal clock calibration, practice for 5 to 10 minutes a day. Consistent daily exposure trains the brain's temporal processing centers better than long, infrequent sessions." />
                </div>
              </div>
            </div>
          </section>
