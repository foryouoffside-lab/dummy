...
export default function ReactionChainProPage() {
  ...
  const addPenalty = (reason) => {
    ...
    showFeedback(`âœ— ${reason}! -15`, 'error');
    playSound('miss');
  };
    ...
    const handleMouseDown = () => {
      ...
      if (dist < hitRadius) {
        ...
        if (scoreRef.current > bestScore && scoreRef.current > parseInt(localStorage.getItem('reactionChainBestScore') || '0', 10)) {
          ...
          playSound('hit');
          showFeedback(`âœ“ ${reactionMs}ms | +10`, 'success');
        }
      } else {
        addPenalty('Miss');
        nodesRef.current = [];
          ...
          {gameState === 'start' && (
            <div className={`absolute inset-0 flex items-center justify-center backdrop-blur-sm rounded-xl z-40 ${isBoxDarkMode ? 'bg-gray-900/95' : 'bg-white/95'}`}>
              <div className={`rounded-2xl p-8 text-center max-w-md mx-4 shadow-xl border ${isBoxDarkMode ? 'bg-gray-800 border-gray-700' : 'bg-white border-gray-200'}`}>
                ...
                <p className={`mb-6 ${isBoxDarkMode ? 'text-gray-300' : 'text-gray-600'}`}>60-second challenge â€¢ Click targets before they disappear (300ms)</p>
                <button 
                  onClick={startGame} 
            ...
            <div className={`rounded-xl border overflow-hidden ${isDarkMode ? 'bg-gray-800 border-gray-700' : 'bg-white border-gray-200'}`}>
              ...
              <div className="p-4">
                ...
                <div className={`mt-3 pt-3 border-t text-xs ${isDarkMode ? 'border-gray-700 text-gray-400' : 'border-gray-200 text-gray-500'} flex items-center justify-between`}>
                  <span>ðŸŽ¯ Hit: +10pts â€¢ Higher tempo = faster disappearance</span>
                  <span>â±ï¸ Base time: 300ms â€¢ Shrinking ring = remaining time</span>
                </div>
              </div>
            </div>
          </div>
