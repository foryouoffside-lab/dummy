...
export default function ListeningComprehensionPage() {
  ...
  const checkAnswer = () => {
    ...
    if (isCorrect) {
      ...
      // Penalty: -1 point for wrong answer (score never goes below 0)
      setScore(prev => Math.max(0, prev - 1));
      setWrongAnswers(prev => prev + 1);
      showFeedback(`✗ Wrong! -1 point penalty`, 'error');
      playPenaltySound();
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        ...
        >
          ...
          <div className="absolute inset-0 flex items-center justify-center p-8 overflow-y-auto">
              ...
              <div className={`absolute inset-0 flex items-center justify-center backdrop-blur-sm rounded-xl z-40 ${isBoxDarkMode ? 'bg-gray-900/95' : 'bg-white/95'}`}>
                <div className={`rounded-2xl p-8 shadow-xl border w-[480px] ${isBoxDarkMode ? 'bg-gray-800 border-gray-700' : 'bg-white border-gray-200'}`}>
                  ...
                  <div className="grid grid-cols-2 gap-4 mb-8">
                    ...
                    <ResultCard label="Wrong" value={wrongAnswers} icon={<X className="w-4 h-4" />} color="text-red-500" />
            ...
            {/* Complete Screen (All Questions Answered) */}
            {gameState === 'complete' && (
              <div className={`absolute inset-0 flex items-center justify-center backdrop-blur-sm rounded-xl z-40 ${isBoxDarkMode ? 'bg-gray-900/95' : 'bg-white/95'}`}>
                <div className={`rounded-2xl p-8 shadow-xl border w-[480px] ${isBoxDarkMode ? 'bg-gray-800 border-gray-700' : 'bg-white border-gray-200'}`}>
                  <div className="flex items-center justify-center gap-3 mb-6">
