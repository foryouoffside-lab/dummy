...
const guideData = {
  ...
  intro: [
    "스무스 퍼슈트 안구운동(Smooth Pursuit Eye Movement, SPEM / 활동성 안구운동)은 시야에서 연속적으로 이동하는 목표물의 속도에 맞춰 안구를 매끄럽게 회전시켜, 목표물의 상을 망막에서 가장 해상도가 높은 중심와(Fovea centralis)에 안정적으로 고정하는 정밀 안구운동 시스템입니다. 이는 시선의 위치 오차를 급격한 점프로 만회하는 도약 안구운동(Saccade)과는 신경학적으로 완전히 독립된 제어 회로에 의해 작동합니다(Rashbass, 1961).",
    ...
    "인간의 시각-운동 피드백 루프에는 약 100~130ms의 감각운동 지연(Sensorimotor Delay)이 불가피하게 발생합니다. 등속으로 움직이는 물체를 지연 없이 추적할 수 있는 이유는 소뇌 내부에 목표물의 물리적 운동 방정식을 미리 시뮬레이션하는 내부 예측 모델(Internal Predictive Model)이 구축되어 피드포워드(Feedforward) 제어가 작동하기 때문입니다(Barnes, 2008). 이 예측 기능이 미숙하면 안구가 타깃을 놓치게 되고, 시선이 단속적으로 뚝뚝 끊기는 보정 사케이드(Catch-up Saccade)가 발생하여 트래킹 에임이 심하게 요동치게 됩니다.",
    "본 시스템은 초정밀 performance.now() 타이머와 서브픽셀 렌더링 엔진을 통해 이동 타깃에 마우스 커서가 머문 유효 시간 비율(Time on Target), 평균 추적 오차, 방향 전환 시의 반응 잠복기를 실시간으로 정밀 계산합니다. 지속적인 스무스 퍼슈트 훈련은 외안근과 전완근의 운동 협응을 최적화하여 FPS 게임의 자석 같은 트래킹 에임 형성, 구기 종목의 동체시력 강화, 장시간 컴퓨터 작업 시의 시각 피로 경감에 탁월한 효과를 발휘합니다."
  ],
  benchmarks: {
