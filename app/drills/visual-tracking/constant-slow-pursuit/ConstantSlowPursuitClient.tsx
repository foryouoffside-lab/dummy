...
export default function ConstantSlowPursuitClient() {
  ...
  useEffect(() => {
    ...
    try {
      const stored = localStorage.getItem('visualTrackingConstantSlowPursuitClientTrialsCount');
        ...
        setTimeLeft(p => {
          if (p <= 1) {
            ...
            setTotalTrials(prev => {
              const next = prev + 1;
              try {
                localStorage.setItem('visualTrackingConstantSlowPursuitClientTrialsCount', next.toString());
              } catch (e) {}
              return next;
            });
  ...
  useEffect(() => {
    ...
    const draw = (ts: number) => {
      ...
      const getLissajous = (t: number) => {
          const cx = W / 2;
          const cy = H / 2;
          return {
            x: cx + Math.cos(t * 3) * (W * 0.35),
            y: cy + Math.sin(t * 4) * (H * 0.35)
          };
        };

        // Speed update
        const baseSpeed = 0.18;
        trackingState.current.angle += scaledDt * baseSpeed;

        // Path guide rendering
        ctx.strokeStyle = 'rgba(51, 65, 85, 0.35)';
        ctx.lineWidth = 4;
        ctx.beginPath();
        for (let t = 0; t < Math.PI * 2; t += 0.02) {
          const pt = getLissajous(t);
        ...
        // Draw target
        const targetPos = getLissajous(trackingState.current.angle);
        ctx.shadowColor = targetColor;
        ctx.shadowBlur = glowEffect ? 18 : 0;
        ctx.fillStyle = targetColor;
        ctx.beginPath();
        ctx.arc(targetPos.x, targetPos.y, targetSize, 0, Math.PI * 2);
        ctx.fill();
        ctx.shadowBlur = 0;
    ...
    if (typeof navigator !== 'undefined' && navigator.share) {
      try {
        await navigator.share({
          title: 'Free Constant Slow Pursuit Drill | SkillDrills',
      ...
      navigator.clipboard.writeText('https://skilldrills.online/drills/visual-tracking/constant-slow-pursuit');
        ...
        {!isFullscreen && (
          <nav aria-label="Breadcrumb" className="mb-5">
            <ol className="flex items-center gap-2 text-[10px] text-slate-550 uppercase tracking-widest font-mono">
              ...
              <li><span className="text-slate-755">/</span></li>
              <li><span className="text-red-400 font-bold">Constant Slow Pursuit</span></li>
            </ol>
          </nav>
        )}
                ...
                <div className="absolute inset-0 bg-[#05070e]/95 flex flex-col items-center justify-center p-6 z-30 select-none">
                  <div className="max-w-md text-center">
                    ...
                    <p className="text-xs text-slate-400 leading-relaxed mb-6">
                      Condition foveal pursuit along a continuous, visible Lissajous curve at low velocity. Focus your eyes on the moving target. Do not use your head or mouse.
                    </p>
            ...
            <div className="md:col-span-2 bg-[#0b0f19]/40 border border-slate-900 p-6 rounded-2xl flex flex-col justify-between">
              <div>
                ...
                <p className="text-xs leading-relaxed text-slate-400 mb-4 font-sans">
                  The Constant Slow Pursuit Drill develops steady, micro-focal eye tracking. By rendering target movement along a visible Lissajous curve at a slow speed, it prevents jerky saccades and conditions the visual cortex for smooth focal transitions.
                </p>
              </div>
            </div>
            ...
            <div className="bg-[#0b0f19]/40 border border-slate-905 p-6 rounded-2xl">
              ...
              <ul className="space-y-3 text-xs text-slate-400 font-mono">
                <li className="flex items-start gap-2">
                  ...
                  <span><strong>Smooth Pursuit</strong>: Eliminates visual micro-jitter and stabilizes steady focus.</span>
                </li>\n                <li className="flex items-start gap-2">
                  <Check className="w-4 h-4 text-green-500 flex-shrink-0 mt-0.5" />
                  <span><strong>Trajectory Guide</strong>: Employs trace lines to assist path tracking.</span>
                </li>\n                <li className="flex items-start gap-2">
                  <Check className="w-4 h-4 text-green-500 flex-shrink-0 mt-0.5" />
                  <span><strong>Endurance</strong>: Strengthens eye muscles during long-term constant tracking.</span>
                </li>\n
              </ul>
            </div>
          </div>
            ...
            <div className="max-w-7xl mx-auto">
              ...
              <div className="border-t border-slate-900 pt-8 text-center">
                ...
                <p className="text-[10px] max-w-2xl mx-auto leading-relaxed mb-6 font-sans text-slate-700">
                  Free online Constant Slow Pursuit drill for visual gaze tracking and foveal stability training. Train foveal pursuit and coordinate re-acquisition speeds under speed configurations. Perfect for gamers, athletes, and attention diagnostics. No registration required. More free drills at skilldrills.online.
                </p>
