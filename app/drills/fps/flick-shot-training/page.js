      ...
      if (targetRef.current) {
        ...
        if (elapsed < TARGET_DURATION) {
          ...
          if (distance < TARGET_SIZE / 2 + 15) {
            ...
            showFeedback(`✓ +1 | ${Math.round(elapsed)}ms`, 'success');
            
            targetRef.current = null;
          } else {
            // Clicked but missed target - lose 1 life
            handleMissClick();
          }
        } else {
          // Too slow (shouldn't happen as target disappears)
          targetRef.current = null;
        }
      } else {
        // Clicked when no target - lose 1 life
        handleMissClick();
      }
    ...
    function draw(currentTime) {
      ...
      if (targetRef.current) {
        ...
        if (elapsed < TARGET_DURATION) {
          ...
          // Target expired - no penalty, just remove
          targetRef.current = null;
        }
      }
          ...
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <p className={`${isDarkMode ? 'text-gray-400' : 'text-gray-500'}`}>Flick shots • 400ms targets • 5 lives system</p>
              </div>
            </div>
          ...
          {gameState === 'start' && (
            <div className={`absolute inset-0 flex items-center justify-center backdrop-blur-sm rounded-xl z-40 ${isBoxDarkMode ? 'bg-gray-900/95' : 'bg-white/95'}`}>
              <div className={`rounded-2xl p-8 text-center max-w-md mx-4 shadow-xl border ${isBoxDarkMode ? 'bg-gray-800 border-gray-700' : 'bg-white border-gray-200'}`}>
                ...
                <p className={`mb-6 ${isBoxDarkMode ? 'text-gray-300' : 'text-gray-600'}`}>60-second challenge • 400ms targets • 5 lives system</p>
                <button 
                  onClick={startGame} 
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  ...
                  <div className="space-y-2">
                    <div className="flex items-start gap-2">
                      ...
                      <p className={`text-sm ${isDarkMode ? 'text-gray-300' : 'text-gray-600'}`}>
                        <span className="font-semibold text-red-500">5 Lives system</span> • Each miss/click off target uses 1 life
                      </p>
                    </div>
                    <div className="flex items-start gap-2">
                    ...
                    <div className="flex items-start gap-2">
                      ...
                      <p className={`text-sm ${isDarkMode ? 'text-gray-300' : 'text-gray-600'}`}>
                        <span className="font-semibold text-yellow-500">No penalty for not clicking</span> • Target simply disappears
                      </p>
                    </div>
                  </div>
                </div>
