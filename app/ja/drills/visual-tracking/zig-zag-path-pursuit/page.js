...
const guide = {
  ...
  intro: [
    ...
    "本『ジグザグ経路追従（Zig-Zag Path Pursuit）』ドリルは、ブラウザ上でこの高度な運動適応を促す専門トレーニングツールです。高頻度に連続する鋭角コーナーを正確にトレースすることで、エイムの切り返しにおける手ブレ・視線ブレを徹底的に排除します。『Hide Line』により視覚ガイドを遮断した純粋な空間予測を鍛え、『Random Speed』で変則的なリズムへの即応力を磨くことができます。最新ゲーミングハードウェア環境（Woods et al., 2015）で、プロレベルの幾何学トラッキング精度を獲得してください。"
  ],
  benchmarks: {
