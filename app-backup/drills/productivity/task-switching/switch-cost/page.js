...
export default function QuadZoneSwitcherPage() {
  ...
  useEffect(() => {
    const savedBestScore = localStorage.getItem('quadZoneBestScore');
    if (savedBestScore) {
  ...
  const updateBestScore = (finalScore) => {
    const currentBestScore = parseInt(localStorage.getItem('quadZoneBestScore') || '0', 10);
    if (finalScore > currentBestScore) {
      localStorage.setItem('quadZoneBestScore', finalScore.toString());
  ...
  // Calculate potential points based on current streak and reaction time
  const calculatePoints = (reaction) => {
    let points = 10;
    if (reaction < 500) points = 35;
    else if (reaction < 800) points = 25;
    else if (reaction < 1200) points = 15;
  ...
  const spawn = () => {
    ...
    currentRuleRef.current = isTop ? "PARITY" : "MAGNITUDE";
  ...
  const fail = (reason) => {
    ...
    // Calculate penalty (would-be points for a slow reaction)
    const penaltyPoints = calculatePoints(1500);
  ...
  const handleSuccess = (reaction) => {
    ...
    if (newStreak % 5 === 0 && newStreak > 0) {
      ...
      playSound('hit');
      showFeedback(`✓ ${reaction}ms | +${pointsEarned}`, 'success');
    }
  ...
  useEffect(() => {
    const handleMouseDown = (e) => {
      ...
      const isTop = mouse.y < cvs.height / 2;
      const isLeft = mouse.x < cvs.width / 2;
      
      const reaction = Math.floor(performance.now() - startTimeRef.current);
      let hit = false;
      
      if (isTop && currentRuleRef.current === "PARITY") {
        const isEven = currentValRef.current % 2 === 0;
        if ((isEven && isLeft) || (!isEven && !isLeft)) hit = true;
      } else if (!isTop && currentRuleRef.current === "MAGNITUDE") {
        const isLess = currentValRef.current < 5;
        if ((isLess && isLeft) || (!isLess && !isLeft)) hit = true;
      }
      
      totalAttemptsRef.current++;
      
      if (hit) {
        handleSuccess(reaction);
      } else {
        fail("WRONG ZONE");
      }
    };
  ...
  useEffect(() => {
    ...
    function draw() {
      ...
      // Top Left - EVEN
      ctx.fillText("EVEN", cvs.width * 0.25, cvs.height * 0.35);
      // Top Right - ODD
      ctx.fillText("ODD", cvs.width * 0.75, cvs.height * 0.35);
      // Bottom Left - < 5
      ctx.fillText("< 5", cvs.width * 0.25, cvs.height * 0.75);
      // Bottom Right - > 5
      ctx.fillText("> 5", cvs.width * 0.75, cvs.height * 0.75);
      
      // Rule indicator
      if (stateRef.current === "ACTIVE" && currentRuleRef.current) {
        ctx.font = "bold 12px monospace";
        ctx.fillStyle = "#00ff88";
        ctx.fillText(currentRuleRef.current, cvs.width / 2, 30);
        
        // Highlight active zone
        ctx.fillStyle = "rgba(0, 255, 136, 0.05)";
        if (currentRuleRef.current === "PARITY") {
          ctx.fillRect(0, 0, cvs.width, cvs.height / 2);
        } else {
      ...
      if (stateRef.current === "ACTIVE" && currentValRef.current !== null) {
        ...
        ctx.textAlign = "center";
        const yPos = currentRuleRef.current === "PARITY" ? cvs.height * 0.45 : cvs.height * 0.85;
        ctx.fillText(currentValRef.current, cvs.width / 2, yPos);
      }
        ...
        <div className="mb-6">
          ...
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-3">
              ...
              <div>
                <h1 className={`text-3xl font-bold ${isDarkMode ? 'text-white' : 'text-gray-900'}`}>Quad-Zone Switcher</h1>
                <p className={`${isDarkMode ? 'text-gray-400' : 'text-gray-500'}`}>4-zone rule switching • 2 sec per question</p>
              </div>
            </div>
          ...
          {gameState === 'start' && (
            <div className={`absolute inset-0 flex items-center justify-center backdrop-blur-sm rounded-xl z-40 ${isBoxDarkMode ? 'bg-gray-900/95' : 'bg-white/95'}`}>
              <div className={`rounded-2xl p-8 text-center max-w-md mx-4 shadow-xl border ${isBoxDarkMode ? 'bg-gray-800 border-gray-700' : 'bg-white border-gray-200'}`}>
                ...
                <h3 className={`text-2xl font-bold mb-2 ${isBoxDarkMode ? 'text-white' : 'text-gray-900'}`}>Quad-Zone Switcher</h3>
                <p className={`mb-6 ${isBoxDarkMode ? 'text-gray-300' : 'text-gray-600'}`}>60-second challenge • 2 sec per question • 4 zones</p>
                <button 
                  onClick={startGame} 
            ...
            <div className={`rounded-xl border overflow-hidden ${isDarkMode ? 'bg-gray-800 border-gray-700' : 'bg-white border-gray-200'}`}>
              ...
              <div className="p-4">
                  ...
                  <div className="space-y-2">
                    <div className="flex items-start gap-2">
                      ...
                      <p className={`text-sm ${isDarkMode ? 'text-gray-300' : 'text-gray-600'}`}>
                        <span className="font-semibold text-purple-500">TOP ZONE (Parity):</span> Even → Top-Left / Odd → Top-Right
                      </p>
                    </div>
                    <div className="flex items-start gap-2">
                      ...
                      <p className={`text-sm ${isDarkMode ? 'text-gray-300' : 'text-gray-600'}`}>
                        <span className="font-semibold text-blue-500">BOTTOM ZONE (Magnitude):</span> &lt;5 → Bottom-Left / &gt;5 → Bottom-Right
                      </p>
                    </div>
                    <div className="flex items-start gap-2">
                      ...
                      <p className={`text-sm ${isDarkMode ? 'text-gray-300' : 'text-gray-600'}`}>
                        <span className="font-semibold text-green-500">2 seconds per question</span> - Click the correct quadrant
                      </p>
                    </div>
                  </div>
                  <div className="space-y-2">
                    ...
                    <div className="flex items-start gap-2">
                      ...
                      <p className={`text-sm ${isDarkMode ? 'text-gray-300' : 'text-gray-600'}`}>
                        <span className="font-semibold text-orange-500">Faster = More Points</span> (&lt;500ms: +35, &lt;800ms: +25, &lt;1200ms: +15)
                      </p>
                    </div>
                    <div className="flex items-start gap-2">
                ...
                <div className={`mt-3 pt-3 border-t text-xs ${isDarkMode ? 'border-gray-700 text-gray-400' : 'border-gray-200 text-gray-500'} flex items-center justify-between`}>
                  <span>🎯 Active zone is highlighted • Rule shown at top center</span>
                  <span>⚡ Streak bonus: +5 per 5 streak</span>
                </div>
              </div>
            </div>
          </div>
