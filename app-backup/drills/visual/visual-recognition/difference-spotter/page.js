...
export default function DifferenceSpotterPage() {
  ...
  const handleSuccess = (reactionTime) => {
    ...
    let pointsEarned = 0;
    if (reactionTime < 500) pointsEarned = 40;
    else if (reactionTime < 1000) pointsEarned = 25;
    else if (reactionTime < 1500) pointsEarned = 15;
    else pointsEarned = 10;
    
    scoreRef.current += pointsEarned;
    setScore(scoreRef.current);
    showFeedback(`✓ Found! ${reactionTime}ms | +${pointsEarned}`, 'success');
    playSound('correct');
    ...
    <div className={`min-h-screen select-none ${isDarkMode ? 'bg-gray-900' : 'bg-gray-50'}`}>
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
          ...
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <p className={`${isDarkMode ? 'text-gray-400' : 'text-gray-500'}`}>Spot the change • Study time adapts</p>
              </div>
            </div>
        ...
        <div className="grid grid-cols-7 gap-3 mb-4 h-[88px]">
          ...
          <StatCard icon={<Timer className="text-purple-600" />} value={`${studyTime}s`} label="Study Time" isDark={isDarkMode} />
          ...
          <StatCard icon={<Clock className="text-cyan-600" />} value={bestDetection || '-'} label="Best" unit="ms" isDark={isDarkMode} />
          ...
          {gameState === 'start' && (
            <div className={`absolute inset-0 flex items-center justify-center backdrop-blur-sm rounded-xl z-40 ${isBoxDarkMode ? 'bg-gray-900/95' : 'bg-white/95'}`}>
              <div className={`rounded-2xl p-8 text-center max-w-md mx-4 shadow-xl border ${isBoxDarkMode ? 'bg-gray-800 border-gray-700' : 'bg-white border-gray-200'}`}>
                ...
                <p className={`mb-6 ${isBoxDarkMode ? 'text-gray-300' : 'text-gray-600'}`}>Memorize the pattern • Spot the change</p>
                <button 
                  onClick={startGame} 
            ...
            <div className={`rounded-xl border overflow-hidden ${isDarkMode ? 'bg-gray-800 border-gray-700' : 'bg-white border-gray-200'}`}>
              ...
              <div className="p-4">
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  <div className="space-y-2">
                    ...
                    <div className="flex items-start gap-2">
                      ...
                      <p className={`text-sm ${isDarkMode ? 'text-gray-300' : 'text-gray-600'}`}>Correct: <span className="font-semibold text-green-500">10-40 points</span> based on speed</p>
                    </div>
                    <div className="flex items-start gap-2">
                      <div className="w-5 h-5 rounded-full bg-red-500 flex items-center justify-center text-white text-xs font-bold mt-0.5">3</div>
                      <p className={`text-sm ${isDarkMode ? 'text-gray-300' : 'text-gray-600'}`}>Wrong object: <span className="font-semibold text-red-500">-10 points</span></p>
                    </div>
                  </div>
                  <div className="space-y-2">
                    <div className="flex items-start gap-2">
                ...
                <div className={`mt-3 pt-3 border-t text-xs ${isDarkMode ? 'border-gray-700 text-gray-400' : 'border-gray-200 text-gray-500'} flex items-center justify-between`}>
                  <span>🎯 &lt;500ms: 40pts • &lt;1000ms: 25pts • &lt;1500ms: 15pts • 1500ms+: 10pts</span>
