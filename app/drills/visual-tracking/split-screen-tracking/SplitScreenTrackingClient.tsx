...
export default function SplitScreenTrackingClient() {
  ...
  useEffect(() => {
    ...
    try {
      const stored = localStorage.getItem('visualTrackingSplitScreenTrackingClientTrialsCount');
        ...
        setTimeLeft(p => {
          if (p <= 1) {
            ...
            setTotalTrials(prev => {
              const next = prev + 1;
              try {
                localStorage.setItem('visualTrackingSplitScreenTrackingClientTrialsCount', next.toString());
              } catch (e) {}
              return next;
            });
  ...
  useEffect(() => {
    ...
    const updateSize = () => {
      ...
      trackingState.current.ly = h / 2;
      trackingState.current.lvy = 420;
      trackingState.current.rx = w * 0.75;
      trackingState.current.rvx = 480;
    };
    ...
    const draw = (ts: number) => {
      ...
      // Draw center split boundary
        ...
        if (trackingState.current.ly < H * 0.15) {
          trackingState.current.ly = H * 0.15;
          trackingState.current.lvy = Math.abs(trackingState.current.lvy);
          playSound('transition');
        } else if (trackingState.current.ly > H * 0.85) {
          trackingState.current.ly = H * 0.85;
          trackingState.current.lvy = -Math.abs(trackingState.current.lvy);
          playSound('transition');
        }
        ...
        // Render right target
        ctx.beginPath();
        ctx.arc(trackingState.current.rx, rightDotY, targetSize, 0, Math.PI * 2);
        ctx.fill();
        ctx.shadowBlur = 0;
        ...
        ctx.fill();
        ctx.beginPath();
        ctx.arc(trackingState.current.rx, rightDotY, 3, 0, Math.PI * 2);
        ctx.fill();
    ...
    if (typeof navigator !== 'undefined' && navigator.share) {
      try {
        await navigator.share({
          title: 'Free Split-Screen Tracking Drill | SkillDrills',
      ...
      navigator.clipboard.writeText('https://skilldrills.online/drills/visual-tracking/split-screen-tracking');
        ...
        {!isFullscreen && (
          <nav aria-label="Breadcrumb" className="mb-5">
            <ol className="flex items-center gap-2 text-[10px] text-slate-550 uppercase tracking-widest font-mono">
              ...
              <li><span className="text-slate-755">/</span></li>
              <li><span className="text-red-400 font-bold">Split-Screen Tracking</span></li>
            </ol>
          </nav>
        )}
            ...
            <div className="md:col-span-2 bg-[#0b0f19]/40 border border-slate-900 p-6 rounded-2xl flex flex-col justify-between">
              <div>
                ...
                <p className="text-xs leading-relaxed text-slate-400 mb-4 font-sans">
                  The Split-Screen Tracking Drill trains divided visual attention. With one target moving vertically on the left screen and another moving horizontally on the right, it forces the brain to coordinate dual-focus tracking.
                </p>
              </div>
            </div>
            ...
            <div className="bg-[#0b0f19]/40 border border-slate-905 p-6 rounded-2xl">
              ...
              <ul className="space-y-3 text-xs text-slate-400 font-mono">
                <li className="flex items-start gap-2">
                  ...
                  <span><strong>Divided Attention</strong>: Maximizes peripheral and split-focus ocular scans.</span>
                </li>\n                <li className="flex items-start gap-2">
                  <Check className="w-4 h-4 text-green-500 flex-shrink-0 mt-0.5" />
                  <span><strong>Cross Plane Tracking</strong>: Conditions vertical and horizontal tracking components simultaneously.</span>
                </li>\n                <li className="flex items-start gap-2">
                  <Check className="w-4 h-4 text-green-500 flex-shrink-0 mt-0.5" />
                  <span><strong>Gaze Sweep stability</strong>: Stabilizes center coordinate boundaries.</span>
                </li>\n
              </ul>
            </div>
          </div>
            ...
            <div className="max-w-7xl mx-auto">
              ...
              <div className="border-t border-slate-900 pt-8 text-center">
                ...
                <p className="text-[10px] max-w-2xl mx-auto leading-relaxed mb-6 font-sans text-slate-700">
                  Free online Split-Screen Tracking drill for visual gaze tracking and foveal stability training. Train foveal pursuit and coordinate re-acquisition speeds under speed configurations. Perfect for gamers, athletes, and attention diagnostics. No registration required. More free drills at skilldrills.online.
                </p>
