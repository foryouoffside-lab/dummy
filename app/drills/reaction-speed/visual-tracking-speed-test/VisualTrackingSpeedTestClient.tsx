...
const getLevelParameters = (lvl: number) => {
  ...
  if (lvl === 1) {
    ...
    targetDurationLimit = 1.0;
    baseSpeed = 225;
    dashSpeed = 1150;
  } else if (lvl === 5) {
    targetDurationLimit = 0.80;
    baseSpeed = 270;
    dashSpeed = 1300;
  } else {
    targetDurationLimit = Math.max(0.40, 0.65 - (lvl - 6) * 0.04);
    baseSpeed = Math.min(450, 310 + (lvl - 6) * 20);
    dashSpeed = Math.min(2200, 1450 + (lvl - 6) * 100);
  }
  return { targetDurationLimit, baseSpeed, dashSpeed };
};
...
export default function VisualTrackingSpeedTestClient() {
  ...
  // Unique physics tracking for Slide Dash Acceleration
  ...
  useEffect(() => {
    ...
    if (typeof window !== 'undefined') {
      ...
      try {
        const storedScore = localStorage.getItem('skilldrills_slide-dash-acceleration_best');
        const storedAcc = localStorage.getItem('skilldrills_slide-dash-acceleration_best_acc');
        const storedLvl = localStorage.getItem('skilldrills_slide-dash-acceleration_best_lvl');
        const storedCombo = localStorage.getItem('skilldrills_slide-dash-acceleration_best_combo');
        const storedSessions = localStorage.getItem('skilldrills_slide-dash-acceleration_sessions');
      ...
      timerIntervalRef.current = setInterval(() => {
        ...
        if (timeLeftRef.current <= 0) {
          ...
          try {
            ...
            const sessionsCount = (parseInt(localStorage.getItem('skilldrills_slide-dash-acceleration_sessions') || '0', 10) || 0) + 1;
            localStorage.setItem('skilldrills_slide-dash-acceleration_sessions', sessionsCount.toString());
            ...
            if (scoreRef.current > currentBestScore) {
              ...
              localStorage.setItem('skilldrills_slide-dash-acceleration_best_acc', accuracyVal.toString());
              localStorage.setItem('skilldrills_slide-dash-acceleration_best_lvl', levelRef.current.toString());
              localStorage.setItem('skilldrills_slide-dash-acceleration_best_combo', maxComboRef.current.toString());
  ...
  useEffect(() => {
    ...
    const drawLoop = (ts: number) => {
      ...
      const { targetDurationLimit, baseSpeed, dashSpeed } = getLevelParameters(currentLevel);
...

      {
        ...
        if (!trackingState.current.isDashing && trackingState.current.timer > 1500) {
          ...
          trackingState.current.isDashing = false;
          trackingState.current.currentSpeed = baseSpeed;
          trackingState.current.timer = 0;
...
animationRef.current = requestAnimationFrame(drawLoop);
  ...
  const shareScore = useCallback(async () => {
    const text = `🎮 I scored ${score} PTS on Visual Tracking Speed Test! Average reaction: ${avgReactionTime}ms. Practice free reflex drills at skilldrills.online! ⚡`;
  ...
  const copyPageLink = useCallback(() => {
    if (typeof navigator !== 'undefined' && navigator.clipboard) {
      navigator.clipboard.writeText('https://skilldrills.online/drills/reaction-speed/visual-tracking-speed-test');
  ...
  let diagnostics = 'Solid attempt! Maintain broad foveal vision. Anticipate target dash acceleration burst intervals.';
  if (overallAccPercent < 75) {
    diagnostics = 'Low Accuracy detected. Rushing clicks causes penalty chains. Stabilize cursor alignment before clicking.';
  } else if (avgReactionTime > 450 && hits > 0) {
    diagnostics = 'Slow visual acquisition delay. Practice fast eye jumps (saccades) to lock onto the target immediately as it speeds up.';
  } else if (maxCombo < 10 && hits > 8) {
    diagnostics = 'Combo streak broken too often. Build steady rhythms and track the base sliding speeds consistently.';
  }
          ...
          <div className="mb-6">
            <nav className="mb-4">
              <ol className="flex flex-wrap items-center gap-2 text-sm text-gray-500">
                ...
                <li><ChevronRight className="w-4 h-4 text-gray-600" /></li>
                <li className="text-amber-400 font-medium">Visual Tracking Speed Test</li>
              </ol>
            </nav>
            ...
            <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
              <div className="flex items-center gap-4">
                ...
                <div>
                  <h1 className="text-2xl sm:text-3xl font-bold tracking-tight">Visual Tracking Speed Test</h1>
                  <p className="text-sm text-gray-400 mt-1 font-medium">Slide Dash Acceleration Drill • Device Adaptive</p>
                </div>
              </div>
            ...
            >
              ...
              {(isFullscreen || isMobileLandscape) && gameState === 'playing' && (
                <>
                  ...
                  {combo > 1 && (
                    <div className="absolute top-4 left-4 z-[35] pointer-events-none">
                      <div className="bg-black/60 border border-gray-700 rounded-xl px-3 py-1.5 flex items-center gap-1.5 font-mono text-xs">
                        <Zap className="w-3 h-3 text-yellow-400" />
                        <span className="text-yellow-300 font-black">{combo}x</span>
                        <span className="text-gray-500 text-[9px] uppercase tracking-wider">combo</span>
                      </div>
                    </div>
                  )}
                  {/* Controls only – top right */}
                  <div className="absolute top-4 right-4 z-[35] flex items-center gap-3 text-xs font-mono bg-black/60 border border-gray-800 rounded-xl px-3 py-2 pointer-events-auto">
                    ...
                    <button onClick={() => setHudLocked(v => !v)} className="text-gray-400 hover:text-red-400 transition-colors" title={hudLocked ? "Unlock HUD" : "Lock HUD"}>
                  ...
                  <div className="min-h-full flex flex-col justify-center items-center py-4 w-full">
                    <div className="max-w-md w-full text-center">
                      ...
                      <p className="text-xs text-slate-555 uppercase tracking-widest mb-6">
                        Slide Dash Acceleration Drill • Mouse / Touch
                      </p>
          ...
          <article className="mt-12 text-gray-300">
            <div className="rounded-2xl border border-gray-800 overflow-hidden bg-gray-900 shadow-xl">
              <div className="px-6 py-5 border-b border-gray-800 bg-black/40 flex items-center gap-3">
                ...
                <h2 className="font-bold text-white text-lg tracking-wide font-mono">About Visual Tracking Speed Test</h2>
              </div>
              ...
              <div className="p-8 space-y-8">
                <section>
                  ...
                  <p className="text-sm leading-relaxed mb-4">
                    The Visual Tracking Speed Test (internally known as Slide Dash Acceleration) is a premium visual tracking speed test designed to isolate and optimize your tracking and motor response reflexes against sudden bursts of target speed. It is ideal for gamers and athletes who need to stabilize their foveal tracking when targets accelerate.
                  </p>
                </section>
                ...
                <section className="border-t border-gray-800 pt-6">
                  ...
                  <p className="text-sm leading-relaxed mb-4">
                    A high-contrast target slides continuously across the screen area, bouncing off screen borders. At random intervals, it undergoes sudden, high-speed burst dash accelerations. Your goal is to track the target with your eyes and click it. Hits prolong your training timer, while misses or timeouts penalize it.
                  </p>
                </section>
                ...
                <section className="border-t border-gray-800 pt-6">
                  ...
                  <p className="text-sm leading-relaxed mb-4">
                    Reacting to sudden, unpredictable speed spikes conditions your visual-motor pathway to trigger hand movements faster. This reduces the cognitive latency between seeing a speed transition and executing a click.
                  </p>
                </section>
                ...
                <section className="border-t border-gray-800 pt-6">
                  ...
                  <p className="text-sm leading-relaxed mb-4">
                    The high-speed burst phases require rapid, fast-twitch muscle adjustments, training your hand reflexes to correct crosshair placement dynamically.
                  </p>
                </section>
                ...
                <section className="border-t border-gray-800 pt-6">
                  ...
                  <p className="text-sm leading-relaxed mb-4">
                    Visual tracking is the ability to follow moving objects smoothly. This drill challenges smooth pursuit eye movements by introducing sudden horizontal and diagonal speed burst dashes.
                  </p>
                </section>
                ...
                <section className="border-t border-gray-800 pt-6">
                  ...
                  <p className="text-sm leading-relaxed mb-4">
                    In competitive gaming, targets frequently slide-cancel, dash, or counter-strafe. This test trains you to maintain consistent gaze tracking when opponents undergo sudden velocity spikes.
                  </p>
                </section>
                ...
                <section className="border-t border-gray-800 pt-6">
                  ...
                  <p className="text-sm leading-relaxed mb-4">
                    First-Person Shooter (FPS) aiming requires tracking targets that change direction and speed erratically. Developing dash intercept muscle memory directly translates to hitting jumping or sliding targets.
                  </p>
                </section>
                ...
                <section className="border-t border-gray-800 pt-6">
                  ...
                  <p className="text-sm leading-relaxed mb-4">
                    Professional esports players require reaction times under 150ms. Conditioning with adaptive levels challenges elite players by reducing target sizes and increasing dash speeds up to Level 6+.
                  </p>
                </section>
                ...
                <section className="border-t border-gray-800 pt-6">
                  ...
                  <p className="text-sm leading-relaxed mb-4">
                    Traditional athletes in fast-paced sports (like tennis, baseball, or squash) benefit from training ocular tracking agility and rapid hand-eye synchronization.
                  </p>
                </section>
                ...
                <section className="border-t border-gray-800 pt-6">
                  ...
                  <p className="text-sm leading-relaxed mb-4">
                    Aligning visual tracking coordinates with manual mouse movements is critical. Correcting aiming errors mid-dash improves physical cursor precision.
                  </p>
                </section>
                ...
                <section className="border-t border-gray-800 pt-6">
                  ...
                  <p className="text-sm leading-relaxed mb-4">
                    Your visual cortex must quickly parse velocity and direction changes. This training refines spatial filtering and helps predict moving target trajectories.
                  </p>
                </section>
                ...
                <section className="border-t border-gray-800 pt-6">
                  ...
                  <p className="text-sm leading-relaxed mb-4">
                    Practice for 5-10 minutes daily as an aiming warmup. Prioritize visual tracking and accuracy over hasty clicking to establish stable neurological reflexes.
                  </p>
                </section>
                ...
                <section className="border-t border-gray-800 pt-6">
                  ...
                  <p className="text-sm leading-relaxed mb-4">
                    Difficulty scales dynamically with your score. As you reach higher levels, base sliding speeds and burst dash velocities increase, target exposure lifespans decrease, and target sizes shrink.
                  </p>
                </section>
                ...
                <section className="border-t border-gray-800 pt-6">
                  ...
                  <p className="text-sm leading-relaxed mb-4">
                    Ideal for competitive gamers, traditional sports athletes, cognitive researchers, or anyone seeking to enhance smooth ocular tracking and visual reaction speeds.
                  </p>
                </section>
                ...
                <section className="border-t border-gray-800 pt-6">
                  ...
                  <p className="text-sm leading-relaxed mb-4">
                    Flick-clicking during high-speed burst frames often leads to misses. It is better to track the target smoothly and click it during the normal sliding speed phases.
                  </p>
                </section>
                ...
                <section className="border-t border-gray-800 pt-6">
                  ...
                  <p className="text-sm leading-relaxed mb-4">
                    Train in full-screen landscape mode to maximize the tracking area. Adjust target colors to High-contrast Neon Green or Cyber Orange to ease foveal targeting.
                  </p>
                </section>
                ...
                <section className="border-t border-gray-800 pt-6">
                  ...
                  <p className="text-sm leading-relaxed mb-4">
                    Bronze: Under 100 • Silver: 100-250 • Gold: 250-500 • Platinum: 500-800 • Diamond: 800-1200 • Master/Grandmaster: 1200+. Aim for over 80% accuracy.
                  </p>
                </section>
              </div>
              ...
              <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-5 gap-8 mb-8">
                <div>
                  ...
                  <ul className="space-y-2">
                    ...
                    <li><Link href="/drills/reaction-speed/visual-tracking-speed-test" className="hover:text-amber-400 transition-colors text-amber-400">Visual Tracking</Link></li>
