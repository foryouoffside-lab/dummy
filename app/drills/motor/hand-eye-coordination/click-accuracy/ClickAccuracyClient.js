...
import { 
  ...
  GraduationCap, Lightbulb, TrendingUp, CheckCircle2, Star, ArrowRight, Share2, Copy, Lock, AlertCircle
} from 'lucide-react';
...
export default function ClickAccuracyClient() {
  ...
  useEffect(() => { try { const s = localStorage.getItem('clickAccuracyBest'); if(s) { const p = parseInt(s,10); if(!isNaN(p)) setBestScore(p); } } catch(e) {} }, []);
  useEffect(() => { gameStateRef.current = gameState; }, [gameState]);

  const updateBestScore = useCallback((fs) => { try { const c = parseInt(localStorage.getItem('clickAccuracyBest')||'0',10); if(fs>c) { localStorage.setItem('clickAccuracyBest',fs.toString()); setBestScore(fs); } } catch(e) {} }, []);
  ...
  const playSound = useCallback((type) => { if(!soundEnabled)return; try { const ctx=initAudio(); if(!ctx)return; const o=ctx.createOscillator(),g=ctx.createGain(); o.connect(g);g.connect(ctx.destination); const now=ctx.currentTime; const f={hit:880,miss:440,streak:1046.5,highstreak:1318.52,teleport:660}; o.frequency.setValueAtTime(f[type]||660,now); g.gain.setValueAtTime(type==='streak'||type==='highstreak'?0.12:0.1,now); g.gain.exponentialRampToValueAtTime(0.001,now+0.15); o.start(now);o.stop(now+0.15); } catch(e){} }, [soundEnabled,initAudio]);
  ...
  const addPenalty = useCallback((reason) => {
    ...
    if(livesRef.current>0){livesRef.current-=1;setLives(livesRef.current);showFeedback(`✗ ${reason}! -1 life`,'error');playSound('miss');if(livesRef.current===0)showFeedback('No lives! Penalties deduct points','warning');}
    else{scoreRef.current=Math.max(0,scoreRef.current-1);setScore(scoreRef.current);playSound('miss');showFeedback(`✗ ${reason}! -1 point`,'error');}
  }, [showFeedback,playSound]);

  const teleportTarget = useCallback(() => {
    const cvs=canvasRef.current; if(!cvs||!isActiveRef.current)return;
    const pad=150, nr=Math.max(7,12-(streakRef.current*0.12));
    targetRef.current={x:pad+Math.random()*(cvs.width-pad*2),y:pad+Math.random()*(cvs.height-pad*2),r:nr};
    setTargetSize(Math.round(nr*10)/10); lastSpawnTimeRef.current=performance.now();
    playSound('teleport');
  }, [playSound]);
  ...
  useEffect(() => {
    const h = (e) => {
      ...
      const ch=virtualCrosshair.current, t=targetRef.current, dist=Math.hypot(ch.x-t.x,ch.y-t.y);
      const rt=Math.floor(performance.now()-lastSpawnTimeRef.current);
      reactionTimesRef.current.push(rt); setBestReaction(prev=>prev===0||rt<prev?rt:prev);
      if(dist<t.r){successfulHitsRef.current++;setTargetsHit(successfulHitsRef.current);const ns=streakRef.current+1;streakRef.current=ns;setStreak(ns);if(ns>bestStreakRef.current){bestStreakRef.current=ns;setBestStreak(ns);}scoreRef.current+=1;setScore(scoreRef.current);if(ns%10===0){playSound(ns>=20?'highstreak':'streak');showFeedback(`🔥 ${ns} Streak! +1`,'success');}else{playSound('hit');showFeedback(`✓ ${rt}ms | +1`,'success');}teleportTarget();}
      else{addPenalty('Miss');teleportTarget();}
    };
    document.addEventListener('mousedown',h);
    return () => document.removeEventListener('mousedown',h);
  }, [gameState,addPenalty,teleportTarget,playSound,showFeedback]);

  // Timer
  useEffect(() => {
    if(gameState!=='playing')return;
    timerIntervalRef.current=setInterval(()=>{setTimeLeft(prev=>{if(prev<=1){setGameState('gameOver');gameStateRef.current='gameOver';isActiveRef.current=false;if(timerIntervalRef.current){clearInterval(timerIntervalRef.current);timerIntervalRef.current=null;}if(teleportIntervalRef.current){clearInterval(teleportIntervalRef.current);teleportIntervalRef.current=null;}updateBestScore(scoreRef.current);document.exitPointerLock();return 0;}return prev-1;});},1000);
  ...
  useEffect(() => {
    ...
    const update=()=>{const cr=containerRef.current;if(!cr)return;const rr=cr.getBoundingClientRect();let w=rr.width,h=w*(9/16);if(h>rr.height){h=rr.height;w=h*(16/9);}cvs.width=w;cvs.height=h;cvs.style.position='absolute';cvs.style.left=`${(rr.width-w)/2}px`;cvs.style.top=`${(rr.height-h)/2}px`;if(!crosshairInitRef.current)virtualCrosshair.current={x:w/2,y:h/2};spawn(cvs);if(teleportIntervalRef.current)clearInterval(teleportIntervalRef.current);teleportIntervalRef.current=setInterval(()=>{if(isActiveRef.current&&gameStateRef.current==='playing')teleportTarget();},1500);};
    update();
    ...
    const loop=()=>{
      ...
      ctx.strokeStyle=isBoxDarkMode?'rgba(255,255,255,0.03)':'rgba(0,0,0,0.03)';ctx.lineWidth=1;
      for(let i=0;i<cvs.width;i+=50){ctx.beginPath();ctx.moveTo(i,0);ctx.lineTo(i,cvs.height);ctx.stroke();ctx.moveTo(0,i);ctx.lineTo(cvs.width,i);ctx.stroke();}
      const t=targetRef.current, isHS=streakRef.current>10;
      ctx.beginPath();ctx.arc(t.x,t.y,t.r,0,Math.PI*2);ctx.strokeStyle=isHS?"#00FFFF":streakRef.current>5?"#00ff88":"#FFFFFF";ctx.lineWidth=2.5;ctx.stroke();
      ctx.beginPath();ctx.arc(t.x,t.y,2.5,0,Math.PI*2);ctx.fillStyle="#00ff88";ctx.fill();
      if(isHS){ctx.beginPath();ctx.arc(t.x,t.y,t.r+3,0,Math.PI*2);ctx.strokeStyle="rgba(0,255,136,0.3)";ctx.stroke();}
    ...
    animationRef.current=requestAnimationFrame(loop);
    return()=>{cancelAnimationFrame(animationRef.current);window.removeEventListener('resize',update);ro.disconnect();if(teleportIntervalRef.current)clearInterval(teleportIntervalRef.current);};
  }, [gameState,isBoxDarkMode,pointerLocked,spawn,teleportTarget]);
  ...
  const startGame = useCallback(() => {
    ...
    setScore(0);setStreak(0);setBestStreak(0);setBestReaction(0);setTargetSize(12);setTimeLeft(60);setLives(3);setFeedback('');setTargetsHit(0);
    isActiveRef.current=true;scoreRef.current=0;streakRef.current=0;bestStreakRef.current=0;livesRef.current=3;totalClicksRef.current=0;successfulHitsRef.current=0;reactionTimesRef.current=[];
    crosshairInitRef.current=false;
    if(canvasRef.current)spawn(canvasRef.current);
    setTimeout(()=>requestPointerLock(),200);
    setTimeout(()=>{crosshairInitRef.current=true;},400);
  }, [spawn,requestPointerLock]);
  ...
  const sharePage = async () => { if(navigator.share){try{await navigator.share({title:'Click Accuracy Elite | SkillDrills',text:'Train mouse precision with a teleporting target.',url:'https://skilldrills.online/drills/motor/hand-eye-coordination/click-accuracy'});}catch(e){}}else{navigator.clipboard.writeText('https://skilldrills.online/drills/motor/hand-eye-coordination/click-accuracy');} };
  const copyPageLink = () => { navigator.clipboard.writeText('https://skilldrills.online/drills/motor/hand-eye-coordination/click-accuracy'); };

  useEffect(()=>()=>{if(timerIntervalRef.current)clearInterval(timerIntervalRef.current);if(teleportIntervalRef.current)clearInterval(teleportIntervalRef.current);if(animationRef.current)cancelAnimationFrame(animationRef.current);document.exitPointerLock();},[]);
...
function RC({label,v,unit='',i,c,d}){const m={yellow:'bg-yellow-500/10 border-yellow-500/30 text-yellow-500',green:'bg-green-500/10 border-green-500/30 text-green-500',orange:'bg-orange-500/10 border-orange-500/30 text-orange-500',cyan:'bg-cyan-500/10 border-cyan-500/30 text-cyan-500',red:'bg-red-500/10 border-red-500/30 text-red-500'};const o=m[c]||m.yellow;const[bg,border,text]=o.split(' ');return(<div className={`flex items-center justify-between p-3 rounded-lg border ${bg} ${border}`}><div className="flex items-center gap-2"><div className={text}>{i}</div><span className={`text-xs sm:text-sm ${d?'text-gray-300':'text-gray-600'}`}>{label}</span></div><span className={`font-bold text-base sm:text-lg ${text}`}>{v}{unit}</span></div>);}