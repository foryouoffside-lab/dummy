...
const GAME_YAWS = {
  ...
  useEffect(() => {
    ...
    function draw(ct) {
      ...
      // Draw Virtual Crosshair reticle (automatic rifle circle)
      if (ch.x > 0 && ch.x < cvs.width && ch.y > 0 && ch.y < cvs.height) {
        ctx.strokeStyle = pointerLocked ? "#00ff88" : "#ffaa00";
        ctx.lineWidth = 1.2;
        
        // Tracking circle reticle
        ctx.beginPath();
        ctx.arc(ch.x + recoilShakeX, ch.y + recoilShakeY, 12, 0, Math.PI*2);
        ctx.stroke();

        ctx.fillStyle = pointerLocked ? "#00ff88" : "#ffaa00";
        ctx.beginPath();
        ctx.arc(ch.x + recoilShakeX, ch.y + recoilShakeY, 1.2, 0, Math.PI*2);
        ctx.fill();
      }
