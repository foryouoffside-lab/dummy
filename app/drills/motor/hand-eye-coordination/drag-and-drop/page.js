...
export default function ExtremeConvergencePage() {
  ...
  // Teleport function - moves both ball and bucket to new random positions
  const teleportPositions = () => {
    if (!isActiveRef.current || gameStateRef.current !== 'playing') return;
    if (ballRef.current.dragging) return; // Don't teleport while dragging
    ...
    if (distance < 200) {
      // Recursive call to reposition if too close
      teleportPositions();
      return;
    }
          ...
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <p className={`${isDarkMode ? 'text-gray-400' : 'text-gray-500'}`}>3 sec per drop • Teleports every 3 sec</p>
              </div>
            </div>
          ...
          {gameState === 'start' && (
            <div className={`absolute inset-0 flex items-center justify-center backdrop-blur-sm rounded-xl z-40 ${isBoxDarkMode ? 'bg-gray-900/95' : 'bg-white/95'}`}>
              <div className={`rounded-2xl p-8 text-center max-w-md mx-4 shadow-xl border ${isBoxDarkMode ? 'bg-gray-800 border-gray-700' : 'bg-white border-gray-200'}`}>
                ...
                <p className={`mb-6 ${isBoxDarkMode ? 'text-gray-300' : 'text-gray-600'}`}>3 seconds to drop • Teleports every 3 sec • 3 lives</p>
                <button 
                  onClick={startGame} 
                  ...
                  <div className="space-y-2">
                    ...
                    <div className="flex items-start gap-2">
                      ...
                      <p className={`text-sm ${isDarkMode ? 'text-gray-300' : 'text-gray-600'}`}>
                        <span className="font-semibold text-green-500">Successful drop = +1 point</span> (must be within 3 seconds)
                      </p>
                    </div>
                    <div className="flex items-start gap-2">
                      <div className="w-5 h-5 rounded-full bg-red-500 flex items-center justify-center text-white text-xs font-bold mt-0.5">3</div>
                      <p className={`text-sm ${isDarkMode ? 'text-gray-300' : 'text-gray-600'}`}>
                        <span className="font-semibold text-red-500">Missing or timeout = -1 life</span> (3 lives total)
                      </p>
                    </div>
                  </div>
                  <div className="space-y-2">
                    <div className="flex items-start gap-2">
                      ...
                      <p className={`text-sm ${isDarkMode ? 'text-gray-300' : 'text-gray-600'}`}>
                        <span className="font-semibold text-orange-500">Positions teleport every 3 seconds!</span> Stay alert!
                      </p>
                    </div>
                    <div className="flex items-start gap-2">
                    ...
                    <div className="flex items-start gap-2">
                      ...
                      <p className={`text-sm ${isDarkMode ? 'text-gray-300' : 'text-gray-600'}`}>Challenge lasts <span className="font-semibold text-yellow-500">60 seconds</span> - Best Score saves locally</p>
                    </div>
                  </div>
                </div>
