...
export default function MomentumTeleportPursuitClient({ copy }: { copy?: { title?: string; subtitle?: string; description?: string } } = {}) {
  ...
  const sharePage = useCallback(async () => {
    ...
    try {
      ...
      const text = 'Momentum Teleport — Free Anticipatory Eye Tracking Drill!';
      if (typeof navigator !== 'undefined' && navigator.share) {
        try {
          await navigator.share({ title: 'Momentum Teleport - Anticipatory Eye Tracking Drill', text, url });
        } catch (e) {}
