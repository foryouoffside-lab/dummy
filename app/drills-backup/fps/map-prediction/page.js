...
import { 
  ...
  Sun, Moon, Eye, Timer, Trophy, Map, Zap, Activity, X, Award, Info, Check
} from 'lucide-react';
...
export default function MapPredictionPage() {
  ...
  useEffect(() => {
    const savedBestScore = localStorage.getItem('mapPredictionDrillBestScore');
    if (savedBestScore) {
  ...
  const updateBestScore = (finalScore) => {
    const currentBestScore = parseInt(localStorage.getItem('mapPredictionDrillBestScore') || '0', 10);
    if (finalScore > currentBestScore) {
      localStorage.setItem('mapPredictionDrillBestScore', finalScore.toString());
  ...
  const calculateScoreFromSpeed = (reactionTimeMs) => {
    if (reactionTimeMs <= 150) return 100;
    if (reactionTimeMs <= 250) return 80;
    if (reactionTimeMs <= 350) return 60;
    if (reactionTimeMs <= 450) return 40;
    if (reactionTimeMs <= 550) return 25;
    if (reactionTimeMs <= 650) return 15;
    return 10;
  };
  ...
  const spawnTarget = () => {
    ...
    pathRef.current.startX = Math.random() * cvs.width;
    pathRef.current.startY = Math.random() * cvs.height;
    
    targetRef.current.entryX = cvs.width * (0.2 + Math.random() * 0.6);
    targetRef.current.entryY = cvs.height * (0.2 + Math.random() * 0.6);
    
    pathRef.current.progress = 0;
  ...
  useEffect(() => {
    const handleMouseDown = () => {
      ...
      const dist = Math.hypot(mouse.x - target.entryX, mouse.y - target.entryY);
      
      if (dist < 40 && target.visible) {
        ...
        showFeedback(`âœ“ ${Math.round(reactionTime)}ms | +${totalPoints}`, 'success');
  ...
  useEffect(() => {
    ...
    function drawPredictionTrail() {
      ...
      if (path.progress < 0.7) {
        ctx.setLineDash([8, 12]);
        ctx.strokeStyle = 'rgba(0, 255, 136, 0.4)';
        ctx.lineWidth = 3;
        ctx.beginPath();
        ctx.moveTo(path.startX, path.startY);
        ctx.lineTo(targetRef.current.entryX, targetRef.current.entryY);
        ctx.stroke();
        ctx.setLineDash([]);
      }
      
      if (path.progress < 0.7) {
        const midX = (path.startX + targetRef.current.entryX) / 2;
        const midY = (path.startY + targetRef.current.entryY) / 2;
        const angle = Math.atan2(targetRef.current.entryY - path.startY, targetRef.current.entryX - path.startX);
        const arrowSize = 12;
      ...
      ctx.beginPath();
      ctx.arc(targetRef.current.entryX, targetRef.current.entryY, 30, 0, Math.PI * 2);
      ctx.strokeStyle = 'rgba(0, 255, 136, 0.6)';
      ctx.lineWidth = 3;
      ctx.stroke();
      
      ctx.beginPath();
      ctx.arc(targetRef.current.entryX, targetRef.current.entryY, 22, 0, Math.PI * 2);
    ...
    function drawTarget() {
      ...
      if (pathRef.current.progress >= 0.95) {
        target.visible = true;
        
        const fadeProgress = Math.min(1, (pathRef.current.progress - 0.95) / 0.05);
        
        ctx.shadowBlur = 30;
        ctx.shadowColor = '#00ff88';
        
        ctx.fillStyle = `rgba(0, 255, 136, ${fadeProgress})`;
        ctx.beginPath();
        ctx.arc(target.entryX, target.entryY, target.r, 0, Math.PI * 2);
        ctx.fill();
        
        ctx.fillStyle = `rgba(255, 255, 255, ${fadeProgress})`;
        ctx.beginPath();
        ctx.arc(target.entryX, target.entryY, target.r * 0.4, 0, Math.PI * 2);
        ctx.fill();
    ...
    function drawCrosshair() {
      ...
      if (mouse.x > 0 && mouse.x < cvs.width && mouse.y > 0 && mouse.y < cvs.height) {
        ...
        ctx.lineTo(target.entryX, target.entryY);
    ...
    function render(now) {
      ...
      if (pathRef.current.progress > 1.3) {
        ...
        <div className="mb-6">
          ...
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-3">
              ...
              <div>
                <h1 className={`text-3xl font-bold ${isDarkMode ? 'text-white' : 'text-gray-900'}`}>Map Prediction</h1>
                <p className={`${isDarkMode ? 'text-gray-400' : 'text-gray-500'}`}>Predict movement â€¢ Pre-fire corners â€¢ Penalty = 100</p>
              </div>
            </div>
          ...
          {gameState === 'start' && (
            <div className={`absolute inset-0 flex items-center justify-center backdrop-blur-sm rounded-xl z-40 ${isBoxDarkMode ? 'bg-gray-900/95' : 'bg-white/95'}`}>
              <div className={`rounded-2xl p-8 text-center max-w-md mx-4 shadow-xl border ${isBoxDarkMode ? 'bg-gray-800 border-gray-700' : 'bg-white border-gray-200'}`}>
                ...
                <h3 className={`text-2xl font-bold mb-2 ${isBoxDarkMode ? 'text-white' : 'text-gray-900'}`}>Map Prediction</h3>
                <p className={`mb-6 ${isBoxDarkMode ? 'text-gray-300' : 'text-gray-600'}`}>60-second challenge â€¢ Predict enemy movement</p>
                <button 
                  onClick={startGame} 
            ...
            <div className={`rounded-xl border overflow-hidden ${isDarkMode ? 'bg-gray-800 border-gray-700' : 'bg-white border-gray-200'}`}>
              ...
              <div className="p-4">
                  ...
                  <div className="space-y-2">
                    <div className="flex items-start gap-2">
                      ...
                      <p className={`text-sm ${isDarkMode ? 'text-gray-300' : 'text-gray-600'}`}>
                        <span className="font-semibold text-green-500">Dashed line shows enemy path</span> â€¢ Arrow indicates direction
                      </p>
                    </div>
                    <div className="flex items-start gap-2">
                      ...
                      <p className={`text-sm ${isDarkMode ? 'text-gray-300' : 'text-gray-600'}`}>
                        <span className="font-semibold text-blue-500">Green ring shows spawn location</span> â€¢ Click when target appears
                      </p>
                    </div>
                    <div className="flex items-start gap-2">
                      ...
                      <p className={`text-sm ${isDarkMode ? 'text-gray-300' : 'text-gray-600'}`}>
                        <span className="font-semibold text-purple-500">Combo bonus: +10 per 3 hits</span> â€¢ 5 combo streak bonus
                      </p>
                    </div>
                  </div>
                  <div className="space-y-2">
                    <div className="flex items-start gap-2">
                      ...
                      <p className={`text-sm ${isDarkMode ? 'text-gray-300' : 'text-gray-600'}`}>
                        <span className="font-semibold text-red-500">Miss/Timeout = -100 points</span> â€¢ Penalty equals max points
                      </p>
                    </div>
                    <div className="flex items-start gap-2">
                      ...
                      <p className={`text-sm ${isDarkMode ? 'text-gray-300' : 'text-gray-600'}`}>
                        <span className="font-semibold text-orange-500">Hit = Speed increases</span> â€¢ Miss = Speed decreases
                      </p>
                    </div>
                    <div className="flex items-start gap-2">
                ...
                <div className={`mt-3 pt-3 border-t text-xs ${isDarkMode ? 'border-gray-700 text-gray-400' : 'border-gray-200 text-gray-500'} flex items-center justify-between`}>
                  <span>ðŸŽ¯ Faster reactions = Higher scores (&lt;150ms: +100)</span>
