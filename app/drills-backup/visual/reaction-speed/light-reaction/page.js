...
import { 
  ...
  Eye, Timer, TrendingUp, AlertCircle, Brain, X, Trophy, Info, Check
} from 'lucide-react';
...
export default function StrobeLatencyPage() {
  ...
  useEffect(() => {
    const handleMouseMove = (e) => {
      ...
      if (gameState === 'playing' && stateRef.current === "WAITING" && !cycleTimeoutRef.current && isActiveRef.current) {
        startCycle();
      }
    };
  ...
  const startCycle = () => {
    ...
    cycleTimeoutRef.current = setTimeout(() => {
      ...
      stateRef.current = "FLASHING";
      startTimeRef.current = performance.now();
      playSound('ready');
      
      flashTimeoutRef.current = setTimeout(() => {
        if (stateRef.current === "FLASHING" && gameState === 'playing' && isActiveRef.current) {
          fail("MISSED");
        }
      }, stayTimeRef.current);
    }, delay);
  ...
  useEffect(() => {
    ...
    function draw() {
      ...
      ctx.fillStyle = stateRef.current === "FLASHING" ? "#000000" : (isBoxDarkMode ? "#333333" : "#999999");
      ctx.fill();
            ...
            <div className={`rounded-xl border overflow-hidden ${isDarkMode ? 'bg-gray-800 border-gray-700' : 'bg-white border-gray-200'}`}>
              ...
              <div className="p-4">
                  ...
                  <div className="space-y-2">
                    <div className="flex items-start gap-2">
                      ...
                      <p className={`text-sm ${isDarkMode ? 'text-gray-300' : 'text-gray-600'}`}>
                        <span className="font-semibold text-blue-500">Wait for ball to flash WHITE</span> • Ball stays gray until signal
                      </p>
                    </div>
                    <div className="flex items-start gap-2">
                      <div className="w-5 h-5 rounded-full bg-green-500 flex items-center justify-center text-white text-xs font-bold mt-0.5">2</div>
                      <p className={`text-sm ${isDarkMode ? 'text-gray-300' : 'text-gray-600'}`}>
                        <span className="font-semibold text-green-500">Click during flash</span> • Faster = More points
                      </p>
                    </div>
                    <div className="flex items-start gap-2">
                      ...
                      <p className={`text-sm ${isDarkMode ? 'text-gray-300' : 'text-gray-600'}`}>
                        <span className="font-semibold text-cyan-500">&lt;150ms: +30 • &lt;200ms: +20 • &lt;250ms: +10</span>
                      </p>
                    </div>
                  </div>
                  <div className="space-y-2">
                    <div className="flex items-start gap-2">
                      ...
                      <p className={`text-sm ${isDarkMode ? 'text-gray-300' : 'text-gray-600'}`}>
                        <span className="font-semibold text-red-500">Penalty = Potential Gain</span> • Miss/Early loses what correct would earn
                      </p>
                    </div>
                    <div className="flex items-start gap-2">
                      ...
                      <p className={`text-sm ${isDarkMode ? 'text-gray-300' : 'text-gray-600'}`}>
                        <span className="font-semibold text-purple-500">Window tightens with success (-5ms)</span> • Loosens with mistakes (+10ms)
                      </p>
                    </div>
                    <div className="flex items-start gap-2">
                ...
                <div className={`mt-3 pt-3 border-t text-xs ${isDarkMode ? 'border-gray-700 text-gray-400' : 'border-gray-200 text-gray-500'} flex items-center justify-between`}>
                  <span>⚪ Goal: Reach 100ms reaction window • 5 streak bonus</span>
                  <span>⚡ 1-3 second random delay between flashes</span>
                </div>
              </div>
            </div>
          </div>
