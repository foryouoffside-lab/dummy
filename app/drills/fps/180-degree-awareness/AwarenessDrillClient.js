...
export default function AwarenessDrillClient() {
  ...
  useEffect(() => { try { const s = localStorage.getItem('awarenessDrillBest'); if(s) { const p = parseInt(s,10); if(!isNaN(p)) setBestScore(p); } } catch(e){} }, []);
  useEffect(() => { gameStateRef.current = gameState; }, [gameState]);
  
  const updateBestScore = useCallback((fs) => { try { const c = parseInt(localStorage.getItem('awarenessDrillBest')||'0',10); if(fs>c) { localStorage.setItem('awarenessDrillBest',fs.toString()); setBestScore(fs); } } catch(e){} }, []);
  ...
  const handleShot = useCallback(() => {
    ...
    if(t.active){
      ...
      if(dist<t.r+10){
        ...
        scoreRef.current+=1;setScore(scoreRef.current);targetsHitRef.current++;setTargetsHit(targetsHitRef.current);
        ...
        t.active=false;spawnTimerRef.current=0;
      }else{handleMiss('Miss');t.active=false;spawnTimerRef.current=0;}
  ...
  useEffect(() => {
    ...
    const loop=(now)=>{if(!isActiveRef.current){animationRef.current=requestAnimationFrame(loop);return;}
      ...
      if(!targetRef.current.active&&canvasSizeRef.current.width>0){spawnTimerRef.current+=dt;if(spawnTimerRef.current>0.25){const{width,height}=canvasSizeRef.current;if(width>0&&height>0){const side=Math.random()>0.5?0.08:0.92;targetRef.current.x=width*side;targetRef.current.y=height*(0.15+Math.random()*0.7);targetRef.current.active=true;targetSpawnTimeRef.current=performance.now();}spawnTimerRef.current=0;}}
      ...
      ctx.fillStyle=isBoxDarkMode?'rgba(0,255,136,0.015)':'rgba(0,200,100,0.015)';ctx.fillRect(0,0,50,cvs.height);ctx.fillRect(cvs.width-50,0,50,cvs.height);
      ctx.strokeStyle='#00ff88';ctx.lineWidth=2;ctx.shadowBlur=6;ctx.shadowColor='#00ff88';
      ctx.beginPath();ctx.moveTo(cvs.width/2-20,cvs.height/2);ctx.lineTo(cvs.width/2+20,cvs.height/2);ctx.moveTo(cvs.width/2,cvs.height/2-20);ctx.lineTo(cvs.width/2,cvs.height/2+20);ctx.stroke();ctx.shadowBlur=0;
      ctx.beginPath();ctx.arc(cvs.width/2,cvs.height/2,4,0,Math.PI*2);ctx.fillStyle='#00ff88';ctx.fill();
      if(targetRef.current.active){
        const t=targetRef.current;
        ctx.shadowBlur=12;ctx.shadowColor='#ffffff';
        const g=ctx.createRadialGradient(t.x-8,t.y-8,0,t.x,t.y,t.r);g.addColorStop(0,'#ffffff');g.addColorStop(0.8,'#e0e0e0');g.addColorStop(1,'#cccccc');
        ctx.fillStyle=g;ctx.beginPath();ctx.arc(t.x,t.y,t.r,0,Math.PI*2);ctx.fill();ctx.shadowBlur=0;
        ctx.strokeStyle='rgba(0,0,0,0.1)';ctx.lineWidth=1.5;ctx.beginPath();ctx.arc(t.x,t.y,t.r,0,Math.PI*2);ctx.stroke();
        ctx.beginPath();ctx.arc(t.x-6,t.y-6,6,0,Math.PI*2);ctx.fillStyle='rgba(255,255,255,0.8)';ctx.fill();
        ctx.beginPath();ctx.arc(t.x,t.y,3,0,Math.PI*2);ctx.fillStyle='#000000';ctx.fill();
      }
  ...
  const startGame = useCallback(() => {
    ...
    targetsHitRef.current=0;missedTargetsRef.current=0;
    targetRef.current={x:0,y:0,r:25,active:false};spawnTimerRef.current=0;
  ...
  const resetGame = useCallback(() => {
    ...
    setScore(0);setTargetsHit(0);setMissedTargets(0);setCombo(0);setBestCombo(0);
    timeLeftRef.current=60;setTimeLeft(60);setBestReaction(0);setAccuracy(100);setLives(3);setFeedback('');
    targetRef.current={x:0,y:0,r:25,active:false};
