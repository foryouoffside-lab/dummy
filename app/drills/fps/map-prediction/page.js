...
import { 
  ...
  Info, Award, Activity, Check, Map, RefreshCw
} from 'lucide-react';
  ...
  useEffect(() => {
    const handleMouseDown = (e) => {
      ...
      if (targetObj.visible) {
        // Target is visible - check if hit
        const dist = Math.hypot(mouse.x - targetObj.entryX, mouse.y - targetObj.entryY);
        ...
        if (dist < 40) {
          ...
          // Clicked but missed the target - penalty
          handleMissClick('Miss');
        }
      } else {
        // Target not visible yet but user clicked - penalty for clicking wrong screen
        handleMissClick('Wrong click');
      }
          ...
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <p className={`${isDarkMode ? 'text-gray-400' : 'text-gray-500'}`}>Predict movement • 5 lives system</p>
              </div>
            </div>
          ...
          {isFullscreen && gameState === 'playing' && (
            <>
              ...
              <div className="absolute top-4 left-4 z-20 bg-black/50 backdrop-blur-sm rounded-lg px-4 py-2 text-white text-sm">
                Score: <span className="text-yellow-400">{score}</span> | Hits: <span className="text-green-400">{successfulHits}</span> | Lives: <span className="text-red-400">{lives}</span>
              </div>
            </>
          )}
          ...
          {gameState === 'start' && (
            <div className={`absolute inset-0 flex items-center justify-center backdrop-blur-sm rounded-xl z-40 ${isBoxDarkMode ? 'bg-gray-900/95' : 'bg-white/95'}`}>
              <div className={`rounded-2xl p-8 text-center max-w-md mx-4 shadow-xl border ${isBoxDarkMode ? 'bg-gray-800 border-gray-700' : 'bg-white border-gray-200'}`}>
                ...
                <p className={`mb-6 ${isBoxDarkMode ? 'text-gray-300' : 'text-gray-600'}`}>60-second challenge • Click target to score • Wrong click costs 1 life</p>
                <button 
                  onClick={startGame} 
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  ...
                  <div className="space-y-2">
                    ...
                    <div className="flex items-start gap-2">
                      ...
                      <p className={`text-sm ${isDarkMode ? 'text-gray-300' : 'text-gray-600'}`}>
                        <span className="font-semibold text-cyan-500">Hit = Speed increases</span> • Miss = Speed decreases
                      </p>
                    </div>
                  </div>
                </div>
                <div className={`mt-3 pt-3 border-t text-xs ${isDarkMode ? 'border-gray-700 text-gray-400' : 'border-gray-200 text-gray-500'} flex items-center justify-between`}>
                  <span>🎯 No penalty for timeout • Target simply respawns</span>
                  <span>⚡ Click ONLY on green target to score</span>
                </div>
              </div>
            </div>
          </div>
