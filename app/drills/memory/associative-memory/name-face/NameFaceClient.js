...
import { 
  ...
  BookOpen, Users, Star, ArrowRight, Share2, Copy, Heart, Hash
} from 'lucide-react';
...
export default function NameFaceClient() {
  ...
  const playSound = useCallback((type) => {
    ...
    try {
      ...
      const freqMap = { correct: 880, wrong: 330, streak: 1046.5 };
      
      osc.frequency.setValueAtTime(freqMap[type] || 880, now);
      gain.gain.setValueAtTime(type === 'wrong' || type === 'streak' ? 0.12 : 0.1, now);
  ...
  const startGame = useCallback(() => {
    ...
    startRound();
    showFeedback('60 seconds • Memorize names with faces!', 'success');
  ...
  }, [isProcessing, totalCorrect, totalAttempts, bestStreak, playSound, showFeedback, startRound]);
    ...
    if (navigator.share) {
      try {
        await navigator.share({
          title: 'Free Name-Face Memory Drill | SkillDrills',
          text: 'Train face-name association memory with emoji profiles. Free!',
          url: 'https://skilldrills.online/drills/memory/associative-memory/name-face'
        });
      } catch (e) {}
    } else {
      navigator.clipboard.writeText('https://skilldrills.online/drills/memory/associative-memory/name-face');
      alert('Link copied!');
    }
  };
    ...
    return (
      <div className="min-h-screen flex items-center justify-center bg-gray-50">
        <div className="text-center">
          ...
          <p className="text-gray-600">Loading name-face memory drill...</p>
        </div>
      </div>
    );
  }
    ...
    <div className={`min-h-screen select-none ${isDarkMode ? 'bg-gray-900' : 'bg-gray-50'}`}>
      <script
        ...
        dangerouslySetInnerHTML={{
          __html: JSON.stringify({
            ...
            "@type": "WebApplication",
            "name": "Name-Face Memory Drill - Face-Name Association Training",
            "url": "https://skilldrills.online/drills/memory/associative-memory/name-face",
            "description": "Free associative memory drill that trains face-name recall using 30 unique emoji profiles with names and roles. 5-second memorization phase studying 3-8 profiles then multiple choice recall from 3 options. Adaptive difficulty increases profile count with correct answers.",
            "applicationCategory": "EducationalApplication",
            "operatingSystem": "All",
            "offers": { "@type": "Offer", "price": "0", "priceCurrency": "USD", "availability": "https://schema.org/OnlineOnly" },
            "author": { "@type": "Organization", "name": "SkillDrills", "url": "https://skilldrills.online" },
            "publisher": { "@type": "Organization", "name": "SkillDrills" },
            "educationalUse": ["Face-Name Association", "Associative Memory", "Social Memory", "Cognitive Training", "Name Recall"],
            "learningResourceType": ["Interactive Exercise", "Memory Drill", "Association Training"],
            "timeRequired": "PT60S",
            "interactivityType": "active",
            "inLanguage": "en-US",
            "teaches": ["Name Recall", "Face Association", "Associative Memory", "Social Recognition", "Working Memory"],
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
            ...
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <p className={`text-sm sm:text-base ${isDarkMode ? 'text-gray-400' : 'text-gray-500'}`}>
                  Free associative recall drill • +1 correct / -1 wrong • Adaptive profiles • 60s
                </p>
              </div>
            </div>
        ...
        <section className="sr-only" aria-label="Drill description for search engines">
          <h2>Name-Face Memory - Associative Face-Name Recall Training</h2>
          <p>
            Train your ability to remember names associated with faces using emoji avatars.
            30 unique profiles with names, roles, and emoji representations.
            5-second memorization phase to study 3-8 profiles, then recall the correct name from 3 options.
            Adaptive difficulty increases profile count with consecutive correct answers.
            +1 point for correct recall, -1 for wrong. 60-second timed challenge with streak tracking.
          </p>
        </section>
            ...
            <div className="w-full max-w-lg text-center">
              ...
              {gameState === 'start' && (
                <div className={`absolute inset-0 flex items-center justify-center backdrop-blur-sm rounded-xl z-40 ${isBoxDarkMode ? 'bg-gray-900/95' : 'bg-white/95'}`}>
                  <div className={`rounded-2xl p-6 sm:p-8 text-center max-w-md mx-4 shadow-xl border ${isBoxDarkMode ? 'bg-gray-800 border-gray-700' : 'bg-white border-gray-200'}`}>
                    <div className="mb-4">
                      <UserCircle className="w-16 h-16 text-amber-500 mx-auto" aria-hidden="true" />
                    </div>
                    <h2 className={`text-2xl font-bold mb-2 ${isBoxDarkMode ? 'text-white' : 'text-gray-900'}`}>
                      Name-Face Memory
                    </h2>
                    <p className={`mb-2 ${isBoxDarkMode ? 'text-gray-300' : 'text-gray-600'}`}>
                      60-second challenge • 30 unique profiles • Adaptive difficulty
                    </p>
                    <p className={`mb-6 text-sm ${isBoxDarkMode ? 'text-gray-400' : 'text-gray-500'}`}>
                      Memorize names with emoji faces for 5 seconds, then pick the correct name. Profiles increase as you improve.
                    </p>
                    <button 
                      onClick={startGame} 
                      ...
                      aria-label="Start name-face memory drill"
                    >
                      Start Free Drill
                    </button>
                  </div>
              ...
              {gameState === 'playing' && phase === "learning" && (
                <div className="space-y-4">
                  <div className="flex items-center justify-between mb-4">
                    <span className={`text-sm font-bold px-3 py-1 rounded-full ${isBoxDarkMode ? 'bg-amber-500/20 text-amber-400' : 'bg-amber-100 text-amber-600'}`}>
                    ...
                    <button 
                      ...
                      aria-label="Skip memorization and start testing"
                    >
                      <SkipForward className="w-3.5 h-3.5" />
                      Skip
                    </button>
                  </div>
                  <p className={`text-sm mb-2 ${isBoxDarkMode ? 'text-gray-400' : 'text-gray-500'}`}>
                    Memorize these {profileCount} profiles:
                  </p>
                  <div className="grid grid-cols-1 gap-2">
                    {currentProfiles.map((profile, i) => (
                      <div key={i} className={`p-3 rounded-xl flex items-center gap-4 transition-all ${
                        isBoxDarkMode ? 'bg-white/10 border border-white/10' : 'bg-amber-50 border border-amber-200'
                      }`}>
                        <span className="text-4xl" role="img" aria-label={profile.role}>{profile.emoji}</span>
              ...
              {gameState === 'playing' && phase === "testing" && testProfile && (
                <div className="space-y-6">
                  <div className="text-7xl mb-2" role="img" aria-label={testProfile.role}>{testProfile.emoji}</div>
                  ...
                  <div className="space-y-3" role="radiogroup" aria-label="Name options">
                    {options.map((name, i) => (
                      <button
                        key={i}
                        ...
                        className={`w-full font-semibold py-4 rounded-xl transition text-lg disabled:opacity-50 ${
                          isBoxDarkMode 
                            ? 'bg-white/10 hover:bg-white/20 text-white border border-white/10 hover:border-amber-400/50' 
                            : 'bg-amber-100 hover:bg-amber-200 text-gray-900 border border-amber-200 hover:border-amber-400'
                        } focus:outline-none focus:ring-2 focus:ring-amber-500`}
              ...
              {gameState === 'playing' && phase === "result" && testProfile && (
                <div className="space-y-4">
                  <div className="text-5xl" role="img" aria-label={testProfile.role}>{testProfile.emoji}</div>
            ...
            <div className="absolute inset-0 flex items-center justify-center backdrop-blur-sm rounded-xl z-40" style={{ background: isBoxDarkMode ? 'rgba(10,10,26,0.95)' : 'rgba(255,251,235,0.95)' }}>
              <div className={`rounded-2xl p-6 sm:p-8 shadow-xl border w-full max-w-[480px] mx-4 ${isBoxDarkMode ? 'bg-gray-800 border-gray-700' : 'bg-white border-gray-200'}`}>
                ...
                <p className={`text-center text-sm mb-6 ${isBoxDarkMode ? 'text-gray-400' : 'text-gray-500'}`}>
                  Keep practicing to strengthen your face-name association memory.
                </p>
                ...
                <div className="grid grid-cols-2 gap-3 mb-6">
                  ...
                  <ResultCard label="Rounds" value={roundsCompleted} icon={<UserCircle className="w-4 h-4" />} color="emerald" isDark={isBoxDarkMode} />
                  <ResultCard label="Max Profiles" value={profileCount} icon={<Brain className="w-4 h-4" />} color="cyan" isDark={isBoxDarkMode} />
                </div>
                  ...
                  <div className="space-y-3">
                    <div className="flex items-start gap-2">
                      ...
                      <p className={`text-sm ${isDarkMode ? 'text-gray-300' : 'text-gray-600'}`}>
                        Memorize <span className="font-semibold text-amber-500">emoji, name, and role</span> for 5 seconds
                      </p>
                    </div>
                    <div className="flex items-start gap-2">
                      ...
                      <p className={`text-sm ${isDarkMode ? 'text-gray-300' : 'text-gray-600'}`}>
                        Correct name = <span className="font-semibold text-green-500">+1 point</span> | Wrong = -1
                      </p>
                    </div>
                    <div className="flex items-start gap-2">
                      ...
                      <p className={`text-sm ${isDarkMode ? 'text-gray-300' : 'text-gray-600'}`}>
                        Emoji + role shown • <span className="font-semibold text-red-500">Pick from 3 names</span>
                      </p>
                    </div>
                  </div>
                  <div className="space-y-3">
                    ...
                    <div className="flex items-start gap-2">
                      ...
                      <p className={`text-sm ${isDarkMode ? 'text-gray-300' : 'text-gray-600'}`}>
                        Profiles increase <span className="font-semibold text-purple-500">3 → 8</span> with consecutive correct answers
                      </p>
                    </div>
                    <div className="flex items-start gap-2">
                      ...
                      <p className={`text-sm ${isDarkMode ? 'text-gray-300' : 'text-gray-600'}`}>
                        Challenge lasts <span className="font-semibold text-yellow-500">60 seconds</span> • Auto-advances
                      </p>
                    </div>
                  </div>
                </div>
        ...
        {!isFullscreen && (
          <section className="mt-8" aria-label="About this name-face memory drill">
            <div className={`rounded-xl border overflow-hidden ${isDarkMode ? 'bg-gray-800 border-gray-700' : 'bg-white border-gray-200'}`}>
              <div className={`px-4 py-3 border-b ${isDarkMode ? 'border-gray-700 bg-gray-800/50' : 'border-gray-200 bg-gray-50'}`}>
                <div className="flex items-center gap-2">
                  <GraduationCap className={`w-5 h-5 ${isDarkMode ? 'text-amber-400' : 'text-amber-600'}`} aria-hidden="true" />
                  <h2 className={`font-semibold ${isDarkMode ? 'text-white' : 'text-gray-900'}`}>About This Free Name-Face Memory Drill</h2>
                </div>
              </div>
              <div className="p-5">
                <p className={`text-sm leading-relaxed mb-5 ${isDarkMode ? 'text-gray-300' : 'text-gray-600'}`}>
                  This free name-face memory drill trains one of the most practical memory skills: remembering names associated with faces. Using 30 unique emoji profiles across diverse professions, each round challenges you to memorize multiple name-face-role associations in just 5 seconds. Then you must pick the correct name from 3 options when shown only the emoji and role. The adaptive difficulty system increases from 3 to 8 profiles as you answer correctly, keeping you challenged at your current skill level. Perfect for networking professionals, teachers, social workers, and anyone wanting to never forget a name again.
                </p>
                <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 mb-5">
                  <div className={`p-4 rounded-xl border ${isDarkMode ? 'bg-gray-700/50 border-gray-600' : 'bg-amber-50 border-amber-100'}`}>
                    ...
                    <p className={`text-xs leading-relaxed ${isDarkMode ? 'text-gray-400' : 'text-gray-500'}`}>
                      Networking professionals, teachers, sales representatives, healthcare workers, social workers, event organizers, and anyone who regularly meets new people and needs to remember names and faces.
                    </p>
                  </div>
                  <div className={`p-4 rounded-xl border ${isDarkMode ? 'bg-gray-700/50 border-gray-600' : 'bg-green-50 border-green-100'}`}>
                    ...
                    <p className={`text-xs leading-relaxed ${isDarkMode ? 'text-gray-400' : 'text-gray-500'}`}>
                      Face-name association, social memory, rapid name encoding, visual-verbal association, working memory capacity, and the ability to recall names under time pressure.
                    </p>
                  </div>
                  <div className={`p-4 rounded-xl border ${isDarkMode ? 'bg-gray-700/50 border-gray-600' : 'bg-purple-50 border-purple-100'}`}>
                    ...
                    <p className={`text-xs leading-relaxed ${isDarkMode ? 'text-gray-400' : 'text-gray-500'}`}>
                      Score, accuracy percentage, streak of consecutive correct answers, profile count progression, rounds completed, and best performance records saved locally.
                    </p>
                  </div>
                </div>
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                  <div className={`p-4 rounded-xl border ${isDarkMode ? 'bg-gray-700/50 border-gray-600' : 'bg-yellow-50 border-yellow-100'}`}>
                    <div className="flex items-center gap-2 mb-3">
                      ...
                      <h3 className={`text-sm font-semibold ${isDarkMode ? 'text-white' : 'text-gray-900'}`}>Why Practice Name-Face Memory?</h3>
                    </div>
                    <ul className={`text-xs space-y-2 ${isDarkMode ? 'text-gray-400' : 'text-gray-600'}`}>
                      ...
                      <li className="flex items-start gap-2">
                        <CheckCircle2 className="w-3 h-3 text-yellow-500 mt-0.5 flex-shrink-0" />
                        Strengthens the brain's ability to link visual and verbal information
                      </li>
                      <li className="flex items-start gap-2">
                        <CheckCircle2 className="w-3 h-3 text-yellow-500 mt-0.5 flex-shrink-0" />
                        Transfers to real-world situations like meetings and networking events
                      </li>
                    </ul>
                  </div>
                  <div className={`p-4 rounded-xl border ${isDarkMode ? 'bg-gray-700/50 border-gray-600' : 'bg-orange-50 border-orange-100'}`}>
                    ...
                    <ol className={`text-xs space-y-2 ${isDarkMode ? 'text-gray-400' : 'text-gray-600'}`}>
                      <li className="flex items-start gap-2">
                        ...
                        Study all three pieces of information: emoji, name, and role
                      </li>
                      <li className="flex items-start gap-2">
                        ...
                        Create mental associations between the emoji and the name
                      </li>
                      <li className="flex items-start gap-2">
                        ...
                        Aim for consecutive correct answers to increase profile count
                      </li>
                      <li className="flex items-start gap-2">
        ...
        {!isFullscreen && (
          <section className="mt-8" aria-label="Related memory and cognitive drills">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
              ...
              <Link href="/drills/memory/short-term/digit-span" className={`group relative overflow-hidden rounded-xl border transition-all duration-300 hover:shadow-lg hover:-translate-y-1 ${isDarkMode ? 'bg-gray-800 border-gray-700 hover:border-purple-500' : 'bg-white border-gray-200 hover:border-purple-300'}`}>
                ...
                <div className="p-4">
                  ...
                  <h3 className={`font-semibold text-sm mb-1 ${isDarkMode ? 'text-white group-hover:text-purple-400' : 'text-gray-900 group-hover:text-purple-600'} transition-colors`}>Digit Span</h3>
              ...
              <Link href="/drills/memory/spatial-memory/pattern-recall" className={`group relative overflow-hidden rounded-xl border transition-all duration-300 hover:shadow-lg hover:-translate-y-1 ${isDarkMode ? 'bg-gray-800 border-gray-700 hover:border-orange-500' : 'bg-white border-gray-200 hover:border-orange-300'}`}>
                ...
                <div className="p-4">
                  ...
                  <h3 className={`font-semibold text-sm mb-1 ${isDarkMode ? 'text-white group-hover:text-orange-400' : 'text-gray-900 group-hover:text-orange-600'} transition-colors`}>Pattern Recall</h3>
              ...
              <Link href="/drills/memory/associative/paired-associates" className={`group relative overflow-hidden rounded-xl border transition-all duration-300 hover:shadow-lg hover:-translate-y-1 ${isDarkMode ? 'bg-gray-800 border-gray-700 hover:border-cyan-500' : 'bg-white border-gray-200 hover:border-cyan-300'}`}>
                ...
                <div className="p-4">
                  ...
                  <h3 className={`font-semibold text-sm mb-1 ${isDarkMode ? 'text-white group-hover:text-cyan-400' : 'text-gray-900 group-hover:text-cyan-600'} transition-colors`}>Paired Associates</h3>
                  <p className={`text-xs leading-relaxed ${isDarkMode ? 'text-gray-500' : 'text-gray-400'}`}>Learn word pairs and recall the partner when shown one item.</p>
              ...
              <Link href="/drills/memory/episodic/scene-recall" className={`group relative overflow-hidden rounded-xl border transition-all duration-300 hover:shadow-lg hover:-translate-y-1 ${isDarkMode ? 'bg-gray-800 border-gray-700 hover:border-teal-500' : 'bg-white border-gray-200 hover:border-teal-300'}`}>
                ...
                <div className="p-4">
                  ...
                  <h3 className={`font-semibold text-sm mb-1 ${isDarkMode ? 'text-white group-hover:text-teal-400' : 'text-gray-900 group-hover:text-teal-600'} transition-colors`}>Scene Recall</h3>
              ...
              <Link href="/drills/memory/prospective/time-based" className={`group relative overflow-hidden rounded-xl border transition-all duration-300 hover:shadow-lg hover:-translate-y-1 ${isDarkMode ? 'bg-gray-800 border-gray-700 hover:border-indigo-500' : 'bg-white border-gray-200 hover:border-indigo-300'}`}>
                ...
                <div className="p-4">
                  ...
                  <h3 className={`font-semibold text-sm mb-1 ${isDarkMode ? 'text-white group-hover:text-indigo-400' : 'text-gray-900 group-hover:text-indigo-600'} transition-colors`}>Prospective Memory</h3>
                  <p className={`text-xs leading-relaxed ${isDarkMode ? 'text-gray-500' : 'text-gray-400'}`}>Remember to perform actions at specific times while doing other tasks.</p>
            ...
            <div className="max-w-7xl mx-auto">
              ...
              <div className="border-t border-gray-800 pt-8 text-center">
                ...
                <p className="text-xs max-w-2xl mx-auto leading-relaxed mb-6">
                  Free online name-face memory drill for associative memory training. 30 unique emoji profiles with names and roles across diverse professions. 5-second memorization phase with multiple choice recall from 3 options. Adaptive difficulty increases from 3 to 8 profiles. Perfect for networking professionals, teachers, and anyone wanting to improve name recall. No registration required. More free memory drills at skilldrills.online.
                </p>
                <div className="flex items-center justify-center gap-5 flex-wrap">
                  <button onClick={sharePage} className="text-gray-500 hover:text-white transition-colors" title="Share this drill" aria-label="Share this free name-face memory drill">
