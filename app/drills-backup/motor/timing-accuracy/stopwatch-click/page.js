...
export default function SustainedInterceptElitePage() {
  ...
  // Calculate points based on how close to target
  const calculatePoints = (error) => {
    const errorAbs = Math.abs(error);
    const targetSec = targetTimeRef.current / 1000;
    
    if (errorAbs < 25) {
      // PERFECT: Within 25ms
      return Math.floor(800 * targetSec);
    } else if (errorAbs < 75) {
      // GOOD: Within 75ms
      return Math.floor(200 * targetSec);
    } else if (errorAbs < 150) {
      // OK: Within 150ms
      return Math.floor(50 * targetSec);
    } else {
      // MISS: Outside range
      return 0;
    }
  ...
  const addPenalty = () => {
    ...
    const penaltyPoints = Math.floor(200 * (targetTimeRef.current / 1000));
  ...
  useEffect(() => {
    const handleMouseDown = () => {
      ...
      if (stateRef.current === 'TIMER') {
        ...
        const errorAbs = Math.abs(error);
        const pointsEarned = calculatePoints(error);
        ...
        if (pointsEarned > 0) {
          ...
          if (newStreak % 5 === 0 && newStreak > 0) {
            ...
            showFeedback(`✓ ${rating}! +${pointsEarned}`, 'success');
          }
        } else {
          addPenalty();
        }
          ...
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <p className={`${isDarkMode ? 'text-gray-400' : 'text-gray-500'}`}>Click at target time • Score based on accuracy</p>
              </div>
            </div>
            ...
            <div className={`rounded-xl border overflow-hidden ${isDarkMode ? 'bg-gray-800 border-gray-700' : 'bg-white border-gray-200'}`}>
              ...
              <div className="p-4">
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  <div className="space-y-2">
                    ...
                    <div className="flex items-start gap-2">
                      ...
                      <p className={`text-sm ${isDarkMode ? 'text-gray-300' : 'text-gray-600'}`}>
                        <span className="font-semibold text-green-500">Click when timer matches target</span> • PERFECT (&lt;25ms): max points
                      </p>
                    </div>
                    <div className="flex items-start gap-2">
                      ...
                      <p className={`text-sm ${isDarkMode ? 'text-gray-300' : 'text-gray-600'}`}>
                        <span className="font-semibold text-cyan-500">GOOD (&lt;75ms) • OK (&lt;150ms)</span> • Scaled scoring
                      </p>
                    </div>
                  </div>
                  <div className="space-y-2">
                    <div className="flex items-start gap-2">
                      ...
                      <p className={`text-sm ${isDarkMode ? 'text-gray-300' : 'text-gray-600'}`}>
                        <span className="font-semibold text-red-500">MISS (≥150ms): Penalty</span> • Loses points
                      </p>
                    </div>
                    <div className="flex items-start gap-2">
                      ...
                      <p className={`text-sm ${isDarkMode ? 'text-gray-300' : 'text-gray-600'}`}>
                        <span className="font-semibold text-purple-500">Longer targets = More points</span> • Streak bonus at 5
                      </p>
                    </div>
                    <div className="flex items-start gap-2">
                ...
                <div className={`mt-3 pt-3 border-t text-xs ${isDarkMode ? 'border-gray-700 text-gray-400' : 'border-gray-200 text-gray-500'} flex items-center justify-between`}>
                  ...
                  <span>⚡ Click RESULT screen for next round</span>
                </div>
              </div>
            </div>
          </div>
