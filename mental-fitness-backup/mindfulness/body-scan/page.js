  ...
  const runStep = () => {
    ...
    if (currentIdxRef.current >= parts.length) {
      ...
      if (comboRef.current % 3 === 0) {
        playSound('combo');
        showFeedback(`ðŸ”¥ ${comboRef.current} Scan Combo! +10`, 'success');
      }
      return;
    }
            ...
            <div className={`rounded-xl border overflow-hidden ${isDarkMode ? 'bg-gray-800 border-gray-700' : 'bg-white border-gray-200'}`}>
              ...
              <div className="p-4">
                ...
                <div className={`mt-3 pt-3 border-t text-xs ${isDarkMode ? 'border-gray-700 text-gray-400' : 'border-gray-200 text-gray-500'} flex items-center justify-between`}>
                  <span>ðŸ§˜ Cultivates body awareness and present-moment focus</span>
                  <span>ðŸŽµ Gentle audio cues at each transition</span>
                </div>
              </div>
            </div>
          </div>
