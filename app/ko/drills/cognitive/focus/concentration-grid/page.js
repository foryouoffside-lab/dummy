...
export default function ConcentrationGridPageKo() {
  ...
  const concentrationGridGuide = {
    ...
    intro: [
      "슐테 테이블(Schulte Table)은 1962년 독일 튀빙겐 대학의 정신과 의사 발터 슐테(Walter Schulte)가 고안한 고전적인 심리 진단 및 시지각 훈련 도구입니다. 원래 주의력의 안정성과 작업 능력을 평가하기 위해 만들어졌으나, 현대에는 속독 능력 향상, 시각 탐색 속도 강화, 주변 시야 확장 훈련의 세계 표준으로 널리 활용되고 있습니다.",
      "시각 인지 심리학에서는 선명하게 초점이 맺히는 중심와(Fovea) 시야와 그 바깥의 주변 시야를 구분합니다 (Rayner, 1998). 비훈련자는 글이나 시각 정보를 접할 때 안구를 끊임없이 도약(Saccade)시키며 일일이 초점을 맞추지만, 슐테 격자 훈련을 거친 사람은 중심 시선을 고정한 채 주변 시야만으로 넓은 영역의 복수 정보를 동시에 병렬 처리할 수 있습니다.",
      "앤 트리즈먼(Anne Treisman)의 특징 통합 이론(Feature Integration Theory, 1980)과 제레미 울프(Jeremy M. Wolfe)의 유도 탐색 모델(Guided Search, 2007)에 따르면, 고밀도 숫자들 사이에서 목표 숫자를 찾아내는 과정은 시각 피질의 선택적 주의 억제와 신호 증폭을 극대화합니다.",
      "SkillDrills 집중력 격자의 모든 반응 시간은 performance.now() 고해상도 디지털 시간 측정(Woods et al., 2015)을 거쳐 오차 없이 정확하게 기록됩니다."
    ],
    benchmarks: {
      title: "슐테 테이블 (5x5 표준 격자) 성능 벤치마크",
      headers: ["퍼포먼스 티어", "완료 시간 (5x5 기준)", "안구 운동 방식", "인지 퍼포먼스 특성"],
      rows: [
        ["Tier 1 (마스터 / 속독 엘리트)", "22초 미만", "완벽한 중앙 고정 (순수 주변 시야)", "최상위 1% 시야각; 1분에 600단어 이상의 텍스트를 무리 없이 속독"],
        ["Tier 2 (숙련자 / 엘리트 선수급)", "22 – 32초", "넓은 시야각, 최소한의 미세 사케드", "탁월한 시각 탐색 속도; E-스포츠 및 구기 종목 반응성이 극대화된 상태"],
        ["Tier 3 (양호한 일반인)", "32 – 45초", "중간 수준 시야각과 부분적 눈동자 이동", "안정적인 주의 집중력 유지; 분당 250~350단어 수준의 일반 독서 속도"],
        ["Tier 4 (초급 / 일반 기준치)", "45 – 60초", "숫자마다 빈번한 안구 도약 발생", "직렬 순차 탐색 의존; 장시간 문서 열람 시 시각 피로도가 쉽게 누적됨"],
        ["Tier 5 (입문 / 시각 피로 상태)", "60초 초과", "좁은 터널 시야, 파편화된 탐색", "주변 시야 활용 부족; 기초적인 시각 집중력 반복 훈련 권장"]
      ],
      note: "5x5 표준 격자(1~25 숫자)를 중앙 고정 시선으로 완료했을 때의 공인 벤치마크 기준입니다 (Rayner, 1998; Lu et al., 2022)."
    },
    techniques: {
