...
export default function AwarenessDrillPage() {
  ...
  useEffect(() => {
    ...
    function drawTarget() {
      ...
      // White ball - constant size
      ctx.beginPath();
      ctx.arc(target.x, target.y, target.r, 0, Math.PI * 2);
      
      const gradient = ctx.createRadialGradient(target.x - 8, target.y - 8, 0, target.x, target.y, target.r);
      ...
      // Inner highlight
      ctx.beginPath();
      ctx.arc(target.x - 6, target.y - 6, 6, 0, Math.PI * 2);
      ctx.fillStyle = 'rgba(255, 255, 255, 0.8)';
      ctx.fill();
      
      // Small black pupil/center dot
      ctx.beginPath();
          ...
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <p className={`${isDarkMode ? 'text-gray-400' : 'text-gray-500'}`}>Extreme edge targets • 5 lives system</p>
              </div>
            </div>
          ...
          {isFullscreen && gameState === 'playing' && (
            <>
              ...
              <div className="absolute top-4 left-4 z-20 bg-black/50 backdrop-blur-sm rounded-lg px-4 py-2 text-white text-sm">
                Score: <span className="text-yellow-400">{score}</span> | Hits: <span className="text-green-400">{targetsHit}</span> | Lives: <span className="text-red-400">{lives}</span>
              </div>
            </>
          )}
          ...
          {gameState === 'start' && (
            <div className={`absolute inset-0 flex items-center justify-center backdrop-blur-sm rounded-xl z-40 ${isBoxDarkMode ? 'bg-gray-900/95' : 'bg-white/95'}`}>
              <div className={`rounded-2xl p-8 text-center max-w-md mx-4 shadow-xl border ${isBoxDarkMode ? 'bg-gray-800 border-gray-700' : 'bg-white border-gray-200'}`}>
                ...
                <p className={`mb-6 ${isBoxDarkMode ? 'text-gray-300' : 'text-gray-600'}`}>60-second challenge • Extreme edge targets • 5 lives system</p>
                <button 
                  onClick={startGame} 
                  ...
                  <div className="space-y-2">
                    <div className="flex items-start gap-2">
                      ...
                      <p className={`text-sm ${isDarkMode ? 'text-gray-300' : 'text-gray-600'}`}>
                        <span className="font-semibold text-green-500">White targets spawn at extreme edges</span> • Train peripheral vision
                      </p>
                    </div>
                    <div className="flex items-start gap-2">
