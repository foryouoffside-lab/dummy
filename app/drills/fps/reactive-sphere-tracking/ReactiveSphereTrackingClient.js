...
// Dynamic level configurations based on score (increases every 500 score)
const getLevelStats = (score) => {
  const lvl = Math.floor(score / 500) + 1;
  let radius, speed, minDelay, maxDelay, lerpFactor;
  ...
  if (lvl === 1) {
    ...
    minDelay = 450;
    maxDelay = 800;
    lerpFactor = 12; // Smoother, wider arcs
  } else if (lvl === 2) {
    radius = 20;
    speed = 420;
    minDelay = 300;
    maxDelay = 600;
    lerpFactor = 16; // Faster strafe response
  } else if (lvl === 3) {
    radius = 17;
    speed = 500;
    minDelay = 250;
    maxDelay = 500;
    lerpFactor = 20; // High speed counter strafes + acceleration
  } else if (lvl === 4) {
    radius = 14;
    speed = 580;
    minDelay = 180;
    maxDelay = 350;
    lerpFactor = 24; // Micro jukes, small corrections required
  } else if (lvl === 5) {
    radius = 11;
    speed = 660;
    minDelay = 120;
    maxDelay = 250;
    lerpFactor = 28; // Tracer-style rapid counter strafes
  } else {
    // Level 6+ infinite progression
    const scaleFactor = lvl - 5;
    radius = Math.max(8, 11 - scaleFactor * 0.5);
    speed = 660 + scaleFactor * 45;
    minDelay = Math.max(80, 120 - scaleFactor * 8);
    maxDelay = Math.max(160, 250 - scaleFactor * 15);
    lerpFactor = Math.min(40, 28 + scaleFactor * 2);
  }
  
  return { lvl, radius, speed, minDelay, maxDelay, lerpFactor };
};
...
const getSuggestion = (rank, accuracy, stability) => {
  ...
  return "Excellent baseline. Focus on maintaining a tight lock during rapid counter-strafes (ADAD transitions) to build higher combo chains.";
};
...
export default function ReactiveSphereTrackingClient() {
  ...
  const engine = useRef({
    target: { x: 0, y: 0, vx: 0, vy: 0, radius: 24 },
    ...
    totalSwaps: 0,
    successfulSwaps: 0,
    microCorrectionSum: 0,
    microCorrectionCount: 0,
    
    // Reaction & swap tracking
    lastSwapTime: 0,
    nextSwapDelay: 600,
    isAdjusting: false,
    swapTime: 0,
  ...
  useEffect(() => {
    try {
      const savedSens = localStorage.getItem('reactiveSphere_sens');
      if (savedSens) setUniversalSens(parseFloat(savedSens));
      const savedBest = localStorage.getItem('reactiveSphere_bestScore');
      if (savedBest) setBestScore(parseInt(savedBest, 10));
      const savedSpray = localStorage.getItem('reactiveSphere_spray');
  ...
  useEffect(() => {
    if (gameState !== 'playing') {
      try { 
        localStorage.setItem('reactiveSphere_sens', universalSens.toString());
        localStorage.setItem('reactiveSphere_spray', weaponSpray.toString());
      } catch (e) {}
    }
  ...
  const playSound = useCallback((type) => {
    ...
    try {
      ...
      if (type === 'score') {
        ...
        o.frequency.exponentialRampToValueAtTime(880.00, now + 0.12); 
  ...
  const endGame = useCallback(() => {
    ...
    const finalAccuracy = weaponSpray 
      ...
      : (e.totalTicks > 0 ? Math.round((e.onTargetTicks / e.totalTicks) * 100) : 0);
    ...
    const avgReaction = e.reactionTimes.length > 0 ? Math.round(e.reactionTimes.reduce((a,b)=>a+b,0) / e.reactionTimes.length) : 0;
    const longestTrack = Math.round((e.longestContinuousTrack || 0) * 10) / 10;
    const avgReacquisition = e.reacquisitionTimes.length > 0 ? Math.round(e.reacquisitionTimes.reduce((a,b)=>a+b,0) / e.reacquisitionTimes.length) : 0;
    const dirSuccess = e.totalSwaps > 0 ? Math.round((e.successfulSwaps / e.totalSwaps) * 100) : 0;
    ...
    setAnalytics({
      ...
      longestContinuousTrack: longestTrack,
      avgReacquisitionTime: avgReacquisition,
      directionChangeSuccess: dirSuccess,
    ...
    setBestScore(prev => {
      if (e.score > prev) {
        setIsNewBest(true);
        try { localStorage.setItem('reactiveSphere_bestScore', e.score.toString()); } catch(e){}
        return e.score;
      }
      return prev;
    });
  ...
  const startGame = useCallback(() => {
    ...
    const spawnY = containerRef.current?.clientHeight / 2 || 225;
    ...
    engine.current = {
      ...
      speed: 350,
      score: 0, combo: 0, bestCombo: 0, timeLeft: DRILL_DURATION,
      
      // Reset accumulators
      totalTicks: 0, onTargetTicks: 0,
      continuousTrackTime: 0,
      comboAccumulator: 0,
      longestContinuousTrack: 0,
      reacquisitionTimes: [],
      ...
      // Swap adjustments
      lastSwapTime: performance.now(), nextSwapDelay: 600,
      isAdjusting: true, swapTime: performance.now(), swapHasLeft: false,
      reactionTimes: [],
      
      deviationSum: 0, deviationCount: 0,
      wasOffTarget: true, timeOfLeavingTarget: performance.now(),
      level: 1,
    ...
    const loop = (time) => {
      ...
      if (gameState === 'playing' && pointerLocked) {
        ...
        if (progressBarRef.current) {
          ...
          progressBarRef.current.className = `h-full ${e.timeLeft <= 10 ? 'bg-red-500 animate-pulse' : 'bg-blue-500'}`;
        }
        ...
        if (weaponSpray && e.isMouseDown) {
          if (time - e.lastFireTime >= 80) {
            ...
            const dist = Math.hypot(e.crosshair.x - t.x, e.crosshair.y - t.y);
            ...
            if (isTargetHit) {
              ...
              createExplosion(e.crosshair.x, e.crosshair.y, '#00ff88');
      ...
      // Vector direction change timing
      if (time - e.lastSwapTime > e.nextSwapDelay) {
        e.totalSwaps++;
        e.lastSwapTime = time;
        e.isAdjusting = true;
        e.swapTime = time;
        ...
        if (stats.lvl === 1) {
          ...
          const isADAD = Math.random() < 0.75;
          if (isADAD) {
            const currentVx = t.vx || 1;
            const goLeft = currentVx > 0;
            const deviation = (Math.random() - 0.5) * 0.4;
            angle = goLeft ? Math.PI + deviation : deviation;
          } else {
            angle = Math.random() * Math.PI * 2;
          }
        }
        ...
        if (stats.lvl >= 3) {
          const minM = stats.lvl >= 6 ? 0.7 : 0.8;
          const maxM = stats.lvl >= 6 ? 1.5 : 1.3;
          speedMultiplier = minM + Math.random() * (maxM - minM);
        }

        e.speed = stats.speed * speedMultiplier;
        t.targetVx = Math.cos(angle) * e.speed;
        t.targetVy = Math.sin(angle) * e.speed;
        
        e.nextSwapDelay = stats.minDelay + Math.random() * (stats.maxDelay - stats.minDelay);
      ...
      // Physics vector interpolation (Lerp Velocity)
      if (t.targetVx === undefined) {
        t.targetVx = t.vx || 0;
        t.targetVy = t.vy || 0;
      }
      t.vx += (t.targetVx - t.vx) * Math.min(1.0, stats.lerpFactor * deltaSec);
      t.vy += (t.targetVy - t.vy) * Math.min(1.0, stats.lerpFactor * deltaSec);

      t.x += t.vx * deltaSec;
      t.y += t.vy * deltaSec;

      // Elastic borders collision
      if (t.x < stats.radius) { t.x = stats.radius; t.vx *= -1; t.targetVx *= -1; }
      if (t.x > cvs.width - stats.radius) { t.x = cvs.width - stats.radius; t.vx *= -1; t.targetVx *= -1; }
      if (t.y < stats.radius) { t.y = stats.radius; t.vy *= -1; t.targetVy *= -1; }
      if (t.y > cvs.height - stats.radius) { t.y = cvs.height - stats.radius; t.vy *= -1; t.targetVy *= -1; }

      // Target verification & scoring engine
      if (gameState === 'playing' && pointerLocked) {
        const dist = Math.hypot(e.crosshair.x - t.x, e.crosshair.y - t.y);
        ...
        if (isHit) {
          ...
          if (e.continuousTrackTime >= 1000) {
            ...
            const currentAcc = weaponSpray 
              ...
              : (e.totalTicks > 0 ? Math.round((e.onTargetTicks / e.totalTicks) * 100) : 100);

            let pts = 1;
            if (currentAcc >= 90) pts = 5;
            else if (currentAcc >= 80) pts = 4;
            else if (currentAcc >= 70) pts = 3;
            else if (currentAcc >= 60) pts = 2;
          ...
          e.comboAccumulator += deltaTimeMs;
          if (e.comboAccumulator >= 100) {
            e.comboAccumulator -= 100;
            e.combo++;
            if (e.combo > e.bestCombo) e.bestCombo = e.combo;
            
            const currentTrackSec = e.combo * 0.1;
            if (currentTrackSec > e.longestContinuousTrack) {
              e.longestContinuousTrack = currentTrackSec;
            }
          ...
          if (e.isAdjusting) {
            if (e.swapHasLeft) {
              const delay = time - e.swapTime;
              e.reactionTimes.push(delay);
              e.isAdjusting = false;
              if (delay <= 350) e.successfulSwaps++;
            } else {
              if (time - e.swapTime >= 150) {
                e.successfulSwaps++;
                e.isAdjusting = false;
              }
            }
          ...
          if (e.wasOffTarget) {
            const reaq = time - e.timeOfLeavingTarget;
            e.reacquisitionTimes.push(reaq);
            e.wasOffTarget = false;
          }

        } else {
          // Off target/Not firing: break combo immediately
          if (e.combo > 0) {
            e.combo = 0;
            e.comboAccumulator = 0;
          }
          ...
          if (dist <= stats.radius * 2.5) {
            const microRating = Math.max(0, 100 - ((dist - stats.radius) / (stats.radius * 1.5)) * 100);
            e.microCorrectionSum += microRating;
            e.microCorrectionCount++;
          }

          if (!e.wasOffTarget) {
            e.timeOfLeavingTarget = time;
            e.wasOffTarget = true;
          }
        }
      ...
      e.decals.forEach((d) => {
        ...
        ctx.fillStyle = d.type === 'hit' ? `rgba(0, 255, 136, ${opacity})` : `rgba(239, 68, 68, ${opacity})`;
        ctx.beginPath();
        ctx.arc(d.x, d.y, 2.5, 0, Math.PI * 2);
        ctx.fill();
      });

      const distToCrosshair = Math.hypot(e.crosshair.x - t.x, e.crosshair.y - t.y);
      const isActuallyHit = distToCrosshair <= stats.radius && gameState === 'playing' && pointerLocked && (!weaponSpray || e.isMouseDown);

      if (e.flashTarget > 0) e.flashTarget -= deltaSec;
      
      const isFlashed = e.flashTarget > 0;
      ctx.shadowBlur = isActuallyHit ? 20 : (isFlashed ? 25 : 5);
      ctx.shadowColor = isActuallyHit ? "#00ff88" : (isFlashed ? "#ffffff" : "#3b82f6");
      ctx.fillStyle = isActuallyHit ? "rgba(0, 255, 136, 0.85)" : (isFlashed ? "rgba(255, 255, 255, 0.95)" : "rgba(59, 130, 246, 0.75)");
      
      ctx.beginPath();
      ctx.arc(t.x, t.y, stats.radius, 0, Math.PI * 2);
      ctx.fill();
      ctx.shadowBlur = 0;

      ctx.strokeStyle = isActuallyHit ? "rgba(255,255,255,0.3)" : "rgba(255,255,255,0.1)";
      ctx.lineWidth = 2;
      ctx.beginPath();
      ctx.arc(t.x, t.y, stats.radius, 0, Math.PI * 2);
      ctx.stroke();
      ...
      // Render Crosshair (raw lock state)
  ...
  const shareScore = useCallback(async () => {
    const text = `🎯 I scored ${score} PTS (Level ${level}) on Reactive Sphere Tracking Pro! Stability: ${analytics.stabilityScore}%. Practice your smooth pursuit tracking at skilldrills.online!`;
  ...
  const schemaData = {
    ...
    "@graph": [
      ...
      {
        "@type": "WebPage",
        "@id": "https://skilldrills.online/drills/fps/reactive-sphere-tracking",
        "url": "https://skilldrills.online/drills/fps/reactive-sphere-tracking",
        "name": "Reactive Sphere Tracking Pro – FPS Aim Trainer",
        "description": "Improve your smooth pursuit aiming and visual-spatial reflexes against erratic target strafes with this professional FPS tracking trainer.",
        "breadcrumb": { "@id": "https://skilldrills.online/drills/fps/reactive-sphere-tracking#breadcrumb" },
        "inLanguage": "en-US"
      }
    ]
  };
  ...
  return (
    <div className="min-h-screen select-none bg-[#050508] text-white">
      <Head>
        <title>Reactive Sphere Tracking Pro – Smooth Pursuit Aim Trainer</title>
        <meta name="description" content="Improve your smooth pursuit aiming and visual-spatial reflexes against erratic target strafes with this professional FPS tracking trainer." />
        <script type="application/ld+json" dangerouslySetInnerHTML={{ __html: JSON.stringify(schemaData) }} />
      </Head>

      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        
        {/* Page Title & Navigation (Hidden in Fullscreen) */}
        {!isFullscreen && (
          <div className="mb-6">
            <nav className="mb-4">
              <ol className="flex flex-wrap items-center gap-2 text-sm text-gray-500">
                ...
                <li><ChevronRight className="w-4 h-4 text-gray-600" /></li>
                <li className="text-blue-400 font-medium">Reactive Sphere Tracking</li>
              </ol>
            </nav>
            ...
            <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
              <div className="flex items-center gap-4">
                ...
                <div>
                  <h1 className="text-2xl sm:text-3xl font-bold tracking-tight">Reactive Sphere Tracking Pro</h1>
        ...
        >
          ...
          {gameState === 'playing' && (
            <div className="absolute top-0 left-0 right-0 h-1.5 bg-gray-900 z-[60]">
              <div ref={progressBarRef} className="h-full bg-blue-500" style={{ width: '100%' }} />
            </div>
          )}
              ...
              <div className="absolute top-6 left-6 z-[60] flex flex-col gap-2 pointer-events-none">
                <div className="bg-black/40 backdrop-blur border border-gray-800 px-4 py-2 rounded-xl flex items-center gap-4">
                  ...
                  <div className="text-center">
                    ...
                    <p className="text-2xl font-black text-blue-400 leading-none">{level}</p>
                  </div>
                </div>
              ...
              <div className="max-w-md w-full text-center">
                ...
                <div className="grid grid-cols-2 gap-3 mb-6 text-left">
                  <div className="bg-slate-900/60 border border-slate-800 p-3 rounded-xl">
                    ...
                    <span className="text-sm font-black text-white">Track the Sphere</span>
                  </div>
                  <div className="bg-slate-900/60 border border-slate-800 p-3 rounded-xl">
                    <span className="text-[8px] text-slate-500 block uppercase font-bold">Difficulty</span>
                    <span className="text-sm font-black text-blue-400">Erratic Strafing</span>
                  </div>
                  <div className="bg-slate-900/60 border border-slate-800 p-3 rounded-xl">
          ...
          <section className="mt-10">
            <div className="rounded-2xl border border-gray-800 overflow-hidden bg-gray-900 shadow-2xl pointer-events-none">
              <div className="px-6 py-5 border-b border-gray-800 bg-black/40 flex items-center gap-3">
                <Info className="w-5 h-5 text-blue-400" /><h2 className="font-bold text-white text-lg tracking-wide">Progression & Scoring Rules</h2>
              </div>
              <div className="p-6 grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div className="space-y-5">
                  <RuleItem num="3" color="fuchsia" text="Level Progression" highlight="Every 500 PTS" result="Target speed & strafes scale" />
          ...
          <article className="mt-12 text-gray-300">
            <div className="rounded-2xl border border-gray-800 overflow-hidden bg-gray-900 shadow-xl">
              <div className="px-6 py-5 border-b border-gray-800 bg-black/40 flex items-center gap-3">
                ...
                <h2 className="font-bold text-white text-lg tracking-wide">About Reactive Sphere Tracking</h2>
              </div>
              
              <div className="p-8 space-y-8">
                <section>
                  <h3 className="text-xl font-bold text-white mb-3 flex items-center gap-2">
                    <Eye className="w-5 h-5 text-blue-400" /> What is Reactive Tracking?
                  </h3>
                  <p className="text-sm leading-relaxed mb-4">
                    <strong>Reactive tracking</strong> is the mechanical discipline of instantly responding to erratic target direction swaps. Unlike predictive aiming, where a player anticipates shifts and pre-aims angles, a <strong>reactive tracking drill</strong> conditions you to focus purely on visual cues, wait for the target to commit to a movement, and adjust the crosshair path fluidly.
                  </p>
                  <p className="text-sm leading-relaxed">
                    By training your arm and eyes to work in sync, you map spatial coordinate changes to motor outputs, keeping your weapon steady through ADAD counter-strafes and sudden vertical jumps.
                  </p>
                </section>
                ...
                <div className="grid grid-cols-1 sm:grid-cols-3 gap-5 py-4 border-y border-gray-800/50">
                  <div className="p-5 rounded-xl border border-gray-800 bg-black/40 hover:bg-gray-800/50 transition-colors">
                    ...
                    <p className="text-xs leading-relaxed text-gray-400">Esports athletes, particularly entry fraggers and close-quarters duelists in high time-to-kill (TTK) games like Apex Legends, Overwatch 2, and The Finals.</p>
                  </div>
                  <div className="p-5 rounded-xl border border-gray-800 bg-black/40 hover:bg-gray-800/50 transition-colors">
                    <div className="flex items-center gap-3 mb-3">
                      ...
                      <h4 className="text-sm font-bold text-white">Pursuit Consistency</h4>
                    </div>
                    <p className="text-xs leading-relaxed text-gray-400">Improves smooth pursuit ocular movement. Reduces arm tension and micro-jitters, standardizing muscle memory for long tracking sequences.</p>
                  </div>
                  <div className="p-5 rounded-xl border border-gray-800 bg-black/40 hover:bg-gray-800/50 transition-colors">
                    <div className="flex items-center gap-3 mb-3">
                      <div className="w-8 h-8 rounded-lg bg-orange-600 flex items-center justify-center"><Zap className="w-4 h-4 text-white" /></div>
                      <h4 className="text-sm font-bold text-white">ADAD Strafe Defense</h4>
                    </div>
                    <p className="text-xs leading-relaxed text-gray-400">Conditions you to react to sudden direction shifts immediately. Overcomes the cognitive lag associated with reacquiring targets after jukes.</p>
                  </div>
                </div>
              </div>
              ...
              <div className="bg-[#0b0f19] border-t border-gray-800 p-8">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                  <FAQItem q="1. What is reactive tracking?" a="Reactive tracking is the mechanical ability to visually track a target and adjust your crosshair position instantly as the target changes direction. Unlike predictive tracking, it relies purely on visual response time and motor correction rather than anticipating movements." />
                  <FAQItem q="2. How do I improve tracking aim?" a="You can improve tracking aim by performing deliberate smooth pursuit drills, focusing your eyes entirely on the target rather than the crosshair, practicing against erratic direction-changing targets, and keeping your wrist and arm relaxed to avoid jittery movements." />
                  <FAQItem q="3. What is smooth pursuit aiming?" a="Smooth pursuit aiming refers to keeping the eyes locked onto a moving object smoothly without rapid, jerky eye adjustments (saccades). In games, this translates to smooth, continuous cursor movement that matches the speed and direction of the opponent." />
                  <FAQItem q="4. Why is tracking harder than flicking?" a="Tracking is harder than flicking because it requires continuous micro-corrections over a long duration rather than a single, rapid adjustment. You must match the speed, acceleration, and sudden direction changes of the target in real time." />
                  <FAQItem q="5. How do pro players track targets?" a="Professional players track targets by focusing intently on the enemy model, maintaining high-refresh-rate hardware, employing smooth and consistent arm movements, and relying on reactive adjustments rather than lazy prediction." />
                  <FAQItem q="6. What is ADAD tracking?" a="ADAD tracking is the skill of tracking a target that strafes left and right rapidly (using the A and D keys). It requires high responsiveness to read the direction change and correct your aim without overshooting." />
                  <FAQItem q="7. Can tracking drills improve Apex Legends aim?" a="Yes. Apex Legends has a very high time-to-kill (TTK), meaning players must stay locked onto enemies through multiple strafes. Tracking drills directly translate to keeping weapons steady during intense firefights in Apex." />
                  <FAQItem q="8. Does this help Overwatch players?" a="Absolutely. Overwatch 2 features characters with extremely fast movement and no acceleration delay, making reactive tracking and target reacquisition critical for heroes like Soldier: 76, Tracer, Zarya, and Sombra." />
                  <FAQItem q="9. How often should I train tracking?" a="We recommend training tracking for 10 to 15 minutes as a warm-up before your gaming sessions, or up to 30 minutes for dedicated mechanical training. Consistency is key to building muscle memory." />
                  <FAQItem q="10. What is target reacquisition?" a="Target reacquisition is the action of locating and locking onto a target again after losing visual contact or after the target performs an extreme movement that breaks your tracking streak." />
                  <FAQItem q="11. How can I reduce overflicking?" a="Reduce overflicking by lowering your sensitivity to a range of 25–45 cm/360, practicing smooth target transitions, and focusing on smooth mouse stop-control during your drills." />
                  <FAQItem q="12. Why do I lose targets while tracking?" a="Losing targets is usually caused by predicting movement rather than reacting to visual cues, looking at your crosshair instead of the target, or dynamic muscle tension that makes your hand move in jerky increments." />
                  <FAQItem q="13. What skills does this drill improve?" a="This drill improves smooth pursuit, visual response latency, ADAD strafe reading, target reacquisition speed, wrist micro-corrections, and tracking stability under pressure." />
                  <FAQItem q="14. Is this tracking trainer free?" a="Yes, this Reactive Tracking Aim Trainer is 100% free, runs in any modern desktop browser, and uses raw pointer input with no ads or registration required." />
                  <FAQItem q="15. Can tracking practice improve reaction speed?" a="Yes. Tracking practice trains your brain to process visual direction changes and command your hand muscles to react faster, which reduces your average reaction delay when targets change vectors." />
                </div>
              </div>
            </div>
          </article>
        ...
        {!isFullscreen && (
          <section className="mt-14" aria-label="Explore related aim and response drills">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
              <RelatedCard href="/drills/fps/anti-strafe-jitter-duel" title="Anti-Strafe Jitter" desc="Close-range tracking against jitter strafes." color="blue" icon={<Flame className="w-4 h-4" />} />
              <RelatedCard href="/drills/fps/180-degree-awareness" title="180° Awareness" desc="Situational awareness and peripheral flicks." color="orange" icon={<Zap className="w-4 h-4" />} />
              <RelatedCard href="/drills/fps/vertical-air-track" title="Vertical Air Track" desc="Vertical axis mouse control and prediction." color="green" icon={<Target className="w-4 h-4" />} />
              <RelatedCard href="/drills/fps/recoil-control" title="Recoil Control" desc="Weapon recoil pattern compensation trainer." color="red" icon={<Activity className="w-4 h-4" />} />
            </div>
          </section>
        )}
