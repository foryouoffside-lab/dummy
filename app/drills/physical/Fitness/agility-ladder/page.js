...
export default function MonochromeAgilityLadderPage() {
    ...
    checkStep(mouseX, mouseY, scrollOffset, canvasWidth, canvasHeight) {
      ...
      if (dist < 15 && !currentRung.stepped) {
        ...
        if (this.currentRungIndex >= 4) {
          this.completed = true;
          return { type: 'complete', points: 2 }; // +2 points for complete ladder
        }
        return { type: 'step', points: 1 }; // +1 point per rung
      }
  ...
  const applyPenalty = () => {
    ...
    // Apply -10 points penalty for missing a whole ladder
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  <div className="space-y-2">
                    ...
                    <div className="flex items-start gap-2">
                      ...
                      <p className={`text-sm ${isDarkMode ? 'text-gray-300' : 'text-gray-600'}`}>Correct step = <span className="font-semibold text-green-500">+1 point</span></p>
                    </div>
                    <div className="flex items-start gap-2">
                      ...
                      <p className={`text-sm ${isDarkMode ? 'text-gray-300' : 'text-gray-600'}`}>Complete all 4 rungs = <span className="font-semibold text-green-500">+2 points bonus</span> + speed increase</p>
                    </div>
                  </div>
                  <div className="space-y-2">
                    <div className="flex items-start gap-2">
                      <div className="w-5 h-5 rounded-full bg-red-500 flex items-center justify-center text-white text-xs font-bold mt-0.5">4</div>
                      <p className={`text-sm ${isDarkMode ? 'text-gray-300' : 'text-gray-600'}`}>Miss a whole ladder = <span className="font-semibold text-red-500">-10 points penalty</span></p>
                    </div>
                    <div className="flex items-start gap-2">
