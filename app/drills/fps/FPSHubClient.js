...
import { 
  Crosshair, Zap, Eye, 
  Gamepad2, Home, ChevronRight, Sparkles
} from "lucide-react";
...
// page into four separate walls of cards.
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8 relative z-10">
        ...
        <nav aria-label="Breadcrumb" className="mb-8">
          <ol className="flex items-center gap-2 text-xs font-mono text-ink-3 uppercase tracking-wider">
            <li><Link href={localizeHref('/')} className="flex items-center gap-1.5 hover:text-red-400 transition-colors"><Home className="w-3.5 h-3.5" /><span>HQ</span></Link></li>
            <li><ChevronRight className="w-3 h-3 text-hairline-2" /></li>
            <li><Link href={localizeHref('/drills')} className="hover:text-red-400 transition-colors">{t('header.allHubs', 'Drills')}</Link></li>
            <li><ChevronRight className="w-3 h-3 text-hairline-2" /></li>
            <li><span className="text-red-400 font-bold" aria-current="page">{t('header.fps', 'FPS Sector')}</span></li>
          </ol>
        </nav>
        ...
        <Reveal>
          <div className="mb-8 bg-surface-1/80 border border-hairline rounded-3xl p-6 sm:p-8 flex flex-col md:flex-row md:items-center justify-between gap-6 relative overflow-hidden backdrop-blur-xl shadow-xl">
            ...
            <div className="flex items-start gap-4">
              <div className="p-3.5 bg-gradient-to-br from-red-500 to-orange-500 rounded-2xl text-white shrink-0">
              ...
              <div>
                ...
                <p className="text-ink-2 mt-2 text-sm sm:text-base max-w-xl leading-relaxed">
                  {t('hubs.fps.desc', 'Hone mouse raw-input reflexes, smooth target tracking, and extreme 180° awareness indicators.')}
                </p>
              </div>
            </div>
          </div>
