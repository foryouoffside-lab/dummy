...
import { 
  ...
  Sun, Moon, Eye, Timer, Trophy, Compass, Zap, Activity, Award, Info
} from 'lucide-react';
  ...
  useEffect(() => {
    ...
    function render(now) {
      ...
      if (isHit && !wasHitRef.current) {
        ...
        const pointsEarned = 5 + comboBonus;
        ...
        if (comboRef.current % 3 === 0) playSound('combo');
        if (comboRef.current % 5 === 0) playSound('perfect');
      } else if (!isHit && wasHitRef.current) {
        comboRef.current = Math.max(0, comboRef.current - 2);
        setTrackingCombo(comboRef.current);
        if (comboRef.current > 3) {
          playSound('miss');
          showFeedback('Lost target!', 'error');
        }
      }
          ...
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <p className={`${isDarkMode ? 'text-gray-400' : 'text-gray-500'}`}>Track orbiting target â€¢ Dynamic speed & radius</p>
              </div>
            </div>
          ...
          {gameState === 'start' && (
            <div className={`absolute inset-0 flex items-center justify-center backdrop-blur-sm rounded-xl z-40 ${isBoxDarkMode ? 'bg-gray-900/95' : 'bg-white/95'}`}>
              <div className={`rounded-2xl p-8 text-center max-w-md mx-4 shadow-xl border ${isBoxDarkMode ? 'bg-gray-800 border-gray-700' : 'bg-white border-gray-200'}`}>
                ...
                <p className={`mb-6 ${isBoxDarkMode ? 'text-gray-300' : 'text-gray-600'}`}>60-second challenge â€¢ Track orbiting target</p>
                <button 
                  onClick={startGame} 
                  ...
                  <div className="space-y-2">
                    <div className="flex items-start gap-2">
                      ...
                      <p className={`text-sm ${isDarkMode ? 'text-gray-300' : 'text-gray-600'}`}>
                        <span className="font-semibold text-blue-500">Target orbits around center</span> â€¢ Follow the circular path
                      </p>
                    </div>
                    <div className="flex items-start gap-2">
                      <div className="w-5 h-5 rounded-full bg-green-500 flex items-center justify-center text-white text-xs font-bold mt-0.5">2</div>
                      <p className={`text-sm ${isDarkMode ? 'text-gray-300' : 'text-gray-600'}`}>
                        <span className="font-semibold text-green-500">Keep cursor on target to score</span> â€¢ Green when tracked
                      </p>
                    </div>
                    <div className="flex items-start gap-2">
                      ...
                      <p className={`text-sm ${isDarkMode ? 'text-gray-300' : 'text-gray-600'}`}>
                        <span className="font-semibold text-purple-500">85%+ = 2pts â€¢ 60%+ = 1pt â€¢ &lt;30% = -1pt</span>
                      </p>
                    </div>
                  </div>
                  <div className="space-y-2">
                    <div className="flex items-start gap-2">
                      ...
                      <p className={`text-sm ${isDarkMode ? 'text-gray-300' : 'text-gray-600'}`}>
                        <span className="font-semibold text-orange-500">Speed increases with combo</span> â€¢ Up to 40% faster
                      </p>
                    </div>
                    <div className="flex items-start gap-2">
                      ...
                      <p className={`text-sm ${isDarkMode ? 'text-gray-300' : 'text-gray-600'}`}>
                        <span className="font-semibold text-cyan-500">Radius expands/contracts dynamically</span> â€¢ Stay adaptive
                      </p>
                    </div>
                    <div className="flex items-start gap-2">
