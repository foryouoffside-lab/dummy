...
import { 
  ...
  Eye, Timer, ScanEye, Trophy, Info, Check, X
} from 'lucide-react';
...
export default function AwarenessDrillPage() {
  ...
  useEffect(() => {
    const savedBestScore = localStorage.getItem('awarenessDrillBestScore');
    if (savedBestScore) {
  ...
  const updateBestScore = (finalScore) => {
    const currentBestScore = parseInt(localStorage.getItem('awarenessDrillBestScore') || '0', 10);
    if (finalScore > currentBestScore) {
      localStorage.setItem('awarenessDrillBestScore', finalScore.toString());
  ...
  const calculateScoreFromSpeed = (reactionTimeMs) => {
    ...
    if (reactionTimeMs <= 300) return 80;
    if (reactionTimeMs <= 400) return 60;
    if (reactionTimeMs <= 500) return 40;
    if (reactionTimeMs <= 600) return 25;
    if (reactionTimeMs <= 800) return 15;
    return 10;
  };
    ...
    timerIntervalRef.current = setInterval(() => {
      if (gameStateRef.current === 'playing' && isActiveRef.current) {
        ...
        if (timeLeftRef.current <= 0) {
          ...
          const total = targetsHit + missedTargets;
          const finalAccuracy = total === 0 ? 100 : Math.round((targetsHit / total) * 100);
          setAccuracy(finalAccuracy);
          updateBestScore(scoreRef.current);
        }
      }
    }, 1000);
  }, [targetsHit, missedTargets]);
  ...
  useEffect(() => {
    ...
    function spawn() {
      ...
      targetRef.current.x = width * side;
      targetRef.current.y = height * (0.15 + Math.random() * 0.7);
    ...
    function drawBackground() {
      ...
      ctx.lineWidth = 2;
      ctx.beginPath();
      ctx.moveTo(cvs.width / 2 - 15, cvs.height / 2);
      ctx.lineTo(cvs.width / 2 + 15, cvs.height / 2);
      ctx.moveTo(cvs.width / 2, cvs.height / 2 - 15);
      ctx.lineTo(cvs.width / 2, cvs.height / 2 + 15);
      ctx.stroke();
    ...
    function drawTarget(now) {
      ...
      const gradient = ctx.createRadialGradient(target.x, target.y, 0, target.x, target.y, currentR);
      ...
      ctx.lineTo(indicatorX - arrowSize * Math.cos(angle - 0.8), indicatorY - arrowSize * Math.sin(angle - 0.8));
      ctx.lineTo(indicatorX - arrowSize * Math.cos(angle + 0.8), indicatorY - arrowSize * Math.sin(angle + 0.8));
      ctx.closePath();
      ctx.fillStyle = '#00ff88';
      ctx.fill();
    ...
    function render(now) {
      ...
      if (!targetRef.current.active && canvasSizeRef.current.width > 0) {
        spawnTimerRef.current += dt;
        if (spawnTimerRef.current > 0.25) {
          spawn();
          spawnTimerRef.current = 0;
        }
      }
  ...
  const startGame = () => {
    ...
    scoreRef.current = 0;
    comboRef.current = 0;
    targetRef.current = { x: 0, y: 0, r: 35, active: false };
    spawnTimerRef.current = 0;
    ...
    <div className={`min-h-screen select-none ${isDarkMode ? 'bg-gray-900' : 'bg-gray-50'}`}>
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        <div className="mb-6">
          ...
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-3">
              ...
              <div>
                <h1 className={`text-3xl font-bold ${isDarkMode ? 'text-white' : 'text-gray-900'}`}>180Â° Awareness Drill</h1>
                <p className={`${isDarkMode ? 'text-gray-400' : 'text-gray-500'}`}>Extreme edge targets â€¢ Penalty = 100</p>
              </div>
            </div>
        ...
        <div className="grid grid-cols-6 gap-3 mb-4 h-[88px]">
          ...
          <StatCard icon={<Check className="text-green-500" />} value={targetsHit} label="Hits" isDark={isDarkMode} />
          ...
          {gameState === 'start' && (
            <div className={`absolute inset-0 flex items-center justify-center backdrop-blur-sm rounded-xl z-40 ${isBoxDarkMode ? 'bg-gray-900/95' : 'bg-white/95'}`}>
              <div className={`rounded-2xl p-8 text-center max-w-md mx-4 shadow-xl border ${isBoxDarkMode ? 'bg-gray-800 border-gray-700' : 'bg-white border-gray-200'}`}>
                <ScanEye className="w-16 h-16 text-green-500 mx-auto mb-4" />
                <h3 className={`text-2xl font-bold mb-2 ${isBoxDarkMode ? 'text-white' : 'text-gray-900'}`}>180Â° Awareness Drill</h3>
                <p className={`mb-6 ${isBoxDarkMode ? 'text-gray-300' : 'text-gray-600'}`}>60-second challenge â€¢ Extreme edge targets</p>
                <button 
                  onClick={startGame} 
            ...
            <div className={`absolute inset-0 flex items-center justify-center backdrop-blur-sm rounded-xl z-40 ${isBoxDarkMode ? 'bg-gray-900/95' : 'bg-white/95'}`}>
              <div className={`rounded-2xl p-8 shadow-xl border w-[480px] ${isBoxDarkMode ? 'bg-gray-800 border-gray-700' : 'bg-white border-gray-200'}`}>
                ...
                <div className="grid grid-cols-2 gap-4 mb-8">
                  ...
                  <ResultCard label="Hits" value={targetsHit} icon={<Check className="w-4 h-4" />} color="text-green-500" />
            ...
            <div className={`rounded-xl border overflow-hidden ${isDarkMode ? 'bg-gray-800 border-gray-700' : 'bg-white border-gray-200'}`}>
              ...
              <div className="p-4">
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  <div className="space-y-2">
                    <div className="flex items-start gap-2">
                      ...
                      <p className={`text-sm ${isDarkMode ? 'text-gray-300' : 'text-gray-600'}`}>
                        <span className="font-semibold text-green-500">Targets spawn at extreme edges</span> â€¢ Train peripheral vision
                      </p>
                    </div>
                    <div className="flex items-start gap-2">
                      ...
                      <p className={`text-sm ${isDarkMode ? 'text-gray-300' : 'text-gray-600'}`}>
                        <span className="font-semibold text-blue-500">Click targets quickly</span> â€¢ Faster = More points
                      </p>
                    </div>
                    <div className="flex items-start gap-2">
                  ...
                  <div className="space-y-2">
                    <div className="flex items-start gap-2">
                      ...
                      <p className={`text-sm ${isDarkMode ? 'text-gray-300' : 'text-gray-600'}`}>
                        <span className="font-semibold text-red-500">Miss = -100 points</span> â€¢ Penalty equals max points
                      </p>
                    </div>
                    <div className="flex items-start gap-2">
                      ...
                      <p className={`text-sm ${isDarkMode ? 'text-gray-300' : 'text-gray-600'}`}>
                        <span className="font-semibold text-orange-500">Green arrow points to target</span> â€¢ Use for quick location
                      </p>
                    </div>
                    <div className="flex items-start gap-2">
                ...
                <div className={`mt-3 pt-3 border-t text-xs ${isDarkMode ? 'border-gray-700 text-gray-400' : 'border-gray-200 text-gray-500'} flex items-center justify-between`}>
                  <span>ðŸŽ¯ New target every 250ms after hit/miss</span>
