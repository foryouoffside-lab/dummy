...
export default function AdjacentHubs({ currentCat, limit = 4 }) {
  ...
  return (
    <section className="mt-12 mb-8 border-t border-hairline pt-12" aria-labelledby="adjacent-hubs-heading">
      ...
      >
        {t('ui.exploreAdjacent', 'Explore Adjacent Hubs')}
      </h2>

      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4 max-w-5xl mx-auto">
        {neighbours.map((cat) => {
          const Icon = cat.icon;
          const href = hasLocalizedRoute(locale, cat.href) ? localizeHref(cat.href) : cat.href;
          const headerKey = cat.cat === 'visual-tracking' ? 'tracking' : cat.cat === 'reaction-speed' ? 'reaction' : cat.cat;
          const name = t(`header.${headerKey}`, cat.name);
          return (
            <Link
              key={cat.cat}
              href={href}
            ...
            >
              ...
              <h3 className="font-bold text-ink-1 uppercase text-xs font-mono">{name}</h3>
              <p className="text-2xs text-ink-3 mt-1 font-mono">{getCategoryCount(cat.cat)} {t('ui.drills', 'drills')}</p>
            </Link>
          );
        })}
      </div>
