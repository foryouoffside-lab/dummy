...
export default function JapaneseColorSequencePage() {
  ...
  const sequenceGuide = {
    ...
    intro: [
      '色の順番を記憶して正確に再現する課題は、人間の視覚的および系列的な作業記憶（Working Memory）を測定する上で最も信頼性の高い古典的アプローチです。1978年にラルフ・ベアらによって開発された電子玩具サイモンを原点とし、感覚入力の瞬間的な符号化と想起を鍛えます。',
      'この訓練の核となる神経基盤は、アラン・バドリー（Baddeley & Hitch, 1974）が提唱したワーキングメモリモデルの「視空間スケッチパッド」にあります。スティーブン・ラックとエドワード・ボーゲル（Luck & Vogel, 1997）およびネルソン・コーワン（Cowan, 2001）の研究は、ヒトの作業記憶が一度に保持できる情報の上限は約3〜4項目であると明らかにしました。',
      'そのため、5ステップ以上の長大なシーケンスを攻略するには、ジョージ・ミラー（Miller, 1956）のチャンキング技術が不可欠です。複数の色をひとつのリズミカルな音階や空間幾何パターンとして再符号化することで、限られた脳のメモリ容量を最大限に活用します。',
      'SkillDrillsはブラウザの高精度 performance.now() APIを用い、ミリ秒単位の正確な入力判定とリアルタイムな適応型難易度調整を提供します（Woods et al., 2015）。'
    ],
    benchmarks: {
