...
export default function StrobePredictionPursuitClient() {
  ...
  useEffect(() => {
    ...
    try {
      const stored = localStorage.getItem('visualTrackingStrobePredictionPursuitClientTrialsCount');
        ...
        setTimeLeft(p => {
          if (p <= 1) {
            ...
            setTotalTrials(prev => {
              const next = prev + 1;
              try {
                localStorage.setItem('visualTrackingStrobePredictionPursuitClientTrialsCount', next.toString());
              } catch (e) {}
              return next;
            });
  ...
  useEffect(() => {
    ...
    const draw = (ts: number) => {
      ...
      // Move target
        ...
        // Strobe logic: visible for 60 frames (1s), hidden for 30 frames (0.5s)
        trackingState.current.strobeTimer += speedMultiplier;
        if (trackingState.current.strobeTimer > 90) {
          trackingState.current.strobeTimer = 0;
          playSound('transition');
        }

        if (trackingState.current.strobeTimer < 60) {
          // Render solid target
          ...
          // Render ghost outline during dark phase
    ...
    if (typeof navigator !== 'undefined' && navigator.share) {
      try {
        await navigator.share({
          title: 'Free Strobe Prediction Pursuit Drill | SkillDrills',
      ...
      navigator.clipboard.writeText('https://skilldrills.online/drills/visual-tracking/strobe-prediction-pursuit');
        ...
        {!isFullscreen && (
          <nav aria-label="Breadcrumb" className="mb-5">
            <ol className="flex items-center gap-2 text-[10px] text-slate-550 uppercase tracking-widest font-mono">
              ...
              <li><span className="text-slate-755">/</span></li>
              <li><span className="text-red-400 font-bold">Strobe Prediction Pursuit</span></li>
            </ol>
          </nav>
        )}
                ...
                <div className="absolute inset-0 bg-[#05070e]/95 flex flex-col items-center justify-center p-6 z-30 select-none">
                  <div className="max-w-md text-center">
                    ...
                    <p className="text-xs text-slate-400 leading-relaxed mb-6">
                      Predict target locations during cyclic strobe blank intervals to train visual memory. Focus your eyes on the moving target. Do not use your head or mouse.
                    </p>
            ...
            <div className="md:col-span-2 bg-[#0b0f19]/40 border border-slate-900 p-6 rounded-2xl flex flex-col justify-between">
              <div>
                ...
                <p className="text-xs leading-relaxed text-slate-400 mb-4 font-sans">
                  The Strobe Prediction Pursuit isolates the brain's visual memory system. By cyclically blanking the moving target, the drill forces the ocular motor system to predict the target's trajectory without sensory feedback, checking eye positioning accuracy when the target re-solidifies.
                </p>
              </div>
            </div>
            ...
            <div className="bg-[#0b0f19]/40 border border-slate-905 p-6 rounded-2xl">
              ...
              <ul className="space-y-3 text-xs text-slate-400 font-mono">
                <li className="flex items-start gap-2">
                  ...
                  <span><strong>Trajectory Prediction</strong>: Refines mental modeling of velocity and headings.</span>
                </li>\n                <li className="flex items-start gap-2">
                  <Check className="w-4 h-4 text-green-500 flex-shrink-0 mt-0.5" />
                  <span><strong>Visual Persistence</strong>: Enhances object tracking during environmental blocks.</span>
                </li>\n                <li className="flex items-start gap-2">
                  <Check className="w-4 h-4 text-green-500 flex-shrink-0 mt-0.5" />
                  <span><strong>Gaze Calibration</strong>: Prevents over-correction when targets reappear.</span>
                </li>\n
              </ul>
            </div>
          </div>
