...
export default function FineMotorClient() {
  ...
  const updateBestScore = useCallback((finalScore) => {
    try {
      ...
      if (finalScore > currentBestScore) { localStorage.setItem('calibratedDynamicFlowBestScore', finalScore.toString()); setBestScore(finalScore); }
    } catch (e) {}
  }, []);
  ...
  const playSound = useCallback((type) => {
    ...
    try {
      ...
      osc.frequency.setValueAtTime(type === 'lock' ? 880 : 1046.5, now);
      gain.gain.setValueAtTime(type === 'streak' ? 0.12 : 0.03, now);
  ...
  const initPoints = useCallback((cvs) => {
    pointsRef.current = []; phaseOffsetRef.current = Math.random() * Math.PI * 2; flowTimeRef.current = 0;
  ...
  useEffect(() => {
    ...
    function update(dt) {
      ...
      for (let i = 0; i < points.length - 1; i++) {
        ...
        if (mouse.y >= p2.y && mouse.y <= p1.y) { const t = (mouse.y - p1.y) / (p2.y - p1.y); if (Math.abs(mouse.x - (p1.x + t * (p2.x - p1.x))) <= 7.0) { onPath = true; break; } }
      }

      const wasLocked = isLockedRef.current; isLockedRef.current = onPath; setIsLocked(onPath);
      if (onPath && !wasLocked) playSound('lock');
      if (onPath) {
        lockedFramesRef.current++; streakRef.current++; setStreak(streakRef.current);
        if (streakRef.current > bestStreakRef.current) { bestStreakRef.current = streakRef.current; setBestStreak(streakRef.current); try { localStorage.setItem('calibratedDynamicFlowBestStreak', streakRef.current.toString()); } catch (e) {} }
        scoreAccumulatorRef.current += dt;
        if (scoreAccumulatorRef.current >= 1.0) { const pts = Math.floor(scoreAccumulatorRef.current); scoreRef.current += pts; setScore(scoreRef.current); scoreAccumulatorRef.current -= pts; if (pts > 0 && Math.random() < 0.3) showFeedback(`+${pts} Focus!`, 'success'); }
    ...
    function drawFluidPath() {
      const points = pointsRef.current; ctx.moveTo(points[0].x, points[0].y + scrollPosRef.current);
      for (let i = 0; i < points.length - 1; i++) { const p1 = points[i]; const p2 = points[i + 1]; ctx.quadraticCurveTo(p1.x, p1.y + scrollPosRef.current, (p1.x + p2.x) / 2, (p1.y + p2.y) / 2 + scrollPosRef.current); }
    }
    ...
    function draw() {
      ...
      drawFluidPath(); ctx.stroke();
      if (isLockedRef.current) { ctx.shadowBlur = 15; ctx.shadowColor = isBoxDarkMode ? "#ffffff" : "#000000"; ctx.stroke(); ctx.shadowBlur = 0; }

      const mouse = mousePositionRef.current;
      if (mouse.x > 0 && mouse.x < cvs.width && mouse.y > 0 && mouse.y < cvs.height) {
        ctx.beginPath(); ctx.arc(mouse.x, mouse.y, 10, 0, Math.PI * 2); ctx.strokeStyle = isLockedRef.current ? "#00ff88" : "#ff2222"; ctx.lineWidth = 2; ctx.stroke();
        ctx.beginPath(); ctx.arc(mouse.x, mouse.y, 4, 0, Math.PI * 2); ctx.fillStyle = isLockedRef.current ? "#00ff88" : "#ff2222"; ctx.fill();
  ...
  }, [gameState, isBoxDarkMode, updateAccuracy, initPoints, addNewPoint, playSound, showFeedback]);

  const startGame = useCallback(() => {
    setGameState('playing'); gameStateRef.current = 'playing'; setScore(0); setTimeLeft(60); setIsLocked(false);
    setScrollSpeed(180); setCurrentAmplitude(220); setCurrentFrequency(0.35); setAccuracy(100); setPhase('Dynamic');
    setStreak(0); setFeedback('');
    isActiveRef.current = true; startTimeRef.current = performance.now();
    scrollPosRef.current = 0; scrollSpeedRef.current = 180; currentAmplitudeRef.current = 220; currentFrequencyRef.current = 0.35;
    isLockedRef.current = false; scoreRef.current = 0; streakRef.current = 0; bestStreakRef.current = 0;
    scoreAccumulatorRef.current = 0; totalFramesRef.current = 0; lockedFramesRef.current = 0; flowTimeRef.current = 0;
    if (canvasRef.current) initPoints(canvasRef.current);
    showFeedback('60 seconds • Stay on the white path! (+1/sec)', 'success');
  }, [initPoints, showFeedback]);
  ...
  const resetGame = useCallback(() => {
    ...
    setGameState('start'); gameStateRef.current = 'start'; setScore(0); setTimeLeft(60); setIsLocked(false);
    setScrollSpeed(180); setCurrentAmplitude(220); setCurrentFrequency(0.35); setAccuracy(100); setPhase('Dynamic');
    setStreak(0); setFeedback('');
  }, []);

  const sharePage = async () => { if (navigator.share) { try { await navigator.share({ title: 'Free Fine Motor Control Drill | SkillDrills', text: 'Train precision cursor control with this free motor drill!', url: 'https://skilldrills.online/drills/motor/precision-control/fine-motor' }); } catch (e) {} } else { navigator.clipboard.writeText('https://skilldrills.online/drills/motor/precision-control/fine-motor'); alert('Link copied!'); } };
  const copyPageLink = () => { navigator.clipboard.writeText('https://skilldrills.online/drills/motor/precision-control/fine-motor'); alert('Link copied!'); };

  if (loading || !isClient) {
    return <div className="min-h-screen flex items-center justify-center bg-gray-50"><div className="text-center"><div className="w-16 h-16 border-4 border-blue-600 border-t-transparent rounded-full animate-spin mx-auto mb-4"></div><p className="text-gray-600">Loading fine motor drill...</p></div></div>;
  }

  return (
    <div className={`min-h-screen select-none ${isDarkMode ? 'bg-gray-900' : 'bg-gray-50'}`}>
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        <nav aria-label="Breadcrumb" className="mb-4"><ol className="flex flex-wrap items-center gap-2 text-sm"><li><Link href="/" className={`hover:underline transition-colors ${isDarkMode ? 'text-gray-400 hover:text-gray-200' : 'text-gray-600 hover:text-gray-900'}`}>Home</Link></li><li className={`${isDarkMode ? 'text-gray-500' : 'text-gray-400'}`} aria-hidden="true">/</li><li><Link href="/drills/motor" className={`hover:underline transition-colors ${isDarkMode ? 'text-gray-400 hover:text-gray-200' : 'text-gray-600 hover:text-gray-900'}`}>Motor Drills</Link></li><li className={`${isDarkMode ? 'text-gray-500' : 'text-gray-400'}`} aria-hidden="true">/</li><li className={`${isDarkMode ? 'text-gray-500' : 'text-gray-400'}`}>Precision Control</li><li className={`${isDarkMode ? 'text-gray-500' : 'text-gray-400'}`} aria-hidden="true">/</li><li className={`font-medium ${isDarkMode ? 'text-blue-400' : 'text-blue-600'}`} aria-current="page">Calibrated Dynamic Flow</li></ol></nav>
        
        <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 mb-6">
          <div className="flex items-center gap-3"><div className="p-3 bg-gradient-to-r from-blue-500 to-indigo-600 rounded-xl flex-shrink-0"><Waves className="w-6 h-6 text-white" /></div><div><h1 className={`text-2xl sm:text-3xl font-bold ${isDarkMode ? 'text-white' : 'text-gray-900'}`}>Calibrated Dynamic Flow</h1><p className={`text-sm sm:text-base ${isDarkMode ? 'text-gray-400' : 'text-gray-500'}`}>+1/sec on path • No penalties • 2 phases • 60s challenge • Free motor training</p></div></div>
          <div className="flex gap-2 flex-shrink-0">
            {gameState === 'playing' && <button onClick={resetGame} className={`p-2 rounded-lg border transition-all hover:scale-105 active:scale-95 ${isDarkMode ? 'bg-gray-800 border-gray-700 text-gray-300 hover:bg-gray-700' : 'bg-white border-gray-200 text-gray-700 hover:bg-gray-100'}`} title="Reset" aria-label="Reset fine motor drill"><RefreshCw className="w-5 h-5" /></button>}
        ...
        <section className="sr-only"><h2>Free Fine Motor Control Drill - Calibrated Dynamic Flow Path Tracking for Precision Training</h2><p>Fine motor precision drill. Track a scrolling wave path with your cursor. +1 point per second while on the white path. Two phases: Dynamic (0-30s) with moderate waves, Extreme (30-60s) with amplified amplitude and frequency. No penalties - pure flow state training. Adaptive scroll speed slows when locked on path. Perfect for precision control training, mouse accuracy, and hand-eye coordination. No registration required.</p></section>
        ...
        <div ref={containerRef} className={`relative ${isFullscreen ? 'fixed inset-0 z-50' : 'rounded-xl border-2'}`} style={{ background: isBoxDarkMode ? "#020202" : "#ffffff", aspectRatio: isFullscreen ? 'auto' : '16/9', maxWidth: '100%', margin: '0 auto', borderColor: isDarkMode ? '#374151' : '#e5e7eb', overflow: 'hidden', cursor: 'none' }}>
          ...
          <canvas ref={canvasRef} style={{ display: 'block', position: 'absolute' }} aria-label="Fine motor tracking canvas. Keep your cursor on the white path." />
          
          {gameState === 'start' && (<div className={`absolute inset-0 flex items-center justify-center backdrop-blur-sm rounded-xl z-40 ${isBoxDarkMode ? 'bg-gray-900/95' : 'bg-white/95'}`}><div className={`rounded-2xl p-6 sm:p-8 text-center max-w-md mx-4 shadow-xl border ${isBoxDarkMode ? 'bg-gray-800 border-gray-700' : 'bg-white border-gray-200'}`}><div className="mb-4"><Waves className="w-16 h-16 text-blue-500 mx-auto" aria-hidden="true" /></div><h2 className={`text-2xl font-bold mb-2 ${isBoxDarkMode ? 'text-white' : 'text-gray-900'}`}>Calibrated Dynamic Flow</h2><p className={`mb-2 ${isBoxDarkMode ? 'text-gray-300' : 'text-gray-600'}`}>60-second challenge • +1/sec on path • No penalties</p><p className={`mb-6 text-sm ${isBoxDarkMode ? 'text-gray-400' : 'text-gray-500'}`}>Keep your cursor on the white scrolling path. Dynamic phase (0-30s) then Extreme phase (30-60s). Pure flow state training for precision control.</p><button onClick={startGame} className="px-8 py-3 bg-gradient-to-r from-blue-500 to-indigo-600 text-white rounded-xl font-semibold hover:shadow-lg w-full transition-all transform hover:scale-[1.02] active:scale-[0.98] focus:outline-none focus:ring-2 focus:ring-blue-500 focus:ring-offset-2" aria-label="Start free fine motor flow training">Start Free Drill</button></div></div>)}
          
          {gameState === 'gameOver' && (<div className={`absolute inset-0 flex items-center justify-center backdrop-blur-sm rounded-xl z-40 ${isBoxDarkMode ? 'bg-gray-900/95' : 'bg-white/95'}`}><div className={`rounded-2xl p-6 sm:p-8 shadow-xl border w-full max-w-[480px] mx-4 ${isBoxDarkMode ? 'bg-gray-800 border-gray-700' : 'bg-white border-gray-200'}`}><div className="flex items-center justify-center gap-3 mb-4"><Timer className="w-10 h-10 text-orange-500" aria-hidden="true" /><h2 className={`text-2xl font-bold ${isBoxDarkMode ? 'text-white' : 'text-gray-900'}`}>Time&apos;s Up!</h2></div><p className={`text-center text-sm mb-6 ${isBoxDarkMode ? 'text-gray-400' : 'text-gray-500'}`}>Keep practicing to improve your path tracking precision and flow state endurance for fine motor control.</p><div className="grid grid-cols-2 gap-3 mb-6"><ResultCard label="Final Score" value={score} icon={<Target className="w-4 h-4" />} color="blue" isDark={isBoxDarkMode} /><ResultCard label="Best Score" value={bestScore} icon={<Trophy className="w-4 h-4" />} color="yellow" isDark={isBoxDarkMode} /><ResultCard label="Best Streak" value={bestStreak} icon={<Zap className="w-4 h-4" />} color="orange" isDark={isBoxDarkMode} /><ResultCard label="Accuracy" value={accuracy} unit="%" icon={<BarChart3 className="w-4 h-4" />} color="purple" isDark={isBoxDarkMode} /><ResultCard label="Max Phase" value={phase} icon={<Waves className="w-4 h-4" />} color="cyan" isDark={isBoxDarkMode} /></div><div className="flex gap-3"><Link href="/drills/motor" className="flex-1"><button className={`w-full px-4 py-2.5 rounded-lg font-semibold transition-all ${isDarkMode ? 'bg-gray-700 text-gray-300 hover:bg-gray-600' : 'bg-gray-200 text-gray-700 hover:bg-gray-300'}`}>← Back to Motor</button></Link><button onClick={startGame} className="flex-1 px-4 py-2.5 bg-gradient-to-r from-blue-500 to-indigo-600 text-white rounded-lg font-semibold hover:shadow-lg transition-all transform hover:scale-[1.02] active:scale-[0.98] focus:outline-none focus:ring-2 focus:ring-blue-500 focus:ring-offset-2">Play Again →</button></div></div></div>)}
        </div>

        {/* 1. DRILL RULES & SCORING */}
        {!isFullscreen && (<footer className="mt-6" aria-label="Drill rules"><div className={`rounded-xl border overflow-hidden ${isDarkMode ? 'bg-gray-800 border-gray-700' : 'bg-white border-gray-200'}`}><div className={`px-4 py-3 border-b ${isDarkMode ? 'border-gray-700 bg-gray-800/50' : 'border-gray-200 bg-gray-50'}`}><div className="flex items-center gap-2"><Info className={`w-4 h-4 ${isDarkMode ? 'text-blue-400' : 'text-blue-600'}`} aria-hidden="true" /><h2 className={`font-semibold ${isDarkMode ? 'text-white' : 'text-gray-900'}`}>Drill Rules & Scoring</h2></div></div><div className="p-4"><div className="grid grid-cols-1 md:grid-cols-2 gap-4"><div className="space-y-3"><div className="flex items-start gap-2"><div className="w-5 h-5 rounded-full bg-blue-500 flex items-center justify-center text-white text-xs font-bold flex-shrink-0 mt-0.5">1</div><p className={`text-sm ${isDarkMode ? 'text-gray-300' : 'text-gray-600'}`}>Keep cursor on the <span className="font-semibold text-blue-500">white path</span> (7px tolerance)</p></div><div className="flex items-start gap-2"><div className="w-5 h-5 rounded-full bg-green-500 flex items-center justify-center text-white text-xs font-bold flex-shrink-0 mt-0.5">2</div><p className={`text-sm ${isDarkMode ? 'text-gray-300' : 'text-gray-600'}`}>On path: <span className="font-semibold text-green-500">+1 point per second</span></p></div><div className="flex items-start gap-2"><div className="w-5 h-5 rounded-full bg-orange-500 flex items-center justify-center text-white text-xs font-bold flex-shrink-0 mt-0.5">3</div><p className={`text-sm ${isDarkMode ? 'text-gray-300' : 'text-gray-600'}`}>Phases: <span className="font-semibold text-orange-500">Dynamic (0-30s) → Extreme (30-60s)</span></p></div></div><div className="space-y-3"><div className="flex items-start gap-2"><div className="w-5 h-5 rounded-full bg-purple-500 flex items-center justify-center text-white text-xs font-bold flex-shrink-0 mt-0.5">4</div><p className={`text-sm ${isDarkMode ? 'text-gray-300' : 'text-gray-600'}`}><span className="font-semibold text-purple-500">No penalties!</span> Just pure flow tracking</p></div><div className="flex items-start gap-2"><div className="w-5 h-5 rounded-full bg-pink-500 flex items-center justify-center text-white text-xs font-bold flex-shrink-0 mt-0.5">5</div><p className={`text-sm ${isDarkMode ? 'text-gray-300' : 'text-gray-600'}`}>Speed <span className="font-semibold text-pink-500">adapts to performance</span> (160-900)</p></div><div className="flex items-start gap-2"><div className="w-5 h-5 rounded-full bg-yellow-500 flex items-center justify-center text-white text-xs font-bold flex-shrink-0 mt-0.5">6</div><p className={`text-sm ${isDarkMode ? 'text-gray-300' : 'text-gray-600'}`}>Challenge lasts <span className="font-semibold text-yellow-500">60 seconds</span></p></div></div></div><div className={`mt-4 pt-3 border-t flex flex-col sm:flex-row items-start sm:items-center justify-between gap-2 text-xs ${isDarkMode ? 'border-gray-700 text-gray-400' : 'border-gray-200 text-gray-500'}`}><span>⚪ White = On path • 🔴 Red = Off path • Speed adapts</span><span>⚡ Best Score saves locally • No lives, no penalties! • Free forever</span></div></div></div></footer>)}

        {/* 2. ABOUT THIS DRILL */}
        {!isFullscreen && (
          <section className="mt-8" aria-label="About this fine motor drill">
            <div className={`rounded-xl border overflow-hidden ${isDarkMode ? 'bg-gray-800 border-gray-700' : 'bg-white border-gray-200'}`}>
              <div className={`px-4 py-3 border-b ${isDarkMode ? 'border-gray-700 bg-gray-800/50' : 'border-gray-200 bg-gray-50'}`}>
                <div className="flex items-center gap-2"><GraduationCap className={`w-5 h-5 ${isDarkMode ? 'text-blue-400' : 'text-blue-600'}`} aria-hidden="true" /><h2 className={`font-semibold ${isDarkMode ? 'text-white' : 'text-gray-900'}`}>About This Free Fine Motor Drill</h2></div>
              </div>
              <div className="p-5">
                <p className={`text-sm leading-relaxed mb-5 ${isDarkMode ? 'text-gray-300' : 'text-gray-600'}`}>This free fine motor control drill trains precision cursor tracking by following a dynamically scrolling wave path. Earn +1 point per second while staying on the white path with no penalties. Two phases increase difficulty: Dynamic (0-30s) and Extreme (30-60s). Adaptive scroll speed responds to your performance.</p>
