...
export default function MultiplicationDrill() {
  ...
  // Get points based on range (difficulty)
  ...
  // Get penalty based on range (same as correct points)
  ...
  const generateProblem = useCallback(() => {
    ...
    while (options.length < 4) {
      let fakeAnswer;
      const variation = range <= 12 ? 20 : range <= 20 ? 50 : 100;
  ...
  const handleOptionSelect = (selectedAnswer) => {
    ...
    if (selectedAnswer === currentProblem.answer) {
      // Correct answer: points based on range
  ...
  // Get point display for range selector
  const getPointDisplay = (r) => {
    if (r === 20) return '1.5pt';
    if (r === 15) return '1pt';
    return '0.5pt';
  };
  ...
  return (
    <div className={`min-h-screen select-none ${isDarkMode ? 'bg-gray-900' : 'bg-gray-50'}`}>
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        ...
        <div className="mb-6">
          ...
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-3">
              ...
              <div>
                <h1 className={`text-3xl font-bold ${isDarkMode ? 'text-white' : 'text-gray-900'}`}>Factor Recall Lab</h1>
                <p className={`${isDarkMode ? 'text-gray-400' : 'text-gray-500'}`}>Multiplication automaticity • 60 seconds</p>
              </div>
            </div>
        ...
        {/* Stats Board - Factor Recall specific metrics with Lives */}
          ...
          <div className="absolute inset-0 flex items-center justify-center p-8 overflow-y-auto">
            ...
            {gameState === 'start' && (
              <div className={`absolute inset-0 flex items-center justify-center backdrop-blur-sm rounded-xl z-40 ${isBoxDarkMode ? 'bg-gray-900/95' : 'bg-white/95'}`}>
                <div className={`rounded-2xl p-8 text-center max-w-md mx-4 shadow-xl border ${isBoxDarkMode ? 'bg-gray-800 border-gray-700' : 'bg-white border-gray-200'}`}>
                  ...
                  <h3 className={`text-2xl font-bold mb-2 ${isBoxDarkMode ? 'text-white' : 'text-gray-900'}`}>Factor Recall Lab</h3>
                  <p className={`mb-6 ${isBoxDarkMode ? 'text-gray-300' : 'text-gray-600'}`}>60-second challenge • Up to {range}×{range}</p>
                  <button 
                    onClick={startGame} 
            ...
            {gameState === 'playing' && (
              <div className="w-full max-w-2xl">
                <div className={`text-6xl md:text-8xl font-bold text-center mb-8 ${
              ...
              <div className={`absolute inset-0 flex items-center justify-center backdrop-blur-sm rounded-xl z-40 ${isBoxDarkMode ? 'bg-gray-900/95' : 'bg-white/95'}`}>
                <div className={`rounded-2xl p-8 shadow-xl border w-[480px] ${isBoxDarkMode ? 'bg-gray-800 border-gray-700' : 'bg-white border-gray-200'}`}>
                  ...
                  <div className="grid grid-cols-2 gap-4 mb-8">
                    <ResultCard label="Final Score" value={score.toFixed(1)} icon={<Target className="w-4 h-4" />} color="text-yellow-500" />
                    ...
                    <ResultCard label="Range" value={`${range}×${range}`} icon={<Hash className="w-4 h-4" />} color="text-cyan-500" />
                  </div>
            ...
            <div className={`rounded-xl border overflow-hidden ${isDarkMode ? 'bg-gray-800 border-gray-700' : 'bg-white border-gray-200'}`}>
              ...
              <div className="p-4">
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  <div className="space-y-2">
                    ...
                    <div className="flex items-start gap-2">
                      ...
                      <p className={`text-sm ${isDarkMode ? 'text-gray-300' : 'text-gray-600'}`}>Correct: <span className="font-semibold text-green-500">12×12: 0.5pt • 15×15: 1pt • 20×20: 1.5pt</span></p>
                    </div>
                    <div className="flex items-start gap-2">
                      <div className="w-5 h-5 rounded-full bg-red-500 flex items-center justify-center text-white text-xs font-bold mt-0.5">3</div>
                      <p className={`text-sm ${isDarkMode ? 'text-gray-300' : 'text-gray-600'}`}>Wrong: <span className="font-semibold text-red-500">-1 life (3 lives total)</span></p>
                    </div>
                  </div>
                  <div className="space-y-2">
                    <div className="flex items-start gap-2">
                    ...
                    <div className="flex items-start gap-2">
                      ...
                      <p className={`text-sm ${isDarkMode ? 'text-gray-300' : 'text-gray-600'}`}>After lives reach 0 → <span className="font-semibold text-yellow-500">Penalty equals correct points</span></p>
                    </div>
                    <div className="flex items-start gap-2">
                ...
                <div className={`mt-3 pt-3 border-t text-xs ${isDarkMode ? 'border-gray-700 text-gray-400' : 'border-gray-200 text-gray-500'} flex items-center justify-between`}>
                  <span> Range: 12×12 • 15×15 • 20×20</span>
