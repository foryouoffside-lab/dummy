...
export default function BoxBreathingClient() {
  ...
  return (
    <div className={`min-h-screen select-none ${isDarkMode?'bg-gray-900':'bg-gray-50'}`}>
      <script type="application/ld+json" dangerouslySetInnerHTML={{__html:JSON.stringify({"@context":"https://schema.org","@type":"WebApplication","name":"Box Breathing Drill - 4-4-4-4 Tactical Breathing","url":"https://skilldrills.online/drills/mental-fitness/breathing-exercises/box-breathing","description":"Box breathing (4-4-4-4 technique) used by Navy SEALs and first responders. Square visual pacer guides 4s inhale, 4s hold, 4s exhale, 4s hold. 5 points per breath with combo bonuses every 3 breaths.","applicationCategory":"HealthApplication","operatingSystem":"Web","offers":{"@type":"Offer","price":"0","priceCurrency":"USD"},"author":{"@type":"Organization","name":"SkillDrills"},"educationalUse":["Breathing Exercise","Stress Relief","Focus Training","Tactical Breathing"],"learningResourceType":"Interactive Exercise","interactivityType":"active","inLanguage":"en-US","teaches":["Box Breathing","4-4-4-4 Breathing","Tactical Breathing","Stress Management","Focus Enhancement"]})}}/>
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        <nav className="mb-4">
          <ol className="flex flex-wrap items-center gap-2 text-sm">
            ...
            <li className={isDarkMode?'text-gray-500':'text-gray-400'}>/</li>
            <li className={`font-medium ${isDarkMode?'text-cyan-400':'text-cyan-600'}`}>Box Breathing</li>
          </ol>
        </nav>
        
        <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 mb-4">
          <div className="flex items-center gap-3">
            <div className="p-3 bg-gradient-to-r from-cyan-500 to-blue-600 rounded-xl"><Square className="w-6 h-6 text-white"/></div>
            <div>
              <h1 className={`text-2xl sm:text-3xl font-bold ${isDarkMode?'text-white':'text-gray-900'}`}>Box Breathing</h1>
              <p className={`text-sm sm:text-base ${isDarkMode?'text-gray-400':'text-gray-500'}`}>Tactical 4-4-4-4 pattern • 5 points per breath • Combo bonuses • Free stress relief</p>
            </div>
          </div>
        ...
        <div className="grid grid-cols-5 gap-3 mb-4 h-[88px]">
          <SCard icon={<Brain className="text-cyan-500"/>} value={score} label="Focus Score" dark={isDarkMode}/>
          ...
          <SCard icon={<Wind className="text-green-500"/>} value={totalBreaths} label="Breaths" dark={isDarkMode}/>
          <SCard icon={<TrendingUp className="text-purple-500"/>} value={combo} label="Combo" dark={isDarkMode}/>
        </div>
        ...
        <div ref={containerRef} className={`relative ${isFullscreen?'fixed inset-0 z-50':'rounded-xl border-2'}`} style={{background:isBoxDarkMode?'#050505':'#ffffff',aspectRatio:isFullscreen?'auto':'16/9',maxWidth:'100%',margin:'0 auto',borderColor:isDarkMode?'#374151':'#e5e7eb'}}>
          {isFullscreen&&gameState==='playing'&&(
            <>
              ...
              <div className="absolute top-4 left-4 z-30 bg-black/50 backdrop-blur-sm rounded-lg px-4 py-2 text-white text-sm">Score: <span className="text-yellow-400 font-bold">{score}</span> | Breaths: <span className="text-green-400 font-bold">{totalBreaths}</span> | Combo: <span className="text-purple-400 font-bold">{combo}</span></div>
            </>
          )}
          ...
          <div className="absolute inset-0 flex items-center justify-center">
            {gameState==='start'&&(
              <div className="absolute inset-0 flex items-center justify-center rounded-xl z-10 backdrop-blur-sm" style={{background:isBoxDarkMode?'rgba(5,5,5,0.95)':'rgba(255,255,255,0.95)'}}>
                <div className={`rounded-2xl p-6 sm:p-8 text-center max-w-md mx-4 shadow-xl border ${isBoxDarkMode?'bg-gray-800 border-gray-700':'bg-white border-gray-200'}`}>
                  ...
                  <h2 className={`text-2xl font-bold mb-2 ${isBoxDarkMode?'text-white':'text-gray-900'}`}>Box Breathing</h2>
                  <p className={`mb-2 ${isBoxDarkMode?'text-gray-300':'text-gray-600'}`}>5 points per breath • Combo bonuses</p>
                  <p className={`mb-6 text-sm ${isBoxDarkMode?'text-gray-400':'text-gray-500'}`}>4-4-4-4 tactical breathing used by Navy SEALs. Follow the dot around the square.</p>
                  <button onClick={startSession} className="px-8 py-3 bg-gradient-to-r from-cyan-500 to-blue-600 text-white rounded-xl font-semibold hover:shadow-lg w-full transition-all transform hover:scale-[1.02] active:scale-[0.98]">Start Free Breathing</button>
                </div>
              </div>
            )}
            ...
            {gameState==='playing'&&(
              <div className="text-center">
                <div className="hud-frame" style={{position:'relative',width:isFullscreen?'450px':'300px',height:isFullscreen?'450px':'300px',display:'flex',justifyContent:'center',alignItems:'center'}}>
                  <div className="box-outline" style={{position:'absolute',width:'100%',height:'100%',border:`1px solid ${isBoxDarkMode?'rgba(0,242,254,0.1)':'rgba(2,132,199,0.15)'}`,borderRadius:'4px'}}/>
                  <div ref={dotRef} style={{position:'absolute',width:isFullscreen?'18px':'12px',height:isFullscreen?'18px':'12px',background:isBoxDarkMode?'#00f2fe':'#0284c7',borderRadius:'2px',boxShadow:`0 0 15px ${isBoxDarkMode?'#00f2fe':'#0284c7'}`,top:'calc(0% - 6px)',left:'calc(0% - 6px)',transition:'all 4000ms linear'}}/>
                  <div className="center-label" style={{textAlign:'center',zIndex:10}}>
                    <div style={{fontSize:isFullscreen?'2rem':'1.5rem',letterSpacing:'4px',textTransform:'uppercase',color:isBoxDarkMode?'#00f2fe':'#0284c7',marginBottom:'10px',fontWeight:'300'}}>{action}</div>
                    <div style={{fontFamily:'monospace',fontSize:isFullscreen?'3rem':'2rem',opacity:0.5,color:isBoxDarkMode?'#ffffff':'#000000'}}>{timer}</div>
                  </div>
                </div>
            ...
            {gameState==='gameOver'&&(
              <div className="absolute inset-0 flex items-center justify-center rounded-xl z-20 backdrop-blur-sm" style={{background:isBoxDarkMode?'rgba(5,5,5,0.95)':'rgba(255,255,255,0.95)'}}>
                <div className={`rounded-2xl p-6 sm:p-8 shadow-xl border w-full max-w-[520px] mx-4 ${isBoxDarkMode?'bg-gray-800 border-gray-700':'bg-white border-gray-200'}`}>
                  ...
                  <p className={`text-center text-sm mb-6 ${isBoxDarkMode?'text-gray-400':'text-gray-500'}`}>Regular box breathing improves focus, reduces stress, and enhances performance under pressure.</p>
                  <div className="grid grid-cols-2 gap-3 mb-6">
                    <RCard label="Focus Score" value={score} icon={<Brain className="w-4 h-4"/>} color="cyan" dark={isBoxDarkMode}/>
                    <RCard label="Best Score" value={bestScore} icon={<Trophy className="w-4 h-4"/>} color="yellow" dark={isBoxDarkMode}/>
                    <RCard label="Breaths" value={totalBreaths} icon={<Wind className="w-4 h-4"/>} color="green" dark={isBoxDarkMode}/>
                    <RCard label="Duration" value={formatTime(timeElapsed)} icon={<Timer className="w-4 h-4"/>} color="blue" dark={isBoxDarkMode}/>
                    <RCard label="Max Combo" value={combo} icon={<TrendingUp className="w-4 h-4"/>} color="purple" dark={isBoxDarkMode}/>
                    <RCard label="Focus Level" value={Math.min(100,Math.floor(score/2))} unit="%" icon={<Square className="w-4 h-4"/>} color="orange" dark={isBoxDarkMode}/>
                  </div>
                  <div className="flex gap-3">
                    ...
                    <button onClick={startSession} className="flex-1 px-4 py-2.5 bg-gradient-to-r from-cyan-500 to-blue-600 text-white rounded-lg font-semibold hover:shadow-lg transition-all transform hover:scale-[1.02] active:scale-[0.98]">Breathe Again →</button>
                  </div>
                </div>
              </div>
            )}
          ...
          <footer className="mt-6">
            <div className={`rounded-xl border overflow-hidden ${isDarkMode?'bg-gray-800 border-gray-700':'bg-white border-gray-200'}`}>
              <div className={`px-4 py-3 border-b ${isDarkMode?'border-gray-700 bg-gray-800/50':'border-gray-200 bg-gray-50'}`}>
                <div className="flex items-center gap-2"><Info className={`w-4 h-4 ${isDarkMode?'text-cyan-400':'text-cyan-600'}`}/><h2 className={`font-semibold text-lg ${isDarkMode?'text-white':'text-gray-900'}`}>Box Breathing Instructions</h2></div>
              </div>
              <div className="p-6">
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                  ...
                  <div className="space-y-3">
                    <div className="flex items-start gap-2"><div className="w-5 h-5 rounded-full bg-purple-500 flex items-center justify-center text-white text-xs font-bold flex-shrink-0 mt-0.5">4</div><p className={`text-sm ${isDarkMode?'text-gray-300':'text-gray-600'}`}><span className="font-semibold text-purple-500">HOLD</span> again for <span className="font-semibold">4 seconds</span> to complete the box</p></div>
                    <div className="flex items-start gap-2"><div className="w-5 h-5 rounded-full bg-orange-500 flex items-center justify-center text-white text-xs font-bold flex-shrink-0 mt-0.5">5</div><p className={`text-sm ${isDarkMode?'text-gray-300':'text-gray-600'}`}>Each breath: <span className="font-semibold text-orange-500">5 points</span> + combo bonuses every 3 breaths</p></div>
                    <div className="flex items-start gap-2"><div className="w-5 h-5 rounded-full bg-yellow-500 flex items-center justify-center text-white text-xs font-bold flex-shrink-0 mt-0.5">6</div><p className={`text-sm ${isDarkMode?'text-gray-300':'text-gray-600'}`}>Used by <span className="font-semibold text-yellow-500">Navy SEALs</span> for stress control and focus</p></div>
                  </div>
                </div>
            ...
            <div className={`rounded-xl border overflow-hidden ${isDarkMode ? 'bg-gray-800 border-gray-700' : 'bg-white border-gray-200'}`}>
              ...
              <div className="p-5">
                ...
                <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 mb-5">
                  ...
                  <div className={`p-4 rounded-xl border ${isDarkMode ? 'bg-gray-700/50 border-gray-600' : 'bg-purple-50 border-purple-100'}`}><div className="flex items-center gap-2 mb-2"><div className="w-8 h-8 rounded-lg bg-purple-500 flex items-center justify-center"><Brain className="w-4 h-4 text-white" /></div><h3 className={`text-sm font-semibold ${isDarkMode ? 'text-white' : 'text-gray-900'}`}>What You'll Track</h3></div><p className={`text-xs leading-relaxed ${isDarkMode ? 'text-gray-400' : 'text-gray-500'}`}>Focus score, breath count, session duration, combo streaks, and best performance.</p></div>
                </div>
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                  <div className={`p-4 rounded-xl border ${isDarkMode ? 'bg-gray-700/50 border-gray-600' : 'bg-yellow-50 border-yellow-100'}`}><div className="flex items-center gap-2 mb-3"><div className="w-8 h-8 rounded-lg bg-yellow-500 flex items-center justify-center"><Lightbulb className="w-4 h-4 text-white" /></div><h3 className={`text-sm font-semibold ${isDarkMode ? 'text-white' : 'text-gray-900'}`}>When to Practice</h3></div><ul className={`text-xs space-y-2 ${isDarkMode ? 'text-gray-400' : 'text-gray-600'}`}><li className="flex items-start gap-2"><CheckCircle2 className="w-3 h-3 text-yellow-500 mt-0.5" /> Before stressful meetings or presentations</li><li className="flex items-start gap-2"><CheckCircle2 className="w-3 h-3 text-yellow-500 mt-0.5" /> During study breaks to reset focus</li><li className="flex items-start gap-2"><CheckCircle2 className="w-3 h-3 text-yellow-500 mt-0.5" /> Morning routine for calm start to the day</li></ul></div>
                  <div className={`p-4 rounded-xl border ${isDarkMode ? 'bg-gray-700/50 border-gray-600' : 'bg-amber-50 border-amber-100'}`}><div className="flex items-center gap-2 mb-3"><div className="w-8 h-8 rounded-lg bg-amber-500 flex items-center justify-center"><Clock className="w-4 h-4 text-white" /></div><h3 className={`text-sm font-semibold ${isDarkMode ? 'text-white' : 'text-gray-900'}`}>How to Practice Effectively</h3></div><ol className={`text-xs space-y-2 ${isDarkMode ? 'text-gray-400' : 'text-gray-600'}`}><li className="flex items-start gap-2"><span className="w-5 h-5 rounded-full bg-amber-500 text-white text-xs flex items-center justify-center flex-shrink-0 mt-0.5">1</span> Sit comfortably with back straight</li><li className="flex items-start gap-2"><span className="w-5 h-5 rounded-full bg-amber-500 text-white text-xs flex items-center justify-center flex-shrink-0 mt-0.5">2</span> Follow the dot's movement around the square</li><li className="flex items-start gap-2"><span className="w-5 h-5 rounded-full bg-amber-500 text-white text-xs flex items-center justify-center flex-shrink-0 mt-0.5">3</span> Breathe with the visual pacer, not against it</li><li className="flex items-start gap-2"><span className="w-5 h-5 rounded-full bg-amber-500 text-white text-xs flex items-center justify-center flex-shrink-0 mt-0.5">4</span> Practice 5-10 minutes daily for best results</li></ol></div>
                </div>
              </div>
            </div>
          </section>
        ...
        {!isFullscreen && (
          <section className="mt-8" aria-label="Related training drills">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
              <Link href="/drills/mental-fitness/breathing-exercises/4-7-8" className={`group relative overflow-hidden rounded-xl border transition-all duration-300 hover:shadow-lg hover:-translate-y-1 ${isDarkMode ? 'bg-gray-800 border-gray-700 hover:border-indigo-500' : 'bg-white border-gray-200 hover:border-indigo-300'}`}><div className="absolute top-0 left-0 right-0 h-1 bg-gradient-to-r from-indigo-500 to-purple-500"></div><div className="p-4"><div className="flex items-center gap-2 mb-2"><div className="w-8 h-8 rounded-lg bg-indigo-100 flex items-center justify-center"><Wind className="w-4 h-4 text-indigo-600" /></div><span className={`text-xs px-2 py-0.5 rounded-full font-medium ${isDarkMode ? 'bg-gray-700 text-gray-400' : 'bg-gray-100 text-gray-500'}`}>Breathing</span></div><h3 className={`font-semibold text-sm mb-1 ${isDarkMode ? 'text-white group-hover:text-indigo-400' : 'text-gray-900 group-hover:text-indigo-600'} transition-colors`}>4-7-8 Breathing</h3><p className={`text-xs leading-relaxed ${isDarkMode ? 'text-gray-500' : 'text-gray-400'}`}>Deep relaxation technique for sleep and anxiety relief.</p><div className="flex items-center gap-1 mt-3 text-indigo-500 text-xs font-medium opacity-0 group-hover:opacity-100 transition-opacity">Start Drill <ArrowRight className="w-3 h-3" /></div></div></Link>
...
function RCard({ label, value, unit='', icon, color, dark }) {
  const m={cyan:'bg-cyan-500/10 border-cyan-500/30 text-cyan-500',yellow:'bg-yellow-500/10 border-yellow-500/30 text-yellow-500',green:'bg-green-500/10 border-green-500/30 text-green-500',blue:'bg-blue-500/10 border-blue-500/30 text-blue-500',purple:'bg-purple-500/10 border-purple-500/30 text-purple-500',orange:'bg-orange-500/10 border-orange-500/30 text-orange-500'};
