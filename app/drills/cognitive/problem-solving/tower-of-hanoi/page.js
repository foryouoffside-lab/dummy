...
import { 
  ...
  BarChart3, Info, Layers, RotateCcw, Move, Award
} from 'lucide-react';
...
export default function TowerOfHanoiPage() {
  ...
  const handleTowerClick = (towerIndex) => {
    ...
    if (fromDisk && (!toDisk || fromDisk < toDisk)) {
      ...
      // Invalid move - just show feedback without penalty
    ...
    <div className={`min-h-screen select-none ${isDarkMode ? 'bg-gray-900' : 'bg-gray-50'}`}>
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
          ...
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <p className={`${isDarkMode ? 'text-gray-400' : 'text-gray-500'}`}>Move disks to right • +10 per level • No penalties</p>
              </div>
            </div>
        ...
        <div className="grid grid-cols-7 gap-3 mb-4 h-[88px]">
          ...
          <StatCard icon={<BarChart3 className="text-emerald-600" />} value={getEfficiency()} label="Eff" unit="%" isDark={isDarkMode} />
            ...
            {gameState === 'start' && (
              <div className={`absolute inset-0 flex items-center justify-center backdrop-blur-sm rounded-xl z-40 ${isBoxDarkMode ? 'bg-gray-900/95' : 'bg-white/95'}`}>
                <div className={`rounded-2xl p-8 text-center max-w-md mx-4 shadow-xl border ${isBoxDarkMode ? 'bg-gray-800 border-gray-700' : 'bg-white border-gray-200'}`}>
                  ...
                  <p className={`mb-6 ${isBoxDarkMode ? 'text-gray-300' : 'text-gray-600'}`}>60-second challenge • +10 per level • No penalties</p>
                  <button 
                    onClick={startGame}
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  ...
                  <div className="space-y-2">
                    ...
                    <div className="flex items-start gap-2">
                      ...
                      <p className={`text-sm ${isDarkMode ? 'text-gray-300' : 'text-gray-600'}`}>Perfect moves = <span className="font-semibold text-orange-500">special celebration</span></p>
                    </div>
                  </div>
                </div>
