...
const STORAGE_KEY = 'skilldrills_visual_tracking_spatial_shift_pursuit_v2';
...
export default function SpatialShiftPursuitClient() {
  ...
  const sharePage = useCallback(async () => {
    const url = 'https://skilldrills.online/drills/visual-tracking/spatial-shift-pursuit';
    try {
      const canvas = generateSessionCard({
        drillName: 'Spatial Shift Pursuit',
      ...
      const text = 'Spatial Shift Pursuit — Free Visual Tracking & Gaze Calibration Drill!';
      if (typeof navigator !== 'undefined' && navigator.share) {
        try {
          await navigator.share({ title: 'Spatial Shift Pursuit Drill', text, url });
        } catch (e) {}
  ...
  const enterDrill = useCallback(async () => {
    ...
    trackingState.current = {
      ...
      vy: (Math.random() > 0.5 ? 1 : -1) * (5 + Math.random() * 3),
      trail: []
    };
  ...
  // Canvas Render Loop (Spatial Shift Physics)
  useEffect(() => {
    ...
    const draw = (ts: number) => {
      ...
      // Random Velocity Spatial Shift Logic
      const shiftChance = randomSpeed ? 0.03 : 0.015;
      if (Math.random() < shiftChance) {
        trackingState.current.vx *= (Math.random() > 0.5 ? 1 : -1) * (0.8 + Math.random() * 0.5);
        trackingState.current.vy *= (Math.random() > 0.5 ? 1 : -1) * (0.8 + Math.random() * 0.5);

        const maxS = 18;
        trackingState.current.vx = Math.max(Math.min(trackingState.current.vx, maxS), -maxS);
        trackingState.current.vy = Math.max(Math.min(trackingState.current.vy, maxS), -maxS);
      }
      ...
      // Render Spatial Velocity Direction Line ONLY if math is NOT invisible
      if (!mathInvisible) {
        ...
        ctx.lineTo(
          trackingState.current.px + trackingState.current.vx * 7,
          trackingState.current.py + trackingState.current.vy * 7
        );
        ctx.stroke();
      }
        ...
        <header className="border-b border-white/5 bg-[#080811]/80 backdrop-blur-md sticky top-0 z-50">
          <div className="max-w-6xl mx-auto px-4 h-14 flex items-center justify-between">
            <div className="flex items-center gap-2 text-xs text-slate-400">
              ...
              <span>/</span>
              <span className="text-red-400 font-medium">Spatial Shift Pursuit</span>
            </div>
        ...
        {!isFullscreen && (
          <div className="text-center">
            ...
            <p className="text-xs text-slate-400 mt-1">
              Dynamic Spatial Vector & Instant Directional Adaptation
            </p>
          </div>
        )}
          ...
          <div className="[&>div]:!mt-0">
            ...
            >
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4 font-sans">
                <RuleItem num="1" text="Spatial Velocity Shifts" highlight="Pure Visual" result="Adapt to vector changes" />
            ...
            <DrillAccordion
              id="about"
              title="About Spatial Shift Pursuit"
              ...
              <div className="space-y-6 font-sans">
                <section>
                  <h4 className="text-base font-bold text-white mb-2">
                    What Is Spatial Shift Pursuit Training?
                  </h4>
                  <p className="text-sm leading-relaxed mb-3 text-gray-300">
                    <strong>Spatial Shift Pursuit Training</strong> conditions visual tracking agility by forcing your eyes to adapt to sudden randomized shifts in target velocity and spatial vectors.
                  </p>
                  <p className="text-sm leading-relaxed text-gray-300">
                    By utilizing features like <strong>Hide Line</strong> and <strong>Random Speed Acceleration</strong>, your visual cortex learns to track unpredictable spatial direction shifts independently.
                  </p>
                </section>
                ...
                <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
                  <div className="p-4 rounded-xl border border-gray-800 bg-white/[0.02]">
                    ...
                    <p className="text-xs text-gray-300 leading-relaxed">Gamers, athletes, and vision training practitioners looking to build dynamic spatial tracking agility.</p>
                  </div>
                  <div className="p-4 rounded-xl border border-gray-800 bg-white/[0.02]">
                    <h5 className="text-xs font-bold text-white mb-1.5">Spatial Adaptability</h5>
                    <p className="text-xs text-gray-300 leading-relaxed">Condition eye muscles to re-acquire target trajectories immediately post-vector shift.</p>
                  </div>
                  <div className="p-4 rounded-xl border border-gray-800 bg-white/[0.02]">
            ...
            >
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4 font-sans">
                <FAQItem q="What is the Spatial Shift Pursuit drill?" a="The Spatial Shift Pursuit drill conditions visual tracking agility by forcing your eyes to adapt to sudden randomized shifts in target velocity and spatial vectors." />
                <FAQItem q="Why use the 'Hide Line' setting?" a="Hiding velocity path vectors forces your visual system to track instantaneous direction shifts without visual motion cues." />
                <FAQItem q="What does the Random Speed feature do?" a="Random Speed introduces unpredictable shift frequencies and magnitude spikes, strengthening ocular motor control." />
