...
export default function SingleLegEquilibriumPage() {
  ...
  const update = (dt, cvs) => {
    ...
    if (dist < 60) {
      ...
      scoreRef.current += Math.floor(120 * dt);
      streakRef.current++;
      ...
      if (streakRef.current % 50 === 0 && streakRef.current > 0) {
        playSound('streak');
        showFeedback(`ðŸ”¥ ${streakRef.current} Streak!`, 'success');
      } else if (Math.random() < 0.008) {
        playSound('focus');
      }
    } else {
      const wasLinked = streakRef.current > 0;
      scoreRef.current = Math.max(0, scoreRef.current - Math.floor(200 * dt));
      streakRef.current = Math.max(0, streakRef.current - 2);
      
      if (streakRef.current === 0 && wasLinked) {
        showFeedback('âš ï¸ Link broken!', 'warning');
        playSound('break');
      }
    }
    ...
    <div className={`min-h-screen select-none ${isDarkMode ? 'bg-gray-900' : 'bg-gray-50'}`}>
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
          ...
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <p className={`${isDarkMode ? 'text-gray-400' : 'text-gray-500'}`}>Keep the link connected - 60 second balance challenge</p>
              </div>
            </div>
        ...
        <div className="grid grid-cols-5 gap-3 mb-4 h-[88px]">
          ...
          <StatCard icon={<LinkIcon className="text-green-600" />} value={linkStability} label="Link Stability" unit="%" isDark={isDarkMode} />
          ...
          {gameState === 'start' && (
            <div className={`absolute inset-0 flex items-center justify-center backdrop-blur-sm rounded-xl z-40 ${isBoxDarkMode ? 'bg-gray-900/95' : 'bg-white/95'}`}>
              <div className={`rounded-2xl p-8 text-center max-w-md mx-4 shadow-xl border ${isBoxDarkMode ? 'bg-gray-800 border-gray-700' : 'bg-white border-gray-200'}`}>
                ...
                <p className={`mb-6 ${isBoxDarkMode ? 'text-gray-300' : 'text-gray-600'}`}>60-second challenge â€¢ Keep the green link connected</p>
                <button 
                  onClick={startGame} 
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  <div className="space-y-2">
                    ...
                    <div className="flex items-start gap-2">
                      ...
                      <p className={`text-sm ${isDarkMode ? 'text-gray-300' : 'text-gray-600'}`}>Broken link causes <span className="font-semibold text-red-500">score decay and streak reset</span></p>
                    </div>
                    <div className="flex items-start gap-2">
                      ...
                      <p className={`text-sm ${isDarkMode ? 'text-gray-300' : 'text-gray-600'}`}>Every <span className="font-semibold text-orange-500">50 consecutive seconds linked</span> triggers a bonus</p>
                    </div>
                  </div>
                  <div className="space-y-2">
                    <div className="flex items-start gap-2">
                      ...
                      <p className={`text-sm ${isDarkMode ? 'text-gray-300' : 'text-gray-600'}`}>The anchor moves with <span className="font-semibold text-blue-500">random velocity and bounces</span> off walls</p>
                    </div>
                    <div className="flex items-start gap-2">
                      ...
                      <p className={`text-sm ${isDarkMode ? 'text-gray-300' : 'text-gray-600'}`}>Challenge lasts <span className="font-semibold text-purple-500">60 seconds</span> - maintain equilibrium</p>
                    </div>
                    <div className="flex items-start gap-2">
                      <div className="w-5 h-5 rounded-full bg-yellow-500 flex items-center justify-center text-white text-xs font-bold mt-0.5">6</div>
                      <p className={`text-sm ${isDarkMode ? 'text-gray-300' : 'text-gray-600'}`}>Your <span className="font-semibold text-yellow-500">Best Score</span> is saved locally across sessions</p>
                    </div>
                  </div>
                </div>
                <div className={`mt-3 pt-3 border-t text-xs ${isDarkMode ? 'border-gray-700 text-gray-400' : 'border-gray-200 text-gray-500'} flex items-center justify-between`}>
                  <span>ðŸ”— Link Stability = Time connected Ã· Total time</span>
                  <span>ðŸŸ¢ Green link = Connected â€¢ ðŸ”´ Red dashed = Broken</span>
                </div>
              </div>
            </div>
          </div>
