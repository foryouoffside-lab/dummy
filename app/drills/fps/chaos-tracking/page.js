...
import { 
  ...
  Move, Activity, Award, Info, RefreshCw
} from 'lucide-react';
...
export default function ReactiveTrackingPage() {
  ...
  // Tracking update interval - 0.5 sec tracking = 1 score
  useEffect(() => {
    ...
    const interval = setInterval(() => {
      ...
      // 0.5 second tracking = 1 score
      if (newAccuracy > 50) {
        trackingTimeRef.current += 0.1; // Add 0.1s every 100ms interval
        
        if (trackingTimeRef.current >= 0.5) {
          const halfSecondsTracked = Math.floor(trackingTimeRef.current / 0.5);
          const pointsEarned = halfSecondsTracked * 1; // 1 point per 0.5 seconds
          scoreRef.current += pointsEarned;
          setTrackingScore(scoreRef.current);
          trackingTimeRef.current -= halfSecondsTracked * 0.5;
          
          comboRef.current += pointsEarned;
          ...
          if (comboRef.current % 10 === 0) playSound('perfect');
        }
      } else {
        // Reset tracking time and combo
        trackingTimeRef.current = 0;
        comboRef.current = 0;
        setTrackingCombo(0);
      }
    }, 100); // Run every 100ms for smooth tracking
    
    return () => clearInterval(interval);
  }, [gameState, bestAccuracy, bestCombo]);
  ...
  useEffect(() => {
        ...
        targetsRef.current = [
          {
            ...
            dir: 1,
            color: '#00f2ff',
            isPrimary: true  // Primary target - focus on this
          },
          {
            x: width / 2,
            y: height / 2 + 80,
            ...
            dir: -1,
            color: '#ff00ff',
            isPrimary: false  // Secondary target - track with peripheral
          }
        ];
      } else {
    ...
    function drawSpheres() {
      targetsRef.current.forEach(t => {
        // Primary target gets brighter glow
        ...
        gradient.addColorStop(1, t.color + (t.isPrimary ? 'ff' : 'cc'));
        ...
        // Label for primary target
        if (t.isPrimary) {
          ctx.font = 'bold 14px monospace';
          ctx.fillStyle = '#00f2ff';
          ctx.textAlign = 'center';
          ctx.fillText('FOCUS', t.x, t.y - 40);
        } else {
          ctx.font = '12px monospace';
          ctx.fillStyle = '#ff00ff';
          ctx.textAlign = 'center';
          ctx.fillText('PERIPHERAL', t.x, t.y - 40);
        }
      });
    }
      ...
      if (mouse.x > 0 && mouse.x < cvs.width && mouse.y > 0 && mouse.y < cvs.height) {
        ...
        targetsRef.current.forEach(t => {
          ...
          if (dist < 150) {
            ...
            ctx.lineTo(t.x, t.y);
            ctx.strokeStyle = t.isPrimary 
              ? `rgba(0, 255, 136, ${0.4 * (1 - dist / 150)})`
              : `rgba(255, 0, 255, ${0.2 * (1 - dist / 150)})`;
        ...
        <div className="mb-6">
          ...
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-3">
              ...
              <div>
                <h1 className={`text-3xl font-bold ${isDarkMode ? 'text-white' : 'text-gray-900'}`}>Peripheral Tracking</h1>
                <p className={`${isDarkMode ? 'text-gray-400' : 'text-gray-500'}`}>Focus on cyan • 0.5s tracking = 1 score</p>
              </div>
            </div>
          ...
          {gameState === 'start' && (
            <div className={`absolute inset-0 flex items-center justify-center backdrop-blur-sm rounded-xl z-40 ${isBoxDarkMode ? 'bg-gray-900/95' : 'bg-white/95'}`}>
              <div className={`rounded-2xl p-8 text-center max-w-md mx-4 shadow-xl border ${isBoxDarkMode ? 'bg-gray-800 border-gray-700' : 'bg-white border-gray-200'}`}>
                ...
                <h3 className={`text-2xl font-bold mb-2 ${isBoxDarkMode ? 'text-white' : 'text-gray-900'}`}>Peripheral Tracking</h3>
                <p className={`mb-6 ${isBoxDarkMode ? 'text-gray-300' : 'text-gray-600'}`}>Focus on CYAN • 0.5s tracking = 1 score</p>
                <button 
                  onClick={startGame} 
            ...
            <div className={`rounded-xl border overflow-hidden ${isDarkMode ? 'bg-gray-800 border-gray-700' : 'bg-white border-gray-200'}`}>
              <div className={`px-4 py-3 border-b ${isDarkMode ? 'border-gray-700 bg-gray-800/50' : 'border-gray-200 bg-gray-50'}`}>
                <div className="flex items-center gap-2">
                  ...
                  <h3 className={`font-semibold ${isDarkMode ? 'text-white' : 'text-gray-900'}`}>Peripheral Vision Training Rules</h3>
                </div>
              </div>
              <div className="p-4">
                  ...
                  <div className="space-y-2">
                    <div className="flex items-start gap-2">
                      ...
                      <p className={`text-sm ${isDarkMode ? 'text-gray-300' : 'text-gray-600'}`}>
                        <span className="font-semibold text-cyan-500">FOCUS on the CYAN sphere</span> • Keep your eyes on this target
                      </p>
                    </div>
                    <div className="flex items-start gap-2">
                      ...
                      <p className={`text-sm ${isDarkMode ? 'text-gray-300' : 'text-gray-600'}`}>
                        <span className="font-semibold text-purple-500">Track MAGENTA with peripheral vision</span> • Don't look directly at it
                      </p>
                    </div>
                    <div className="flex items-start gap-2">
                      ...
                      <p className={`text-sm ${isDarkMode ? 'text-gray-300' : 'text-gray-600'}`}>
                        <span className="font-semibold text-green-500">Keep cursor centered between both</span> • Train spatial awareness
                      </p>
                    </div>
                  </div>
                  <div className="space-y-2">
                    <div className="flex items-start gap-2">
                      ...
                      <p className={`text-sm ${isDarkMode ? 'text-gray-300' : 'text-gray-600'}`}>
                        <span className="font-semibold text-blue-500">0.5 sec tracking = 1 score</span> • Build your score over time
                      </p>
                    </div>
                    <div className="flex items-start gap-2">
                      ...
                      <p className={`text-sm ${isDarkMode ? 'text-gray-300' : 'text-gray-600'}`}>
                        <span className="font-semibold text-orange-500">Poor tracking resets combo</span> • No score penalty
                      </p>
                    </div>
                    <div className="flex items-start gap-2">
                ...
                <div className={`mt-3 pt-3 border-t text-xs ${isDarkMode ? 'border-gray-700 text-gray-400' : 'border-gray-200 text-gray-500'} flex items-center justify-between`}>
                  <span>🎯 Cyan = Primary Focus • Magenta = Peripheral Track</span>
                  <span>⚡ Green connection lines show tracking quality</span>
                </div>
              </div>
            </div>
          </div>
