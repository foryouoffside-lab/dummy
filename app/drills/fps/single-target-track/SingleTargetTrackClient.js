...
import { 
  ...
  Lock, AlertCircle, RefreshCw, Crosshair, Dumbbell, Database, Keyboard, Star, Users,
  GraduationCap, Lightbulb, TrendingUp, Clock, ArrowRight, BookOpen, Brain, Code2, Hash
} from 'lucide-react';

const TARGET_RADIUS = 30;
const SPEED = 8;
const SCORE_INTERVAL = 200;
...
export default function SingleTargetTrackClient() {
  ...
  const sharePage = async () => { if (navigator.share) { try { await navigator.share({ title: 'Free Single Target Track Drill | SkillDrills', text: 'Smooth tracking aim trainer with raw mouse input!', url: 'https://skilldrills.online/drills/fps/single-target-track' }); } catch (e) {} } else { navigator.clipboard.writeText('https://skilldrills.online/drills/fps/single-target-track'); alert('Link copied!'); } };
  const copyPageLink = () => { navigator.clipboard.writeText('https://skilldrills.online/drills/fps/single-target-track'); alert('Link copied!'); };
          ...
          <section className="mt-8" aria-label="About this tracking drill">
            <div className={`rounded-xl border overflow-hidden ${isDarkMode ? 'bg-gray-800 border-gray-700' : 'bg-white border-gray-200'}`}>
              <div className={`px-4 py-3 border-b ${isDarkMode ? 'border-gray-700 bg-gray-800/50' : 'border-gray-200 bg-gray-50'}`}>
                <div className="flex items-center gap-2"><GraduationCap className={`w-5 h-5 ${isDarkMode ? 'text-gray-400' : 'text-gray-600'}`} /><h2 className={`font-semibold ${isDarkMode ? 'text-white' : 'text-gray-900'}`}>About This Free Tracking Drill</h2></div>
              </div>
              <div className="p-5">
                <p className={`text-sm leading-relaxed mb-5 ${isDarkMode ? 'text-gray-300' : 'text-gray-600'}`}>This free single target tracking drill uses raw mouse input via Pointer Lock API for precise aim training. Track a smoothly moving target with real-time accuracy feedback and lock-on visual effects. Perfect for building mouse control for FPS games like Valorant, CS2, Apex Legends, and Overwatch.</p>
                <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
                  <div className={`p-4 rounded-xl border ${isDarkMode ? 'bg-gray-700/50 border-gray-600' : 'bg-gray-50 border-gray-100'}`}><div className="flex items-center gap-2 mb-2"><div className="w-8 h-8 rounded-lg bg-gray-500 flex items-center justify-center"><Crosshair className="w-4 h-4 text-white" /></div><h3 className={`text-sm font-semibold ${isDarkMode ? 'text-white' : 'text-gray-900'}`}>Who It's For</h3></div><p className={`text-xs leading-relaxed ${isDarkMode ? 'text-gray-400' : 'text-gray-500'}`}>FPS gamers wanting smoother tracking aim in Valorant, CS2, Apex, Overwatch, and competitive shooters.</p></div>
                  <div className={`p-4 rounded-xl border ${isDarkMode ? 'bg-gray-700/50 border-gray-600' : 'bg-gray-50 border-gray-100'}`}><div className="flex items-center gap-2 mb-2"><div className="w-8 h-8 rounded-lg bg-green-500 flex items-center justify-center"><TrendingUp className="w-4 h-4 text-white" /></div><h3 className={`text-sm font-semibold ${isDarkMode ? 'text-white' : 'text-gray-900'}`}>Skills Improved</h3></div><p className={`text-xs leading-relaxed ${isDarkMode ? 'text-gray-400' : 'text-gray-500'}`}>Mouse control, smooth tracking, hand-eye coordination, and sustained aim precision.</p></div>
                  <div className={`p-4 rounded-xl border ${isDarkMode ? 'bg-gray-700/50 border-gray-600' : 'bg-gray-50 border-gray-100'}`}><div className="flex items-center gap-2 mb-2"><div className="w-8 h-8 rounded-lg bg-purple-500 flex items-center justify-center"><Activity className="w-4 h-4 text-white" /></div><h3 className={`text-sm font-semibold ${isDarkMode ? 'text-white' : 'text-gray-900'}`}>What You'll Track</h3></div><p className={`text-xs leading-relaxed ${isDarkMode ? 'text-gray-400' : 'text-gray-500'}`}>Score, accuracy percentage, tracking streak, and best performance across sessions.</p></div>
                </div>
              </div>
            </div>
          </section>
