...
export default function MultiplicationClient() {
  ...
  const getPointsForCorrect = useCallback(() => { if (range === 20) return 3; if (range === 15) return 2; return 1; }, [range]);
  const getPenaltyPoints = useCallback(() => { if (range === 20) return 3; if (range === 15) return 2; return 1; }, [range]);

  const generateProblem = useCallback(() => {
    if (timeoutRef.current) { clearTimeout(timeoutRef.current); timeoutRef.current = null; }
    const weights = [3, 4, 6, 7, 8, 9, 11, 12];
    const getNum = () => { const useWeight = Math.random() > 0.3; return useWeight ? weights[Math.floor(Math.random() * weights.length)] : Math.floor(Math.random() * range) + 2; };
    let a, b, answer, questionKey; let attempts = 0; const maxAttempts = 100; const allowRepeats = usedQuestionsRef.current.size > 50;
    do { a = getNum(); b = getNum(); answer = a * b; questionKey = `${a}×${b}`; attempts++; if (attempts >= maxAttempts) break; } while (!allowRepeats && usedQuestionsRef.current.has(questionKey));
    usedQuestionsRef.current.add(questionKey);
    const options = [answer]; while (options.length < 4) { let fa; const v = range <= 12 ? 20 : range <= 15 ? 40 : 80; fa = answer + (Math.floor(Math.random() * v * 2) - v); if (fa > 0 && !options.includes(fa)) options.push(fa); }
    const shuffledOptions = [...options].sort(() => Math.random() - 0.5);
    setCurrentProblem({ a, b, answer, options: shuffledOptions }); setSelectedOption(null); setCanSelect(true); setShowCorrectAnswer(false);
  }, [range]);

  const moveToNextQuestion = useCallback(() => { if (timeLeft > 0) generateProblem(); }, [timeLeft, generateProblem]);
  ...
  const handleOptionSelect = useCallback((selectedAnswer) => {
    ...
    if (selectedAnswer === currentProblem.answer) {
      ...
      timeoutRef.current = setTimeout(() => { moveToNextQuestion(); timeoutRef.current = null; }, 400);
    } else {
      ...
      if (timeoutRef.current) clearTimeout(timeoutRef.current);
      timeoutRef.current = setTimeout(() => { moveToNextQuestion(); timeoutRef.current = null; }, 800);
    }
 ...
 if (timerRef.current) clearInterval(timerRef.current); if (timeoutRef.current) clearTimeout(timeoutRef.current); setGameState('playing'); setScore(0); setTimeLeft(60); setSolvedCount(0); setErrorCount(0); setCombo(0); setLives(3); setSelectedOption(null); setCanSelect(true); setFeedback(''); setShowCorrectAnswer(false); usedQuestionsRef.current = new Set(); livesRef.current = 3; scoreRef.current = 0; comboRef.current = 0; generateProblem(); }, [generateProblem]);
  ...
  const resetGame = useCallback(() => { if (timerRef.current) clearInterval(timerRef.current); if (timeoutRef.current) clearTimeout(timeoutRef.current); if (feedbackTimeoutRef.current) clearTimeout(feedbackTimeoutRef.current); setGameState('start'); setScore(0); setTimeLeft(60); setSolvedCount(0); setErrorCount(0); setCombo(0); setLives(3); setSelectedOption(null); setCanSelect(true); setFeedback(''); setShowCorrectAnswer(false); usedQuestionsRef.current = new Set(); livesRef.current = 3; scoreRef.current = 0; comboRef.current = 0; }, []);
  ...
  const sharePage = async () => { if (navigator.share) { try { await navigator.share({ title: 'Free Multiplication Tables Drill | SkillDrills', text: 'Master times tables with intelligent practice. Free!', url: 'https://skilldrills.online/drills/academic/math-speed/multiplication-tables' }); } catch (e) {} } else { navigator.clipboard.writeText('https://skilldrills.online/drills/academic/math-speed/multiplication-tables'); alert('Link copied!'); } };
  const copyPageLink = () => { navigator.clipboard.writeText('https://skilldrills.online/drills/academic/math-speed/multiplication-tables'); alert('Link copied!'); };

  if (loading || !isClient) { return (<div className="min-h-screen flex items-center justify-center bg-gray-50"><div className="text-center"><div className="w-16 h-16 border-4 border-indigo-600 border-t-transparent rounded-full animate-spin mx-auto mb-4"></div><p className="text-gray-600">Loading multiplication tables drill...</p></div></div>); }
  ...
  return (
    <div className={`min-h-screen select-none ${isDarkMode ? 'bg-gray-900' : 'bg-gray-50'}`}>
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        <nav aria-label="Breadcrumb" className="mb-4">
          <ol className="flex flex-wrap items-center gap-2 text-sm">
            ...
            <li className={`${isDarkMode ? 'text-gray-500' : 'text-gray-400'}`} aria-hidden="true">/</li>
            <li className={`${isDarkMode ? 'text-gray-500' : 'text-gray-400'}`}>Math Speed</li>
            <li className={`${isDarkMode ? 'text-gray-500' : 'text-gray-400'}`} aria-hidden="true">/</li>
            <li className={`font-medium ${isDarkMode ? 'text-indigo-400' : 'text-indigo-600'}`} aria-current="page">Multiplication Tables</li>
          </ol>
        </nav>
        
        <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 mb-6">
          <div className="flex items-center gap-3">
            <div className="p-3 bg-gradient-to-r from-indigo-500 to-purple-600 rounded-xl flex-shrink-0"><Hash className="w-6 h-6 text-white" /></div>
            <div><h1 className={`text-2xl sm:text-3xl font-bold ${isDarkMode ? 'text-white' : 'text-gray-900'}`}>Multiplication Tables</h1><p className={`text-sm sm:text-base ${isDarkMode ? 'text-gray-400' : 'text-gray-500'}`}>Intelligent practice • 60-second challenge • 3 ranges • Free times tables training</p></div>
          </div>
        ...
        <section className="sr-only" aria-label="Drill description for search engines">
          <h2>Free Multiplication Tables Drill - Times Tables Speed Practice for SAT GRE GMAT CAT UPSC SSC Banking Exams</h2>
          <p>Master multiplication tables with intelligent practice that prioritizes high friction numbers 6 through 12 for maximum learning efficiency. Choose from 3 ranges 12x12 15x15 and 20x20. 4 option multiple choice with plausible distractors. 3 life protection system combo streaks every 3 correct answers and keyboard shortcuts 1 4 keys for rapid answering. 60 second timed format with adaptive scoring. Perfect for students competitive exam preparation and brain training. No registration required.</p>
        </section>
        ...
        <div className="flex justify-center gap-3 mb-4">
          <div className={`flex p-1 rounded-xl ${isDarkMode ? 'bg-gray-800' : 'bg-gray-200'}`} role="radiogroup" aria-label="Multiplication range">
            {[12, 15, 20].map(r => { 
              const pts = r === 20 ? '3' : r === 15 ? '2' : '1'; 
              return (<button key={r} onClick={() => setRange(r)} role="radio" aria-checked={range === r} className={`px-6 py-2 rounded-lg text-sm font-bold transition-all ${range === r ? 'bg-gradient-to-r from-indigo-500 to-purple-600 text-white shadow-lg' : `${isDarkMode ? 'text-gray-400 hover:text-gray-200' : 'text-gray-500 hover:text-gray-700'}`} focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:ring-offset-2`} aria-label={`${r}×${r} range - ${pts} points per correct answer`}>{r}×{r} ({pts}pt)</button>); 
            })}
          </div>
        </div>
      ...
      {showRotateWarning && (
        <div className="absolute inset-0 z-50 flex flex-col items-center justify-center bg-gray-950/95 text-center p-6" aria-hidden="true">
          ...
          {isFullscreen && gameState === 'playing' && (
            <div className="absolute top-4 right-4 z-30 flex gap-3">
              <button onClick={resetGame} className="p-2.5 bg-black/50 backdrop-blur-sm rounded-lg text-white hover:bg-black/70 transition-all" title="Reset session" aria-label="Reset drill session"><RefreshCw className="w-5 h-5" /></button>
          ...
          <div className="absolute inset-0 flex items-center justify-center p-4 sm:p-8 overflow-y-auto">
            {gameState === 'start' && (
              <div className={`absolute inset-0 flex items-center justify-center backdrop-blur-sm rounded-xl z-40 ${isBoxDarkMode ? 'bg-gray-900/95' : 'bg-white/95'}`}>
                <div className={`rounded-2xl p-6 sm:p-8 text-center max-w-md mx-4 shadow-xl border ${isBoxDarkMode ? 'bg-gray-800 border-gray-700' : 'bg-white border-gray-200'}`}>
                  <div className="mb-4"><Hash className="w-16 h-16 text-indigo-500 mx-auto" aria-hidden="true" /></div>
                  <h2 className={`text-2xl font-bold mb-2 ${isBoxDarkMode ? 'text-white' : 'text-gray-900'}`}>Multiplication Tables</h2>
                  <p className={`mb-2 ${isBoxDarkMode ? 'text-gray-300' : 'text-gray-600'}`}>60-second challenge • Up to {range}×{range} • +{getPointsForCorrect()}pt per correct</p>
                  <p className={`mb-6 text-sm ${isBoxDarkMode ? 'text-gray-400' : 'text-gray-500'}`}>Intelligent practice prioritizes harder facts 6-12. 3 lives protect your score. Use keys 1-4 for faster answering. Perfect for students and competitive exam math preparation.</p>
                  <button onClick={startGame} className="px-8 py-3 bg-gradient-to-r from-indigo-500 to-purple-600 text-white rounded-xl font-semibold hover:shadow-lg w-full transition-all transform hover:scale-[1.02] active:scale-[0.98] focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:ring-offset-2" aria-label="Start free multiplication tables drill">Start Free Drill</button>
                </div>
              </div>
            )}
            ...
            {gameState === 'playing' && (
              <div className="w-full max-w-2xl">
                <div className={`text-5xl sm:text-6xl md:text-8xl font-bold text-center mb-6 sm:mb-8 ${isBoxDarkMode ? 'text-white' : 'text-gray-900'}`}>{currentProblem.a} <span className="text-indigo-500">×</span> {currentProblem.b} = ?</div>
                <div className="grid grid-cols-2 gap-3 sm:gap-4" role="radiogroup" aria-label="Answer options">
                  {currentProblem.options.map((option, index) => { 
                    ...
                    let bc = `p-4 sm:p-6 rounded-2xl text-xl sm:text-2xl md:text-3xl font-bold transition-all relative `; 
                    if (isSelected || (showCorrectAnswer && isCorrect)) { 
                      if (isSelected && !isCorrect) bc += 'bg-red-500 text-white scale-95 shadow-lg'; 
                      else if (isCorrect && showCorrectAnswer) bc += 'bg-green-500 text-white scale-105 shadow-lg ring-2 ring-green-300'; 
                      else if (isSelected && isCorrect) bc += 'bg-green-500 text-white scale-105 shadow-lg ring-2 ring-green-300'; 
                    } else { 
                      bc += isBoxDarkMode ? 'bg-gray-800 text-white hover:bg-gray-700 border border-gray-700' : 'bg-gray-100 text-gray-900 hover:bg-gray-200 border border-gray-200'; 
                      bc += canSelect && selectedOption === null ? ' hover:scale-105 hover:shadow-lg cursor-pointer active:scale-[0.98]' : ' opacity-60 cursor-not-allowed'; 
                    } 
                    return (<button key={index} onClick={() => canSelect && selectedOption === null && handleOptionSelect(option)} disabled={!canSelect || selectedOption !== null} className={bc} aria-label={`Option ${keyNum}: ${option}`}><span className="absolute top-2 left-3 text-xs opacity-50 font-mono">{keyNum}</span>{option}</button>); 
                  })}
                </div>
                <p className={`text-center mt-4 text-xs ${isBoxDarkMode ? 'text-gray-500' : 'text-gray-400'}`}>Press 1-4 keys to answer faster</p>
              </div>
            )}
            ...
            {gameState === 'gameOver' && (
              <div className={`absolute inset-0 flex items-center justify-center backdrop-blur-sm rounded-xl z-40 ${isBoxDarkMode ? 'bg-gray-900/95' : 'bg-white/95'}`}>
                <div className={`rounded-2xl p-6 sm:p-8 shadow-xl border w-full max-w-[480px] mx-4 ${isBoxDarkMode ? 'bg-gray-800 border-gray-700' : 'bg-white border-gray-200'}`}>
                  ...
                  <p className={`text-center text-sm mb-6 ${isBoxDarkMode ? 'text-gray-400' : 'text-gray-500'}`}>Keep practicing to master your multiplication tables for competitive exams and daily calculations.</p>
                  ...
                  <div className="flex gap-3">
                    <Link href="/drills/academic" className="flex-1">
                      <button className={`w-full px-4 py-2.5 rounded-lg font-semibold transition-all ${isDarkMode ? 'bg-gray-700 text-gray-300 hover:bg-gray-600' : 'bg-gray-200 text-gray-700 hover:bg-gray-300'}`}>← Back to Drills</button>
                    </Link>
                    <button onClick={resetGame} className="flex-1 px-4 py-2.5 bg-gradient-to-r from-indigo-500 to-purple-600 text-white rounded-lg font-semibold hover:shadow-lg transition-all transform hover:scale-[1.02] active:scale-[0.98] focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:ring-offset-2">Play Again →</button>
                  </div>
                </div>
              </div>
            )}
            ...
            <div className={`rounded-xl border overflow-hidden ${isDarkMode ? 'bg-gray-800 border-gray-700' : 'bg-white border-gray-200'}`}>
              ...
              <div className="p-4">
                ...
                <div className={`mt-4 pt-3 border-t flex flex-col sm:flex-row items-start sm:items-center justify-between gap-2 text-xs ${isDarkMode ? 'border-gray-700 text-gray-400' : 'border-gray-200 text-gray-500'}`}>
                  ...
                  <span>🏆 Score never below 0 • Best Score saves locally • Free forever</span>
                </div>
              </div>
            </div>
          </footer>
        ...
        {/* ABOUT THIS DRILL */}
        {!isFullscreen && (
          <section className="mt-8" aria-label="About this multiplication tables drill">
            <div className={`rounded-xl border overflow-hidden ${isDarkMode ? 'bg-gray-800 border-gray-700' : 'bg-white border-gray-200'}`}>
              <div className={`px-4 py-3 border-b ${isDarkMode ? 'border-gray-700 bg-gray-800/50' : 'border-gray-200 bg-gray-50'}`}>
                <div className="flex items-center gap-2"><GraduationCap className={`w-5 h-5 ${isDarkMode ? 'text-indigo-400' : 'text-indigo-600'}`} aria-hidden="true" /><h2 className={`font-semibold ${isDarkMode ? 'text-white' : 'text-gray-900'}`}>About This Free Multiplication Drill</h2></div>
              </div>
              <div className="p-5">
                <p className={`text-sm leading-relaxed mb-5 ${isDarkMode ? 'text-gray-300' : 'text-gray-600'}`}>This free multiplication tables drill uses intelligent practice that prioritizes high-friction numbers (6-12) for maximum learning efficiency. Choose from 3 ranges with adaptive scoring that rewards harder facts. Perfect for students and competitive exam preparation.</p>
                <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 mb-5">
                  <div className={`p-4 rounded-xl border ${isDarkMode ? 'bg-gray-700/50 border-gray-600' : 'bg-indigo-50 border-indigo-100'}`}><div className="flex items-center gap-2 mb-2"><div className="w-8 h-8 rounded-lg bg-indigo-500 flex items-center justify-center"><GraduationCap className="w-4 h-4 text-white" /></div><h3 className={`text-sm font-semibold ${isDarkMode ? 'text-white' : 'text-gray-900'}`}>Who It&apos;s For</h3></div><p className={`text-xs leading-relaxed ${isDarkMode ? 'text-gray-400' : 'text-gray-500'}`}>Students learning times tables, competitive exam aspirants, and anyone wanting faster multiplication recall.</p></div>
                  <div className={`p-4 rounded-xl border ${isDarkMode ? 'bg-gray-700/50 border-gray-600' : 'bg-green-50 border-green-100'}`}><div className="flex items-center gap-2 mb-2"><div className="w-8 h-8 rounded-lg bg-green-500 flex items-center justify-center"><TrendingUp className="w-4 h-4 text-white" /></div><h3 className={`text-sm font-semibold ${isDarkMode ? 'text-white' : 'text-gray-900'}`}>Skills Improved</h3></div><p className={`text-xs leading-relaxed ${isDarkMode ? 'text-gray-400' : 'text-gray-500'}`}>Multiplication fluency, factor recall, mental math speed, and number sense.</p></div>
                  <div className={`p-4 rounded-xl border ${isDarkMode ? 'bg-gray-700/50 border-gray-600' : 'bg-purple-50 border-purple-100'}`}><div className="flex items-center gap-2 mb-2"><div className="w-8 h-8 rounded-lg bg-purple-500 flex items-center justify-center"><BarChart3 className="w-4 h-4 text-white" /></div><h3 className={`text-sm font-semibold ${isDarkMode ? 'text-white' : 'text-gray-900'}`}>What You&apos;ll Track</h3></div><p className={`text-xs leading-relaxed ${isDarkMode ? 'text-gray-400' : 'text-gray-500'}`}>Score, accuracy, combo streaks, lives remaining, and best performance across ranges.</p></div>
                </div>
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                  <div className={`p-4 rounded-xl border ${isDarkMode ? 'bg-gray-700/50 border-gray-600' : 'bg-yellow-50 border-yellow-100'}`}><div className="flex items-center gap-2 mb-3"><div className="w-8 h-8 rounded-lg bg-yellow-500 flex items-center justify-center"><Lightbulb className="w-4 h-4 text-white" /></div><h3 className={`text-sm font-semibold ${isDarkMode ? 'text-white' : 'text-gray-900'}`}>Why Practice Times Tables?</h3></div><ul className={`text-xs space-y-2 ${isDarkMode ? 'text-gray-400' : 'text-gray-600'}`}><li className="flex items-start gap-2"><CheckCircle2 className="w-3 h-3 text-yellow-500 mt-0.5 flex-shrink-0" /> Foundation for all advanced mathematics</li><li className="flex items-start gap-2"><CheckCircle2 className="w-3 h-3 text-yellow-500 mt-0.5 flex-shrink-0" /> Essential for quick mental calculations</li><li className="flex items-start gap-2"><CheckCircle2 className="w-3 h-3 text-yellow-500 mt-0.5 flex-shrink-0" /> Builds confidence in numerical ability</li></ul></div>
                  <div className={`p-4 rounded-xl border ${isDarkMode ? 'bg-gray-700/50 border-gray-600' : 'bg-amber-50 border-amber-100'}`}><div className="flex items-center gap-2 mb-3"><div className="w-8 h-8 rounded-lg bg-amber-500 flex items-center justify-center"><Clock className="w-4 h-4 text-white" /></div><h3 className={`text-sm font-semibold ${isDarkMode ? 'text-white' : 'text-gray-900'}`}>How to Practice Effectively</h3></div><ol className={`text-xs space-y-2 ${isDarkMode ? 'text-gray-400' : 'text-gray-600'}`}><li className="flex items-start gap-2"><span className="w-5 h-5 rounded-full bg-amber-500 text-white text-xs flex items-center justify-center flex-shrink-0 mt-0.5">1</span> Start with 12×12 range and progress upward</li><li className="flex items-start gap-2"><span className="w-5 h-5 rounded-full bg-amber-500 text-white text-xs flex items-center justify-center flex-shrink-0 mt-0.5">2</span> Use keyboard shortcuts (1-4) for faster answering</li><li className="flex items-start gap-2"><span className="w-5 h-5 rounded-full bg-amber-500 text-white text-xs flex items-center justify-center flex-shrink-0 mt-0.5">3</span> Focus on accuracy first, then speed</li><li className="flex items-start gap-2"><span className="w-5 h-5 rounded-full bg-amber-500 text-white text-xs flex items-center justify-center flex-shrink-0 mt-0.5">4</span> Practice 2-3 times daily for best improvement in 2-3 weeks</li></ol></div>
                </div>
              </div>
            </div>
          </section>
        ...
        {!isFullscreen && (
          <section className="mt-8" aria-label="Related training drills and resources">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
              ...
              <Link href="/drills/academic/math-speed/Math-Reaction" className={`group relative overflow-hidden rounded-xl border transition-all duration-300 hover:shadow-lg hover:-translate-y-1 ${isDarkMode ? 'bg-gray-800 border-gray-700 hover:border-cyan-500' : 'bg-white border-gray-200 hover:border-cyan-300'}`}><div className="absolute top-0 left-0 right-0 h-1 bg-gradient-to-r from-cyan-500 to-blue-500"></div><div className="p-4"><div className="flex items-center gap-2 mb-2"><div className="w-8 h-8 rounded-lg bg-cyan-100 flex items-center justify-center"><Zap className="w-4 h-4 text-cyan-600" /></div><span className={`text-xs px-2 py-0.5 rounded-full font-medium ${isDarkMode ? 'bg-gray-700 text-gray-400' : 'bg-gray-100 text-gray-500'}`}>Math Speed</span></div><h3 className={`font-semibold text-sm mb-1 ${isDarkMode ? 'text-white group-hover:text-cyan-400' : 'text-gray-900 group-hover:text-cyan-600'} transition-colors`}>Math Reaction</h3><p className={`text-xs leading-relaxed ${isDarkMode ? 'text-gray-500' : 'text-gray-400'}`}>Odd/even parity identification under adaptive time pressure.</p><div className="flex items-center gap-1 mt-3 text-cyan-500 text-xs font-medium opacity-0 group-hover:opacity-100 transition-opacity">Start Drill <ArrowRight className="w-3 h-3" /></div></div></Link>
              <Link href="/drills/cognitive/processing-speed/quick-math" className={`group relative overflow-hidden rounded-xl border transition-all duration-300 hover:shadow-lg hover:-translate-y-1 ${isDarkMode ? 'bg-gray-800 border-gray-700 hover:border-teal-500' : 'bg-white border-gray-200 hover:border-teal-300'}`}><div className="absolute top-0 left-0 right-0 h-1 bg-gradient-to-r from-teal-500 to-green-500"></div><div className="p-4"><div className="flex items-center gap-2 mb-2"><div className="w-8 h-8 rounded-lg bg-teal-100 flex items-center justify-center"><Zap className="w-4 h-4 text-teal-600" /></div><span className={`text-xs px-2 py-0.5 rounded-full font-medium ${isDarkMode ? 'bg-gray-700 text-gray-400' : 'bg-gray-100 text-gray-500'}`}>Processing Speed</span></div><h3 className={`font-semibold text-sm mb-1 ${isDarkMode ? 'text-white group-hover:text-teal-400' : 'text-gray-900 group-hover:text-teal-600'} transition-colors`}>Quick Math</h3><p className={`text-xs leading-relaxed ${isDarkMode ? 'text-gray-500' : 'text-gray-400'}`}>Rapid fire mental arithmetic practice for brain training.</p><div className="flex items-center gap-1 mt-3 text-teal-500 text-xs font-medium opacity-0 group-hover:opacity-100 transition-opacity">Start Drill <ArrowRight className="w-3 h-3" /></div></div></Link>
          ...
          <footer className="mt-12 bg-gray-900 text-gray-400 rounded-xl py-10 px-6" role="contentinfo">
            <div className="max-w-7xl mx-auto">
              ...
              <div className="border-t border-gray-800 pt-8 text-center">
                ...
                <p className="text-xs max-w-2xl mx-auto leading-relaxed mb-6">Free online multiplication tables drill with intelligent practice prioritizing high friction numbers 6-12. Practice across 3 ranges 12x12 15x15 and 20x20 with adaptive scoring. Perfect for students competitive exam preparation SAT GRE GMAT CAT UPSC SSC banking exams and brain training. No registration required. More free drills at skilldrills.online.</p>
                <div className="flex items-center justify-center gap-5 flex-wrap">
                  <button onClick={sharePage} className="text-gray-500 hover:text-white transition-colors" title="Share this drill" aria-label="Share this free multiplication drill">
