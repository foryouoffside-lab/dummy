...
export default function KineticTrainerClient() {
  ...
  useEffect(() => { try { const s = localStorage.getItem('kineticDrillBest'); if(s) { const p = parseInt(s,10); if(!isNaN(p)) setBestScore(p); } } catch(e){} }, []);
  useEffect(() => { gameStateRef.current = gameState; }, [gameState]);
  
  const updateBestScore = useCallback((fs) => { try { const c = parseInt(localStorage.getItem('kineticDrillBest')||'0',10); if(fs>c) { localStorage.setItem('kineticDrillBest',fs.toString()); setBestScore(fs); } } catch(e){} }, []);
  ...
  const playSound = useCallback((type) => { if(!soundEnabled)return; try { const ctx=initAudio(); if(!ctx)return; const o=ctx.createOscillator(),g=ctx.createGain(); o.connect(g);g.connect(ctx.destination); const now=ctx.currentTime; const f={success:880,fail:440,teleport:660,combo:1046}; o.frequency.setValueAtTime(f[type]||440,now); g.gain.setValueAtTime(type==='combo'?0.12:type==='fail'?0.1:0.08,now); g.gain.exponentialRampToValueAtTime(0.001,now+0.1); o.start(now);o.stop(now+0.1); } catch(e){} }, [soundEnabled,initAudio]);
  ...
  const getRandomPosition = useCallback((avoidCurrent=true) => {
    const {width,height}=canvasSizeRef.current; if(width===0||height===0)return{x:width/2,y:height/2};
    const pad=TARGET_SIZE+25; let nx,ny,tooClose=true,att=0;
    while(tooClose&&att<100){nx=pad+Math.random()*(width-pad*2);ny=pad+Math.random()*(height-pad*2);if(avoidCurrent&&Math.hypot(nx-targetRef.current.x,ny-targetRef.current.y)<120){tooClose=true;}else{tooClose=false;}if(!tooClose&&lastPositionsRef.current.length>0){for(const p of lastPositionsRef.current){if(Math.hypot(nx-p.x,ny-p.y)<100){tooClose=true;break;}}}att++;}
    lastPositionsRef.current.push({x:nx,y:ny});if(lastPositionsRef.current.length>5)lastPositionsRef.current.shift();
    return{x:nx,y:ny};
  }, []);

  const teleport = useCallback((isHit=false) => {
    const {width,height}=canvasSizeRef.current; if(width===0||height===0)return;
    const np=getRandomPosition(true); targetRef.current.x=np.x; targetRef.current.y=np.y;
    const sm=1+Math.min(0.8,comboRef.current/30),bs=6+Math.random()*14;
    velRef.current={x:(Math.random()>0.5?1:-1)*(bs*sm),y:(Math.random()>0.5?1:-1)*(bs*sm)};
    velRef.current.x=Math.max(-30,Math.min(30,velRef.current.x));velRef.current.y=Math.max(-30,Math.min(30,velRef.current.y));
    setTargetSpeed(Math.round(Math.abs(velRef.current.x)+Math.abs(velRef.current.y)));
    if(!isHit)playSound('teleport');
  }, [getRandomPosition,playSound]);
  ...
  const handleMiss = useCallback((r) => {
    ...
    missesRef.current++;setMisses(missesRef.current);comboRef.current=0;setCombo(0);
    playSound('fail');showFeedback(`✗ ${r}!`,'error');
    const t=hitsRef.current+missesRef.current;setAccuracy(t===0?100:Math.round((hitsRef.current/t)*100));
  ...
  const handleShot = useCallback(() => {
    ...
    if(Math.hypot(ch.x-t.x,ch.y-t.y)<t.r+10){
      scoreRef.current+=1;setKineticScore(scoreRef.current);hitsRef.current++;setHits(hitsRef.current);
      ...
      playSound('success');showFeedback('✓ +1','success');
      if(comboRef.current%5===0){playSound('combo');showFeedback(`🔥 ${comboRef.current} Combo!`,'success');}
      teleport(true);
      const total=hitsRef.current+missesRef.current;setAccuracy(total===0?100:Math.round((hitsRef.current/total)*100));
    }else handleMiss('Miss');
  }, [teleport,handleMiss,playSound,showFeedback]);
  ...
  useEffect(() => {
    ...
    const loop=(now)=>{const dt=Math.min(0.033,(now-lt)/1000);lt=now;
      if(isActiveRef.current){const t=targetRef.current,{width,height}=canvasSizeRef.current;if(width>0&&height>0){t.x+=velRef.current.x*dt*60;t.y+=velRef.current.y*dt*60;const r=t.r;if(t.x-r<0){t.x=r;velRef.current.x=Math.abs(velRef.current.x);}else if(t.x+r>width){t.x=width-r;velRef.current.x=-Math.abs(velRef.current.x);}if(t.y-r<0){t.y=r;velRef.current.y=Math.abs(velRef.current.y);}else if(t.y+r>height){t.y=height-r;velRef.current.y=-Math.abs(velRef.current.y);}teleportTimerRef.current+=dt;if(teleportTimerRef.current>Math.max(0.7,1.8-comboRef.current*0.02)){teleportTimerRef.current=0;if(Math.random()<0.55)teleport(false);}}}
      ...
      const g=ctx.createRadialGradient(tg.x-5,tg.y-5,0,tg.x,tg.y,cr);g.addColorStop(0,'#00ff88');g.addColorStop(0.6,'#00cc66');g.addColorStop(1,'#009955');
      ctx.fillStyle=g;ctx.beginPath();ctx.arc(tg.x,tg.y,cr,0,Math.PI*2);ctx.fill();ctx.shadowBlur=0;
      ctx.strokeStyle='#ffffff';ctx.lineWidth=2;ctx.beginPath();ctx.arc(tg.x,tg.y,cr*0.5,0,Math.PI*2);ctx.stroke();
      ...
      if(ch.x>0&&ch.x<cvs.width&&ch.y>0&&ch.y<cvs.height){
        ...
        ctx.beginPath();ctx.moveTo(ch.x,ch.y);ctx.lineTo(tg.x,tg.y);ctx.strokeStyle='rgba(0,255,136,0.15)';ctx.lineWidth=1;ctx.stroke();
      }
      animationRef.current=requestAnimationFrame(loop);};
    animationRef.current=requestAnimationFrame(loop);
    return()=>{cancelAnimationFrame(animationRef.current);window.removeEventListener('resize',update);ro.disconnect();};
  }, [gameState,isBoxDarkMode,pointerLocked,teleport,getRandomPosition]);
  ...
  const startGame = useCallback(() => {
    ...
    targetRef.current.r=TARGET_SIZE/2;velRef.current={x:8,y:6};setTargetSpeed(14);teleportTimerRef.current=0;
  ...
  const sharePage = async () => { if (navigator.share) { try { await navigator.share({ title: 'Free Kinetic Trainer FPS Aim Drill | SkillDrills', text: 'Train raw mouse input with bouncing teleporting targets. Free!', url: 'https://skilldrills.online/drills/fps/high-speed-kinetic-trainer' }); } catch (e) {} } else { navigator.clipboard.writeText('https://skilldrills.online/drills/fps/high-speed-kinetic-trainer'); alert('Link copied!'); } };
  const copyPageLink = () => { navigator.clipboard.writeText('https://skilldrills.online/drills/fps/high-speed-kinetic-trainer'); alert('Link copied!'); };
  ...
  return (
    <div className={`min-h-screen select-none ${isDarkMode?'bg-gray-900':'bg-gray-50'}`}>
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        {/* Breadcrumb */}
        {!isFullscreen&&(<nav className="mb-4"><ol className="flex flex-wrap items-center gap-2 text-sm"><li><Link href="/" className={`hover:underline ${isDarkMode?'text-gray-400 hover:text-gray-200':'text-gray-600 hover:text-gray-900'}`}>Home</Link></li><li className={isDarkMode?'text-gray-500':'text-gray-400'}>/</li><li><Link href="/drills/fps" className={`hover:underline ${isDarkMode?'text-gray-400 hover:text-gray-200':'text-gray-600 hover:text-gray-900'}`}>FPS Drills</Link></li><li className={isDarkMode?'text-gray-500':'text-gray-400'}>/</li><li className={`font-medium ${isDarkMode?'text-blue-400':'text-blue-600'}`}>Kinetic Trainer</li></ol></nav>)}
        
        {/* Header */}
        {!isFullscreen&&(<div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 mb-6"><div className="flex items-center gap-3"><div className="p-3 bg-gradient-to-r from-blue-500 to-cyan-600 rounded-xl"><Wind className="w-6 h-6 text-white"/></div><div><h1 className={`text-2xl sm:text-3xl font-bold ${isDarkMode?'text-white':'text-gray-900'}`}>High-Speed Kinetic Trainer</h1><p className={`text-sm sm:text-base ${isDarkMode?'text-gray-400':'text-gray-500'}`}>{pointerLocked?'🟢 Raw input active':'🔴 Click canvas'} • Bouncing • Teleports • Free FPS aim training</p></div></div><div className="flex gap-2"><button onClick={resetGame} className={`p-2 rounded-lg border ${isDarkMode?'bg-gray-800 border-gray-700 text-gray-300':'bg-white border-gray-200 text-gray-700'}`} title="Reset"><RefreshCw className="w-5 h-5"/></button><button onClick={()=>setIsDarkMode(!isDarkMode)} className={`p-2 rounded-lg border ${isDarkMode?'bg-gray-800 border-gray-700 text-gray-300':'bg-white border-gray-200 text-gray-700'}`}>{isDarkMode?<Sun className="w-5 h-5"/>:<Moon className="w-5 h-5"/>}</button><button onClick={()=>setIsBoxDarkMode(!isBoxDarkMode)} className={`p-2 rounded-lg border ${isDarkMode?'bg-gray-800 border-gray-700 text-gray-300':'bg-white border-gray-200 text-gray-700'}`}><Eye className="w-5 h-5"/></button><button onClick={()=>setSoundEnabled(!soundEnabled)} className={`p-2 rounded-lg border ${isDarkMode?'bg-gray-800 border-gray-700 text-gray-300':'bg-white border-gray-200 text-gray-700'}`}>{soundEnabled?<Volume2 className="w-5 h-5"/>:<VolumeX className="w-5 h-5"/>}</button><button onClick={toggleFullscreen} className={`p-2 rounded-lg border ${isDarkMode?'bg-gray-800 border-gray-700 text-gray-300':'bg-white border-gray-200 text-gray-700'}`}>{isFullscreen?<Minimize2 className="w-5 h-5"/>:<Maximize2 className="w-5 h-5"/>}</button><button onClick={pointerLocked?()=>{document.exitPointerLock();setLockCooldown(true);setTimeout(()=>setLockCooldown(false),1000);}:requestPointerLock} className={`p-2 rounded-lg border ${pointerLocked?'bg-green-500 border-green-600 text-white':isDarkMode?'bg-gray-800 border-gray-700 text-gray-300':'bg-white border-gray-200 text-gray-700'}`}><Lock className="w-5 h-5"/></button></div></div>)}

        {/* SEO Content */}
        <section className="sr-only" aria-label="Drill description">
          <h2>Free High-Speed Kinetic Trainer - FPS Aim Training with Raw Mouse Input</h2>
          <p>Train your FPS aim with this free kinetic trainer featuring raw mouse input via Pointer Lock API. Click bouncing teleporting targets that speed up with combo streaks. 60-second challenge with no penalties - pure aim practice for Valorant, CS2, Overwatch, Apex Legends and all FPS games.</p>
        </section>

        {/* Stats */}
        {!isFullscreen&&(<div className="grid grid-cols-6 gap-3 mb-4 h-[88px]"><StatCard icon={<Target className="text-blue-600"/>} value={kineticScore} label="Score" d={isDarkMode}/><StatCard icon={<Trophy className="text-yellow-500"/>} value={bestScore} label="Best" d={isDarkMode}/><StatCard icon={<Timer className={timeLeft<=10?'text-red-600':'text-green-600'}/>} value={timeLeft} label="Time" unit="s" d={isDarkMode}/><StatCard icon={<Zap className="text-orange-500"/>} value={combo} label="Combo" d={isDarkMode}/><StatCard icon={<Activity className="text-purple-500"/>} value={accuracy} label="Accuracy" unit="%" d={isDarkMode}/><StatCard icon={<Wind className="text-cyan-500"/>} value={targetSpeed} label="Speed" unit="px/s" d={isDarkMode}/></div>)}
        ...
        <div ref={containerRef} className={`relative ${isFullscreen?'fixed inset-0 z-50':'rounded-xl border-2'}`} style={{background:isBoxDarkMode?"#020202":"#fff",aspectRatio:isFullscreen?'auto':'16/9',maxWidth:'100%',margin:'0 auto',borderColor:isDarkMode?'#374151':'#e5e7eb',overflow:'hidden'}}>
          ...
          {gameState==='start'&&(<div className={`absolute inset-0 flex items-center justify-center backdrop-blur-sm z-40 ${isBoxDarkMode?'bg-gray-900/95':'bg-white/95'}`}><div className={`rounded-2xl p-6 sm:p-8 text-center max-w-md mx-4 shadow-xl border ${isBoxDarkMode?'bg-gray-800 border-gray-700':'bg-white border-gray-200'}`}><Wind className="w-16 h-16 text-blue-500 mx-auto mb-4"/><h2 className={`text-2xl font-bold mb-2 ${isBoxDarkMode?'text-white':'text-gray-900'}`}>High-Speed Kinetic Trainer</h2><p className={`mb-4 ${isBoxDarkMode?'text-gray-300':'text-gray-600'}`}>Raw input • Bouncing • Teleports • No penalties</p><div className={`mb-6 p-3 rounded-lg border ${isBoxDarkMode?'border-yellow-600 bg-yellow-900/20':'border-yellow-200 bg-yellow-50'}`}><div className="flex items-center gap-2 mb-2"><AlertCircle className="w-4 h-4 text-yellow-500"/><p className={`text-sm font-medium ${isBoxDarkMode?'text-yellow-400':'text-yellow-700'}`}>Raw Input via Pointer Lock API</p></div><p className={`text-xs ${isBoxDarkMode?'text-gray-400':'text-gray-600'}`}>Click bouncing targets. Cursor locks for raw mouse input. Speed increases with combos. Press ESC to unlock. Perfect for Valorant, CS2, Overwatch, and Apex Legends aim training.</p></div><button onClick={startGame} className="px-8 py-3 bg-gradient-to-r from-blue-500 to-cyan-600 text-white rounded-xl font-semibold hover:shadow-lg w-full">Start Free Training</button></div></div>)}

          {gameState==='gameOver'&&(<div className={`absolute inset-0 flex items-center justify-center backdrop-blur-sm z-40 ${isBoxDarkMode?'bg-gray-900/95':'bg-white/95'}`}><div className={`rounded-2xl p-6 sm:p-8 shadow-xl border max-w-[520px] mx-4 ${isBoxDarkMode?'bg-gray-800 border-gray-700':'bg-white border-gray-200'}`}><div className="flex items-center justify-center gap-3 mb-4"><Trophy className="w-10 h-10 text-yellow-500"/><h2 className={`text-2xl font-bold ${isBoxDarkMode?'text-white':'text-gray-900'}`}>Training Complete</h2></div><p className={`text-center text-sm mb-4 ${isBoxDarkMode?'text-gray-400':'text-gray-500'}`}>Keep practicing to improve tracking speed for competitive FPS games.</p><div className="grid grid-cols-2 gap-3 mb-4"><RC label="Score" v={kineticScore} i={<Target className="w-4 h-4"/>} c="blue" d={isBoxDarkMode}/><RC label="Best" v={bestScore} i={<Trophy className="w-4 h-4"/>} c="yellow" d={isBoxDarkMode}/><RC label="Hits" v={hits} i={<Target className="w-4 h-4"/>} c="emerald" d={isBoxDarkMode}/><RC label="Combo" v={bestCombo} i={<Zap className="w-4 h-4"/>} c="orange" d={isBoxDarkMode}/><RC label="Accuracy" v={accuracy} u="%" i={<Activity className="w-4 h-4"/>} c="purple" d={isBoxDarkMode}/><RC label="Speed" v={targetSpeed} u="px/s" i={<Wind className="w-4 h-4"/>} c="cyan" d={isBoxDarkMode}/></div><div className="flex gap-3"><Link href="/drills/fps" className="flex-1"><button className={`w-full px-4 py-2.5 rounded-lg font-semibold ${isDarkMode?'bg-gray-700 text-gray-300':'bg-gray-200 text-gray-700'}`}>← Back</button></Link><button onClick={startGame} className="flex-1 px-4 py-2.5 bg-gradient-to-r from-blue-500 to-cyan-600 text-white rounded-lg font-semibold">Train Again →</button></div></div></div>)}
        </div>

        {/* 1. DRILL RULES */}
        {!isFullscreen&&(<footer className="mt-6"><div className={`rounded-xl border overflow-hidden ${isDarkMode?'bg-gray-800 border-gray-700':'bg-white border-gray-200'}`}><div className={`px-4 py-3 border-b ${isDarkMode?'border-gray-700 bg-gray-800/50':'border-gray-200 bg-gray-50'}`}><div className="flex items-center gap-2"><Info className={`w-4 h-4 ${isDarkMode?'text-blue-400':'text-blue-600'}`}/><h2 className={`font-semibold ${isDarkMode?'text-white':'text-gray-900'}`}>Drill Rules & Scoring</h2></div></div><div className="p-4"><div className="grid grid-cols-1 md:grid-cols-3 gap-4"><div className="space-y-2"><p className={`text-sm ${isDarkMode?'text-gray-300':'text-gray-600'}`}><span className="font-semibold text-blue-400">+1 point</span> per hit</p><p className={`text-sm ${isDarkMode?'text-gray-300':'text-gray-600'}`}><span className="font-semibold text-orange-400">Combo</span> every 5 hits</p><p className={`text-sm ${isDarkMode?'text-gray-300':'text-gray-600'}`}><span className="font-semibold text-red-400">Miss</span> breaks combo only</p></div><div className="space-y-2"><p className={`text-sm ${isDarkMode?'text-gray-300':'text-gray-600'}`}><span className="font-semibold text-purple-400">No penalties</span> - pure practice</p><p className={`text-sm ${isDarkMode?'text-gray-300':'text-gray-600'}`}><span className="font-semibold text-green-400">60 second</span> challenge</p><p className={`text-sm ${isDarkMode?'text-gray-300':'text-gray-600'}`}><span className="font-semibold text-cyan-400">Speed</span> scales with combo</p></div><div className="space-y-2"><p className={`text-sm ${isDarkMode?'text-gray-300':'text-gray-600'}`}><span className="font-semibold text-blue-400">Raw Input</span> via Pointer Lock</p><p className={`text-sm ${isDarkMode?'text-gray-300':'text-gray-600'}`}><span className="font-semibold text-orange-400">ESC</span> to unlock cursor</p><p className={`text-sm ${isDarkMode?'text-gray-300':'text-gray-600'}`}><span className="font-semibold text-purple-400">Click canvas</span> to re-lock</p></div></div></div></div></footer>)}

        {/* 2. ABOUT THIS DRILL */}
        {!isFullscreen && (
          <section className="mt-8" aria-label="About this kinetic trainer drill">
            <div className={`rounded-xl border overflow-hidden ${isDarkMode ? 'bg-gray-800 border-gray-700' : 'bg-white border-gray-200'}`}>
              <div className={`px-4 py-3 border-b ${isDarkMode ? 'border-gray-700 bg-gray-800/50' : 'border-gray-200 bg-gray-50'}`}>
                <div className="flex items-center gap-2"><GraduationCap className={`w-5 h-5 ${isDarkMode ? 'text-blue-400' : 'text-blue-600'}`} aria-hidden="true" /><h2 className={`font-semibold ${isDarkMode ? 'text-white' : 'text-gray-900'}`}>About This Free Kinetic Trainer</h2></div>
              </div>
              <div className="p-5">
                <p className={`text-sm leading-relaxed mb-5 ${isDarkMode ? 'text-gray-300' : 'text-gray-600'}`}>This free high-speed kinetic trainer uses raw mouse input via the Pointer Lock API for the most responsive FPS aim training. Click bouncing targets that teleport randomly and increase speed with your combo streak. No penalties - just pure aim practice for competitive FPS games.</p>
                <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 mb-5">
                  <div className={`p-4 rounded-xl border ${isDarkMode ? 'bg-gray-700/50 border-gray-600' : 'bg-blue-50 border-blue-100'}`}><div className="flex items-center gap-2 mb-2"><div className="w-8 h-8 rounded-lg bg-blue-500 flex items-center justify-center"><GraduationCap className="w-4 h-4 text-white" /></div><h3 className={`text-sm font-semibold ${isDarkMode ? 'text-white' : 'text-gray-900'}`}>Who It's For</h3></div><p className={`text-xs leading-relaxed ${isDarkMode ? 'text-gray-400' : 'text-gray-500'}`}>FPS gamers playing Valorant, CS2, Overwatch, Apex Legends, and any competitive shooter requiring fast target acquisition and tracking.</p></div>
                  <div className={`p-4 rounded-xl border ${isDarkMode ? 'bg-gray-700/50 border-gray-600' : 'bg-green-50 border-green-100'}`}><div className="flex items-center gap-2 mb-2"><div className="w-8 h-8 rounded-lg bg-green-500 flex items-center justify-center"><TrendingUp className="w-4 h-4 text-white" /></div><h3 className={`text-sm font-semibold ${isDarkMode ? 'text-white' : 'text-gray-900'}`}>Skills Improved</h3></div><p className={`text-xs leading-relaxed ${isDarkMode ? 'text-gray-400' : 'text-gray-500'}`}>Target acquisition speed, mouse control precision, tracking accuracy, reaction time, and combo consistency.</p></div>
                  <div className={`p-4 rounded-xl border ${isDarkMode ? 'bg-gray-700/50 border-gray-600' : 'bg-purple-50 border-purple-100'}`}><div className="flex items-center gap-2 mb-2"><div className="w-8 h-8 rounded-lg bg-purple-500 flex items-center justify-center"><BarChart3 className="w-4 h-4 text-white" /></div><h3 className={`text-sm font-semibold ${isDarkMode ? 'text-white' : 'text-gray-900'}`}>What You'll Track</h3></div><p className={`text-xs leading-relaxed ${isDarkMode ? 'text-gray-400' : 'text-gray-500'}`}>Score, accuracy, combo streaks, target speed, and best performance across sessions.</p></div>
                </div>
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                  <div className={`p-4 rounded-xl border ${isDarkMode ? 'bg-gray-700/50 border-gray-600' : 'bg-yellow-50 border-yellow-100'}`}><div className="flex items-center gap-2 mb-3"><div className="w-8 h-8 rounded-lg bg-yellow-500 flex items-center justify-center"><Lightbulb className="w-4 h-4 text-white" /></div><h3 className={`text-sm font-semibold ${isDarkMode ? 'text-white' : 'text-gray-900'}`}>Why Practice Kinetic Tracking?</h3></div><ul className={`text-xs space-y-2 ${isDarkMode ? 'text-gray-400' : 'text-gray-600'}`}><li className="flex items-start gap-2"><CheckCircle2 className="w-3 h-3 text-yellow-500 mt-0.5 flex-shrink-0" /> Raw input eliminates mouse acceleration</li><li className="flex items-start gap-2"><CheckCircle2 className="w-3 h-3 text-yellow-500 mt-0.5 flex-shrink-0" /> Bouncing targets simulate unpredictable enemy movement</li><li className="flex items-start gap-2"><CheckCircle2 className="w-3 h-3 text-yellow-500 mt-0.5 flex-shrink-0" /> Teleports train quick target re-acquisition</li></ul></div>
                  <div className={`p-4 rounded-xl border ${isDarkMode ? 'bg-gray-700/50 border-gray-600' : 'bg-orange-50 border-orange-100'}`}><div className="flex items-center gap-2 mb-3"><div className="w-8 h-8 rounded-lg bg-orange-500 flex items-center justify-center"><Clock className="w-4 h-4 text-white" /></div><h3 className={`text-sm font-semibold ${isDarkMode ? 'text-white' : 'text-gray-900'}`}>How to Practice Effectively</h3></div><ol className={`text-xs space-y-2 ${isDarkMode ? 'text-gray-400' : 'text-gray-600'}`}><li className="flex items-start gap-2"><span className="w-5 h-5 rounded-full bg-orange-500 text-white text-xs flex items-center justify-center flex-shrink-0 mt-0.5">1</span> Click canvas to lock cursor for raw input</li><li className="flex items-start gap-2"><span className="w-5 h-5 rounded-full bg-orange-500 text-white text-xs flex items-center justify-center flex-shrink-0 mt-0.5">2</span> Click bouncing targets to build combo streaks</li><li className="flex items-start gap-2"><span className="w-5 h-5 rounded-full bg-orange-500 text-white text-xs flex items-center justify-center flex-shrink-0 mt-0.5">3</span> Speed increases with combos - push your limits</li><li className="flex items-start gap-2"><span className="w-5 h-5 rounded-full bg-orange-500 text-white text-xs flex items-center justify-center flex-shrink-0 mt-0.5">4</span> Practice 5-10 minutes before gaming sessions</li></ol></div>
                </div>
              </div>
            </div>
          </section>
