...
export default function ZigZagPathPursuitClient() {
  ...
  useEffect(() => {
    ...
    try {
      const stored = localStorage.getItem('visualTrackingZigZagPathPursuitClientTrialsCount');
        ...
        setTimeLeft(p => {
          if (p <= 1) {
            ...
            setTotalTrials(prev => {
              const next = prev + 1;
              try {
                localStorage.setItem('visualTrackingZigZagPathPursuitClientTrialsCount', next.toString());
              } catch (e) {}
              return next;
            });
  ...
  useEffect(() => {
    ...
    const updateSize = () => {
      ...
      const cx = w / 2;
      const cy = h / 2;
      const graphW = Math.min(w * 0.85, 1200);
      const graphH = h * 0.45;
      const startX = cx - (graphW / 2);
      const startY = cy - (graphH / 2);
      
      trackingState.current.points = [];
      const segments = 10;
      const stepX = graphW / segments;
      for (let i = 0; i <= segments; i++) {
        const xPos = startX + (i * stepX);
        const yPos = (i % 2 === 0) ? (startY + graphH) : startY;
        trackingState.current.points.push({ x: xPos, y: yPos });
      }
      trackingState.current.progress = 0;
      trackingState.current.direction = 1;
    };
    ...
    const draw = (ts: number) => {
      ...
      if (trackingState.current.points.length > 0) {
          const pts = trackingState.current.points;
          // Draw visible zig-zag path guide
          ctx.beginPath();
          ...
          trackingState.current.progress += baseSpeed * trackingState.current.direction * scaledDt;
          if (trackingState.current.progress >= 1) {
            trackingState.current.progress = 1;
            trackingState.current.direction = -1;
            playSound('transition');
          } else if (trackingState.current.progress <= 0) {
            trackingState.current.progress = 0;
            trackingState.current.direction = 1;
            playSound('transition');
          }
          ...
          const scaledProgress = trackingState.current.progress * segmentCount;
          const index = Math.min(Math.floor(scaledProgress), segmentCount - 1);
          const segmentT = scaledProgress - index;

          const targetX = pts[index].x + (pts[index + 1].x - pts[index].x) * segmentT;
          const targetY = pts[index].y + (pts[index + 1].y - pts[index].y) * segmentT;
    ...
    if (typeof navigator !== 'undefined' && navigator.share) {
      try {
        await navigator.share({
          title: 'Free Zig-Zag Path Pursuit Drill | SkillDrills',
      ...
      navigator.clipboard.writeText('https://skilldrills.online/drills/visual-tracking/zig-zag-path-pursuit');
        ...
        {!isFullscreen && (
          <nav aria-label="Breadcrumb" className="mb-5">
            <ol className="flex items-center gap-2 text-[10px] text-slate-550 uppercase tracking-widest font-mono">
              ...
              <li><span className="text-slate-755">/</span></li>
              <li><span className="text-red-400 font-bold">Zig-Zag Path Pursuit</span></li>
            </ol>
          </nav>
        )}
                ...
                <div className="absolute inset-0 bg-[#05070e]/95 flex flex-col items-center justify-center p-6 z-30 select-none">
                  <div className="max-w-md text-center">
                    ...
                    <p className="text-xs text-slate-400 leading-relaxed mb-6">
                      Track target movements along a visible multi-segment zig-zag guide path. Focus your eyes on the moving target. Do not use your head or mouse.
                    </p>
            ...
            <div className="md:col-span-2 bg-[#0b0f19]/40 border border-slate-900 p-6 rounded-2xl flex flex-col justify-between">
              <div>
                ...
                <p className="text-xs leading-relaxed text-slate-400 mb-4 font-sans">
                  The Zig-Zag Path Pursuit Drill trains continuous pursuit along a complex path. The target moves back and forth along a multi-segment line, testing predictive tracking and gaze centering.
                </p>
              </div>
            </div>
            ...
            <div className="bg-[#0b0f19]/40 border border-slate-905 p-6 rounded-2xl">
              ...
              <ul className="space-y-3 text-xs text-slate-400 font-mono">
                <li className="flex items-start gap-2">
                  ...
                  <span><strong>Path Tracking</strong>: Builds foveal stability along visible segmented guide lines.</span>
                </li>\n                <li className="flex items-start gap-2">
                  <Check className="w-4 h-4 text-green-500 flex-shrink-0 mt-0.5" />
                  <span><strong>Directional Reversal</strong>: Conditions gaze recovery at endpoint path loops.</span>
                </li>\n                <li className="flex items-start gap-2">
                  <Check className="w-4 h-4 text-green-500 flex-shrink-0 mt-0.5" />
                  <span><strong>Linear Interpolation</strong>: Stabilizes smooth tracking sweeps.</span>
                </li>\n
              </ul>
            </div>
          </div>
            ...
            <div className="max-w-7xl mx-auto">
              ...
              <div className="border-t border-slate-900 pt-8 text-center">
                ...
                <p className="text-[10px] max-w-2xl mx-auto leading-relaxed mb-6 font-sans text-slate-700">
                  Free online Zig-Zag Path Pursuit drill for visual gaze tracking and foveal stability training. Train foveal pursuit and coordinate re-acquisition speeds under speed configurations. Perfect for gamers, athletes, and attention diagnostics. No registration required. More free drills at skilldrills.online.
                </p>
