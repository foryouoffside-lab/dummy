...
import {
  ...
  Share2, ArrowLeft, RotateCw, Eye, Users, TrendingUp, Zap, Brain, Crosshair, Heart, Trophy
} from 'lucide-react';
...
function draw2dTarget(ctx, x, y, targetVisible, targetType) {
  ...
  ctx.shadowBlur = targetVisible ? 36 : 16;
    ...
    spawnTimeoutRef.current = setTimeout(() => {
      ...
      signalTimeoutRef.current = setTimeout(() => {
        ...
        if (currentTypeRef.current === 'NO_GO' && !hasRespondedRef.current) {
          ...
          setUiScore(e.score);
          drillAudio?.playHit?.();
        } else if (currentTypeRef.current === 'GO' && !hasRespondedRef.current) {
        ...
        <header className="border-b border-white/5 bg-[#080811]/80 backdrop-blur-md sticky top-0 z-50">
          <div className="max-w-6xl mx-auto px-4 h-14 flex items-center justify-between">
            <div className="flex items-center gap-2 text-xs text-slate-400">
              ...
              <span>/</span>
              <span className="text-emerald-400 font-medium">Go/No-Go Pro</span>
            </div>
