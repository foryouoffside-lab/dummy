...
export default function ReactionSimulatorClient() {
  ...
  const trackingState = useRef({
    lastTime: 0,
    targets: [] as { x: number; y: number; vy: number }[],
    spawnTimer: 0,
  ...
  useEffect(() => {
    ...
    try {
      const stored = localStorage.getItem('visualTrackingReactionSimulatorClientTrialsCount');
        ...
        setTimeLeft(p => {
          if (p <= 1) {
            ...
            setTotalTrials(prev => {
              const next = prev + 1;
              try {
                localStorage.setItem('visualTrackingReactionSimulatorClientTrialsCount', next.toString());
              } catch (e) {}
              return next;
            });
  ...
  useEffect(() => {
    ...
    const draw = (ts: number) => {
      ...
      // Update & Spawn
        ...
        if (trackingState.current.spawnTimer > 1.2) {
          const lane = Math.floor(Math.random() * 3);
          trackingState.current.targets.push({
            x: (W * 0.25) + (lane * W * 0.25),
            y: H * 0.2,
            vy: (800 + Math.random() * 400) * (H / 1080)
          });
          trackingState.current.spawnTimer = 0;
        ...
        for (let i = 0; i < 3; i++) {
          ctx.fillRect((W * 0.25 + i * W * 0.25) - 50, H * 0.2, 100, 8);
        }

        // Update & Draw targets
        for (let i = trackingState.current.targets.length - 1; i >= 0; i--) {
          const t = trackingState.current.targets[i];
          t.y += t.vy * scaledDt;
          ...
          if (t.y > H + targetSize) {
            trackingState.current.targets.splice(i, 1);
          }
        }
    ...
    if (typeof navigator !== 'undefined' && navigator.share) {
      try {
        await navigator.share({
          title: 'Free Saccadic Reaction Simulator Drill | SkillDrills',
      ...
      navigator.clipboard.writeText('https://skilldrills.online/drills/visual-tracking/reaction-simulator');
        ...
        {!isFullscreen && (
          <nav aria-label="Breadcrumb" className="mb-5">
            <ol className="flex items-center gap-2 text-[10px] text-slate-550 uppercase tracking-widest font-mono">
              ...
              <li><span className="text-slate-755">/</span></li>
              <li><span className="text-red-400 font-bold">Saccadic Reaction Simulator</span></li>
            </ol>
          </nav>
        )}
                ...
                <div className="absolute inset-0 bg-[#05070e]/95 flex flex-col items-center justify-center p-6 z-30 select-none">
                  <div className="max-w-md text-center">
                    ...
                    <p className="text-xs text-slate-400 leading-relaxed mb-6">
                      Track falling targets along parallel lanes to calibrate vertical reaction limits. Focus your eyes on the moving target. Do not use your head or mouse.
                    </p>
            ...
            <div className="md:col-span-2 bg-[#0b0f19]/40 border border-slate-900 p-6 rounded-2xl flex flex-col justify-between">
              <div>
                ...
                <p className="text-xs leading-relaxed text-slate-400 mb-4 font-sans">
                  The Reaction Simulator Drill trains vertical target interception and lane tracking. Targets slide vertically down parallel paths, forcing the user to estimate speed variations and lock focus. This conditions the eyes to track vertical drop trajectories, common in high-ground drops and airborne situations.
                </p>
              </div>
            </div>
            ...
            <div className="bg-[#0b0f19]/40 border border-slate-905 p-6 rounded-2xl">
              ...
              <ul className="space-y-3 text-xs text-slate-400 font-mono">
                <li className="flex items-start gap-2">
                  ...
                  <span><strong>Lane Tracking</strong>: Stabilizes gaze pursuit along parallel vertical lanes.</span>
                </li>\n                <li className="flex items-start gap-2">
                  <Check className="w-4 h-4 text-green-500 flex-shrink-0 mt-0.5" />
                  <span><strong>Speed Estimation</strong>: Trains ocular muscles to adapt to varying acceleration vectors.</span>
                </li>\n                <li className="flex items-start gap-2">
                  <Check className="w-4 h-4 text-green-500 flex-shrink-0 mt-0.5" />
                  <span><strong>Reaction Calibration</strong>: Overclocks latency when target re-acquisition is required.</span>
                </li>\n
              </ul>
            </div>
          </div>
            ...
            <div className="max-w-7xl mx-auto">
              ...
              <div className="border-t border-slate-900 pt-8 text-center">
                ...
                <p className="text-[10px] max-w-2xl mx-auto leading-relaxed mb-6 font-sans text-slate-700">
                  Free online Saccadic Reaction Simulator drill for visual gaze tracking and foveal stability training. Train foveal pursuit and coordinate re-acquisition speeds under speed configurations. Perfect for gamers, athletes, and attention diagnostics. No registration required. More free drills at skilldrills.online.
                </p>
