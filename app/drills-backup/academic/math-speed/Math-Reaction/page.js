...
import { 
  ...
  Eye, Timer, Trophy, Target, Zap, Activity, Award, Info, Brain, Calculator
} from 'lucide-react';
...
export default function MathReactionPage() {
  ...
  const handleSuccess = useCallback((reactionTime) => {
    ...
    if (streakRef.current % 5 === 0) {
      playSound('streak');
      showFeedback(`ðŸ”¥ ${streakRef.current} Streak! +${pointsEarned}`, 'success');
    } else {
      playSound('correct');
      showFeedback(`âœ“ ${reactionTime}ms | +${pointsEarned}`, 'success');
    }
        ...
        <div className="mb-6">
          ...
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-3">
              ...
              <div>
                <h1 className={`text-3xl font-bold ${isDarkMode ? 'text-white' : 'text-gray-900'}`}>Math-Reaction</h1>
                <p className={`${isDarkMode ? 'text-gray-400' : 'text-gray-500'}`}>Click ODD or EVEN based on the result</p>
              </div>
            </div>
          ...
          {isFullscreen && gameState === 'playing' && (
            <>
              ...
              <div className="absolute top-4 left-4 z-20 bg-black/50 backdrop-blur-sm rounded-lg px-4 py-2 text-white text-sm">
                Score: <span className="text-yellow-400">{score}</span> | Streak: <span className="text-orange-400">{streak}</span> | Window: <span className="text-cyan-400">{windowTime}ms</span>
              </div>
            </>
          )}

          <canvas ref={canvasRef} style={{ display: 'block', position: 'absolute', cursor: 'pointer' }} />

          {/* Functional EVEN/ODD Buttons - Only these two buttons remain */}
          {gameState === 'playing' && (
            <div className="absolute bottom-12 left-0 right-0 flex justify-center gap-8 z-20">
              <button
                onClick={handleOddClick}
                className="w-48 h-24 bg-blue-500/30 hover:bg-blue-500/50 border-3 border-blue-500 rounded-xl transition-all hover:scale-105 active:scale-95 font-bold text-3xl text-blue-500 backdrop-blur-sm shadow-lg"
              ...
              <button
                ...
                className="w-48 h-24 bg-green-500/30 hover:bg-green-500/50 border-3 border-green-500 rounded-xl transition-all hover:scale-105 active:scale-95 font-bold text-3xl text-green-500 backdrop-blur-sm shadow-lg"
                style={{ cursor: 'pointer' }}
              >
                EVEN
              </button>
          ...
          {gameState === 'start' && (
            <div className={`absolute inset-0 flex items-center justify-center backdrop-blur-sm rounded-xl z-40 ${isBoxDarkMode ? 'bg-gray-900/95' : 'bg-white/95'}`}>
              <div className={`rounded-2xl p-8 text-center max-w-md mx-4 shadow-xl border ${isBoxDarkMode ? 'bg-gray-800 border-gray-700' : 'bg-white border-gray-200'}`}>
                ...
                <h3 className={`text-2xl font-bold mb-2 ${isBoxDarkMode ? 'text-white' : 'text-gray-900'}`}>Math-Reaction</h3>
                <p className={`mb-6 ${isBoxDarkMode ? 'text-gray-300' : 'text-gray-600'}`}>60-second challenge â€¢ Click ODD or EVEN based on the result</p>
                <button 
                  onClick={startGame} 
            ...
            <div className={`absolute inset-0 flex items-center justify-center backdrop-blur-sm rounded-xl z-40 ${isBoxDarkMode ? 'bg-gray-900/95' : 'bg-white/95'}`}>
              <div className={`rounded-2xl p-8 shadow-xl border w-[480px] ${isBoxDarkMode ? 'bg-gray-800 border-gray-700' : 'bg-white border-gray-200'}`}>
                ...
                <div className="grid grid-cols-2 gap-4 mb-8">
                  ...
                  <ResultCard label="Correct" value={`${correctAnswers}/${totalAttempts}`} icon={<Calculator className="w-4 h-4" />} color="text-cyan-500" />
                </div>
            ...
            <div className={`rounded-xl border overflow-hidden ${isDarkMode ? 'bg-gray-800 border-gray-700' : 'bg-white border-gray-200'}`}>
              ...
              <div className="p-4">
                  ...
                  <div className="space-y-2">
                    <div className="flex items-start gap-2">
                      ...
                      <p className={`text-sm ${isDarkMode ? 'text-gray-300' : 'text-gray-600'}`}>
                        <span className="font-semibold text-blue-500">Solve the math equation</span> â€¢ Appears for 800ms
                      </p>
                    </div>
                    <div className="flex items-start gap-2">
                      <div className="w-5 h-5 rounded-full bg-green-500 flex items-center justify-center text-white text-xs font-bold mt-0.5">2</div>
                      <p className={`text-sm ${isDarkMode ? 'text-gray-300' : 'text-gray-600'}`}>
                        <span className="font-semibold text-green-500">ODD result = Click ODD button</span> â€¢ EVEN result = Click EVEN button
                      </p>
                    </div>
                    <div className="flex items-start gap-2">
                      ...
                      <p className={`text-sm ${isDarkMode ? 'text-gray-300' : 'text-gray-600'}`}>
                        <span className="font-semibold text-purple-500">Correct = +50 points</span> â€¢ Window tightens (-40ms)
                      </p>
                    </div>
                  </div>
                  <div className="space-y-2">
                    <div className="flex items-start gap-2">
                      ...
                      <p className={`text-sm ${isDarkMode ? 'text-gray-300' : 'text-gray-600'}`}>
                        <span className="font-semibold text-red-500">Wrong/Timeout = -50 points</span> â€¢ Window loosens (+100ms)
                      </p>
                    </div>
                    <div className="flex items-start gap-2">
                      ...
                      <p className={`text-sm ${isDarkMode ? 'text-gray-300' : 'text-gray-600'}`}>
                        <span className="font-semibold text-orange-500">Streak bonus at 5 correct</span> â€¢ Build your streak
                      </p>
                    </div>
                    <div className="flex items-start gap-2">
                ...
                <div className={`mt-3 pt-3 border-t text-xs ${isDarkMode ? 'border-gray-700 text-gray-400' : 'border-gray-200 text-gray-500'} flex items-center justify-between`}>
                  <span>ðŸ§® Operations: +, -, * â€¢ 800-1500ms random delay</span>
                  <span>âš¡ Window range: 600-1500ms</span>
                </div>
              </div>
            </div>
          </div>
