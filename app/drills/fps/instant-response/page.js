...
export default function InstantResponsePage() {
  ...
  const handleMissClick = (reason) => {
    ...
    // Remove one life for wrong click
    ...
    // If no lives left, apply score penalty
    if (livesRef.current === 0) {
    ...
    // Increase response window on miss (makes it easier)
  ...
  useEffect(() => {
    const handleMouseDown = (e) => {
      ...
      if (targetObj.active) {
        // Target is active - check if hit or miss
        ...
        if (dist < targetObj.r + 15) {
          ...
          // Shrink response window on successful hit
        ...
        // No active target but user clicked - lose 1 life (prevents spam clicking)
    ...
    function render(now) {
      ...
      if (!targetRef.current.active) {
        ...
        if (elapsed > responseWindowRef.current) {
          // Timeout - no penalty, just reset target
          ...
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <p className={`${isDarkMode ? 'text-gray-400' : 'text-gray-500'}`}>Click only when target appears • 5 lives system</p>
              </div>
            </div>
          ...
          {gameState === 'start' && (
            <div className={`absolute inset-0 flex items-center justify-center backdrop-blur-sm rounded-xl z-40 ${isBoxDarkMode ? 'bg-gray-900/95' : 'bg-white/95'}`}>
              <div className={`rounded-2xl p-8 text-center max-w-md mx-4 shadow-xl border ${isBoxDarkMode ? 'bg-gray-800 border-gray-700' : 'bg-white border-gray-200'}`}>
                ...
                <p className={`mb-6 ${isBoxDarkMode ? 'text-gray-300' : 'text-gray-600'}`}>60-second challenge • Click target to score • Any miss costs 1 life</p>
                <button 
                  onClick={startGame} 
            ...
            <div className={`rounded-xl border overflow-hidden ${isDarkMode ? 'bg-gray-800 border-gray-700' : 'bg-white border-gray-200'}`}>
              ...
              <div className="p-4">
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  <div className="space-y-2">
                    ...
                    <div className="flex items-start gap-2">
                      ...
                      <p className={`text-sm ${isDarkMode ? 'text-gray-300' : 'text-gray-600'}`}>
                        <span className="font-semibold text-cyan-500">Window shrinks with fast hits</span> • Down to 80ms elite level
                      </p>
                    </div>
                  </div>
                  <div className="space-y-2">
                    ...
                    <div className="flex items-start gap-2">
                      ...
                      <p className={`text-sm ${isDarkMode ? 'text-gray-300' : 'text-gray-600'}`}>
                        <span className="font-semibold text-yellow-500">No penalty for timeout</span> • Target simply disappears
                      </p>
                    </div>
                  </div>
                </div>
                <div className={`mt-3 pt-3 border-t text-xs ${isDarkMode ? 'border-gray-700 text-gray-400' : 'border-gray-200 text-gray-500'} flex items-center justify-between`}>
                  ...
                  <span>⚡ Click ONLY on white target to score</span>
                </div>
              </div>
            </div>
          </div>
