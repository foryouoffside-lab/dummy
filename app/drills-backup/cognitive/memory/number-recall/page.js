...
import { 
  ...
  Sun, Moon, Eye, Timer,
  Trophy, Crown, Hash, Award, BarChart3, Info
} from 'lucide-react';
...
export default function NumberRecallDrill() {
  ...
  // Get accuracy
  const getAccuracy = () => {
    const totalAttempts = totalSequencesCompleted + (gameState === 'fail' ? 1 : 0);
    if (totalAttempts === 0) return 100;
    return Math.round((totalSequencesCompleted / totalAttempts) * 100);
  };

  // Start new round
  const startNewRound = useCallback(async (digits) => {
  ...
  const handleSuccess = () => {
    const pointsEarned = digitCount * 15 + (comboRef.current * 5);
  ...
  const handleFailure = () => {
    ...
    setGameState('fail');
    showFeedback(`✗ Incorrect! -10`, 'error');
  ...
  const startGame = useCallback(() => {
    ...
    setTimeout(() => startNewRound(4), 100);
          ...
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <p className={`${isDarkMode ? 'text-gray-400' : 'text-gray-500'}`}>Remember and recall digits • 60s</p>
              </div>
            </div>
          ...
          <div className="absolute inset-0 flex flex-col items-center justify-center p-8">
            ...
            {gameState === 'start' && (
              <div className={`absolute inset-0 flex items-center justify-center backdrop-blur-sm rounded-xl z-40 ${isBoxDarkMode ? 'bg-gray-900/95' : 'bg-white/95'}`}>
                <div className={`rounded-2xl p-8 text-center max-w-md mx-4 shadow-xl border ${isBoxDarkMode ? 'bg-gray-800 border-gray-700' : 'bg-white border-gray-200'}`}>
                  ...
                  <p className={`mb-6 ${isBoxDarkMode ? 'text-gray-300' : 'text-gray-600'}`}>60-second challenge • 4 → 49 digits</p>
                  <button 
                    onClick={startGame}
            ...
            {gameState === 'success' && (
              <div className="text-center">
                ...
                <h2 className={`text-3xl font-bold ${isBoxDarkMode ? 'text-white' : 'text-gray-900'}`}>Correct!</h2>
              </div>
            )}
              ...
              <div className={`absolute inset-0 flex items-center justify-center backdrop-blur-sm rounded-xl z-40 ${isBoxDarkMode ? 'bg-gray-900/95' : 'bg-white/95'}`}>
                <div className={`rounded-2xl p-8 shadow-xl border w-[480px] ${isBoxDarkMode ? 'bg-gray-800 border-gray-700' : 'bg-white border-gray-200'}`}>
                  ...
                  <div className="grid grid-cols-2 gap-4 mb-8">
                    ...
                    <ResultCard label="Completed" value={totalSequencesCompleted} icon={<Target className="w-4 h-4" />} color="text-green-500" />
                    <ResultCard label="Memory Master" value={isMemoryMaster ? '✓ Yes!' : 'Not yet'} icon={<Crown className="w-4 h-4" />} color={isMemoryMaster ? 'text-yellow-500' : 'text-gray-400'} />
                  </div>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  <div className="space-y-2">
                    ...
                    <div className="flex items-start gap-2">
                      ...
                      <p className={`text-sm ${isDarkMode ? 'text-gray-300' : 'text-gray-600'}`}>Success: <span className="font-semibold text-green-500">+15 × digits + 5 × combo</span></p>
                    </div>
                    <div className="flex items-start gap-2">
                      <div className="w-5 h-5 rounded-full bg-red-500 flex items-center justify-center text-white text-xs font-bold mt-0.5">3</div>
                      <p className={`text-sm ${isDarkMode ? 'text-gray-300' : 'text-gray-600'}`}>One mistake = <span className="font-semibold text-red-500">-10 + level reset</span></p>
                    </div>
                  </div>
                  <div className="space-y-2">
                    <div className="flex items-start gap-2">
                    ...
                    <div className="flex items-start gap-2">
                      ...
                      <p className={`text-sm ${isDarkMode ? 'text-gray-300' : 'text-gray-600'}`}>Complete Level 5 to become <span className="font-semibold text-orange-500">Memory Master</span></p>
                    </div>
                    <div className="flex items-start gap-2">
