...
export default function SaccadicGalleryClient() {
  ...
  useEffect(() => {
    ...
    try {
      const stored = localStorage.getItem('visualTrackingSaccadicGalleryClientTrialsCount');
        ...
        setTimeLeft(p => {
          if (p <= 1) {
            ...
            setTotalTrials(prev => {
              const next = prev + 1;
              try {
                localStorage.setItem('visualTrackingSaccadicGalleryClientTrialsCount', next.toString());
              } catch (e) {}
              return next;
            });
  ...
  useEffect(() => {
    ...
    const updateSize = () => {
      ...
      for (let i = 0; i < 5; i++) {
        trackingState.current.spots.push([w * 0.25 + i * (w * 0.12), h / 2 + (i % 2 ? -h * 0.15 : h * 0.15)]);
      }
    ...
    const draw = (ts: number) => {
      ...
      // Update sequence
        ...
        if (trackingState.current.seqT > 0.8) {
        ...
        if (trackingState.current.show > 0) {
          trackingState.current.show -= scaledDt;
        }

        // Draw inactive circular ports
        ctx.strokeStyle = 'rgba(51, 65, 85, 0.4)';
        ctx.lineWidth = 4;
        for (let i = 0; i < 5; i++) {
          ctx.beginPath();
          ctx.arc(trackingState.current.spots[i][0], trackingState.current.spots[i][1], targetSize, 0, Math.PI * 2);
          ctx.stroke();
        }

        // Draw active target
        if (trackingState.current.show > 0 && trackingState.current.spots.length === 5) {
          ...
          ctx.beginPath();
          ctx.arc(s[0], s[1], targetSize, 0, Math.PI * 2);
          ctx.fill();
          ctx.shadowBlur = 0;
    ...
    if (typeof navigator !== 'undefined' && navigator.share) {
      try {
        await navigator.share({
          title: 'Free Saccadic Gallery Drill | SkillDrills',
          text: 'Sequence-based ballistic eye shifts. Track glowing targets flashing in a zig-zag gallery pattern.',
          url: 'https://skilldrills.online/drills/visual-tracking/saccadic-gallery'
        });
      } catch (e) {}
    } else if (typeof navigator !== 'undefined' && navigator.clipboard) {
      navigator.clipboard.writeText('https://skilldrills.online/drills/visual-tracking/saccadic-gallery');
        ...
        {!isFullscreen && (
          <nav aria-label="Breadcrumb" className="mb-5">
            <ol className="flex items-center gap-2 text-[10px] text-slate-550 uppercase tracking-widest font-mono">
              ...
              <li><span className="text-slate-755">/</span></li>
              <li><span className="text-red-400 font-bold">Saccadic Gallery</span></li>
            </ol>
          </nav>
        )}
                ...
                <div className="absolute inset-0 bg-[#05070e]/95 flex flex-col items-center justify-center p-6 z-30 select-none">
                  <div className="max-w-md text-center">
                    ...
                    <p className="text-xs text-slate-400 leading-relaxed mb-6">
                      Sequence-based ballistic eye shifts. Track glowing targets flashing in a zig-zag gallery pattern. Focus your eyes on the moving target. Do not use your head or mouse.
                    </p>
            ...
            <div className="bg-[#0b0f19]/40 border border-slate-905 p-6 rounded-2xl">
              ...
              <ul className="space-y-3 text-xs text-slate-400 font-mono">
                <li className="flex items-start gap-2">
                  ...
                  <span><strong>Saccadic Focus</strong>: Trains fast, ballistic gaze shifts between target positions.</span>
                </li>\n                <li className="flex items-start gap-2">
                  <Check className="w-4 h-4 text-green-500 flex-shrink-0 mt-0.5" />
                  <span><strong>Angles Coverage</strong>: Conditions eyes to clear corners at standard height ranges.</span>
                </li>\n                <li className="flex items-start gap-2">
                  <Check className="w-4 h-4 text-green-500 flex-shrink-0 mt-0.5" />
                  <span><strong>Visual Processing</strong>: Overclocks target verification latency under sequential pressures.</span>
                </li>\n
              </ul>
            </div>
          </div>
            ...
            <div className="max-w-7xl mx-auto">
              ...
              <div className="border-t border-slate-900 pt-8 text-center">
                ...
                <p className="text-[10px] max-w-2xl mx-auto leading-relaxed mb-6 font-sans text-slate-700">
                  Free online Saccadic Gallery drill for visual gaze tracking and foveal stability training. Train foveal pursuit and coordinate re-acquisition speeds under speed configurations. Perfect for gamers, athletes, and attention diagnostics. No registration required. More free drills at skilldrills.online.
                </p>
