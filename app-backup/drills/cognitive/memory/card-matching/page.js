...
import { 
  ...
  Sun, Moon, Eye, Timer, Trophy, Grid, Heart, Star, Circle, Square,
  Triangle, Diamond, Award, Zap, Hexagon, Activity, Clock,
...
export default function CardMatchingPage() {
  ...
  const checkMatch = (idx1, idx2) => {
    ...
    if (card1.name === card2.name) {
      const pointsEarned = 20 + (comboRef.current * 5) + (level * 3);
      ...
      if (comboRef.current % 3 === 0) {
        ...
        playSound('match');
        showFeedback(`✓ Match! +${pointsEarned}`, 'success');
      }
  ...
  const levelComplete = () => {
    ...
    setScore(scoreRef.current);
    showFeedback(`🎯 Level ${level} Complete! +${bonus}`, 'success');
    playSound('levelup');
          ...
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <p className={`${isDarkMode ? 'text-gray-400' : 'text-gray-500'}`}>Progressive memory training • 60s</p>
              </div>
            </div>
            ...
            {gameState === 'start' && (
              <div className={`absolute inset-0 flex items-center justify-center backdrop-blur-sm rounded-xl z-40 ${isBoxDarkMode ? 'bg-gray-900/95' : 'bg-white/95'}`}>
                <div className={`rounded-2xl p-8 text-center max-w-md mx-4 shadow-xl border ${isBoxDarkMode ? 'bg-gray-800 border-gray-700' : 'bg-white border-gray-200'}`}>
                  ...
                  <p className={`mb-6 ${isBoxDarkMode ? 'text-gray-300' : 'text-gray-600'}`}>60-second challenge • Progressive levels</p>
                  <button 
                    onClick={startGame}
              ...
              <div className={`absolute inset-0 flex items-center justify-center backdrop-blur-sm rounded-xl z-40 ${isBoxDarkMode ? 'bg-gray-900/95' : 'bg-white/95'}`}>
                <div className={`rounded-2xl p-8 shadow-xl border w-[480px] ${isBoxDarkMode ? 'bg-gray-800 border-gray-700' : 'bg-white border-gray-200'}`}>
                  ...
                  <div className="grid grid-cols-2 gap-4 mb-8">
                    ...
                    <ResultCard label="Total Moves" value={moves} icon={<Activity className="w-4 h-4" />} color="text-blue-500" />
                    <ResultCard label="Max Combo" value={`${bestCombo}x`} icon={<Zap className="w-4 h-4" />} color="text-orange-500" />
                    <ResultCard label="Grid Size" value={`${gridCols}×${Math.ceil(cards.length / gridCols)}`} icon={<Grid className="w-4 h-4" />} color="text-cyan-500" />
                  </div>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  <div className="space-y-2">
                    ...
                    <div className="flex items-start gap-2">
                      ...
                      <p className={`text-sm ${isDarkMode ? 'text-gray-300' : 'text-gray-600'}`}>Match: <span className="font-semibold text-green-500">20 + (combo × 5) + (level × 3)</span></p>
                    </div>
                    <div className="flex items-start gap-2">
                      ...
                      <p className={`text-sm ${isDarkMode ? 'text-gray-300' : 'text-gray-600'}`}>Level complete: <span className="font-semibold text-blue-500">+100 × level bonus</span></p>
                    </div>
                  </div>
                  <div className="space-y-2">
                    <div className="flex items-start gap-2">
