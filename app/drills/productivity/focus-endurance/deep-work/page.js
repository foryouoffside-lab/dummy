...
export default function DeepWorkLabPage() {
  ...
  // Timer for focus tracking and scoring
  ...
  useEffect(() => {
    ...
    if (savedBestFocus) {
      setBestFocus(parseInt(savedBestFocus, 10));
    }
  }, []);

  // Save best score and best focus whenever they update
  useEffect(() => {
    if (bestScore > 0) {
      localStorage.setItem('deepWorkLabBestScore', bestScore.toString());
    }
  }, [bestScore]);
  
  useEffect(() => {
    if (bestFocus > 0) {
      localStorage.setItem('deepWorkLabBestFocus', bestFocus.toString());
    }
  }, [bestFocus]);
  ...
  const applyPenalty = () => {
    ...
    // Penalty: -1 point for each second of distraction
  ...
  const update = (dt, cvs) => {
    ...
    if (dist < zone.r) {
      ...
      // Accumulate focus time for scoring
      focusTimerRef.current += dt;
      
      // Add 1 point for every 1 second of continuous focus
      if (focusTimerRef.current >= 1.0) {
        ...
        if (pointsToAdd > 0) {
          showFeedback(`+${pointsToAdd} Focus Point!`, 'success');
        }
      }
      ...
      // Apply penalty for distraction (every second outside ring)
      const penaltyAccumulator = focusTimerRef.current;
      if (penaltyAccumulator >= 1.0) {
        const penaltiesToAdd = Math.floor(penaltyAccumulator);
        for (let i = 0; i < penaltiesToAdd; i++) {
          applyPenalty();
        }
        focusTimerRef.current -= penaltiesToAdd;
      } else {
    ...
    if (focusMeterRef.current > bestFocus) {
      setBestFocus(Math.floor(focusMeterRef.current));
    }
  };
  ...
  useEffect(() => {
    ...
    function draw() {
      ...
      // Focus Ring (radius 35)
      ctx.beginPath();
      ctx.arc(zone.x, zone.y, zone.r, 0, Math.PI * 2);
      ctx.lineWidth = 3;
      
      const isFocusLow = focusMeterRef.current < 25;
      ctx.strokeStyle = isFocusLow ? "#FF3E3E" : "#00ff88";
      ctx.stroke();
      ...
      ctx.fillStyle = isFocusLow ? "#FF3E3E" : "#00ff88";
  ...
  return (
    <div className={`min-h-screen select-none ${isDarkMode ? 'bg-gray-900' : 'bg-gray-50'}`}>
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        <div className="mb-6">
          ...
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-3">
              ...
              <div>
                <h1 className={`text-3xl font-bold ${isDarkMode ? 'text-white' : 'text-gray-900'}`}>Deep Work Lab</h1>
                <p className={`${isDarkMode ? 'text-gray-400' : 'text-gray-500'}`}>Track the ring - 1 sec focus = +1, distraction = -1</p>
              </div>
            </div>
        ...
        {/* Stats Board - Added Mistakes counter */}
        <div className="grid grid-cols-6 gap-3 mb-4 h-[88px]">
          ...
          <StatCard icon={<Award className="text-purple-600" />} value={bestFocus} label="Peak Focus" unit="%" isDark={isDarkMode} />
          ...
          {isFullscreen && gameState === 'playing' && (
            <>
              ...
              <div className="absolute top-4 left-4 z-30 bg-black/50 backdrop-blur-sm rounded-lg px-4 py-2 text-white text-sm">
                Score: <span className="text-yellow-400">{score}</span> | Time: <span className={timeLeft < 15 ? 'text-red-400' : 'text-green-400'}>{timeLeft}s</span> | Focus: <span className="text-green-400">{focusMeter}%</span>
              </div>
            </>
          )}
          ...
          {gameState === 'start' && (
            <div className={`absolute inset-0 flex items-center justify-center backdrop-blur-sm rounded-xl z-40 ${isBoxDarkMode ? 'bg-gray-900/95' : 'bg-white/95'}`}>
              <div className={`rounded-2xl p-8 text-center max-w-md mx-4 shadow-xl border ${isBoxDarkMode ? 'bg-gray-800 border-gray-700' : 'bg-white border-gray-200'}`}>
                ...
                <h3 className={`text-2xl font-bold mb-2 ${isBoxDarkMode ? 'text-white' : 'text-gray-900'}`}>Deep Work Lab</h3>
                <p className={`mb-6 ${isBoxDarkMode ? 'text-gray-300' : 'text-gray-600'}`}>60-second challenge • Focus = +1 point • Distraction = -1 point</p>
                <button 
                  onClick={startGame} 
            ...
            <div className={`absolute inset-0 flex items-center justify-center backdrop-blur-sm rounded-xl z-40 ${isBoxDarkMode ? 'bg-gray-900/95' : 'bg-white/95'}`}>
              <div className={`rounded-2xl p-8 shadow-xl border w-[480px] ${isBoxDarkMode ? 'bg-gray-800 border-gray-700' : 'bg-white border-gray-200'}`}>
                ...
                <div className="grid grid-cols-2 gap-4 mb-8">
                  ...
                  <ResultCard label="Peak Focus" value={bestFocus} unit="%" icon={<Activity className="w-4 h-4" />} color="text-green-500" />
                  <ResultCard label="Time Completed" value="60" unit="s" icon={<Timer className="w-4 h-4" />} color="text-purple-500" />
                  <ResultCard label="Distractions" value={mistakes} icon={<AlertCircle className="w-4 h-4" />} color="text-red-500" />
                  <ResultCard label="Net Score" value={score} icon={<Focus className="w-4 h-4" />} color="text-cyan-500" />
                </div>
                  ...
                  <div className="space-y-2">
                    <div className="flex items-start gap-2">
                      ...
                      <p className={`text-sm ${isDarkMode ? 'text-gray-300' : 'text-gray-600'}`}>
                        Keep your cursor <span className="font-semibold text-blue-500">inside the small moving green ring</span>
                      </p>
                    </div>
                    <div className="flex items-start gap-2">
                      <div className="w-5 h-5 rounded-full bg-green-500 flex items-center justify-center text-white text-xs font-bold mt-0.5">2</div>
                      <p className={`text-sm ${isDarkMode ? 'text-gray-300' : 'text-gray-600'}`}>
                        <span className="font-semibold text-green-500">Focus = +1 point per second</span>
                      </p>
                    </div>
                    <div className="flex items-start gap-2">
                      <div className="w-5 h-5 rounded-full bg-red-500 flex items-center justify-center text-white text-xs font-bold mt-0.5">3</div>
                      <p className={`text-sm ${isDarkMode ? 'text-gray-300' : 'text-gray-600'}`}>
                        <span className="font-semibold text-red-500">Distraction = -1 point per second</span>
                      </p>
                    </div>
                  </div>
                  <div className="space-y-2">
                    <div className="flex items-start gap-2">
                      ...
                      <p className={`text-sm ${isDarkMode ? 'text-gray-300' : 'text-gray-600'}`}>
                        Focus meter below <span className="font-semibold text-orange-500">25% is critical zone</span> (red ring)
                      </p>
                    </div>
                    <div className="flex items-start gap-2">
                      ...
                      <p className={`text-sm ${isDarkMode ? 'text-gray-300' : 'text-gray-600'}`}>
                        Challenge lasts <span className="font-semibold text-purple-500">60 seconds</span>
                      </p>
                    </div>
                    <div className="flex items-start gap-2">
                      <div className="w-5 h-5 rounded-full bg-yellow-500 flex items-center justify-center text-white text-xs font-bold mt-0.5">6</div>
                      <p className={`text-sm ${isDarkMode ? 'text-gray-300' : 'text-gray-600'}`}>
                        <span className="font-semibold text-yellow-500">No lives - just point penalties!</span>
                      </p>
                    </div>
                  </div>
                </div>
                <div className={`mt-3 pt-3 border-t text-xs ${isDarkMode ? 'border-gray-700 text-gray-400' : 'border-gray-200 text-gray-500'} flex items-center justify-between`}>
                  <span>🎯 Stay focused to maximize your score!</span>
