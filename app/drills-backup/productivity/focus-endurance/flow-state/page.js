...
import { 
  ...
  Eye, Wind, Brain, X, Trophy, Info, Timer
} from 'lucide-react';
...
export default function FlowInductionPage() {
  ...
  const targetRef = useRef({ x: 0, y: 0, r: 40, vx: 150, vy: 150 });
  ...
  // Timers for 1-second scoring
  const focusTimerRef = useRef(0);
  const distractionTimerRef = useRef(0);

  // Load best score from localStorage on mount
  useEffect(() => {
    const savedBestScore = localStorage.getItem('flowInductionBestScore');
    if (savedBestScore) {
  ...
  const updateBestScore = (finalScore) => {
    const currentBestScore = parseInt(localStorage.getItem('flowInductionBestScore') || '0', 10);
    if (finalScore > currentBestScore) {
      localStorage.setItem('flowInductionBestScore', finalScore.toString());
  ...
  const update = (dt, cvs) => {
    ...
    if (dist < target.r) {
      ...
      // 1 second of flow = +1 point (base) + streak bonus
      focusTimerRef.current += dt;
      if (focusTimerRef.current >= 1.0) {
        const secondsInFlow = Math.floor(focusTimerRef.current);
        const streakBonus = Math.floor(streakRef.current / 50);
        const pointsToAdd = secondsInFlow * (1 + streakBonus);
        scoreRef.current += pointsToAdd;
        setScore(scoreRef.current);
        focusTimerRef.current -= secondsInFlow;
        if (pointsToAdd > 1) {
          showFeedback(`+${pointsToAdd} Flow!`, 'success');
        }
      }
      ...
      if (streakRef.current % 50 === 0 && streakRef.current > 0) {
        playSound('streak');
        showFeedback(`🔥 ${streakRef.current} Streak!`, 'success');
      }
    } else {
      // Cursor outside ring
      const wasHigh = flowAlphaRef.current > 0.3;
      streakRef.current = Math.max(0, streakRef.current - 8);
      setStreak(streakRef.current);
      flowAlphaRef.current = Math.max(0, flowAlphaRef.current - 0.04);
      
      // 1 second outside = 1 penalty
      distractionTimerRef.current += dt;
      if (distractionTimerRef.current >= 1.0) {
        const secondsDistracted = Math.floor(distractionTimerRef.current);
        for (let i = 0; i < secondsDistracted; i++) {
          addPenalty();
        }
        distractionTimerRef.current -= secondsDistracted;
        if (secondsDistracted > 0) {
          showFeedback(`⚠️ ${secondsDistracted} Penalty!`, 'error');
        }
      }
      ...
      if (flowAlphaRef.current < 0.3 && wasHigh) {
        // Penalty handled by timer above
      }
    }
    
    setFlowState(Math.floor(flowAlphaRef.current * 100));
    
    if (flowAlphaRef.current * 100 > bestFlow) {
      setBestFlow(Math.floor(flowAlphaRef.current * 100));
    }
    
    // REMOVED: Best score no longer updates during gameplay
    // It only updates at the end via updateBestScore()
  };
  ...
  useEffect(() => {
    ...
    function draw() {
      ...
      ctx.lineWidth = 2 + (flowAlphaRef.current * 3);
      ctx.stroke();
      ...
      ctx.fillRect(mX, mY, (flowAlphaRef.current) * mWidth, mHeight);
  ...
  const startGame = () => {
    ...
    if (canvasRef.current) {
      ...
      targetRef.current.vx = 150;
      targetRef.current.vy = 150;
    }
  };
    ...
    <div className={`min-h-screen select-none ${isDarkMode ? 'bg-gray-900' : 'bg-gray-50'}`}>
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        <div className="mb-6">
          ...
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-3">
              ...
              <div>
                <h1 className={`text-3xl font-bold ${isDarkMode ? 'text-white' : 'text-gray-900'}`}>Flow Induction</h1>
                <p className={`${isDarkMode ? 'text-gray-400' : 'text-gray-500'}`}>Track the ring - 60 seconds</p>
              </div>
            </div>
        ...
        <div className="grid grid-cols-6 gap-3 mb-4 h-[88px]">
          ...
          <StatCard icon={<Wind className="text-cyan-500" />} value={flowState} label="Flow" unit="%" isDark={isDarkMode} />
          ...
          {isFullscreen && gameState === 'playing' && (
            <>
              ...
              <div className="absolute top-4 left-4 z-30 bg-black/50 backdrop-blur-sm rounded-lg px-4 py-2 text-white text-sm">
                Score: <span className="text-yellow-400">{score}</span> | Time: <span className={timeLeft < 15 ? 'text-red-400' : 'text-green-400'}>{timeLeft}s</span> | Flow: <span className="text-cyan-400">{flowState}%</span>
              </div>
            </>
          )}
          ...
          {gameState === 'start' && (
            <div className={`absolute inset-0 flex items-center justify-center backdrop-blur-sm rounded-xl z-40 ${isBoxDarkMode ? 'bg-gray-900/95' : 'bg-white/95'}`}>
              <div className={`rounded-2xl p-8 text-center max-w-md mx-4 shadow-xl border ${isBoxDarkMode ? 'bg-gray-800 border-gray-700' : 'bg-white border-gray-200'}`}>
                ...
                <h3 className={`text-2xl font-bold mb-2 ${isBoxDarkMode ? 'text-white' : 'text-gray-900'}`}>Flow Induction</h3>
                <p className={`mb-6 ${isBoxDarkMode ? 'text-gray-300' : 'text-gray-600'}`}>60-second challenge • 1 sec flow = +1 point (+streak bonus)</p>
                <button 
                  onClick={startGame} 
            ...
            <div className={`absolute inset-0 flex items-center justify-center backdrop-blur-sm rounded-xl z-40 ${isBoxDarkMode ? 'bg-gray-900/95' : 'bg-white/95'}`}>
              <div className={`rounded-2xl p-8 shadow-xl border w-[480px] ${isBoxDarkMode ? 'bg-gray-800 border-gray-700' : 'bg-white border-gray-200'}`}>
                ...
                <div className="grid grid-cols-2 gap-4 mb-8">
                  ...
                  <ResultCard label="Peak Flow" value={bestFlow} unit="%" icon={<Wind className="w-4 h-4" />} color="text-cyan-500" />
                  ...
                  <div className="space-y-2">
                    <div className="flex items-start gap-2">
                      ...
                      <p className={`text-sm ${isDarkMode ? 'text-gray-300' : 'text-gray-600'}`}>
                        Keep your cursor <span className="font-semibold text-cyan-500">inside the moving ring</span> to build flow state
                      </p>
                    </div>
                    <div className="flex items-start gap-2">
                      <div className="w-5 h-5 rounded-full bg-green-500 flex items-center justify-center text-white text-xs font-bold mt-0.5">2</div>
                      <p className={`text-sm ${isDarkMode ? 'text-gray-300' : 'text-gray-600'}`}>
                        <span className="font-semibold text-green-500">1 second of flow = +1 point</span> • Streak adds bonus points
                      </p>
                    </div>
                    <div className="flex items-start gap-2">
                      <div className="w-5 h-5 rounded-full bg-red-500 flex items-center justify-center text-white text-xs font-bold mt-0.5">3</div>
                      <p className={`text-sm ${isDarkMode ? 'text-gray-300' : 'text-gray-600'}`}>
                        <span className="font-semibold text-red-500">1 second outside ring = 1 penalty</span> • 5 mistakes reset score
                      </p>
                    </div>
                  </div>
                  <div className="space-y-2">
                    <div className="flex items-start gap-2">
                      ...
                      <p className={`text-sm ${isDarkMode ? 'text-gray-300' : 'text-gray-600'}`}>
                        Ring <span className="font-semibold text-orange-500">speeds up with higher streak</span> • Stay focused
                      </p>
                    </div>
                    <div className="flex items-start gap-2">
                      ...
                      <p className={`text-sm ${isDarkMode ? 'text-gray-300' : 'text-gray-600'}`}>
                        Challenge lasts <span className="font-semibold text-purple-500">60 seconds</span> • Achieve maximum flow
                      </p>
                    </div>
                    <div className="flex items-start gap-2">
                      <div className="w-5 h-5 rounded-full bg-yellow-500 flex items-center justify-center text-white text-xs font-bold mt-0.5">6</div>
                      <p className={`text-sm ${isDarkMode ? 'text-gray-300' : 'text-gray-600'}`}>
                        <span className="font-semibold text-yellow-500">Best Score saves locally</span> and persists after refresh
                      </p>
                    </div>
                  </div>
                </div>
                <div className={`mt-3 pt-3 border-t text-xs ${isDarkMode ? 'border-gray-700 text-gray-400' : 'border-gray-200 text-gray-500'} flex items-center justify-between`}>
                  <span>🌀 Flow 60% = Double ring • 50 streak = bonus notification</span>
                  <span>⚡ 1 sec flow = +1 • 1 sec distraction = 1 penalty</span>
                </div>
              </div>
            </div>
          </div>
