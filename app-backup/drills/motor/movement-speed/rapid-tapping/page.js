...
import { 
  ...
  Eye, Brain, BarChart3, Timer, Trophy, Info, Disc
} from 'lucide-react';
...
export default function PerpetualSingularityPage() {
  ...
  useEffect(() => {
    ...
    function draw() {
      ...
      // Simple circle - white in dark mode, black in light mode
      ctx.beginPath();
          ...
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <p className={`${isDarkMode ? 'text-gray-400' : 'text-gray-500'}`}>Click to expand • Survive the collapse</p>
              </div>
            </div>
          ...
          {gameState === 'start' && (
            <div className={`absolute inset-0 flex items-center justify-center backdrop-blur-sm rounded-xl z-40 ${isBoxDarkMode ? 'bg-gray-900/95' : 'bg-white/95'}`}>
              <div className={`rounded-2xl p-8 text-center max-w-md mx-4 shadow-xl border ${isBoxDarkMode ? 'bg-gray-800 border-gray-700' : 'bg-white border-gray-200'}`}>
                ...
                <p className={`mb-6 ${isBoxDarkMode ? 'text-gray-300' : 'text-gray-600'}`}>Click the ball to expand • Survive as long as possible</p>
                <button 
                  onClick={startGame} 
            ...
            <div className={`absolute inset-0 flex items-center justify-center backdrop-blur-sm rounded-xl z-40 ${isBoxDarkMode ? 'bg-gray-900/95' : 'bg-white/95'}`}>
              <div className={`rounded-2xl p-8 shadow-xl border w-[480px] ${isBoxDarkMode ? 'bg-gray-800 border-gray-700' : 'bg-white border-gray-200'}`}>
                ...
                <div className="grid grid-cols-2 gap-4 mb-8">
                  ...
                  <ResultCard label="Survival Time" value={formatTime(survivalTime)} icon={<Timer className="w-4 h-4" />} color="text-green-500" />
                  <ResultCard label="Total Clicks" value={clicks} icon={<Activity className="w-4 h-4" />} color="text-purple-500" />
                  <ResultCard label="Final Size" value={Math.floor(radius)} unit="px" icon={<Disc className="w-4 h-4" />} color="text-cyan-500" />
                  <ResultCard label="Max Difficulty" value={`+${Math.floor((shrinkRate / baseShrinkRef.current - 1) * 100)}%`} icon={<Zap className="w-4 h-4" />} color="text-orange-500" />
                </div>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  ...
                  <div className="space-y-2">
                    <div className="flex items-start gap-2">
                      ...
                      <p className={`text-sm ${isDarkMode ? 'text-gray-300' : 'text-gray-600'}`}>Scoring: <span className="font-semibold text-green-500">1 point per click</span></p>
                    </div>
                    <div className="flex items-start gap-2">
