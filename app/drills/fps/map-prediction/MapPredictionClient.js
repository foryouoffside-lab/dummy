...
export default function MapPredictionClient() {
  ...
  const targetRef = useRef({ x: 0, y: 0, r: 22, visible: false, entryX: 0, entryY: 0 });
  const pathRef = useRef({ startX: 0, startY: 0, endX: 0, endY: 0, progress: 0 });
  ...
  useEffect(() => { try { const s = localStorage.getItem('mapPredictionBest'); if(s) { const p = parseInt(s,10); if(!isNaN(p)) setBestScore(p); } } catch(e){} }, []);
  useEffect(() => { gameStateRef.current = gameState; }, [gameState]);
  
  const updateBestScore = useCallback((fs) => { try { const c = parseInt(localStorage.getItem('mapPredictionBest')||'0',10); if(fs>c) { localStorage.setItem('mapPredictionBest',fs.toString()); setBestScore(fs); } } catch(e){} }, []);
  ...
  const spawnTarget = useCallback(() => {
    ...
    pathRef.current.startX=Math.random()*cvs.width;pathRef.current.startY=Math.random()*cvs.height;
    targetRef.current.entryX=cvs.width*(0.2+Math.random()*0.6);targetRef.current.entryY=cvs.height*(0.2+Math.random()*0.6);
    pathRef.current.progress=0;targetRef.current.visible=false;
  ...
  const handleMiss = useCallback((r) => {
    ...
    speedRef.current=Math.max(MIN_SPEED,speedRef.current-0.005);
    ...
    spawnTarget();
  }, [playSound,showFeedback,spawnTarget]);
  ...
  const handleShot = useCallback(() => {
    ...
    if(t.visible){
      if(Math.hypot(ch.x-t.entryX,ch.y-t.entryY)<40){
        ...
        speedRef.current=Math.min(MAX_SPEED,speedRef.current+0.003);
    ...
    }else{handleMiss('Wrong click');}
  }, [bestReaction,handleMiss,playSound,showFeedback,spawnTarget]);
  ...
  useEffect(() => {
    ...
    const loop=(now)=>{if(!isActiveRef.current){animationRef.current=requestAnimationFrame(loop);return;}
      ...
      pathRef.current.progress+=speedRef.current;
      if(pathRef.current.progress>1.3)spawnTarget();
      ...
      const p=pathRef.current, t=targetRef.current;
      if(p.progress<0.7){ctx.setLineDash([8,12]);ctx.strokeStyle='rgba(0,255,136,0.4)';ctx.lineWidth=3;ctx.beginPath();ctx.moveTo(p.startX,p.startY);ctx.lineTo(t.entryX,t.entryY);ctx.stroke();ctx.setLineDash([]);const mx=(p.startX+t.entryX)/2,my=(p.startY+t.entryY)/2,angle=Math.atan2(t.entryY-p.startY,t.entryX-p.startX);ctx.beginPath();ctx.moveTo(mx,my);ctx.lineTo(mx-12*Math.cos(angle-0.5),my-12*Math.sin(angle-0.5));ctx.lineTo(mx-12*Math.cos(angle+0.5),my-12*Math.sin(angle+0.5));ctx.closePath();ctx.fillStyle='rgba(0,255,136,0.5)';ctx.fill();}
      ctx.beginPath();ctx.arc(p.startX,p.startY,6,0,Math.PI*2);ctx.fillStyle='rgba(0,255,136,0.4)';ctx.fill();
      ctx.beginPath();ctx.arc(t.entryX,t.entryY,30,0,Math.PI*2);ctx.strokeStyle='rgba(0,255,136,0.6)';ctx.lineWidth=3;ctx.stroke();
      ctx.beginPath();ctx.arc(t.entryX,t.entryY,22,0,Math.PI*2);ctx.strokeStyle='rgba(0,255,136,0.3)';ctx.lineWidth=1.5;ctx.stroke();
      if(p.progress>=0.95){t.visible=true;const fp=Math.min(1,(p.progress-0.95)/0.05);ctx.shadowBlur=12;ctx.shadowColor='#00ff88';ctx.fillStyle=`rgba(0,255,136,${fp})`;ctx.beginPath();ctx.arc(t.entryX,t.entryY,t.r,0,Math.PI*2);ctx.fill();ctx.fillStyle=`rgba(255,255,255,${fp})`;ctx.beginPath();ctx.arc(t.entryX,t.entryY,t.r*0.4,0,Math.PI*2);ctx.fill();ctx.shadowBlur=0;}else{t.visible=false;}
      ...
      if(ch.x>0&&ch.x<cvs.width&&ch.y>0&&ch.y<cvs.height){
        ...
        ctx.beginPath();ctx.moveTo(ch.x,ch.y);ctx.lineTo(t.entryX,t.entryY);ctx.strokeStyle='rgba(0,255,136,0.15)';ctx.lineWidth=1.5;ctx.stroke();
      }
  ...
  const startGame = useCallback(() => {
    ...
    hitsRef.current=0;missesRef.current=0;speedRef.current=0.02;
  ...
  const sharePage = async () => { if (navigator.share) { try { await navigator.share({ title: 'Free Map Prediction Trainer | SkillDrills', text: 'Train predictive tracking with raw input. Free!', url: 'https://skilldrills.online/drills/fps/map-prediction' }); } catch (e) {} } else { navigator.clipboard.writeText('https://skilldrills.online/drills/fps/map-prediction'); alert('Link copied!'); } };
  const copyPageLink = () => { navigator.clipboard.writeText('https://skilldrills.online/drills/fps/map-prediction'); alert('Link copied!'); };
