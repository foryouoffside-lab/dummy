...
export default function TriangularPursuitClient() {
  ...
  useEffect(() => {
    ...
    try {
      const stored = localStorage.getItem('visualTrackingTriangularPursuitClientTrialsCount');
        ...
        setTimeLeft(p => {
          if (p <= 1) {
            ...
            setTotalTrials(prev => {
              const next = prev + 1;
              try {
                localStorage.setItem('visualTrackingTriangularPursuitClientTrialsCount', next.toString());
              } catch (e) {}
              return next;
            });
  ...
  useEffect(() => {
    ...
    const updateSize = () => {
      ...
      trackingState.current.nodes = [
        [cx, cy - size],
        [cx + size, cy + size * 0.8],
        [cx - size, cy + size * 0.8]
      ];
      trackingState.current.currentNode = 0;
      trackingState.current.nextNode = 1;
      trackingState.current.t = 0;
    };
    ...
    const draw = (ts: number) => {
      ...
      if (trackingState.current.nodes.length === 3) {
          const nds = trackingState.current.nodes;
          // Draw triangular path guide
          ctx.strokeStyle = 'rgba(51, 65, 85, 0.4)';
          ctx.lineWidth = 4;
          ctx.lineJoin = 'round';
          ctx.beginPath();
          ctx.moveTo(nds[0][0], nds[0][1]);
          ctx.lineTo(nds[1][0], nds[1][1]);
          ctx.lineTo(nds[2][0], nds[2][1]);
          ctx.closePath();
          ctx.stroke();

          // Update position
          trackingState.current.t += 0.6 * scaledDt;
          if (trackingState.current.t >= 1) {
            trackingState.current.t = 0;
            trackingState.current.currentNode = trackingState.current.nextNode;
            trackingState.current.nextNode = (trackingState.current.nextNode + 1) % 3;
            playSound('transition');
          }

          const nCurr = nds[trackingState.current.currentNode];
          const nNext = nds[trackingState.current.nextNode];
          const targetX = nCurr[0] + (nNext[0] - nCurr[0]) * trackingState.current.t;
          const targetY = nCurr[1] + (nNext[1] - nCurr[1]) * trackingState.current.t;
    ...
    if (typeof navigator !== 'undefined' && navigator.share) {
      try {
        await navigator.share({
          title: 'Free Triangular Pursuit Drill | SkillDrills',
      ...
      navigator.clipboard.writeText('https://skilldrills.online/drills/visual-tracking/triangular-pursuit');
        ...
        {!isFullscreen && (
          <nav aria-label="Breadcrumb" className="mb-5">
            <ol className="flex items-center gap-2 text-[10px] text-slate-550 uppercase tracking-widest font-mono">
              ...
              <li><span className="text-slate-755">/</span></li>
              <li><span className="text-red-400 font-bold">Triangular Pursuit</span></li>
            </ol>
          </nav>
        )}
                ...
                <div className="absolute inset-0 bg-[#05070e]/95 flex flex-col items-center justify-center p-6 z-30 select-none">
                  <div className="max-w-md text-center">
                    ...
                    <p className="text-xs text-slate-400 leading-relaxed mb-6">
                      Track target transitions along a visible triangular guide vector. Focus your eyes on the moving target. Do not use your head or mouse.
                    </p>
            ...
            <div className="md:col-span-2 bg-[#0b0f19]/40 border border-slate-900 p-6 rounded-2xl flex flex-col justify-between">
              <div>
                ...
                <p className="text-xs leading-relaxed text-slate-400 mb-4 font-sans">
                  The Triangular Pursuit conditions ballistic corner re-acquisition. Moving along sharp linear segments, it trains foveal centering during sudden direction changes at vertices.
                </p>
              </div>
            </div>
            ...
            <div className="bg-[#0b0f19]/40 border border-slate-905 p-6 rounded-2xl">
              ...
              <ul className="space-y-3 text-xs text-slate-400 font-mono">
                <li className="flex items-start gap-2">
                  ...
                  <span><strong>Angular Retargeting</strong>: Trains immediate tracking adaptations at sharp vertices.</span>
                </li>\n                <li className="flex items-start gap-2">
                  <Check className="w-4 h-4 text-green-500 flex-shrink-0 mt-0.5" />
                  <span><strong>Linear Segments</strong>: Builds constant-speed straight line smooth pursuit.</span>
                </li>\n                <li className="flex items-start gap-2">
                  <Check className="w-4 h-4 text-green-500 flex-shrink-0 mt-0.5" />
                  <span><strong>Path Estimation</strong>: Helps anticipate corner arrivals.</span>
                </li>\n
              </ul>
            </div>
          </div>
            ...
            <div className="max-w-7xl mx-auto">
              ...
              <div className="border-t border-slate-900 pt-8 text-center">
                ...
                <p className="text-[10px] max-w-2xl mx-auto leading-relaxed mb-6 font-sans text-slate-700">
                  Free online Triangular Pursuit drill for visual gaze tracking and foveal stability training. Train foveal pursuit and coordinate re-acquisition speeds under speed configurations. Perfect for gamers, athletes, and attention diagnostics. No registration required. More free drills at skilldrills.online.
                </p>
