...
const STORAGE_KEY = 'skilldrills_visual_tracking_zig_zag_path_pursuit_v2';
...
const RELATED_DRILLS = [
  ...
  { id: "sine-wave-pursuit", name: "Sine-Wave Pursuit", cat: "Visual Tracking", desc: "Train smooth pursuit along vertical and horizontal sine oscillations.", href: "/drills/visual-tracking/sine-wave-pursuit" },
...
export default function ZigZagPathPursuitClient() {
  ...
  const [glowEffect, setGlowEffect] = useState<boolean>(false); // OFF initially
  ...
  const sharePage = useCallback(async () => {
    const url = 'https://skilldrills.online/drills/visual-tracking/zig-zag-path-pursuit';
    try {
      const canvas = generateSessionCard({
        drillName: 'Zig-Zag Path Pursuit',
  ...
  const enterDrill = useCallback(async () => {
    ...
    // Generate Zig Zag Polyline Points
    const numPoints = 8;
  ...
  // Canvas Render Loop (Zig Zag Path Pursuit Physics)
  useEffect(() => {
    ...
    const updateSize = () => {
      ...
      trackingState.current.points = pts;
    };
    ...
    const draw = (ts: number) => {
      ...
      if (pts.length > 0) {
        // Draw Zig-Zag Guide Path ONLY if math is NOT invisible
        if (!mathInvisible) {
        ...
        // Draw target ball (MATCHING CONSTANT-SLOW-PURSUIT EXACTLY WITH WHITE CENTER CORE)
        ...
        <header className="border-b border-white/5 bg-[#080811]/80 backdrop-blur-md sticky top-0 z-50">
          <div className="max-w-6xl mx-auto px-4 h-14 flex items-center justify-between">
            <div className="flex items-center gap-2 text-xs text-slate-400">
              ...
              <span>/</span>
              <span className="text-red-400 font-medium">Zig-Zag Path Pursuit</span>
            </div>
          ...
          <div className="[&>div]:!mt-0">
            ...
            >
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4 font-sans">
                <RuleItem num="1" text="Zig-Zag Path Pursuit" highlight="Pure Visual" result="Follow multi-segment polyline" />
              ...
              <div className="space-y-6 font-sans">
                <section>
                  <h4 className="text-base font-bold text-white mb-2">
                    What Is Zig-Zag Path Pursuit Training?
                  </h4>
                  <p className="text-sm leading-relaxed mb-3 text-gray-300">
                    <strong>Zig-Zag Path Pursuit Training</strong> conditions visual pursuit accuracy across acute direction changes as a target traverses a multi-segment zig-zag polyline.
                  </p>
                  <p className="text-sm leading-relaxed text-gray-300">
                    By utilizing features like <strong>Hide Line</strong> and <strong>Random Speed Acceleration</strong>, your visual cortex learns to anticipate sharp diagonal turns without visual path indicators.
                  </p>
                </section>
                ...
                <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
                  <div className="p-4 rounded-xl border border-gray-800 bg-white/[0.02]">
                    ...
                    <p className="text-xs text-gray-300 leading-relaxed">Gamers, athletes, and vision training practitioners looking to build diagonal tracking precision.</p>
                  </div>
                  <div className="p-4 rounded-xl border border-gray-800 bg-white/[0.02]">
                    <h5 className="text-xs font-bold text-white mb-1.5">Sharp Segment Angles</h5>
                    <p className="text-xs text-gray-300 leading-relaxed">Conditions foveal pursuit across sharp multi-segment direction reversals.</p>
                  </div>
                  <div className="p-4 rounded-xl border border-gray-800 bg-white/[0.02]">
            ...
            >
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4 font-sans">
                <FAQItem q="What is the Zig-Zag Path Pursuit drill?" a="The Zig-Zag Path Pursuit drill conditions visual pursuit accuracy across acute direction changes as a target traverses a multi-segment zig-zag polyline." />
                <FAQItem q="Why use the 'Hide Line' setting?" a="Hiding the zig-zag guide path forces your eyes to anticipate sharp diagonal turns without visual path indicators." />
                <FAQItem q="What does the Random Speed feature do?" a="Random Speed introduces variable segment traversal speeds and abrupt direction reversals, strengthening ocular motor control." />
