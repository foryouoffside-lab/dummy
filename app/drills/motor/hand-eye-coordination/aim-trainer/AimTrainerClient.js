...
export default function AimTrainerClient() {
  ...
  const engine = useRef({
    ...
    maxTargets: 2, // STRICTLY CAP AT 2
  ...
  useEffect(() => {
    try {
      const savedSens = localStorage.getItem('aimTrainer_sens');
      if (savedSens) setUniversalSens(parseFloat(savedSens));
      const savedBest = localStorage.getItem('aimTrainer_bestScore');
  ...
  useEffect(() => {
    if (gameState !== 'playing') {
      try { localStorage.setItem('aimTrainer_sens', universalSens.toString()); } catch (e) {}
    }
  ...
  const endGame = useCallback(() => {
    ...
    setBestScore(prev => {
      if (e.score > prev) {
        setIsNewBest(true);
        try { localStorage.setItem('aimTrainer_bestScore', e.score.toString()); } catch(err){}
        return e.score;
      }
      return prev;
    });
  ...
  const spawnTarget = useCallback((cvs) => {
    ...
    // Adaptive Sizing: Shrinks as streak increases
    const dynamicMaxR = Math.max(12, 35 - (e.streak * 0.5));
    const padding = 50; 
    ...
    // Lifespan shrinks as difficulty increases
    const maxLife = Math.max(1.0, 3.0 - (e.streak * 0.05));
  ...
  const applyPenalty = useCallback((cvs, reason) => {
    ...
    // Forgiveness: Slow target down slightly to recover
    e.targetSpeedMulti = Math.max(1.0, e.targetSpeedMulti - 0.5);
  ...
  const handleHit = useCallback((cvs, targetIndex) => {
    ...
    e.timeLeft += 2.0; // UPDATED: +2.0s Time Bonus
    ...
    // Adaptive Difficulty: Targets speed up
    if (e.streak % 5 === 0) {
      e.targetSpeedMulti = Math.min(5.0, e.targetSpeedMulti + 0.3);
  ...
  const startGame = useCallback(async () => {
    ...
    e.maxTargets = 2; // Strict 2 Targets
    e.targets = [];
    e.screenShake = 0;
    ...
    setTimeout(() => {
      if (canvasRef.current && !document.pointerLockElement) {
        ...
        // Spawn exactly 2 targets
        for(let i=0; i<e.maxTargets; i++) spawnTarget(canvasRef.current);
      }
    }, 150);
  }, [spawnTarget]);
  ...
  useEffect(() => {
    ...
    const handleMouseDown = (e) => {
      ...
      if (gameState === 'playing' && pointerLocked && isActiveRef.current) {
        ...
        // Find if we hit a target (checking backwards to hit top-most z-index)
        let hitIndex = -1;
        ...
        if (hitIndex !== -1) {
          ...
          applyPenalty(canvasRef.current, "Missed Click");
        }
      }
    };
  ...
  useEffect(() => {
    ...
    const loop = (time) => {
      ...
      if (gameState === 'playing' && pointerLocked && isActiveRef.current) {
        ...
        // Maintain Target Population (Strictly 2)
      ...
      if (gameState === 'playing' || gameState === 'start') {
        ...
        // Render All Targets (Classic Bullseye Style)
        e.targets.forEach(t => {
        ...
        if (ch.initialized) {
          const activeColor = pointerLocked ? '#10b981' : '#ef4444'; // Emerald / Red
  ...
  const shareDrillLink = useCallback(() => {
    ...
    if (navigator.share) {
      navigator.share({ title: 'Aim Trainer Elite', url }).catch(() => {});
    } else {
          ...
          <div className="mb-6">
            <nav className="mb-4">
              <ol className="flex flex-wrap items-center gap-2 text-sm text-gray-500">
                ...
                <li><ChevronRight className="w-4 h-4 text-gray-600" /></li>
                <li className="text-emerald-400 font-medium">Aim Trainer Elite</li>
              </ol>
            </nav>
          ...
          {gameState === 'start' && (
            <div className="absolute inset-0 flex items-center justify-center z-50 bg-black/70 backdrop-blur-md p-4 overflow-y-auto">
              <div className="rounded-3xl p-8 text-center max-w-lg w-full border border-gray-700 bg-gray-900 shadow-2xl my-auto">
                ...
                <h2 className="text-3xl font-black mb-3 tracking-tight text-white uppercase">Aim Trainer Elite</h2>
                <p className="text-sm mb-6 text-gray-400 leading-relaxed">
                  Raw input dynamic aim tracking. Click the moving bullseye targets before they vanish. Missing a click or letting a target expire violently deducts your score and drains your master clock.
                </p>
                ...
                <button 
                  onClick={startGame}
                  className="w-full flex items-center justify-center gap-3 px-6 py-4 bg-gradient-to-r from-green-600 to-emerald-600 text-white rounded-xl font-black text-lg hover:brightness-110 transition-all transform hover:scale-[1.02] active:scale-[0.98] shadow-[0_0_20px_rgba(16,185,129,0.3)]"
                >
                      ...
                      <div className="space-y-3 flex flex-col justify-between">
                        <div>
                          ...
                          <p className="text-gray-400 leading-relaxed font-sans">
                            {analytics.misses > 5 ? (
                              <span className="text-red-300">You are missing too often. Clicking empty space or letting the target expire violently drains your clock and deducts points. Focus purely on accuracy and smooth tracking of the moving target rather than panic-flicking.</span>
                            ) : analytics.peakSpeed < 2.5 ? (
                              <span className="text-yellow-300">Your base accuracy is decent, but you are not hitting the targets fast enough to scale the engine into the tightest difficulty thresholds where the targets shrink drastically.</span>
                            ) : (
                              <span className="text-green-300">Excellent spatial tracking! You are maintaining incredible accuracy despite the violently shrinking and rapidly accelerating dynamic target swarms. Keep pushing your limits.</span>
                            )}
                          </p>
                        </div>
                      </div>
            ...
            <div className="rounded-2xl border border-gray-800 overflow-hidden bg-gray-900 shadow-xl">
              ...
              <div className="p-8">
                <p className="text-sm leading-relaxed mb-6 text-gray-300">
                  This free dynamic click-timing aim trainer tests your reaction speed and clicking precision against moving, shrinking bullseye targets. The engine utilizes random-acceleration vector physics to force the targets to fly erratically around the screen. As you succeed, the targets severely shrink and speed up, demanding high-level mouse control.
                </p>
                ...
                <div className="mb-8 bg-[#0b0f19]/40 border border-gray-800 rounded-xl p-6">
                  ...
                  <div className="grid sm:grid-cols-2 gap-6 text-sm text-gray-300">
                    <ol className="space-y-3 list-decimal pl-5">
                      ...
                      <li>Track the dynamically moving target as it shrinks over time.</li>
