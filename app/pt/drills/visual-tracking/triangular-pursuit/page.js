...
export default function TriangularPursuitPagePT() {
  ...
  const guide = {
    ...
    intro: "O rastreamento ocular ao longo de polígonos fechados impõe uma demanda única sobre o sistema de controle visuomotor. Enquanto a perseguição suave ao longo das arestas diagonais exige a fusão contínua de comandos horizontais (via PPRF pontino) e verticais (via riMLF mesencefálico), os três vértices agudos de 60 graus causam uma queda abrupta da velocidade de deslizamento retiniano e uma explosão no erro de posição foveal. Estudos clássicos de de Brouwer et al. (2002) e Heinen et al. (2005) confirmam que esses cantos acionam sacadas corretivas disparadas pelo córtex frontal de controle ocular (FEF) e ajustadas pelo flóculo cerebelar para eliminar ultrapassagens inerciais.",
    benchmarks: {
