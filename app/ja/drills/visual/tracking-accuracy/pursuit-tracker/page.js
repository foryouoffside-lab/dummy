...
const guideData = {
  ...
  intro: [
    "追従眼球運動（Smooth Pursuit Eye Movement, SPEM / 滑動性眼球運動）は、動く視覚目標の速度に合わせて眼球を滑らかに回転させ、目標の網膜像を中心窩（Fovea centralis）に留め続ける高度な眼球運動システムです。視標の位置誤差を急速なジャンプによって補正するサッケード運動（急速眼球運動）とは生理学的に完全に独立した神経回路によって制御されています（Rashbass, 1961）。",
    ...
    "人間の視覚フィードバックループには約100〜130msの不可避な感覚運動遅延（Sensorimotor Delay）が存在します。等速で動く物体に対して遅延なく追従できるのは、小脳内に目標の物理的運動を先読みする内部モデル（Internal Predictive Model）が形成され、フィードフォワード制御（予測制御）が働くためです（Barnes, 2008）。この予測機能が未発達であったり集中力が低下すると、追従速度が目標に追いつかず、視線が頻繁にカクつく補正サッケード（Catch-up Saccade）が発生してトラッキング精度が著しく低下します。",
    "本システムは、HTML5の高精度タイマー（performance.now()）とサブピクセル描画エンジンを活用し、移動オーブに対するポインタの滞在時間（Time on Target）、平均追従偏差、切り返し時の応答遅延をリアルタイムに算出します。継続的なスムーズパシュート訓練は外眼筋と手指の運動連鎖を最適化し、FPSゲームでの吸い付くようなトラッキングエイムの確立、球技スポーツにおける動体視力強化、そして長時間のスクリーンワークによる視覚疲労の低減を可能にします。"
  ],
  benchmarks: {
