...
export default function VisualSearchPage() {
  ...
  const handleWrongClick = () => {
    ...
    // Penalty: -1 point for wrong click
    ...
    for (let i = 0; i < TOTAL_CELLS; i++) {
      ...
      if (i === targetPos) {
        ...
        div.onclick = (e) => {
          ...
          // +1 point per correct find
          const pointsEarned = 1;
          
          scoreRef.current += pointsEarned;
          setScore(scoreRef.current);
          showFeedback(`✓ +${pointsEarned} point (${searchTimeMs}ms)`, 'success');
          ...
          {gameState === 'start' && (
            <div className={`absolute inset-0 flex items-center justify-center backdrop-blur-sm rounded-xl z-40 ${isBoxDarkMode ? 'bg-gray-900/95' : 'bg-white/95'}`}>
              <div className={`rounded-2xl p-8 text-center max-w-md mx-4 shadow-xl border ${isBoxDarkMode ? 'bg-gray-800 border-gray-700' : 'bg-white border-gray-200'}`}>
                ...
                <p className={`mb-6 ${isBoxDarkMode ? 'text-gray-300' : 'text-gray-600'}`}>60-second challenge • Find the 'C'</p>
                <button 
                  onClick={startGame} 
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  <div className="space-y-2">
                    ...
                    <div className="flex items-start gap-2">
                      ...
                      <p className={`text-sm ${isDarkMode ? 'text-gray-300' : 'text-gray-600'}`}>Wrong click: <span className="font-semibold text-red-500">-1 point penalty</span> • Resets streak</p>
                    </div>
                  </div>
                  <div className="space-y-2">
                    <div className="flex items-start gap-2">
                      ...
                      <p className={`text-sm ${isDarkMode ? 'text-gray-300' : 'text-gray-600'}`}>Every 5 streak = <span className="font-semibold text-blue-500">bonus sound</span></p>
                    </div>
                    <div className="flex items-start gap-2">
                    ...
                    <div className="flex items-start gap-2">
                      ...
                      <p className={`text-sm ${isDarkMode ? 'text-gray-300' : 'text-gray-600'}`}><span className="font-semibold text-yellow-500">No lives system</span> • Just avoid wrong clicks to keep score high</p>
                    </div>
                  </div>
                </div>
                <div className={`mt-3 pt-3 border-t text-xs ${isDarkMode ? 'border-gray-700 text-gray-400' : 'border-gray-200 text-gray-500'} flex items-center justify-between`}>
                  <span>🎯 Find 'C' to get +1 point • Wrong click = -1 point penalty</span>
