...
import { 
  ArrowLeft, Calculator, Zap, Clock, Award, 
  RotateCcw, Volume2, VolumeX, Maximize2, Minimize2,
  Sun, Moon, TrendingUp, Brain, Eye,
  Timer, Target, Activity, Infinity, Trophy,
  BarChart3, Info, CheckCircle, Hash
} from 'lucide-react';
...
export default function QuickMathDrill() {
  ...
  const handleCorrect = (time) => {
    const speedBonus = Math.max(0, Math.floor(10 - time * 2));
    const difficultyBonus = difficulty * 5;
    const comboBonus = Math.floor(comboRef.current / 3) * 5;
    const pointsEarned = 20 + difficultyBonus + speedBonus + comboBonus;
        ...
        <div className="mb-6">
          ...
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-3">
              ...
              <div>
                <h1 className={`text-3xl font-bold ${isDarkMode ? 'text-white' : 'text-gray-900'}`}>Infinite Math Lab</h1>
                <p className={`${isDarkMode ? 'text-gray-400' : 'text-gray-500'}`}>60 seconds • Endless unique problems</p>
              </div>
            </div>
            ...
            {gameState === 'start' && (
              <div className={`absolute inset-0 flex items-center justify-center backdrop-blur-sm rounded-xl z-40 ${isBoxDarkMode ? 'bg-gray-900/95' : 'bg-white/95'}`}>
                <div className={`rounded-2xl p-8 text-center max-w-md mx-4 shadow-xl border ${isBoxDarkMode ? 'bg-gray-800 border-gray-700' : 'bg-white border-gray-200'}`}>
                  ...
                  <h3 className={`text-2xl font-bold mb-2 ${isBoxDarkMode ? 'text-white' : 'text-gray-900'}`}>Infinite Math Lab</h3>
                  <p className={`mb-6 ${isBoxDarkMode ? 'text-gray-300' : 'text-gray-600'}`}>60-second challenge • Unique problems</p>
                  <button 
                    onClick={startDrill}
              ...
              <div className={`absolute inset-0 flex items-center justify-center backdrop-blur-sm rounded-xl z-40 ${isBoxDarkMode ? 'bg-gray-900/95' : 'bg-white/95'}`}>
                <div className={`rounded-2xl p-8 shadow-xl border w-[480px] ${isBoxDarkMode ? 'bg-gray-800 border-gray-700' : 'bg-white border-gray-200'}`}>
                  ...
                  <div className="grid grid-cols-2 gap-4 mb-8">
                    ...
                    <ResultCard label="Wrong" value={wrongAnswers} icon={<Hash className="w-4 h-4" />} color="text-red-500" />
                    ...
                    <ResultCard label="Avg Response" value={`${avgResponseTime}s`} icon={<Timer className="w-4 h-4" />} color="text-blue-500" />
                  </div>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  <div className="space-y-2">
                    ...
                    <div className="flex items-start gap-2">
                      ...
                      <p className={`text-sm ${isDarkMode ? 'text-gray-300' : 'text-gray-600'}`}>Correct: <span className="font-semibold text-green-500">20 + difficulty × 5 + speed + combo</span></p>
                    </div>
                    <div className="flex items-start gap-2">
                      <div className="w-5 h-5 rounded-full bg-red-500 flex items-center justify-center text-white text-xs font-bold mt-0.5">3</div>
                      <p className={`text-sm ${isDarkMode ? 'text-gray-300' : 'text-gray-600'}`}>Wrong: <span className="font-semibold text-red-500">-10 points</span> + combo resets</p>
                    </div>
                  </div>
                  <div className="space-y-2">
                    <div className="flex items-start gap-2">
                      ...
                      <p className={`text-sm ${isDarkMode ? 'text-gray-300' : 'text-gray-600'}`}>Every 3 combo = <span className="font-semibold text-blue-500">+5 bonus points</span></p>
                    </div>
                    <div className="flex items-start gap-2">
                      ...
                      <p className={`text-sm ${isDarkMode ? 'text-gray-300' : 'text-gray-600'}`}>Auto-submit on <span className="font-semibold text-purple-500">correct answer</span></p>
                    </div>
                    <div className="flex items-start gap-2">
                      <div className="w-5 h-5 rounded-full bg-yellow-500 flex items-center justify-center text-white text-xs font-bold mt-0.5">6</div>
                      <p className={`text-sm ${isDarkMode ? 'text-gray-300' : 'text-gray-600'}`}>Operations: +, -, ×, <span className="font-semibold text-yellow-500">÷ (unlocks later)</span></p>
                    </div>
                  </div>
                </div>
