...
export default function ComplexPatternElitePage() {
  ...
  const submitDrawing = () => {
    ...
    if (result.accurate) {
      ...
      playSound('success');
      showFeedback(`âœ“ Success! +${pointsEarned}`, 'success');
    } else {
      ...
      playSound('fail');
      showFeedback(`âœ— Failed! -${penaltyAmount} penalty`, 'error');
    }
          ...
          {gameState === 'start' && (
            <div className={`absolute inset-0 flex items-center justify-center backdrop-blur-sm rounded-xl z-40 ${isBoxDarkMode ? 'bg-gray-900/95' : 'bg-white/95'}`}>
              <div className={`rounded-2xl p-8 text-center max-w-md mx-4 shadow-xl border ${isBoxDarkMode ? 'bg-gray-800 border-gray-700' : 'bg-white border-gray-200'}`}>
                ...
                <p className={`mb-6 ${isBoxDarkMode ? 'text-gray-300' : 'text-gray-600'}`}>60-second challenge â€¢ Memorize and draw patterns</p>
                <button 
                  onClick={startGame} 
            ...
            <div className={`rounded-xl border overflow-hidden ${isDarkMode ? 'bg-gray-800 border-gray-700' : 'bg-white border-gray-200'}`}>
              ...
              <div className="p-4">
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  ...
                  <div className="space-y-2">
                    ...
                    <div className="flex items-start gap-2">
                      ...
                      <p className={`text-sm ${isDarkMode ? 'text-gray-300' : 'text-gray-600'}`}>Success: +100+(20Ã—complexity) pts â€¢ Failure: -25 penalty</p>
                    </div>
                    <div className="flex items-start gap-2">
                ...
                <div className={`mt-3 pt-3 border-t text-xs ${isDarkMode ? 'border-gray-700 text-gray-400' : 'border-gray-200 text-gray-500'} flex items-center justify-between`}>
                  <span>ðŸŸ¢ Cyan = Start â€¢ ðŸŸ£ Magenta = End â€¢ ðŸŸ¢ Green = Waypoints (memorize)</span>
                  <span>âš¡ Draw path must pass near green waypoints to score â€¢ Each failure adds a penalty</span>
                </div>
              </div>
            </div>
          </div>
