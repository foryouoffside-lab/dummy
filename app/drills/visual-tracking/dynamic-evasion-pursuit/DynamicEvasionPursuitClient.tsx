...
export default function DynamicEvasionPursuitClient() {
  ...
  // Canvas Render Loop (Dynamic Evasion Physics)
  useEffect(() => {
    ...
    const draw = (ts: number) => {
      ...
      // Dynamic Evasion Timer Logic (sudden direction shifts)
      trackingState.current.timer += scaledDt * 1000;
      const shiftInterval = randomSpeed ? 350 + Math.random() * 300 : 500;

      if (trackingState.current.timer > shiftInterval) {
        const speedMag = (5 + Math.random() * 5) * effectiveSpeed;
        const angle = Math.random() * Math.PI * 2;
        trackingState.current.vx = Math.cos(angle) * speedMag;
        trackingState.current.vy = Math.sin(angle) * speedMag;
        ...
        {!isFullscreen && (
          <div className="text-center">
            ...
            <p className="text-xs text-slate-400 mt-1">
              Erratic Evasive Motion & Rapid Gaze Re-acquisition
            </p>
          </div>
        )}
