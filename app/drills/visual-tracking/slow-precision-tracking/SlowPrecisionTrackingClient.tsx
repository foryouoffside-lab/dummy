...
export default function SlowPrecisionTrackingClient() {
  ...
  useEffect(() => {
    ...
    try {
      const stored = localStorage.getItem('visualTrackingSlowPrecisionTrackingClientTrialsCount');
        ...
        setTimeLeft(p => {
          if (p <= 1) {
            ...
            setTotalTrials(prev => {
              const next = prev + 1;
              try {
                localStorage.setItem('visualTrackingSlowPrecisionTrackingClientTrialsCount', next.toString());
              } catch (e) {}
              return next;
            });
  ...
  useEffect(() => {
    ...
    const updateSize = () => {
      ...
      trackingState.current.heights = [h * 0.3, h * 0.5, h * 0.7];
      trackingState.current.px = w * 0.5;
      trackingState.current.py = trackingState.current.heights[0];
      trackingState.current.vx = 4;
      trackingState.current.level = 0;
      trackingState.current.levelT = 0;
    };
    ...
    const draw = (ts: number) => {
      ...
      // Update level swap timers
        trackingState.current.levelT += scaledDt;
        if (trackingState.current.levelT > 2.0) {
          trackingState.current.level = (trackingState.current.level + 1) % 3;
          trackingState.current.levelT = 0;
          playSound('transition');
        }
        
        if (trackingState.current.heights.length === 3) {
          trackingState.current.py = trackingState.current.heights[trackingState.current.level];
        }
        ...
        // Border bounce
        const leftBound = W * 0.1;
        const rightBound = W * 0.9;
        if (trackingState.current.px < leftBound) {
          trackingState.current.px = leftBound;
          trackingState.current.vx = 4;
        } else if (trackingState.current.px > rightBound) {
          trackingState.current.px = rightBound;
          trackingState.current.vx = -4;
        }
        ...
        ctx.lineWidth = 4;
        if (trackingState.current.heights.length === 3) {
          for (let h of trackingState.current.heights) {
            ctx.beginPath();
            ctx.moveTo(W * 0.1, h);
            ctx.lineTo(W * 0.9, h);
            ctx.stroke();
        ...
        ctx.beginPath();
        ctx.arc(trackingState.current.px, trackingState.current.py, 3, 0, Math.PI * 2);
        ctx.fill();
    ...
    if (typeof navigator !== 'undefined' && navigator.share) {
      try {
        await navigator.share({
          title: 'Free Slow-Precision Tracking Drill | SkillDrills',
      ...
      navigator.clipboard.writeText('https://skilldrills.online/drills/visual-tracking/slow-precision-tracking');
        ...
        {!isFullscreen && (
          <nav aria-label="Breadcrumb" className="mb-5">
            <ol className="flex items-center gap-2 text-[10px] text-slate-550 uppercase tracking-widest font-mono">
              ...
              <li><span className="text-slate-755">/</span></li>
              <li><span className="text-red-400 font-bold">Slow-Precision Tracking</span></li>
            </ol>
          </nav>
        )}
            ...
            <div className="flex items-center gap-3.5">
              ...
              <div>
                <h1 className="text-xl sm:text-2xl font-black tracking-tight text-white uppercase bg-gradient-to-r from-white via-slate-200 to-slate-400 bg-clip-text text-transparent">
                  Slow-Precision Tracking
                </h1>
                <p className="text-xs text-slate-400 tracking-wider mt-0.5">
                  Visual Gaze Calibration • Beginner Sector
                </p>
              </div>
            </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-4 gap-6 items-start">
          ...
          <div className={`${isFullscreen ? 'col-span-4' : 'lg:col-span-3'} flex flex-col`}>
            ...
            >
              ...
              {/* Screen overlay HUD details */}
              {gameState === 'playing' && (
                <div className="absolute top-4 left-4 z-10 pointer-events-none font-mono text-[9px] text-slate-500 uppercase tracking-widest bg-slate-950/60 border border-slate-900 px-2 py-1 rounded">
                  Drill Speed: {speedMultiplier.toFixed(1)}x • Target: {targetSize}px
                </div>
              )}
                ...
                <div className="absolute inset-0 bg-[#05070e]/95 flex flex-col items-center justify-center p-6 z-30 select-none">
                  <div className="max-w-md text-center">
                    ...
                    <p className="text-xs text-slate-400 leading-relaxed mb-6">
                      Deliberate horizontal plane pursuit with discrete vertical level transitions. Recalibrate smooth eye movements and vertical saccades. Focus your eyes on the moving target. Do not use your head or mouse.
                    </p>

                    <button
                      onClick={startDrill}
                      className="px-6 py-3 bg-red-600 hover:bg-red-700 text-white font-bold rounded-lg text-xs flex items-center justify-center gap-2 shadow-lg shadow-red-600/20 uppercase tracking-widest font-mono transition-all duration-200 active:scale-95 mx-auto"
                    >
            ...
            <div className="md:col-span-2 bg-[#0b0f19]/40 border border-slate-900 p-6 rounded-2xl flex flex-col justify-between">
              <div>
                ...
                <p className="text-xs leading-relaxed text-slate-400 mb-4 font-sans">
                  The Slow-Precision Tracking Drill conditions steady foveal smooth pursuit. By keeping horizontal movement slow and level-bounded, the ocular motor system learns to execute micro-adjustments without visual slip or saccadic overshoot.
                </p>
              </div>
            </div>
            ...
            <div className="bg-[#0b0f19]/40 border border-slate-905 p-6 rounded-2xl">
              ...
              <ul className="space-y-3 text-xs text-slate-400 font-mono">
                <li className="flex items-start gap-2">
                  ...
                  <span><strong>Smooth Pursuit</strong>: Stabilizes horizontal tracking vectors at low velocities.</span>
                </li>\n                <li className="flex items-start gap-2">
                  <Check className="w-4 h-4 text-green-500 flex-shrink-0 mt-0.5" />
                  <span><strong>Level Isolation</strong>: Discretizes eye focus across three vertical heights.</span>
                </li>\n                <li className="flex items-start gap-2">
                  <Check className="w-4 h-4 text-green-500 flex-shrink-0 mt-0.5" />
                  <span><strong>Ocular Calming</strong>: Eliminates visual micro-jitter and tracking instability.</span>
                </li>\n
              </ul>
            </div>
          </div>
            ...
            <div className="max-w-7xl mx-auto">
              <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-5 gap-8 mb-8 font-mono text-xs text-left">
                ...
                <div>
                  ...
                  <ul className="space-y-2">
                    ...
                    <li><Link href="/drills/fps/reactive-tracking" className="hover:text-red-400 transition-colors">Reactive Tracking</Link></li>
                ...
                <div>
                  ...
                  <ul className="space-y-2">
                    ...
                    <li><Link href="/drills/productivity" className="hover:text-red-400 transition-colors">Productivity (10 drills)</Link></li>
                    <li><Link href="/drills/mental-fitness" className="hover:text-red-400 transition-colors">Mental Fitness (6 drills)</Link></li>
              ...
              <div className="border-t border-slate-900 pt-8 text-center">
                ...
                <p className="text-[10px] max-w-2xl mx-auto leading-relaxed mb-6 font-sans text-slate-700">
                  Free online Slow-Precision Tracking drill for visual gaze tracking and foveal stability training. Train foveal pursuit and coordinate re-acquisition speeds under speed configurations. Perfect for gamers, athletes, and attention diagnostics. No registration required. More free drills at skilldrills.online.
                </p>
