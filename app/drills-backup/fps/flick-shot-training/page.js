...
import { 
  ...
  Eye, Crosshair, Timer, Trophy, Target, Zap, Activity, Award, Info, Check
} from 'lucide-react';
...
export default function ProFlickPage() {
  ...
  const TARGET_DURATION = 400;
  const SPAWN_INTERVAL = 500;
  const MAX_POINTS = 100;

  // Load best score from localStorage
  useEffect(() => {
    const savedBestScore = localStorage.getItem('proFlickDrillBestScore');
    if (savedBestScore) {
  ...
  const updateBestScore = (finalScore) => {
    const currentBestScore = parseInt(localStorage.getItem('proFlickDrillBestScore') || '0', 10);
    if (finalScore > currentBestScore) {
      localStorage.setItem('proFlickDrillBestScore', finalScore.toString());
  ...
  // Timer function using ref to avoid dependency issues
    ...
    const handleClick = (e) => {
      ...
      if (targetRef.current) {
        ...
        if (elapsed < TARGET_DURATION) {
          ...
          if (distance < TARGET_SIZE / 2 + 15) {
            const pointsEarned = calculateScoreFromSpeed(elapsed);
            ...
            if (bestReaction === 0 || elapsed < bestReaction) {
              setBestReaction(Math.round(elapsed));
            }
            ...
            showFeedback(`âœ“ ${Math.round(elapsed)}ms | +${totalPoints}`, 'success');
          ...
          handlePenalty('Too slow');
          targetRef.current = null;
        }
      } else {
        handlePenalty('Missed');
      }
    };
  ...
  useEffect(() => {
    ...
    function spawnTarget() {
      ...
      return {
        x: Math.random() * (cvs.width - padding * 2) + padding,
        y: Math.random() * (cvs.height - padding * 2) + padding,
    ...
    function draw(currentTime) {
      ...
      if (!targetRef.current && gameState === 'playing') {
      ...
      if (targetRef.current) {
        const elapsed = currentTime - targetRef.current.startTime;

        if (elapsed < TARGET_DURATION) {
          const opacity = Math.max(0.3, 1 - (elapsed / TARGET_DURATION) * 0.7);
          
          ctx.beginPath();
          ctx.arc(targetRef.current.x, targetRef.current.y, TARGET_SIZE / 2, 0, Math.PI * 2);
          ...
          ctx.beginPath();
          ctx.arc(targetRef.current.x, targetRef.current.y, TARGET_SIZE / 4, 0, Math.PI * 2);
        ...
        <div className="mb-6">
          ...
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-3">
              ...
              <div>
                <h1 className={`text-3xl font-bold ${isDarkMode ? 'text-white' : 'text-gray-900'}`}>Pro Flick Drill</h1>
                <p className={`${isDarkMode ? 'text-gray-400' : 'text-gray-500'}`}>Flick shots â€¢ 400ms targets â€¢ 60s countdown</p>
              </div>
            </div>
          ...
          {gameState === 'start' && (
            <div className={`absolute inset-0 flex items-center justify-center backdrop-blur-sm rounded-xl z-40 ${isBoxDarkMode ? 'bg-gray-900/95' : 'bg-white/95'}`}>
              <div className={`rounded-2xl p-8 text-center max-w-md mx-4 shadow-xl border ${isBoxDarkMode ? 'bg-gray-800 border-gray-700' : 'bg-white border-gray-200'}`}>
                ...
                <h3 className={`text-2xl font-bold mb-2 ${isBoxDarkMode ? 'text-white' : 'text-gray-900'}`}>Pro Flick Drill</h3>
                <p className={`mb-6 ${isBoxDarkMode ? 'text-gray-300' : 'text-gray-600'}`}>60-second challenge â€¢ 400ms targets</p>
                <button 
                  onClick={startGame} 
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  <div className="space-y-2">
                    <div className="flex items-start gap-2">
                      ...
                      <p className={`text-sm ${isDarkMode ? 'text-gray-300' : 'text-gray-600'}`}>
                        <span className="font-semibold text-green-500">Targets appear for 400ms</span> â€¢ Flick and click quickly
                      </p>
                    </div>
                    <div className="flex items-start gap-2">
                      ...
                      <p className={`text-sm ${isDarkMode ? 'text-gray-300' : 'text-gray-600'}`}>
                        <span className="font-semibold text-blue-500">Faster flicks = Higher scores</span> (&lt;100ms: +100, &lt;150ms: +80)
                      </p>
                    </div>
                    <div className="flex items-start gap-2">
                  ...
                  <div className="space-y-2">
                    ...
                    <div className="flex items-start gap-2">
                      ...
                      <p className={`text-sm ${isDarkMode ? 'text-gray-300' : 'text-gray-600'}`}>
                        <span className="font-semibold text-orange-500">Click = Score, Miss = Penalty</span> â€¢ Simple scoring
                      </p>
                    </div>
                    <div className="flex items-start gap-2">
                      <div className="w-5 h-5 rounded-full bg-yellow-500 flex items-center justify-center text-white text-xs font-bold mt-0.5">6</div>
                      <p className={`text-sm ${isDarkMode ? 'text-gray-300' : 'text-gray-600'}`}>
                        <span className="font-semibold text-yellow-500">Best Score saves locally</span> â€¢ Pure flick training
                      </p>
                    </div>
                  </div>
                </div>
                <div className={`mt-3 pt-3 border-t text-xs ${isDarkMode ? 'border-gray-700 text-gray-400' : 'border-gray-200 text-gray-500'} flex items-center justify-between`}>
                  <span>ðŸŽ¯ New target every 500ms â€¢ 60 second countdown</span>
                  <span>âš¡ Click on target to score</span>
                </div>
              </div>
            </div>
          </div>
