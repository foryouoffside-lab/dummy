...
export default function InfinityPursuitClient() {
  ...
  useEffect(() => {
    ...
    try {
      const stored = localStorage.getItem('visualTrackingInfinityPursuitClientTrialsCount');
        ...
        setTimeLeft(p => {
          if (p <= 1) {
            ...
            setTotalTrials(prev => {
              const next = prev + 1;
              try {
                localStorage.setItem('visualTrackingInfinityPursuitClientTrialsCount', next.toString());
              } catch (e) {}
              return next;
            });
  ...
  useEffect(() => {
    ...
    const draw = (ts: number) => {
      ...
      const scale = 0.35;
        // Draw figure-8 path guide
        ctx.strokeStyle = 'rgba(51, 65, 85, 0.4)';
        ctx.lineWidth = 4;
        ctx.beginPath();
        for (let i = 0; i <= Math.PI * 2; i += 0.05) {
          const x = W / 2 + (W * scale * Math.cos(i)) / (1 + Math.sin(i) * Math.sin(i));
          const y = H / 2 + (H * scale * Math.sin(i) * Math.cos(i)) / (1 + Math.sin(i) * Math.sin(i));
          i === 0 ? ctx.moveTo(x, y) : ctx.lineTo(x, y);
        }
        ctx.stroke();

        // Update position
        trackingState.current.t += 1.0 * scaledDt;

        const targetX = W / 2 + (W * scale * Math.cos(trackingState.current.t)) / (1 + Math.sin(trackingState.current.t) * Math.sin(trackingState.current.t));
        const targetY = H / 2 + (H * scale * Math.sin(trackingState.current.t) * Math.cos(trackingState.current.t)) / (1 + Math.sin(trackingState.current.t) * Math.sin(trackingState.current.t));
    ...
    if (typeof navigator !== 'undefined' && navigator.share) {
      try {
        await navigator.share({
          title: 'Free Infinity Pursuit Drill | SkillDrills',
      ...
      navigator.clipboard.writeText('https://skilldrills.online/drills/visual-tracking/infinity-pursuit');
        ...
        {!isFullscreen && (
          <nav aria-label="Breadcrumb" className="mb-5">
            <ol className="flex items-center gap-2 text-[10px] text-slate-550 uppercase tracking-widest font-mono">
              ...
              <li><span className="text-slate-755">/</span></li>
              <li><span className="text-red-400 font-bold">Infinity Pursuit</span></li>
            </ol>
          </nav>
        )}
                ...
                <div className="absolute inset-0 bg-[#05070e]/95 flex flex-col items-center justify-center p-6 z-30 select-none">
                  <div className="max-w-md text-center">
                    ...
                    <p className="text-xs text-slate-400 leading-relaxed mb-6">
                      Track a target moving along a visible figure-8 Lemniscate path at a fluid pace. Focus your eyes on the moving target. Do not use your head or mouse.
                    </p>
            ...
            <div className="md:col-span-2 bg-[#0b0f19]/40 border border-slate-900 p-6 rounded-2xl flex flex-col justify-between">
              <div>
                ...
                <p className="text-xs leading-relaxed text-slate-400 mb-4 font-sans">
                  The Infinity Pursuit Drill conditions complex eye tracking across crossing coordinates. Lemniscate trajectories challenge foveal tracking at the intersection points, preventing target loss during crossover vector changes.
                </p>
              </div>
            </div>
            ...
            <div className="bg-[#0b0f19]/40 border border-slate-905 p-6 rounded-2xl">
              ...
              <ul className="space-y-3 text-xs text-slate-400 font-mono">
                <li className="flex items-start gap-2">
                  ...
                  <span><strong>Intersection Focus</strong>: Trains the visual system to track targets crossing center lanes.</span>
                </li>\n                <li className="flex items-start gap-2">
                  <Check className="w-4 h-4 text-green-500 flex-shrink-0 mt-0.5" />
                  <span><strong>Lemniscate Stability</strong>: Strengthens diagonal eye coordination.</span>
                </li>\n                <li className="flex items-start gap-2">
                  <Check className="w-4 h-4 text-green-500 flex-shrink-0 mt-0.5" />
                  <span><strong>Smooth Pursuit</strong>: Promotes continuous path tracking at fluid velocities.</span>
                </li>\n
              </ul>
            </div>
          </div>
            ...
            <div className="max-w-7xl mx-auto">
              ...
              <div className="border-t border-slate-900 pt-8 text-center">
                ...
                <p className="text-[10px] max-w-2xl mx-auto leading-relaxed mb-6 font-sans text-slate-700">
                  Free online Infinity Pursuit drill for visual gaze tracking and foveal stability training. Train foveal pursuit and coordinate re-acquisition speeds under speed configurations. Perfect for gamers, athletes, and attention diagnostics. No registration required. More free drills at skilldrills.online.
                </p>
