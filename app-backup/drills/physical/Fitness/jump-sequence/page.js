  ...
  useEffect(() => {
    ...
    function draw() {
      ...
      if (!isJumpingRef.current && !isChargingRef.current && gameState === 'playing') {
        ...
        ctx.fillText("⬅ Click & Hold", playerRef.current.x - 35, playerRef.current.y - 10);
        ctx.fillText("Hold to charge ➔", playerRef.current.x + 35, playerRef.current.y - 10);
      }
          ...
          {gameState === 'start' && (
            <div className={`absolute inset-0 flex items-center justify-center backdrop-blur-sm rounded-xl z-40 ${isBoxDarkMode ? 'bg-gray-900/95' : 'bg-white/95'}`}>
              <div className={`rounded-2xl p-8 text-center max-w-md mx-4 shadow-xl border ${isBoxDarkMode ? 'bg-gray-800 border-gray-700' : 'bg-white border-gray-200'}`}>
                ...
                <p className={`mb-6 ${isBoxDarkMode ? 'text-gray-300' : 'text-gray-600'}`}>60-second challenge • Click and hold on the ball to charge, release to jump</p>
                <button 
                  onClick={startGame} 
            ...
            <div className={`rounded-xl border overflow-hidden ${isDarkMode ? 'bg-gray-800 border-gray-700' : 'bg-white border-gray-200'}`}>
              ...
              <div className="p-4">
                ...
                <div className={`mt-3 pt-3 border-t text-xs ${isDarkMode ? 'border-gray-700 text-gray-400' : 'border-gray-200 text-gray-500'} flex items-center justify-between`}>
                  <span>⚡ More charge = Higher jump • 🖱️ Mouse X = Air control</span>
                  <span>🎯 Hit target: +100pts • Streak bonus: +50 every 5 hits</span>
                </div>
              </div>
            </div>
          </div>
