...
export default function AffectiveConditioningPage() {
  ...
  const runStep = (index) => {
    ...
    if (index >= protocol.length) {
      ...
      playSound('complete');
      showFeedback(`ðŸŽ¯ Conditioning Complete! +10 Bonus`, 'success');
      scoreRef.current += 10;
    ...
    if (comboRef.current > 0 && comboRef.current % 3 === 0) {
      playSound('combo');
      showFeedback(`ðŸ”¥ ${comboRef.current} Step Combo! +${comboBonus}`, 'success');
    }
            ...
            <div className={`rounded-xl border overflow-hidden ${isDarkMode ? 'bg-gray-800 border-gray-700' : 'bg-white border-gray-200'}`}>
              ...
              <div className="p-4">
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  ...
                  <div className="space-y-2">
                    <div className="flex items-start gap-2">
                      ...
                      <p className={`text-sm ${isDarkMode ? 'text-gray-300' : 'text-gray-600'}`}>Progression: <span className="font-semibold text-cyan-500">Self â†’ Loved One â†’ Neutral â†’ Difficult â†’ Universal</span></p>
                    </div>
                    <div className="flex items-start gap-2">
                ...
                <div className={`mt-3 pt-3 border-t text-xs ${isDarkMode ? 'border-gray-700 text-gray-400' : 'border-gray-200 text-gray-500'} flex items-center justify-between`}>
                  <span>ðŸ§˜ Cultivates loving-kindness and emotional resilience</span>
                  <span>ðŸŽµ Warm orange glow facilitates pro-social emotions</span>
                </div>
              </div>
            </div>
          </div>
