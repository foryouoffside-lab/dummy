  ...
  const logWander = () => {
    ...
    if (timeSinceLastWander < 30000 && wanderCountRef.current > 1) {
      ...
      if (comboRef.current > 0 && comboRef.current % 3 === 0) {
        ...
        playSound('combo');
        showFeedback(`ðŸ”¥ ${comboRef.current} Notice Combo! +${comboBonus}`, 'success');
      } else {
        comboBonus = Math.floor(comboRef.current / 3);
      }
    } else {
            ...
            <div className={`rounded-xl border overflow-hidden ${isDarkMode ? 'bg-gray-800 border-gray-700' : 'bg-white border-gray-200'}`}>
              ...
              <div className="p-4">
                ...
                <div className={`mt-3 pt-3 border-t text-xs ${isDarkMode ? 'border-gray-700 text-gray-400' : 'border-gray-200 text-gray-500'} flex items-center justify-between`}>
                  <span>ðŸ§˜ Builds meta-awareness and attention regulation</span>
                  <span>ðŸŽµ Gentle audio cues at each noticed wandering</span>
                </div>
              </div>
            </div>
          </div>
