...
export default function TriangularPursuitPageFR() {
  ...
  const guide = {
    ...
    intro: "La poursuite fovéale le long de polygones fermés constitue un test d exigence motrice élevée. Le déplacement le long des segments diagonaux requiert une synthèse motrice harmonieuse entre circuits horizontaux (PPRF) et verticaux (riMLF). Cependant, à chacun des trois sommets aigus de 60 degrés, le glissement rétinien s effondre et l erreur de position fovéale culmine. Les études de de Brouwer et al. (2002) ainsi que de Heinen et al. (2005) démontrent que ces transitions imposent des saccades balistiques de recalage coordonnées par le cortex frontal et tempérées par le cervelet pour supprimer les dépassements inertiels.",
    benchmarks: {
