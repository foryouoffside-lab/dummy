...
import { 
  ...
  Eye, GitBranch, Brain, X, Trophy, Info, Timer, Circle
} from 'lucide-react';
...
export default function PrioritySortingPage() {
  ...
  const calculatePoints = (isPriority) => {
    const basePoints = isPriority ? 20 : 10;
      ...
      timerIntervalRef.current = setInterval(() => {
        setTimeLeft(prev => {
          if (prev <= 1) {
            ...
            const total = totalCleared + mistakes;
            const finalAccuracy = total === 0 ? 100 : Math.round((totalCleared / total) * 100);
  ...
  }, [gameState, timeLeft, totalCleared, mistakes]);

  const fail = (reason) => {
    if (!isActiveRef.current) return;
    
    const penaltyPoints = calculatePoints(false);
    ...
    const handleMouseDown = (e) => {
      ...
      if (hitIdx !== -1) {
        ...
        if (isPriority) {
          ...
          const pointsEarned = calculatePoints(true);
          ...
          const pointsEarned = calculatePoints(false);
          scoreRef.current += pointsEarned;
          setScore(scoreRef.current);
          ...
          {isFullscreen && gameState === 'playing' && (
            <>
              ...
              <div className="absolute top-4 left-4 z-30 bg-black/50 backdrop-blur-sm rounded-lg px-4 py-2 text-white text-sm">
                Score: <span className="text-yellow-400">{score}</span> | Time: <span className={timeLeft < 15 ? 'text-red-400' : 'text-green-400'}>{timeLeft}s</span> | Rule: <span style={{color: colors[currentRule]}}>{currentRule}</span>
              </div>
            </>
          )}
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  <div className="space-y-2">
                    ...
                    <div className="flex items-start gap-2">
                      ...
                      <p className={`text-sm ${isDarkMode ? 'text-gray-300' : 'text-gray-600'}`}>
                        <span className="font-semibold text-green-500">Priority targets: +20 base points</span> + streak bonus
                      </p>
                    </div>
                    <div className="flex items-start gap-2">
                      ...
                      <p className={`text-sm ${isDarkMode ? 'text-gray-300' : 'text-gray-600'}`}>
                        <span className="font-semibold text-blue-500">Noise items: +10 base points</span> + streak bonus
                      </p>
                    </div>
                  </div>
                  <div className="space-y-2">
                    <div className="flex items-start gap-2">
                      ...
                      <p className={`text-sm ${isDarkMode ? 'text-gray-300' : 'text-gray-600'}`}>
                        <span className="font-semibold text-red-500">Penalty = Potential Gain</span> (Wrong click loses what correct would earn)
                      </p>
                    </div>
                    <div className="flex items-start gap-2">
