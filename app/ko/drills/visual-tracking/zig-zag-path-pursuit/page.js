...
const guide = {
  ...
  intro: [
    ...
    "본 '지그재그 안구 추적(Zig-Zag Path Pursuit)' 드릴은 웹 브라우저 상에서 이러한 고난도 운동 적응을 달성하도록 구축된 전문 훈련 도구입니다. 쉴 새 없이 반복되는 예각 코너를 오차 없이 추적함으로써 에임 전환 시의 떨림과 시선 밀림을 근본적으로 제거합니다. 'Hide Line' 모드로 시각 보조선을 지운 채 순수 내재적 공간 예측력을 단련하고, 'Random Speed'를 통해 변칙적인 템포에도 흔들리지 않는 신경 적응력을 완성하십시오. 최신 게이밍 하드웨어 환경(Woods et al., 2015)에서 프로급 궤적 추적 정밀도를 체감해 보십시오."
  ],
  benchmarks: {
