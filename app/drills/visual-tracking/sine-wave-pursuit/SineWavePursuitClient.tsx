...
export default function SineWavePursuitClient() {
  ...
  useEffect(() => {
    ...
    try {
      const stored = localStorage.getItem('visualTrackingSineWavePursuitClientTrialsCount');
        ...
        setTimeLeft(p => {
          if (p <= 1) {
            ...
            setTotalTrials(prev => {
              const next = prev + 1;
              try {
                localStorage.setItem('visualTrackingSineWavePursuitClientTrialsCount', next.toString());
              } catch (e) {}
              return next;
            });
  ...
  useEffect(() => {
    ...
    const draw = (ts: number) => {
      ...
      // Draw visible Sine path
        ctx.beginPath();
        ctx.strokeStyle = 'rgba(51, 65, 85, 0.4)';
        ctx.lineWidth = 4;
        for (let i = W * 0.1; i < W * 0.9; i += 5) {
          const y = (H / 2) + Math.sin(i * 0.01) * (H * 0.2);
          i === W * 0.1 ? ctx.moveTo(i, y) : ctx.lineTo(i, y);
        }
        ctx.stroke();
        ...
        const targetY = (H / 2) + Math.sin(trackingState.current.x * 0.01) * (H * 0.2);
        ...
        ctx.beginPath();
        ctx.arc(trackingState.current.x, targetY, targetSize, 0, Math.PI * 2);
        ctx.fill();
        ctx.shadowBlur = 0;

        ctx.fillStyle = '#ffffff';
        ctx.beginPath();
        ctx.arc(trackingState.current.x, targetY, 3, 0, Math.PI * 2);
        ctx.fill();
    ...
    if (typeof navigator !== 'undefined' && navigator.share) {
      try {
        await navigator.share({
          title: 'Free Sine-Wave Pursuit Drill | SkillDrills',
          text: 'Track target oscillations along a horizontal sine wave guide line.',
          url: 'https://skilldrills.online/drills/visual-tracking/sine-wave-pursuit'
        });
      } catch (e) {}
    } else if (typeof navigator !== 'undefined' && navigator.clipboard) {
      navigator.clipboard.writeText('https://skilldrills.online/drills/visual-tracking/sine-wave-pursuit');
        ...
        {!isFullscreen && (
          <nav aria-label="Breadcrumb" className="mb-5">
            <ol className="flex items-center gap-2 text-[10px] text-slate-550 uppercase tracking-widest font-mono">
              ...
              <li><span className="text-slate-755">/</span></li>
              <li><span className="text-red-400 font-bold">Sine-Wave Pursuit</span></li>
            </ol>
          </nav>
        )}
                ...
                <div className="absolute inset-0 bg-[#05070e]/95 flex flex-col items-center justify-center p-6 z-30 select-none">
                  <div className="max-w-md text-center">
                    ...
                    <p className="text-xs text-slate-400 leading-relaxed mb-6">
                      Track target oscillations along a horizontal sine wave guide line. Focus your eyes on the moving target. Do not use your head or mouse.
                    </p>
            ...
            <div className="md:col-span-2 bg-[#0b0f19]/40 border border-slate-900 p-6 rounded-2xl flex flex-col justify-between">
              <div>
                ...
                <p className="text-xs leading-relaxed text-slate-400 mb-4 font-sans">
                  The Sine-Wave Pursuit Drill builds foveal control under continuous vertical acceleration changes. Moving at a consistent pixels-per-second horizontal velocity, it forces the foveal system to track sinusoids smoothly.
                </p>
              </div>
            </div>
            ...
            <div className="bg-[#0b0f19]/40 border border-slate-905 p-6 rounded-2xl">
              ...
              <ul className="space-y-3 text-xs text-slate-400 font-mono">
                <li className="flex items-start gap-2">
                  ...
                  <span><strong>Oscillation Tracking</strong>: Sharpens vertical eye muscle adaptations during horizontal transitions.</span>
                </li>\n                <li className="flex items-start gap-2">
                  <Check className="w-4 h-4 text-green-500 flex-shrink-0 mt-0.5" />
                  <span><strong>Vertical Acceleration</strong>: Trains prediction under continuous curve acceleration changes.</span>
                </li>\n                <li className="flex items-start gap-2">
                  <Check className="w-4 h-4 text-green-500 flex-shrink-0 mt-0.5" />
                  <span><strong>Trajectory Guide</strong>: Visualizes path parameters to support steady pursuit.</span>
                </li>\n
              </ul>
            </div>
          </div>
            ...
            <div className="max-w-7xl mx-auto">
              ...
              <div className="border-t border-slate-900 pt-8 text-center">
                ...
                <p className="text-[10px] max-w-2xl mx-auto leading-relaxed mb-6 font-sans text-slate-700">
                  Free online Sine-Wave Pursuit drill for visual gaze tracking and foveal stability training. Train foveal pursuit and coordinate re-acquisition speeds under speed configurations. Perfect for gamers, athletes, and attention diagnostics. No registration required. More free drills at skilldrills.online.
                </p>
