...
export default function AimTrainerClient() {
  ...
  const applyPenalty = useCallback((reason) => {
    ...
    else{scoreRef.current=Math.max(0,scoreRef.current-5);setScore(scoreRef.current);playSound('penalty');showFeedback(`${reason}! -5 points`,'error');}
  ...
  useEffect(() => {
    const h = (e) => {
      ...
      if(hit&&hi!==-1){totalAttemptsRef.current++;hitsRef.current++;successfulHitsRef.current++;setTargetsHit(successfulHitsRef.current);reactionTimesRef.current.push(hr);setBestReaction(prev=>prev===0||hr<prev?hr:prev);targetsRef.current.splice(hi,1);const ns=streakRef.current+1;streakRef.current=ns;setStreak(ns);if(ns>bestStreakRef.current){bestStreakRef.current=ns;setBestStreak(ns);}scoreRef.current+=5;setScore(scoreRef.current);if(ns%10===0){playSound('streak');showFeedback(`🔥 ${ns} Streak! +5`,'success');}else{playSound('hit');showFeedback(`✓ Hit! +5`,'success');}}
