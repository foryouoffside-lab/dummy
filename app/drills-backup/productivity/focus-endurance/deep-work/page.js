...
import { 
  ...
  Eye, Focus, Brain, X, Trophy, Info, Timer
} from 'lucide-react';
...
export default function DeepWorkLabPage() {
  ...
  const zoneRef = useRef({ x: 0, y: 0, r: 45, vx: 130, vy: 130 });
  ...
  // Save best score whenever it updates
  useEffect(() => {
    if (bestScore > 0) {
  ...
  const update = (dt, cvs) => {
    ...
    if (dist < zone.r) {
      ...
      // 1 second of concentration = +1 score
      ...
      // 1 second of distraction = 1 penalty
      distractionTimerRef.current += dt;
      if (distractionTimerRef.current >= 1.0) {
        const penaltiesToAdd = Math.floor(distractionTimerRef.current);
        for (let i = 0; i < penaltiesToAdd; i++) {
          addPenalty();
        }
        distractionTimerRef.current -= penaltiesToAdd;
        if (penaltiesToAdd > 0) {
          showFeedback(`⚠️ ${penaltiesToAdd} Penalty!`, 'error');
        }
      }
      ...
      if (focusMeterRef.current < 25 && wasAbove) {
        // Penalty already handled by the 1-second timer above
      }
    }
    ...
    if (scoreRef.current > bestScore) {
      setBestScore(Math.floor(scoreRef.current));
    }
  };
          ...
          {gameState === 'start' && (
            <div className={`absolute inset-0 flex items-center justify-center backdrop-blur-sm rounded-xl z-40 ${isBoxDarkMode ? 'bg-gray-900/95' : 'bg-white/95'}`}>
              <div className={`rounded-2xl p-8 text-center max-w-md mx-4 shadow-xl border ${isBoxDarkMode ? 'bg-gray-800 border-gray-700' : 'bg-white border-gray-200'}`}>
                ...
                <p className={`mb-6 ${isBoxDarkMode ? 'text-gray-300' : 'text-gray-600'}`}>60-second challenge • 1 sec focus = +1 point</p>
                <button 
                  onClick={startGame} 
            ...
            <div className={`absolute inset-0 flex items-center justify-center backdrop-blur-sm rounded-xl z-40 ${isBoxDarkMode ? 'bg-gray-900/95' : 'bg-white/95'}`}>
              <div className={`rounded-2xl p-8 shadow-xl border w-[480px] ${isBoxDarkMode ? 'bg-gray-800 border-gray-700' : 'bg-white border-gray-200'}`}>
                ...
                <div className="grid grid-cols-2 gap-4 mb-8">
                  ...
                  <ResultCard label="Accuracy" value={accuracy} unit="%" icon={<Focus className="w-4 h-4" />} color="text-purple-500" />
                  <ResultCard label="Mistakes" value={mistakes} icon={<X className="w-4 h-4" />} color="text-red-500" />
                  <ResultCard label="Focus Rating" value={bestFocus >= 80 ? 'Elite' : bestFocus >= 60 ? 'Good' : 'Fair'} icon={<Brain className="w-4 h-4" />} color="text-cyan-500" />
                </div>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  <div className="space-y-2">
                    <div className="flex items-start gap-2">
                      ...
                      <p className={`text-sm ${isDarkMode ? 'text-gray-300' : 'text-gray-600'}`}>
                        Keep your cursor <span className="font-semibold text-blue-500">inside the moving green ring</span> to earn points
                      </p>
                    </div>
                    <div className="flex items-start gap-2">
                      <div className="w-5 h-5 rounded-full bg-green-500 flex items-center justify-center text-white text-xs font-bold mt-0.5">2</div>
                      <p className={`text-sm ${isDarkMode ? 'text-gray-300' : 'text-gray-600'}`}>
                        <span className="font-semibold text-green-500">1 second of focus = +1 point</span> • Build your score steadily
                      </p>
                    </div>
                    <div className="flex items-start gap-2">
                  ...
                  <div className="space-y-2">
                    <div className="flex items-start gap-2">
                      ...
                      <p className={`text-sm ${isDarkMode ? 'text-gray-300' : 'text-gray-600'}`}>
                        Focus meter shows <span className="font-semibold text-orange-500">attention level</span> • Below 25% is critical
                      </p>
                    </div>
                    <div className="flex items-start gap-2">
                      ...
                      <p className={`text-sm ${isDarkMode ? 'text-gray-300' : 'text-gray-600'}`}>
                        Challenge lasts <span className="font-semibold text-purple-500">60 seconds</span> • Maximize your focus time
                      </p>
                    </div>
                    <div className="flex items-start gap-2">
                      <div className="w-5 h-5 rounded-full bg-yellow-500 flex items-center justify-center text-white text-xs font-bold mt-0.5">6</div>
                      <p className={`text-sm ${isDarkMode ? 'text-gray-300' : 'text-gray-600'}`}>
                        Your <span className="font-semibold text-yellow-500">Best Score</span> is saved locally and persists across sessions
                      </p>
                    </div>
                  </div>
                </div>
                <div className={`mt-3 pt-3 border-t text-xs ${isDarkMode ? 'border-gray-700 text-gray-400' : 'border-gray-200 text-gray-500'} flex items-center justify-between`}>
                  <span>🎯 Green ring = Safe zone • Red focus bar = Danger zone</span>
                  <span>⚡ 1 sec focus = +1 • 1 sec distraction = 1 penalty</span>
                </div>
              </div>
            </div>
          </div>
...
function ResultCard({ label, value, unit = '', icon, color }) {
  const bgColor = color === 'text-blue-500' ? 'bg-blue-500/10' : 
                   ...
                   color === 'text-red-500' ? 'bg-red-500/10' : 'bg-cyan-500/10';
