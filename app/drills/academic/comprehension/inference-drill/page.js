...
export default function ComprehensionDrill() {
  ...
  const startGame = () => {
    ...
    setTimeout(() => {
      const available = DRILL_SET.filter((_, idx) => !completedQuestions.has(idx));
      if (available.length > 0) {
        const randomQuestion = available[Math.floor(Math.random() * available.length)];
        setCurrentQuestion(randomQuestion);
      }
  ...
  const handleSubmit = (index) => {
    ...
    if (isCorrect) {
      ...
      // Wrong answer: -1 point penalty (no lives)
      setCombo(0);
      setWrongAnswers(prev => prev + 1);
      
      // Penalty: -1 point (score never goes below 0)
              ...
              <div className="w-full">
                <div className="flex justify-between items-center mb-4">
                  ...
                  <div className="flex items-center gap-2">
                    <span className={`text-sm ${isBoxDarkMode ? 'text-gray-400' : 'text-gray-500'}`}>Combo: {combo}x</span>
                  </div>
                </div>
