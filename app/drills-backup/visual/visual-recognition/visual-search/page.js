...
import { 
  ...
  Eye, Timer, Search, Brain, X,
  BarChart3, Trophy, Info, CheckCircle2
} from 'lucide-react';
...
export default function VisualSearchPage() {
  ...
  useEffect(() => {
    const savedBestScore = localStorage.getItem('visualSearchBestScore');
    if (savedBestScore) {
  ...
  const updateBestScore = (finalScore) => {
    const currentBestScore = parseInt(localStorage.getItem('visualSearchBestScore') || '0', 10);
    if (finalScore > currentBestScore) {
      localStorage.setItem('visualSearchBestScore', finalScore.toString());
  ...
  const generateGrid = () => {
    if (!gridContainerRef.current || !isActiveRef.current) return;
    
    const container = gridContainerRef.current;
    container.innerHTML = '';
    
    const targetPos = Math.floor(Math.random() * TOTAL_CELLS);
    ...
    for (let i = 0; i < TOTAL_CELLS; i++) {
      ...
      div.style.color = isBoxDarkMode ? '#888888' : '#666666';
      ...
      if (i === targetPos) {
        ...
        div.onclick = (e) => {
          ...
          const searchTimeMs = Math.floor(performance.now() - startTimeRef.current);
          setSearchTime(searchTimeMs);
          
          if (bestTime === 0 || searchTimeMs < bestTime) {
            setBestTime(searchTimeMs);
          }
          ...
          let pointsEarned = 10;
          if (searchTimeMs < 500) pointsEarned = 30;
          else if (searchTimeMs < 1000) pointsEarned = 20;
          else if (searchTimeMs < 2000) pointsEarned = 15;
          ...
          showFeedback(`✓ +${pointsEarned} (${searchTimeMs}ms)`, 'success');
          ...
          if (isActiveRef.current) generateGrid();
        };
      } else {
        div.textContent = 'O';
  ...
  const startGame = () => {
    ...
    setTimeout(() => {
      if (gridContainerRef.current && isActiveRef.current) {
        generateGrid();
      }
    }, 50);
  };
  ...
  const getAccuracy = () => {
    const total = totalFound + mistakes;
    return total === 0 ? 100 : Math.round((totalFound / total) * 100);
  };
    ...
    <div className={`min-h-screen select-none ${isDarkMode ? 'bg-gray-900' : 'bg-gray-50'}`}>
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        <div className="mb-6">
          ...
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-3">
              ...
              <div>
                <h1 className={`text-3xl font-bold ${isDarkMode ? 'text-white' : 'text-gray-900'}`}>Visual Search</h1>
                <p className={`${isDarkMode ? 'text-gray-400' : 'text-gray-500'}`}>Find 'C' among 'O's • 60 seconds</p>
              </div>
            </div>
        ...
        <div className="grid grid-cols-7 gap-3 mb-4 h-[88px]">
          ...
          <StatCard icon={<CheckCircle2 className="text-emerald-600" />} value={totalFound} label="Found" isDark={isDarkMode} />
          <StatCard icon={<Activity className="text-red-500" />} value={`${mistakes}/5`} label="Mistakes" isDark={isDarkMode} />
        </div>
          ...
          {gameState === 'start' && (
            <div className={`absolute inset-0 flex items-center justify-center backdrop-blur-sm rounded-xl z-40 ${isBoxDarkMode ? 'bg-gray-900/95' : 'bg-white/95'}`}>
              <div className={`rounded-2xl p-8 text-center max-w-md mx-4 shadow-xl border ${isBoxDarkMode ? 'bg-gray-800 border-gray-700' : 'bg-white border-gray-200'}`}>
                <Search className="w-16 h-16 text-cyan-500 mx-auto mb-4" />
                <h3 className={`text-2xl font-bold mb-2 ${isBoxDarkMode ? 'text-white' : 'text-gray-900'}`}>Visual Search</h3>
                <p className={`mb-6 ${isBoxDarkMode ? 'text-gray-300' : 'text-gray-600'}`}>Find 'C' among 'O's • 60 seconds</p>
                <button 
                  onClick={startGame} 
            ...
            <div className={`absolute inset-0 flex items-center justify-center backdrop-blur-sm rounded-xl z-40 ${isBoxDarkMode ? 'bg-gray-900/95' : 'bg-white/95'}`}>
              <div className={`rounded-2xl p-8 shadow-xl border w-[480px] ${isBoxDarkMode ? 'bg-gray-800 border-gray-700' : 'bg-white border-gray-200'}`}>
                <div className="flex items-center justify-center gap-3 mb-6">
                  ...
                  <h3 className={`text-2xl font-bold ${isBoxDarkMode ? 'text-white' : 'text-gray-900'}`}>Search Complete!</h3>
                </div>
                ...
                <div className="grid grid-cols-2 gap-4 mb-8">
                  ...
                  <ResultCard label="Targets Found" value={totalFound} icon={<CheckCircle2 className="w-4 h-4" />} color="text-green-500" />
                  ...
                  <ResultCard label="Best Time" value={bestTime || '-'} unit="ms" icon={<Clock className="w-4 h-4" />} color="text-cyan-500" />
                  <ResultCard label="Last Search" value={searchTime || '-'} unit="ms" icon={<Timer className="w-4 h-4" />} color="text-blue-500" />
                </div>
            ...
            <div className={`rounded-xl border overflow-hidden ${isDarkMode ? 'bg-gray-800 border-gray-700' : 'bg-white border-gray-200'}`}>
              ...
              <div className="p-4">
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  <div className="space-y-2">
                    <div className="flex items-start gap-2">
                      ...
                      <p className={`text-sm ${isDarkMode ? 'text-gray-300' : 'text-gray-600'}`}>Find and click the letter <span className="font-semibold text-cyan-500">'C'</span> among 'O' distractors</p>
                    </div>
                    <div className="flex items-start gap-2">
                      <div className="w-5 h-5 rounded-full bg-green-500 flex items-center justify-center text-white text-xs font-bold mt-0.5">2</div>
                      <p className={`text-sm ${isDarkMode ? 'text-gray-300' : 'text-gray-600'}`}>Correct: <span className="font-semibold text-green-500">10-30 points</span> based on speed</p>
                    </div>
                    <div className="flex items-start gap-2">
                  ...
                  <div className="space-y-2">
                    <div className="flex items-start gap-2">
                      ...
                      <p className={`text-sm ${isDarkMode ? 'text-gray-300' : 'text-gray-600'}`}>Every 5 streak = <span className="font-semibold text-blue-500">+5 bonus points</span></p>
                    </div>
                    <div className="flex items-start gap-2">
                ...
                <div className={`mt-3 pt-3 border-t text-xs ${isDarkMode ? 'border-gray-700 text-gray-400' : 'border-gray-200 text-gray-500'} flex items-center justify-between`}>
                  <span>🎯 &lt;500ms: 30pts • &lt;1000ms: 20pts • &lt;2000ms: 15pts • 2000ms+: 10pts</span>
