...
import { 
  ...
  Eye, Timer, Hourglass, Brain, X, Trophy, Info
} from 'lucide-react';
...
export default function TemporalPrecisionPage() {
  ...
  const calculatePoints = (errorMs) => {
    let points = 10;
    if (errorMs < 30) points = 40;
    else if (errorMs < 60) points = 30;
    else if (errorMs < 90) points = 20;
  ...
  }, [gameState, successfulHits, mistakes]);

  const fail = useCallback((reason) => {
    if (!isActiveRef.current) return;
    
    const penaltyPoints = calculatePoints(120);
  ...
  const handleInputUp = useCallback(() => {
    ...
    if (stateRef.current === "HOLDING") {
      ...
      if (errorSec < 0.120) {
        ...
        if (bestError === 0 || errorMs < bestError) {
          setBestError(errorMs);
        }
        
        const pointsEarned = calculatePoints(errorMs);
        ...
        if (newStreak % 5 === 0 && newStreak > 0) {
          ...
          playSound('correct');
          showFeedback(`✓ ${errorMs}ms error | +${pointsEarned}`, 'success');
        }
      } else {
        fail("TIMING OFF");
        return;
  ...
  }, [gameState, bestError, bestStreak, fail, initRound]);
  ...
  useEffect(() => {
    ...
    function draw() {
      ...
      if (stateRef.current === "HOLDING") {
        ...
        const currentHold = (performance.now() - startTimeRef.current) / 1000;
        if (currentHold > 0) {
          ctx.fillStyle = isBoxDarkMode ? "#00ff88" : "#10b981";
          ctx.font = "bold 24px monospace";
          ctx.fillText(`${currentHold.toFixed(2)}s`, cx, cy - 50);
        }
      }
        ...
        <div className="mb-6">
          ...
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-3">
              ...
              <div>
                <h1 className={`text-3xl font-bold ${isDarkMode ? 'text-white' : 'text-gray-900'}`}>Temporal Precision</h1>
                <p className={`${isDarkMode ? 'text-gray-400' : 'text-gray-500'}`}>Match target duration • &lt;120ms error</p>
              </div>
            </div>
          ...
          {isFullscreen && gameState === 'playing' && (
            <>
              ...
              <div className="absolute top-4 left-4 z-30 bg-black/50 backdrop-blur-sm rounded-lg px-4 py-2 text-white text-sm">
                Score: <span className="text-yellow-400">{score}</span> | Time: <span className={timeLeft < 15 ? 'text-red-400' : 'text-green-400'}>{timeLeft}s</span> | Target: <span className="text-cyan-400">{targetTime}s</span>
              </div>
            </>
          )}
          ...
          {gameState === 'start' && (
            <div className={`absolute inset-0 flex items-center justify-center backdrop-blur-sm rounded-xl z-40 ${isBoxDarkMode ? 'bg-gray-900/95' : 'bg-white/95'}`}>
              <div className={`rounded-2xl p-8 text-center max-w-md mx-4 shadow-xl border ${isBoxDarkMode ? 'bg-gray-800 border-gray-700' : 'bg-white border-gray-200'}`}>
                <Hourglass className="w-16 h-16 text-blue-500 mx-auto mb-4" />
                <h3 className={`text-2xl font-bold mb-2 ${isBoxDarkMode ? 'text-white' : 'text-gray-900'}`}>Temporal Precision</h3>
                <p className={`mb-6 ${isBoxDarkMode ? 'text-gray-300' : 'text-gray-600'}`}>60-second challenge • Match target duration</p>
                <button 
                  onClick={startGame} 
            ...
            <div className={`absolute inset-0 flex items-center justify-center backdrop-blur-sm rounded-xl z-40 ${isBoxDarkMode ? 'bg-gray-900/95' : 'bg-white/95'}`}>
              <div className={`rounded-2xl p-8 shadow-xl border w-[480px] ${isBoxDarkMode ? 'bg-gray-800 border-gray-700' : 'bg-white border-gray-200'}`}>
                ...
                <div className="grid grid-cols-2 gap-4 mb-8">
                  ...
                  <ResultCard label="Best Error" value={bestError || '-'} unit="ms" icon={<Timer className="w-4 h-4" />} color="text-green-500" />
            ...
            <div className={`rounded-xl border overflow-hidden ${isDarkMode ? 'bg-gray-800 border-gray-700' : 'bg-white border-gray-200'}`}>
              ...
              <div className="p-4">
                  ...
                  <div className="space-y-2">
                    <div className="flex items-start gap-2">
                      ...
                      <p className={`text-sm ${isDarkMode ? 'text-gray-300' : 'text-gray-600'}`}>
                        <span className="font-semibold text-blue-500">Hold and release</span> to match the target time
                      </p>
                    </div>
                    <div className="flex items-start gap-2">
                      <div className="w-5 h-5 rounded-full bg-green-500 flex items-center justify-center text-white text-xs font-bold mt-0.5">2</div>
                      <p className={`text-sm ${isDarkMode ? 'text-gray-300' : 'text-gray-600'}`}>
                        <span className="font-semibold text-green-500">Target: 0.5s - 2.5s</span> • New target each round
                      </p>
                    </div>
                    <div className="flex items-start gap-2">
                      ...
                      <p className={`text-sm ${isDarkMode ? 'text-gray-300' : 'text-gray-600'}`}>
                        <span className="font-semibold text-cyan-500">&lt;30ms error: +40</span> • &lt;60ms: +30 • &lt;90ms: +20 • &lt;120ms: +10
                      </p>
                    </div>
                  </div>
                  <div className="space-y-2">
                    <div className="flex items-start gap-2">
                      ...
                      <p className={`text-sm ${isDarkMode ? 'text-gray-300' : 'text-gray-600'}`}>
                        <span className="font-semibold text-red-500">Penalty = Potential Gain</span> (Wrong loses what correct would earn)
                      </p>
                    </div>
                    <div className="flex items-start gap-2">
                ...
                <div className={`mt-3 pt-3 border-t text-xs ${isDarkMode ? 'border-gray-700 text-gray-400' : 'border-gray-200 text-gray-500'} flex items-center justify-between`}>
                  <span>⏱️ Green ring = Holding • Result shows error in ms</span>
                  <span>⚡ 1 second cooldown between rounds</span>
                </div>
              </div>
            </div>
          </div>
