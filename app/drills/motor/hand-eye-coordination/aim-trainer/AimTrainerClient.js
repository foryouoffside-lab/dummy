...
export default function AimTrainerClient() {
  ...
  useEffect(() => { try { const saved = localStorage.getItem('aimTrainerBest'); if (saved) { const p = parseInt(saved,10); if (!isNaN(p)) setBestScore(p); } } catch(e) {} }, []);
  useEffect(() => { gameStateRef.current = gameState; }, [gameState]);
  useEffect(() => { if (totalClicksRef.current > 0) setAccuracy(Math.round((successfulHitsRef.current/totalClicksRef.current)*100)); else setAccuracy(100); }, [score]);

  const updateBestScore = useCallback((fs) => { try { const c = parseInt(localStorage.getItem('aimTrainerBest')||'0',10); if (fs>c) { localStorage.setItem('aimTrainerBest',fs.toString()); setBestScore(fs); } } catch(e) {} }, []);
  ...
  const playSound = useCallback((type) => { if(!soundEnabled)return; try { const ctx=initAudio(); if(!ctx)return; const o=ctx.createOscillator(),g=ctx.createGain(); o.connect(g);g.connect(ctx.destination); const now=ctx.currentTime; const f={hit:880,miss:440,streak:1046.5,penalty:330}; o.frequency.setValueAtTime(f[type]||660,now); g.gain.setValueAtTime(type==='streak'||type==='penalty'?0.12:0.1,now); g.gain.exponentialRampToValueAtTime(0.001,now+0.15); o.start(now);o.stop(now+0.15); } catch(e){} }, [soundEnabled,initAudio]);
  ...
  const applyPenalty = useCallback((reason) => {
    ...
    else{scoreRef.current=Math.max(0,scoreRef.current-1);setScore(scoreRef.current);playSound('penalty');showFeedback(`${reason}! -1 point`,'error');}
    streakRef.current=0;setStreak(0);
  }, [showFeedback,playSound]);

  const spawnTarget = useCallback((cvs) => {
    if(!cvs||!isActiveRef.current)return;
    const pad=100, tr=Math.max(8,20-(streakRef.current*0.15));
    setTargetSize(Math.round(tr));
    targetsRef.current.push({x:pad+Math.random()*(cvs.width-pad*2),y:pad+Math.random()*(cvs.height-pad*2),r:tr,life:1.0/(1+streakRef.current*0.04),born:performance.now()});
  }, []);
  ...
  useEffect(() => {
    const h = (e) => {
      ...
      let hit=false, hi=-1, hr=0;
      for(let i=targetsRef.current.length-1;i>=0;i--){const t=targetsRef.current[i];if(Math.hypot(ch.x-t.x,ch.y-t.y)<t.r+8){hit=true;hi=i;hr=Math.floor(performance.now()-t.born);break;}}
      if(hit&&hi!==-1){totalAttemptsRef.current++;hitsRef.current++;successfulHitsRef.current++;setTargetsHit(successfulHitsRef.current);reactionTimesRef.current.push(hr);setBestReaction(prev=>prev===0||hr<prev?hr:prev);targetsRef.current.splice(hi,1);const ns=streakRef.current+1;streakRef.current=ns;setStreak(ns);if(ns>bestStreakRef.current){bestStreakRef.current=ns;setBestStreak(ns);}scoreRef.current+=1;setScore(scoreRef.current);if(ns%10===0){playSound('streak');showFeedback(`🔥 ${ns} Streak! +1`,'success');}else{playSound('hit');showFeedback(`✓ Hit! +1`,'success');}}
      else applyPenalty('Missed Click');
    };
    document.addEventListener('mousedown',h);
    return () => document.removeEventListener('mousedown',h);
  }, [gameState,applyPenalty,playSound,showFeedback]);
  ...
  useEffect(() => {
    ...
    const loop=(now)=>{const dt=Math.min(0.033,(now-lt)/1000);lt=now;
      if(isActiveRef.current){spawnTimerRef.current+=dt;const iv=Math.max(0.18,0.55-(streakRef.current*0.008));if(spawnTimerRef.current>iv&&targetsRef.current.length<12){spawnTarget(cvs);spawnTimerRef.current=0;}for(let i=targetsRef.current.length-1;i>=0;i--){if((now-targetsRef.current[i].born)/1000>targetsRef.current[i].life)targetsRef.current.splice(i,1);}}
      ...
      const isHS=streakRef.current>15;
      targetsRef.current.forEach(t=>{ctx.beginPath();ctx.arc(t.x,t.y,t.r,0,Math.PI*2);ctx.strokeStyle=isHS?"#00FFFF":streakRef.current>8?"#00ff88":"#FFFFFF";ctx.lineWidth=2.5;ctx.stroke();if(isHS){ctx.beginPath();ctx.arc(t.x,t.y,t.r*0.25,0,Math.PI*2);ctx.fillStyle="#00FFFF";ctx.fill();}const age=(now-t.born)/1000,prog=Math.min(1,age/t.life);ctx.beginPath();ctx.arc(t.x,t.y,t.r+4,-Math.PI/2,(-Math.PI/2)+(Math.PI*2*(1-prog)));ctx.strokeStyle="rgba(0,255,136,0.4)";ctx.lineWidth=2;ctx.stroke();});
  ...
  const startGame = useCallback(() => {
    ...
    setScore(0);setStreak(0);setBestStreak(0);setBestReaction(0);setTargetSize(20);setTimeLeft(60);setLives(3);setFeedback('');setAccuracy(100);setTargetsHit(0);
    isActiveRef.current=true;scoreRef.current=0;streakRef.current=0;bestStreakRef.current=0;livesRef.current=3;targetsRef.current=[];spawnTimerRef.current=0;totalAttemptsRef.current=0;hitsRef.current=0;reactionTimesRef.current=[];totalClicksRef.current=0;successfulHitsRef.current=0;
  ...
  const sharePage = async () => { if(navigator.share){try{await navigator.share({title:'Aim Trainer Elite | SkillDrills',text:'Improve mouse aim with shrinking targets and 3-life system.',url:'https://skilldrills.online/drills/motor/hand-eye-coordination/aim-trainer'});}catch(e){}}else{navigator.clipboard.writeText('https://skilldrills.online/drills/motor/hand-eye-coordination/aim-trainer');} };
  const copyPageLink = () => { navigator.clipboard.writeText('https://skilldrills.online/drills/motor/hand-eye-coordination/aim-trainer'); };

  useEffect(()=>()=>{if(timerIntervalRef.current)clearInterval(timerIntervalRef.current);if(animationRef.current)cancelAnimationFrame(animationRef.current);document.exitPointerLock();},[]);
...
function RC({label,v,unit='',i,c,d}){const m={yellow:'bg-yellow-500/10 border-yellow-500/30 text-yellow-500',purple:'bg-purple-500/10 border-purple-500/30 text-purple-500',green:'bg-green-500/10 border-green-500/30 text-green-500',orange:'bg-orange-500/10 border-orange-500/30 text-orange-500',cyan:'bg-cyan-500/10 border-cyan-500/30 text-cyan-500'};const o=m[c]||m.yellow;const[bg,border,text]=o.split(' ');return(<div className={`flex items-center justify-between p-3 rounded-lg border ${bg} ${border}`}><div className="flex items-center gap-2"><div className={text}>{i}</div><span className={`text-xs sm:text-sm ${d?'text-gray-300':'text-gray-600'}`}>{label}</span></div><span className={`font-bold text-base sm:text-lg ${text}`}>{v}{unit}</span></div>);}