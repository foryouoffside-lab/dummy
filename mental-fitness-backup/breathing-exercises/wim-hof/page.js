...
export default function WHMPowerPage() {
  ...
  const enterRetention = () => {
    ...
    scoreRef.current += 30;
    setScore(scoreRef.current);
    showFeedback(`ðŸŽ¯ Complete! +30 Bonus`, 'success');
            ...
            {gameState === 'start' && (
              <div className="absolute inset-0 flex items-center justify-center rounded-xl z-10 backdrop-blur-sm" style={{ background: isBoxDarkMode ? 'rgba(3,3,3,0.95)' : 'rgba(255,255,255,0.95)' }}>
                <div className={`rounded-2xl p-8 text-center max-w-md mx-4 shadow-xl border ${isBoxDarkMode ? 'bg-gray-800 border-gray-700' : 'bg-white border-gray-200'}`}>
                  ...
                  <p className={`mb-6 ${isBoxDarkMode ? 'text-gray-300' : 'text-gray-600'}`}>30 power breaths + retention â€¢ Wim Hof Method</p>
                  <button
                    onClick={startDrill}
            ...
            <div className={`rounded-xl border overflow-hidden ${isDarkMode ? 'bg-gray-800 border-gray-700' : 'bg-white border-gray-200'}`}>
              ...
              <div className="p-4">
                ...
                <div className={`mt-3 pt-3 border-t text-xs ${isDarkMode ? 'border-gray-700 text-gray-400' : 'border-gray-200 text-gray-500'} flex items-center justify-between`}>
                  <span>ðŸ”¥ Increases oxygen saturation and energy</span>
                  <span>ðŸ§˜ Best practiced on an empty stomach</span>
                </div>
              </div>
            </div>
          </div>
