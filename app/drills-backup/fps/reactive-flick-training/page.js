...
export default function ReactiveFlickPage() {
  ...
  const [currentInterval, setCurrentInterval] = useState(700);
  ...
  const warpIntervalRef = useRef(700);
  ...
  const START_INTERVAL = 700;
  const MIN_INTERVAL = 350;
  const MAX_POINTS = 20;

  // Load best score from localStorage
  useEffect(() => {
    const savedBestScore = localStorage.getItem('reactiveFlickBestScore');
    if (savedBestScore) {
  ...
  const updateBestScore = (finalScore) => {
    const currentBestScore = parseInt(localStorage.getItem('reactiveFlickBestScore') || '0', 10);
    if (finalScore > currentBestScore) {
      localStorage.setItem('reactiveFlickBestScore', finalScore.toString());
  ...
  const calculateScoreFromSpeed = () => {
    const speedBonus = Math.floor((START_INTERVAL - warpIntervalRef.current) / 20);
  ...
  function teleport(cvs) {
    const margin = 80;
    const oldX = targetRef.current.x;
    const oldY = targetRef.current.y;
    
    let newX, newY;
    let attempts = 0;
    do {
      newX = margin + Math.random() * (cvs.width - margin * 2);
      newY = margin + Math.random() * (cvs.height - margin * 2);
      attempts++;
      if (attempts > 30) break;
    } while (Math.hypot(newX - oldX, newY - oldY) < 120);
  ...
  useEffect(() => {
    ...
    const handleClick = (e) => {
      ...
      const dist = Math.hypot(mouseX - targetRef.current.x, mouseY - targetRef.current.y);
      ...
      if (dist < TARGET_RADIUS + 10) {
        ...
        const total = hitCount + 1 + missCount;
        setAccuracy(Math.round(((hitCount + 1) / total) * 100));
      } else {
        handlePenalty('Miss');
      }
    };
    ...
    function drawTarget(now) {
      ...
      const currentR = TARGET_RADIUS + (hitFlashRef.current > 0 ? pulse : 0);
      ...
      ctx.beginPath();
      ctx.arc(targetRef.current.x, targetRef.current.y, currentR, 0, Math.PI * 2);
      ctx.fill();
      ctx.shadowBlur = 0;
    ...
    function render(now) {
      ...
      warpIntervalRef.current = START_INTERVAL - progress * (START_INTERVAL - MIN_INTERVAL);
      setCurrentInterval(Math.round(warpIntervalRef.current));
      
      if (now - lastWarpRef.current > warpIntervalRef.current) {
        teleport(cvs);
        lastWarpRef.current = now;
    ...
    <div className={`min-h-screen select-none ${isDarkMode ? 'bg-gray-900' : 'bg-gray-50'}`}>
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        <div className="mb-6">
          ...
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-3">
              ...
              <div>
                <h1 className={`text-3xl font-bold ${isDarkMode ? 'text-white' : 'text-gray-900'}`}>Reactive Flick</h1>
                <p className={`${isDarkMode ? 'text-gray-400' : 'text-gray-500'}`}>Click teleporting target â€¢ Speed increases</p>
              </div>
            </div>
        ...
        <div className="grid grid-cols-6 gap-3 mb-4 h-[88px]">
          <StatCard icon={<Target className="text-blue-600" />} value={flickScore} label="Flick Score" isDark={isDarkMode} />
          ...
          <StatCard icon={<Zap className="text-orange-500" />} value={flickCombo} label="Combo" isDark={isDarkMode} />
          <StatCard icon={<Check className="text-green-500" />} value={hitCount} label="Hits" isDark={isDarkMode} />
          <StatCard icon={<Activity className="text-purple-500" />} value={currentInterval} label="Interval" unit="ms" isDark={isDarkMode} />
        </div>
          ...
          {isFullscreen && gameState === 'playing' && (
            <>
              ...
              <div className="absolute top-4 left-4 z-20 bg-black/50 backdrop-blur-sm rounded-lg px-4 py-2 text-white text-sm">
                Score: <span className="text-yellow-400">{flickScore}</span> | Combo: <span className="text-purple-400">{flickCombo}x</span> | Interval: <span className="text-cyan-400">{currentInterval}ms</span>
              </div>
            </>
          )}
          ...
          {gameState === 'start' && (
            <div className={`absolute inset-0 flex items-center justify-center backdrop-blur-sm rounded-xl z-40 ${isBoxDarkMode ? 'bg-gray-900/95' : 'bg-white/95'}`}>
              <div className={`rounded-2xl p-8 text-center max-w-md mx-4 shadow-xl border ${isBoxDarkMode ? 'bg-gray-800 border-gray-700' : 'bg-white border-gray-200'}`}>
                ...
                <h3 className={`text-2xl font-bold mb-2 ${isBoxDarkMode ? 'text-white' : 'text-gray-900'}`}>Reactive Flick</h3>
                <p className={`mb-6 ${isBoxDarkMode ? 'text-gray-300' : 'text-gray-600'}`}>60-second challenge â€¢ Speed increases over time</p>
                <button 
                  onClick={startGame} 
            ...
            <div className={`absolute inset-0 flex items-center justify-center backdrop-blur-sm rounded-xl z-40 ${isBoxDarkMode ? 'bg-gray-900/95' : 'bg-white/95'}`}>
              <div className={`rounded-2xl p-8 shadow-xl border w-[480px] ${isBoxDarkMode ? 'bg-gray-800 border-gray-700' : 'bg-white border-gray-200'}`}>
                ...
                <div className="grid grid-cols-2 gap-4 mb-8">
                  <ResultCard label="Flick Score" value={flickScore} icon={<Target className="w-4 h-4" />} color="text-blue-500" />
                  <ResultCard label="Best Score" value={bestScore} icon={<Trophy className="w-4 h-4" />} color="text-yellow-500" />
                  <ResultCard label="Hits" value={hitCount} icon={<Check className="w-4 h-4" />} color="text-green-500" />
                  ...
                  <ResultCard label="Final Interval" value={currentInterval} unit="ms" icon={<Timer className="w-4 h-4" />} color="text-cyan-500" />
                </div>
            ...
            <div className={`rounded-xl border overflow-hidden ${isDarkMode ? 'bg-gray-800 border-gray-700' : 'bg-white border-gray-200'}`}>
              <div className={`px-4 py-3 border-b ${isDarkMode ? 'border-gray-700 bg-gray-800/50' : 'border-gray-200 bg-gray-50'}`}>
                <div className="flex items-center gap-2">
                  ...
                  <h3 className={`font-semibold ${isDarkMode ? 'text-white' : 'text-gray-900'}`}>Reactive Flick Rules</h3>
                </div>
              </div>
              <div className="p-4">
                  ...
                  <div className="space-y-2">
                    <div className="flex items-start gap-2">
                      ...
                      <p className={`text-sm ${isDarkMode ? 'text-gray-300' : 'text-gray-600'}`}>
                        <span className="font-semibold text-green-500">Click the green target</span> â€¢ Teleports on hit/timeout
                      </p>
                    </div>
                    <div className="flex items-start gap-2">
                      ...
                      <p className={`text-sm ${isDarkMode ? 'text-gray-300' : 'text-gray-600'}`}>
                        <span className="font-semibold text-blue-500">Speed increases over time</span> â€¢ 700ms â†’ 350ms
                      </p>
                    </div>
                    <div className="flex items-start gap-2">
                      ...
                      <p className={`text-sm ${isDarkMode ? 'text-gray-300' : 'text-gray-600'}`}>
                        <span className="font-semibold text-purple-500">Combo bonus: +5 per 3 hits</span> â€¢ Speed bonus
                      </p>
                    </div>
                  </div>
                  <div className="space-y-2">
                    <div className="flex items-start gap-2">
                      ...
                      <p className={`text-sm ${isDarkMode ? 'text-gray-300' : 'text-gray-600'}`}>
                        <span className="font-semibold text-red-500">Miss/Timeout = -20 points</span> â€¢ Penalty equals max
                      </p>
                    </div>
                    <div className="flex items-start gap-2">
                      ...
                      <p className={`text-sm ${isDarkMode ? 'text-gray-300' : 'text-gray-600'}`}>
                        <span className="font-semibold text-orange-500">Symmetric scoring</span> â€¢ Penalty matches potential gain
                      </p>
                    </div>
                    <div className="flex items-start gap-2">
