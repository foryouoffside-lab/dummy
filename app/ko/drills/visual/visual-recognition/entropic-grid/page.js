...
const guideData = {
  ...
  intro: [
    ...
    "제레미 울프의 가이드 탐색 모델(Guided Search 4.0, Wolfe, 2007)은 시각 자극 자체의 두드러짐(상향식 현저성)과 관찰자의 목표 지향적 의도(하향식 주의 편향)가 결합하여 뇌 속에서 '우선순위 맵(Priority Map)'을 형성한다고 설명합니다. 본 엔트로픽 그리드 과제처럼 100개의 셀 전반에서 700ms 주기로 동적 잡음이 쏟아지는 고엔트로피 환경에서는 방해 문자의 점멸이 상향식 주의를 극도로 교란합니다. 이를 극복하려면 전전두엽 피질과 후두정엽 피질이 협력하여 강력한 하향식 억제 신호를 보내고, 목표물 이외의 시각 잡음을 뇌 내부에서 능동적으로 소거해야 합니다(Duncan & Humphreys, 1989).",
    "시각 탐색의 수행 속도를 결정짓는 물리적 한계는 안구의 주시 시간(Fixation Duration: 약 200~250ms)과 안구 단속운동(Saccade) 잠복기입니다(Posner, 1980; Woods et al., 2015). 격자판을 한 칸씩 차례대로 훑는 비효율적인 직렬 탐색을 수행할 경우 제한 시간 내에 단 몇 개의 목표만 찾아내는 데 그칩니다. 반면 고도로 훈련된 액션 게임 플레이어나 숙련된 스포츠 선수는 시각적 주의의 스포트라이트를 광역으로 확장하여, 한 번의 주시 동안 4~9개 셀의 복합 정보를 병렬적으로 샘플링하고 판별하는 탁월한 시지각 스팬을 발휘합니다(Green & Bavelier, 2006).",
    "본 엔트로픽 그리드 훈련 도구는 정밀 웹 타이머를 통해 목표물 발견 시의 반응 잠복기, 누적 식별 성공 횟수, 오클릭 페널티를 종합적으로 측정하여 실시간 인지 스코어를 도출합니다. 지속적인 동적 그리드 탐색 훈련은 극심한 시각적 클러터 환경에서도 망막의 유효 시야를 극대화하며, FPS 게임에서의 찰나의 색적, 복잡한 도로 주행 시의 돌발 위험 감지, 방대한 데이터를 정밀 검토하는 전문 직무에서의 정보 처리 속도를 획기적으로 향상시킵니다."
  ],
  benchmarks: {
