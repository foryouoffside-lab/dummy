...
import { 
  ...
  Eye, Timer, Crosshair, Brain, X, Trophy, Info, RefreshCw
} from 'lucide-react';
  ...
  useEffect(() => {
    ...
    function update() {
      ...
      if (dist < target.r) {
        ...
        // Score every 0.5 seconds (30 frames at 60fps)
        if (trackingTimeRef.current >= lastScoreTimeRef.current + 30) {
          lastScoreTimeRef.current = trackingTimeRef.current;
          const pointsEarned = 1;
          scoreRef.current += pointsEarned;
          setScore(scoreRef.current);
          showFeedback(`+${pointsEarned} point!`, 'success');
          playSound('score');
        }
      } else {
      ...
      // Check for record
      if (trackingTimeRef.current > bestTimeRef.current && trackingTimeRef.current > 30) {
      ...
      // Streak bonus notification every 2.5 seconds (150 frames)
      if (trackingTimeRef.current > 0 && trackingTimeRef.current % 150 === 0 && trackingTimeRef.current > 0) {
        playSound('streak');
        showFeedback(`🔥 ${(trackingTimeRef.current / 60).toFixed(1)}s Streak!`, 'success');
      }
    }
    ...
    function draw() {
      ...
      if (m.x > 0 && m.x < cvs.width && m.y > 0 && m.y < cvs.height) {
        const isOverTarget = Math.hypot(m.x - target.x, m.y - target.y) < target.r;
        ctx.strokeStyle = isOverTarget ? "#00ff88" : "#888888";
        ctx.lineWidth = 2;
        ctx.beginPath();
        ...
        ctx.strokeStyle = isOverTarget ? 'rgba(0, 255, 136, 0.3)' : 'rgba(136, 136, 136, 0.3)';
        ctx.stroke();
        ctx.fillStyle = isOverTarget ? '#00ff88' : '#888888';
          ...
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <p className={`${isDarkMode ? 'text-gray-400' : 'text-gray-500'}`}>Smooth pursuit • 0.5 sec tracking = +1 pt</p>
              </div>
            </div>
          ...
          {isFullscreen && gameState === 'playing' && (
            <>
              ...
              <div className="absolute top-4 left-4 z-20 bg-black/50 backdrop-blur-sm rounded-lg px-4 py-2 text-white text-sm">
                Score: <span className="text-yellow-400">{score}</span> | Tracking: <span className="text-green-400">{formatTrackingTime(trackingTime)}s</span>
              </div>
            </>
          )}
            ...
            <div className={`absolute inset-0 flex items-center justify-center backdrop-blur-sm rounded-xl z-40 ${isBoxDarkMode ? 'bg-gray-900/95' : 'bg-white/95'}`}>
              <div className={`rounded-2xl p-8 shadow-xl border w-[480px] ${isBoxDarkMode ? 'bg-gray-800 border-gray-700' : 'bg-white border-gray-200'}`}>
                ...
                <div className="grid grid-cols-2 gap-4 mb-8">
                  ...
                  <ResultCard label="Total Score" value={score} icon={<Award className="w-4 h-4" />} color="text-purple-500" />
                </div>
            ...
            <div className={`rounded-xl border overflow-hidden ${isDarkMode ? 'bg-gray-800 border-gray-700' : 'bg-white border-gray-200'}`}>
              ...
              <div className="p-4">
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  <div className="space-y-2">
                    <div className="flex items-start gap-2">
                      ...
                      <p className={`text-sm ${isDarkMode ? 'text-gray-300' : 'text-gray-600'}`}>
                        <span className="font-semibold text-green-500">Keep cursor on moving target</span> • Turns green when tracking
                      </p>
                    </div>
                    <div className="flex items-start gap-2">
                      ...
                      <p className={`text-sm ${isDarkMode ? 'text-gray-300' : 'text-gray-600'}`}>
                        <span className="font-semibold text-blue-500">0.5 second of tracking = +1 point</span> • Build your score
                      </p>
                    </div>
                    <div className="flex items-start gap-2">
                  ...
                  <div className="space-y-2">
                    <div className="flex items-start gap-2">
                      ...
                      <p className={`text-sm ${isDarkMode ? 'text-gray-300' : 'text-gray-600'}`}>
                        <span className="font-semibold text-purple-500">Beat your record for bonus</span> • 2.5 sec streak notification
                      </p>
                    </div>
                    <div className="flex items-start gap-2">
                      ...
                      <p className={`text-sm ${isDarkMode ? 'text-gray-300' : 'text-gray-600'}`}>
                        <span className="font-semibold text-orange-500">No lives/penalty system</span> • Just pure tracking
                      </p>
                    </div>
                    <div className="flex items-start gap-2">
                ...
                <div className={`mt-3 pt-3 border-t text-xs ${isDarkMode ? 'border-gray-700 text-gray-400' : 'border-gray-200 text-gray-500'} flex items-center justify-between`}>
                  ...
                  <span>⚡ +1 point every 0.5 seconds of tracking</span>
                </div>
              </div>
            </div>
          </div>
