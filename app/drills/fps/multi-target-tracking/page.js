...
export default function GhostLinkPage() {
  ...
  const handleBallClick = (index) => {
    ...
    if (selectedBalls.includes(index)) {
      ...
      // Only allow selecting up to 3 balls
  ...
  useEffect(() => {
    ...
    function drawIdentificationUI() {
      ...
      if (showResults) {
        ctx.fillText(`You tracked ${correctCount}/3 correctly! +${score} points!`, cvs.width / 2, 20);
      } else {
        ctx.fillText(`Click the 3 balls you tracked! (${selectedBalls.length}/3 selected)`, cvs.width / 2, 20);
      }
      
      // Draw confirm button
      if (!showResults && selectedBalls.length === 3) {
  ...
  useEffect(() => {
    const handleConfirmClick = (e) => {
      if (!identificationPhase || showResults) return;
      if (selectedBalls.length !== 3) return;
          ...
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <p className={`${isDarkMode ? 'text-gray-400' : 'text-gray-500'}`}>Memorize 3 targets • Track for 60s • +5 per correct ball</p>
              </div>
            </div>
          ...
          {gameState === 'start' && (
            <div className={`absolute inset-0 flex items-center justify-center backdrop-blur-sm rounded-xl z-40 ${isBoxDarkMode ? 'bg-gray-900/95' : 'bg-white/95'}`}>
              <div className={`rounded-2xl p-8 text-center max-w-md mx-4 shadow-xl border ${isBoxDarkMode ? 'bg-gray-800 border-gray-700' : 'bg-white border-gray-200'}`}>
                ...
                <p className={`mb-6 ${isBoxDarkMode ? 'text-gray-300' : 'text-gray-600'}`}>60s • Memorize 3 targets • Track with cursor • +5 per correct ball</p>
                <button 
                  onClick={startGame} 
            ...
            <div className={`absolute inset-0 flex items-center justify-center backdrop-blur-sm rounded-xl z-40 ${isBoxDarkMode ? 'bg-gray-900/95' : 'bg-white/95'}`}>
              <div className={`rounded-2xl p-8 shadow-xl border w-[400px] ${isBoxDarkMode ? 'bg-gray-800 border-gray-700' : 'bg-white border-gray-200'}`}>
                ...
                <div className="text-center mb-8">
                  ...
                  <p className={`text-5xl font-bold mb-2 ${correctCount === 3 ? 'text-green-500' : correctCount === 2 ? 'text-yellow-500' : 'text-red-500'}`}>
                    {correctCount}/3
                  </p>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  <div className="space-y-2">
                    ...
                    <div className="flex items-start gap-2">
                      ...
                      <p className={`text-sm ${isDarkMode ? 'text-gray-300' : 'text-gray-600'}`}>
                        <span className="font-semibold text-green-500">Identify phase after 60s</span> • Click the 3 balls you tracked
                      </p>
                    </div>
                  </div>
                  <div className="space-y-2">
                    <div className="flex items-start gap-2">
                      ...
                      <p className={`text-sm ${isDarkMode ? 'text-gray-300' : 'text-gray-600'}`}>
                        <span className="font-semibold text-orange-500">+5 points per correct ball</span> • Maximum 15 points
                      </p>
                    </div>
                    <div className="flex items-start gap-2">
                      ...
                      <p className={`text-sm ${isDarkMode ? 'text-gray-300' : 'text-gray-600'}`}>
                        <span className="font-semibold text-yellow-500">Select exactly 3 balls</span> • Confirm to see results
                      </p>
                    </div>
                    <div className="flex items-start gap-2">
                      ...
                      <p className={`text-sm ${isDarkMode ? 'text-gray-300' : 'text-gray-600'}`}>
                        <span className="font-semibold text-cyan-500">Best Score saves locally</span> • Try to get 15 points!
                      </p>
                    </div>
                  </div>
                </div>
