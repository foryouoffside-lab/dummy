...
export default function MonochromeAgilityLadderPage() {
  ...
  const applyPenalty = () => {
    ...
    playSound('fail');
    showFeedback(`âœ— Missed rung! -${penaltyAmount}`, 'error');
      ...
      for (let i = 0; i < ladders.current.length; i++) {
        ...
        if (result) {
          ...
          if (result.type === 'step') {
            ...
            playSound('step');
            showFeedback(`âœ“ Rung! +${result.points}`, 'success');
            ...
            playSound('complete');
            showFeedback(`ðŸŽ‰ Ladder Complete! +${result.points}`, 'success');
          }
          break;
        }
          ...
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <p className={`${isDarkMode ? 'text-gray-400' : 'text-gray-500'}`}>Step on rungs in sequence: Left â†’ Right â†’ Left â†’ Right</p>
              </div>
            </div>
          ...
          {gameState === 'start' && (
            <div className={`absolute inset-0 flex items-center justify-center backdrop-blur-sm rounded-xl z-40 ${isBoxDarkMode ? 'bg-gray-900/95' : 'bg-white/95'}`}>
              <div className={`rounded-2xl p-8 text-center max-w-md mx-4 shadow-xl border ${isBoxDarkMode ? 'bg-gray-800 border-gray-700' : 'bg-white border-gray-200'}`}>
                ...
                <p className={`mb-6 ${isBoxDarkMode ? 'text-gray-300' : 'text-gray-600'}`}>Step on rungs in sequence: Left â†’ Right â†’ Left â†’ Right</p>
                <button 
                  onClick={startGame} 
            ...
            <div className={`rounded-xl border overflow-hidden ${isDarkMode ? 'bg-gray-800 border-gray-700' : 'bg-white border-gray-200'}`}>
              ...
              <div className="p-4">
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  <div className="space-y-2">
                    <div className="flex items-start gap-2">
                      ...
                      <p className={`text-sm ${isDarkMode ? 'text-gray-300' : 'text-gray-600'}`}>Step on rungs in sequence: <span className="font-semibold">Left â†’ Right â†’ Left â†’ Right</span></p>
                    </div>
                    <div className="flex items-start gap-2">
                ...
                <div className={`mt-3 pt-3 border-t text-xs ${isDarkMode ? 'border-gray-700 text-gray-400' : 'border-gray-200 text-gray-500'} flex items-center justify-between`}>
                  <span>â¬…ï¸ Left rung â†’ âž¡ï¸ Right rung â†’ â¬…ï¸ Left rung â†’ âž¡ï¸ Right rung</span>
                  <span>âš¡ Hover over the highlighted rung to step</span>
                </div>
              </div>
            </div>
          </div>
