...
import { 
  ...
  Sun, Moon, Eye, Timer, Trophy, GitBranch, Activity, Zap, Award, Info, Brain
} from 'lucide-react';
...
export default function NeuralTrackerPage() {
  ...
  useEffect(() => {
    const savedBestScore = localStorage.getItem('neuralTrackerBestScore');
    if (savedBestScore) {
  ...
  const updateBestScore = (finalScore) => {
    const currentBestScore = parseInt(localStorage.getItem('neuralTrackerBestScore') || '0', 10);
    if (finalScore > currentBestScore) {
      localStorage.setItem('neuralTrackerBestScore', finalScore.toString());
  ...
  const gameLoop = useCallback(() => {
    ...
    if (target) {
      ...
      if (distance < ballRadius) {
        const pointsEarned = 5 + Math.floor(comboRef.current / 3) * 2;
  ...
  useEffect(() => {
    ...
    const teleportInterval = setInterval(() => {
      if (gameState === 'playing' && ballsRef.current.length >= 2) {
        const count = ballsRef.current.length;
        const a = Math.floor(Math.random() * count);
        let b = Math.floor(Math.random() * count);
        while (b === a) b = Math.floor(Math.random() * count);
        
        const tempX = ballsRef.current[a].x;
        const tempY = ballsRef.current[a].y;
        ballsRef.current[a].x = ballsRef.current[b].x;
        ballsRef.current[a].y = ballsRef.current[b].y;
        ballsRef.current[b].x = tempX;
        ballsRef.current[b].y = tempY;
        
        if (ballsRef.current[a].isTarget) {
          targetIndexRef.current = a;
        } else if (ballsRef.current[b].isTarget) {
          targetIndexRef.current = b;
        }
        ...
        <div className="mb-6">
          ...
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-3">
              ...
              <div>
                <h1 className={`text-3xl font-bold ${isDarkMode ? 'text-white' : 'text-gray-900'}`}>Neural Tracker</h1>
                <p className={`${isDarkMode ? 'text-gray-400' : 'text-gray-500'}`}>Track green target â€¢ Balls swap every 2s</p>
              </div>
            </div>
          ...
          {gameState === 'start' && (
            <div className={`absolute inset-0 flex items-center justify-center backdrop-blur-sm rounded-xl z-40 ${isBoxDarkMode ? 'bg-gray-900/95' : 'bg-white/95'}`}>
              <div className={`rounded-2xl p-8 text-center max-w-md mx-4 shadow-xl border ${isBoxDarkMode ? 'bg-gray-800 border-gray-700' : 'bg-white border-gray-200'}`}>
                ...
                <h3 className={`text-2xl font-bold mb-2 ${isBoxDarkMode ? 'text-white' : 'text-gray-900'}`}>Neural Tracker</h3>
                <p className={`mb-6 ${isBoxDarkMode ? 'text-gray-300' : 'text-gray-600'}`}>60-second challenge â€¢ Track the green target</p>
                <button 
                  onClick={startGame} 
            ...
            <div className={`rounded-xl border overflow-hidden ${isDarkMode ? 'bg-gray-800 border-gray-700' : 'bg-white border-gray-200'}`}>
              <div className={`px-4 py-3 border-b ${isDarkMode ? 'border-gray-700 bg-gray-800/50' : 'border-gray-200 bg-gray-50'}`}>
                <div className="flex items-center gap-2">
                  ...
                  <h3 className={`font-semibold ${isDarkMode ? 'text-white' : 'text-gray-900'}`}>Neural Tracking Rules</h3>
                </div>
              </div>
              <div className="p-4">
                  ...
                  <div className="space-y-2">
                    <div className="flex items-start gap-2">
                      ...
                      <p className={`text-sm ${isDarkMode ? 'text-gray-300' : 'text-gray-600'}`}>
                        <span className="font-semibold text-green-500">Track the GREEN target ball</span> â€¢ Ignore gray decoys
                      </p>
                    </div>
                    <div className="flex items-start gap-2">
                      ...
                      <p className={`text-sm ${isDarkMode ? 'text-gray-300' : 'text-gray-600'}`}>
                        <span className="font-semibold text-blue-500">Balls swap positions every 2 seconds</span> â€¢ Stay alert
                      </p>
                    </div>
                    <div className="flex items-start gap-2">
                      ...
                      <p className={`text-sm ${isDarkMode ? 'text-gray-300' : 'text-gray-600'}`}>
                        <span className="font-semibold text-cyan-500">Cursor on target = +5 + combo bonus</span>
                      </p>
                    </div>
                  </div>
                  <div className="space-y-2">
                    <div className="flex items-start gap-2">
                      ...
                      <p className={`text-sm ${isDarkMode ? 'text-gray-300' : 'text-gray-600'}`}>
                        <span className="font-semibold text-purple-500">80%+ accuracy = +2pts â€¢ 55%+ = +1pt</span>
                      </p>
                    </div>
                    <div className="flex items-start gap-2">
                      <div className="w-5 h-5 rounded-full bg-red-500 flex items-center justify-center text-white text-xs font-bold mt-0.5">5</div>
                      <p className={`text-sm ${isDarkMode ? 'text-gray-300' : 'text-gray-600'}`}>
                        <span className="font-semibold text-red-500">&lt;25% accuracy = -2pts</span> â€¢ Resets combo
                      </p>
                    </div>
                    <div className="flex items-start gap-2">
