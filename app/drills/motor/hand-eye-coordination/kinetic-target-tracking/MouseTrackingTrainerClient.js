...
import { 
  ...
  Share2, CheckCircle2, Zap, Users, Sparkles
} from 'lucide-react';
...
export default function MouseTrackingGameClient() {
  // === UI & Viewport State ===
  ...
  const [currentCombo, setCurrentCombo] = useState(1.0);
  ...
  const engine = useRef({
    ...
    target: { x: 250, y: 250, vx: 200, vy: -150, radius: 26 },
    gravity: { x: 0, y: 0 },
    gravityTimer: 0,
  ...
  useEffect(() => {
    try {
      const savedSens = localStorage.getItem('mouseTrackingGame_sens');
      if (savedSens) setUniversalSens(parseFloat(savedSens));
      const savedBest = localStorage.getItem('mouseTrackingGame_bestScore');
  ...
  useEffect(() => {
    if (gameState !== 'playing') {
      try { localStorage.setItem('mouseTrackingGame_sens', universalSens.toString()); } catch (e) {}
    }
  ...
  const endGame = useCallback(() => {
    ...
    const finalAcc = e.totalFrames > 0 ? Math.round((e.contactFrames / e.totalFrames) * 100) : 0;
    const maxStreakSec = parseFloat((e.maxStreak / 60).toFixed(1)); 
    const timeAlive = parseFloat(((performance.now() - e.gameStartTime) / 1000).toFixed(1));

    let rank = 'Bronze';
    let rankColor = 'text-slate-500';
    
    if (e.score >= 3500 && finalAcc >= 80) {
      rank = 'Master';
      rankColor = 'text-fuchsia-400';
    } else if (e.score >= 2000 && finalAcc >= 70) {
      rank = 'Diamond';
      rankColor = 'text-cyan-400';
    } else if (e.score >= 1000 && finalAcc >= 60) {
      rank = 'Platinum';
      rankColor = 'text-indigo-400';
    } else if (e.score >= 500 && finalAcc >= 50) {
      rank = 'Gold';
    ...
    let advice = 'Excellent hand eye coordination! Keep pushing your multiplier by locking onto the target instantly after gravity shifts.';
    if (finalAcc < 40) {
      advice = 'Your tracking accuracy is low. Focus on smooth pursuit and mouse control rather than flicking. Keep the crosshair inside the target to build your combo.';
    } else if (maxStreakSec < 1.0) {
      advice = 'You are losing your combo multiplier during directional bounces. Anticipate the walls to maintain your tracking aim.';
    }

    setAnalytics({
      accuracy: finalAcc,
      maxStreak: maxStreakSec,
      timeAlive: timeAlive,
    ...
    setScore(Math.round(e.score));

    setBestScore(prev => {
      const roundedScore = Math.round(e.score);
      if (roundedScore > prev) {
        setIsNewBest(true);
        try { localStorage.setItem('mouseTrackingGame_bestScore', roundedScore.toString()); } catch(err){}
        return roundedScore;
      }
      return prev;
    });
  ...
  const startGame = useCallback(() => {
    ...
    engine.current = {
      crosshair: { x: cvs ? cvs.width / 2 : 250, y: cvs ? cvs.height / 2 : 250, initialized: true },
      target: { x: cvs ? cvs.width / 2 : 250, y: cvs ? cvs.height / 2 : 250, vx: 220, vy: -180, radius: 26 },
      gravity: { x: 0, y: 0 },
      gravityTimer: 0,
    ...
    // Synchronous execution fixes the pointer lock pause bug
  ...
  // === Mouse Input Handler (Pointer Lock) ===
  useEffect(() => {
  ...
  useEffect(() => {
    ...
    const handleMouseDown = (e) => {
      ...
      if (gameState === 'playing' && !pointerLocked && canvasRef.current) {
  ...
  useEffect(() => {
    ...
    const loop = (time) => {
      ...
      if (gameState === 'playing' && pointerLocked) {
        ...
        let newLevel = 1;
        if (e.score >= 2500) newLevel = 6;
        else if (e.score >= 1500) newLevel = 5;
        else if (e.score >= 800) newLevel = 4;
        else if (e.score >= 400) newLevel = 3;
        else if (e.score >= 150) newLevel = 2;
        ...
        const gravMag = e.level >= 3 ? 180 * 1.15 : 180;
        const targetRadius = e.level >= 4 ? 26 * 0.9 : 26;
        const shiftInterval = e.level >= 5 ? 1.8 : 2.5;
        const bounceDecay = e.level >= 6 ? 1.05 : 0.95;

        e.target.radius = targetRadius;

        // Physics: Gravity Shifts
        e.gravityTimer += dt;
        if (e.gravityTimer >= shiftInterval) {
          e.gravityTimer = 0;
          const angle = Math.random() * Math.PI * 2;
          e.gravity = {
            x: Math.cos(angle) * gravMag,
            y: Math.sin(angle) * gravMag
          };
        }

        // Physics: Velocity & Drag
        e.target.vx += e.gravity.x * dt;
        e.target.vy += e.gravity.y * dt;
        const drag = 0.35;
        e.target.vx *= (1 - drag * dt);
        e.target.vy *= (1 - drag * dt);
        e.target.x += e.target.vx * dt;
        e.target.y += e.target.vy * dt;

        // Physics: Wall Collisions
        if (e.target.x - e.target.radius < 0) {
          e.target.x = e.target.radius;
          e.target.vx = -e.target.vx * bounceDecay;
        } else if (e.target.x + e.target.radius > cvs.width) {
          e.target.x = cvs.width - e.target.radius;
          e.target.vx = -e.target.vx * bounceDecay;
        }
        if (e.target.y - e.target.radius < 0) {
          e.target.y = e.target.radius;
          e.target.vy = -e.target.vy * bounceDecay;
        } else if (e.target.y + e.target.radius > cvs.height) {
          e.target.y = cvs.height - e.target.radius;
          e.target.vy = -e.target.vy * bounceDecay;
        }

        // Hit Registration & Gamified Combo System
        const dist = Math.hypot(e.target.x - e.crosshair.x, e.target.y - e.crosshair.y);
        const isContact = dist <= e.target.radius;

        if (isContact) {
          e.contactFrames++;
          e.currentStreak++;
          if (e.currentStreak > e.maxStreak) e.maxStreak = e.currentStreak;
          
          // Combo Multiplier: Increases by 0.5x every 60 frames (1 sec) up to 3.0x
          e.comboMultiplier = Math.min(3.0, 1.0 + Math.floor(e.currentStreak / 60) * 0.5);
          
          e.timeLeft = Math.min(60.0, e.timeLeft + 0.6 * dt);
          e.score += (15 + e.level * 5) * e.comboMultiplier * dt; // Score never goes down
          
          e.audioThrottle += dt;
          if (e.audioThrottle >= 0.18) {
            e.audioThrottle = 0;
            if (audioSynth) audioSynth.playSound('tick');
          }
        } else {
          e.currentStreak = 0;
          e.comboMultiplier = 1.0;
          e.timeLeft = Math.max(0, e.timeLeft - 1.6 * dt); // Drain time, but no negative score
          
          e.audioThrottle += dt;
          if (e.audioThrottle >= 0.3) {
            e.audioThrottle = 0;
        ...
        setScore(Math.round(e.score));
        setCurrentCombo(e.comboMultiplier);
        
        if (e.totalFrames % 10 === 0) {
            setLiveAccuracy(Math.round((e.contactFrames / e.totalFrames) * 100));
        }
      ...
      // Draw Target Base (No floating text on canvas)
      if (gameState === 'playing' || gameState === 'start') {
        ...
        ctx.fillStyle = color;
        ctx.beginPath();
        ctx.arc(e.target.x, e.target.y, e.target.radius, 0, Math.PI * 2);
        ctx.fill();
        ctx.shadowBlur = 0;
        
        // Target Kinetic Vectors
        ctx.strokeStyle = isContact ? 'rgba(16, 185, 129, 0.35)' : 'rgba(239, 68, 68, 0.35)';
        ctx.lineWidth = 3;
        ctx.beginPath();
        ctx.moveTo(e.target.x, e.target.y);
        ctx.lineTo(e.target.x - e.target.vx * 0.08, e.target.y - e.target.vy * 0.08);
        ctx.stroke();
  ...
  const shareScore = useCallback(async () => {
    const text = `🎯 I scored ${score} PTS (Level ${analytics.levelReached}) in the Mouse Tracking Game! Accuracy: ${analytics.accuracy}%, Max Combo Streak: ${analytics.maxStreak}s. Practice hand eye coordination at skilldrills.online!`;
    if (typeof navigator !== 'undefined' && navigator.share) {
      try {
        await navigator.share({ title: 'My Mouse Tracking Game Score', text, url: 'https://skilldrills.online/drills/mouse-tracking-game' });
      } catch (e) {}
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
          ...
          <div className="mb-6">
            <nav className="mb-4">
              <ol className="flex flex-wrap items-center gap-2 text-sm text-gray-500">
                ...
                <li><ChevronRight className="w-4 h-4 text-gray-600" /></li>
                <li className="text-cyan-400 font-medium">Mouse Tracking Game</li>
              </ol>
            </nav>
            ...
            <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
              <div className="flex items-center gap-4">
                ...
                <div>
                  <h1 className="text-2xl sm:text-3xl font-bold tracking-tight">Mouse Tracking Game</h1>
                  <p className="text-sm text-gray-400 mt-1 font-medium">Hand Eye Coordination Game • Tracking Accuracy Test</p>
                </div>
              </div>
        ...
        {!isFullscreen && (
          <div className="grid grid-cols-5 gap-2 mb-2">
            ...
            <StatCard icon={<Zap className="text-yellow-400" />} value={`${currentCombo.toFixed(1)}x`} label="Combo" highlight={currentCombo >= 2.0} />
            ...
            <StatCard icon={<Target className="text-green-500" />} value={`${gameState === 'playing' ? liveAccuracy : analytics.accuracy}%`} label="Accuracy" />
        ...
        >
          {gameState === 'playing' && (
            <div className="absolute top-0 left-0 right-0 h-1.5 bg-gray-900 z-[100]">
              <div 
                ...
                style={{ width: `${Math.min(100, (timeLeft / GAME_DURATION) * 100)}%` }} 
              />
            </div>
          )}
          ...
          {isFullscreen && gameState === 'playing' && (
            <>
              {/* Fullscreen Enhanced HUD (Time & Acc Removed) */}
              <div className="absolute top-6 left-6 z-[60] flex flex-col gap-2 pointer-events-none">
                <div className="bg-black/40 backdrop-blur border border-gray-800 px-4 py-2 rounded-xl flex items-center gap-4">
                  ...
                  <div className="text-center">
                    ...
                    <p className="text-2xl font-black text-yellow-400 leading-none">{currentCombo.toFixed(1)}x</p>
                  </div>
                </div>
              </div>
              ...
              <div className="max-w-md w-full text-center">
                ...
                <div className="bg-[#0b0f19] border border-slate-850 p-4 rounded-xl mb-4 text-left text-xs text-slate-400">
                  ...
                  <ul className="list-disc pl-4 space-y-1 text-[10px] text-slate-500 leading-relaxed">
                    <li>Predictive trajectory estimation for tracking aim practice</li>
                    <li>Visual motor coordination and cursor precision training</li>
                    <li>Zero negative scoring—focus entirely on streak building</li>
                  </ul>
                </div>
              ...
              <div className="max-w-md w-full text-center">
                ...
                <div className="grid grid-cols-3 gap-2.5 mb-6 text-left">
                  ...
                  <div className="bg-slate-900/60 border border-slate-800 p-2.5 rounded-xl">
                    ...
                    <span className="text-base font-black text-purple-400">{analytics.timeAlive}s</span>
                  </div>
                  <div className="bg-slate-900/60 border border-slate-800 p-2.5 rounded-xl">
                    <span className="text-[7.5px] text-slate-500 block uppercase font-bold">Motor Rating</span>
                    <span className="text-base font-black text-yellow-400">{analytics.rankData.rank}</span>
                  </div>
                </div>
                ...
                <div className="bg-[#0b0f19] border border-slate-850 p-3 rounded-xl mb-4 text-left">
                  ...
                  <div className="flex items-center gap-1.5 text-[9px] font-bold text-white uppercase mb-1">
                    <Sparkles className="w-3 h-3 text-cyan-400" /> Tracking Diagnostics:
                  </div>
            ...
            <div className="rounded-2xl border border-gray-800 overflow-hidden bg-gray-900 shadow-2xl pointer-events-none">
              ...
              <div className="p-6 grid grid-cols-1 md:grid-cols-2 gap-6">
                <div className="space-y-5">
                  <RuleItem num="1" color="green" text="Continuous Hover" highlight="Builds Combo" result="Multiplier up to 3.0x" />
                  <RuleItem num="2" color="indigo" text="Zero Negative Scoring" highlight="No Lost Points" result="Points only go up, never down" />
                </div>
                <div className="space-y-5">
                  <RuleItem num="3" color="red" text="Time Survival" highlight="Health Drain" result="Clock drops off-target to end game" />
                  <RuleItem num="4" color="cyan" text="Adaptive Difficulty" highlight="Levels 1-6" result="Faster vectors & tighter tracking bounds" />
                </div>
              </div>
            </div>
          </section>
          ...
          <article className="mt-12 text-gray-300">
            <div className="rounded-2xl border border-gray-800 overflow-hidden bg-gray-900 shadow-xl">
              <div className="px-6 py-5 border-b border-gray-800 bg-black/40 flex items-center gap-3">
                ...
                <h2 className="font-bold text-white text-lg tracking-wide">About the Mouse Tracking Game</h2>
              </div>
              
              <div className="p-8 space-y-8">
                <section>
                  <h2 className="text-xl font-bold text-white mb-3">Mastering the Hand Eye Coordination Game</h2>
                  <p className="text-sm leading-relaxed mb-4">
                    This free mouse tracking game online is designed to push your visual motor coordination and cursor control to the absolute limit. By simulating dynamic physics and unpredictable gravity shifts, this tracking accuracy test forces players to transition from static flicking to smooth pursuit tracking. There is no negative scoring—your goal is simply to survive the time drain by maintaining continuous contact to build massive combo multipliers.
                  </p>
                </section>
                ...
                <div className="grid grid-cols-1 sm:grid-cols-3 gap-5 mb-8">
                  <div className="p-5 rounded-xl border bg-black/40 border-gray-800">
                    ...
                    <p className="text-xs leading-relaxed text-slate-400">Gamers seeking mouse control training, FPS players looking for a tracking aim trainer, and anyone taking a hand eye coordination test.</p>
                  </div>
                  <div className="p-5 rounded-xl border bg-black/40 border-gray-800">
                    <div className="flex items-center gap-3 mb-3">
                    ...
                    <p className="text-xs leading-relaxed text-slate-400">Improves visual tracking, smooth pursuit training, aim tracking practice, and fine motor control under pressure.</p>
                  </div>
                  <div className="p-5 rounded-xl border bg-black/40 border-gray-800">
                    <div className="flex items-center gap-3 mb-3">
                    ...
                    <p className="text-xs leading-relaxed text-slate-400">Total gamified score, your tracking accuracy percentage, maximum combo multiplier, and the adaptive speed level.</p>
                  </div>
                </div>
              </div>
              ...
              <div className="bg-[#0b0f19] border-t border-gray-800 p-8">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                  <FAQItem q="What is a mouse tracking game?" a="A mouse tracking game is a specialized browser tool designed to help gamers and athletes improve their tracking accuracy and cursor control by keeping their crosshair continuously locked onto a dynamically moving target." />
                  <FAQItem q="How does tracking aim differ from flick aiming?" a="Tracking aim involves continuously keeping your crosshair locked onto a moving target, while flick aiming focuses on rapidly snapping your crosshair to a static point in a single motion." />
                  <FAQItem q="Can this drill improve FPS tracking accuracy?" a="Yes, by training visual motor coordination and pointer smoothing control under dynamic physics, this exercise directly translates to better tracking accuracy in first-person shooters." />
                  <FAQItem q="Is this tracking trainer good for Valorant and Apex Legends?" a="Absolutely. It is particularly highly effective for high-TTK games like Apex Legends where continuous tracking aim is crucial, but also benefits micro-corrections in tactical shooters like Valorant." />
                  <FAQItem q="How does continuous cursor tracking improve hand-eye coordination?" a="It forces your brain to process spatial velocity and translate it into real-time physical hand movements, reducing the latency between seeing a target move and adjusting your mouse." />
                  <FAQItem q="What skills does this tracking trainer improve?" a="This tool vastly improves fine motor skills, continuous tracking accuracy, visual tracking, cursor control efficiency, and overall hand-eye coordination." />
                  <FAQItem q="Is this mouse tracking game free?" a="Yes, our tracking trainer is a free mouse tracking game online with no downloads required, offering immediate browser-based access to top-tier motor skills training." />
                  <FAQItem q="What is visual motor coordination?" a="Visual motor coordination is the ability to coordinate visual information with motor output, such as using your eyes to track a moving object while simultaneously moving your hand to intercept it." />
                  <FAQItem q="How often should I practice mouse control training?" a="For optimal results, incorporate 10-15 minutes of mouse control training into your daily pre-game warmup routine to establish strong neural pathways." />
                  <FAQItem q="What makes a good tracking accuracy test?" a="A strong tracking accuracy test incorporates unpredictable direction shifts, varying speeds, and strict hitboxes to ensure you are predicting movement rather than just reacting." />
                  <FAQItem q="How does target scaling affect tracking practice?" a="By shrinking the target size at higher difficulty levels, you are forced into tighter micro-adjustments, ensuring that your pursuit stays centered rather than resting on the edge of a large hitbox." />
                  <FAQItem q="Why is smooth tracking important in gaming?" a="Smooth tracking eliminates jittery crosshair movements, which maximizes damage output per second (DPS) and maintains consistent recoil control during continuous fire." />
                  <FAQItem q="How do gravity vectors mimic in-game movement?" a="Gravity shifts simulate the sudden acceleration, deceleration, and directional strafing patterns of human opponents, building your predictive tracking capabilities." />
                  <FAQItem q="Can beginners use this cursor tracking game?" a="Yes, the drill uses a dynamic scoring system that starts at Level 1, allowing beginners to learn the mechanics before automatically scaling up the difficulty." />
                  <FAQItem q="What is predictive target motion?" a="Predictive motion is the anticipation of where a target will be based on its current trajectory and speed, rather than simply dragging the mouse to where the target is right now." />
                </div>
              </div>
            </div>
          </article>
          ...
          <section className="mt-14" aria-label="Explore related hand eye coordination games">
            <div className="flex items-center gap-2 mb-4">
              ...
              <h2 className="text-xs font-bold text-white uppercase tracking-widest font-mono">
                Explore More Tracking Trainers
              </h2>
            </div>
