...
import { 
  ArrowLeft, Brain, Volume2, VolumeX, Maximize2, Minimize2,
  Sun, Moon, Eye, Timer, Trophy, RotateCcw, Grid, Activity,
...
export default function MemorySequenceDrill() {
  ...
  const [maxSequenceForCurrentGrid, setMaxSequenceForCurrentGrid] = useState(9);
  ...
  const getAccuracy = () => {
    ...
    const totalAttempts = totalSequencesCompleted + (3 - attemptsLeft);
    return Math.round((totalSequencesCompleted / totalAttempts) * 100);
  };

  // Get max sequence for grid size
  ...
  // Get starting sequence for grid size
  ...
  // Start new round
  const startNewRound = useCallback(async (size, seqLen, resetAttempts = true) => {
    ...
    const newSequence = generateSequence(seqLen, size);
    setSequence(newSequence);

    const displayTime = Math.max(400, 600 - (size - 3) * 20);
    const pauseTime = Math.max(200, 350 - (size - 3) * 15);
  ...
  const handleSuccess = () => {
    const pointsEarned = sequenceLength * 15 + (currentStreak * 5);
    ...
    const maxSeqForThisGrid = getMaxSequenceForGrid(gridSize);
    
    if (sequenceLength < maxSeqForThisGrid) {
      feedbackTimeoutRef.current = setTimeout(() => {
        setSequenceLength(prev => prev + 1);
        startNewRound(gridSize, sequenceLength + 1, true);
      }, 800);
    } else if (gridSize < 7) {
      ...
      feedbackTimeoutRef.current = setTimeout(() => {
        ...
        startNewRound(newSize, newSeqLen, true);
      }, 800);
    } else {
  ...
  const handleFailure = () => {
    ...
    if (newAttemptsLeft <= 0) {
      ...
      feedbackTimeoutRef.current = setTimeout(() => {
        setGridSize(3);
        setSequenceLength(4);
        setMaxSequenceForCurrentGrid(9);
        setCurrentLevel(1);
        setCurrentStreak(0);
        setAttemptsLeft(3);
        setIsMemoryMaster(false);
        startNewRound(3, 4, true);
      }, 1500);
    } else {
      setGameState('fail');
      showFeedback(`✗ Miss! -5 • ${newAttemptsLeft} chance${newAttemptsLeft > 1 ? 's' : ''} left`, 'error');
      setScore(prev => Math.max(0, prev - 5));
      scoreRef.current = Math.max(0, scoreRef.current - 5);
      playFailSound();
      
      feedbackTimeoutRef.current = setTimeout(() => {
        startNewRound(gridSize, sequenceLength, false);
      }, 1000);
    }
  };
  ...
  const startGame = useCallback(() => {
    ...
    setTimeout(() => startNewRound(3, 4, true), 100);
  ...
  // Calculate grid cell size - keeps grid within container
  const getGridCellSize = () => {
    if (gridSize <= 3) return 'minmax(50px, 80px)';
    if (gridSize === 4) return 'minmax(40px, 60px)';
    if (gridSize === 5) return 'minmax(32px, 48px)';
    if (gridSize === 6) return 'minmax(26px, 40px)';
    return 'minmax(22px, 34px)';
  };

  // Calculate grid max width
  const getGridMaxWidth = () => {
    if (gridSize <= 3) return '300px';
    if (gridSize === 4) return '320px';
    if (gridSize === 5) return '320px';
    if (gridSize === 6) return '300px';
    return '300px';
  };
    ...
    <div className={`min-h-screen select-none ${isDarkMode ? 'bg-gray-900' : 'bg-gray-50'}`}>
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
          ...
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <p className={`${isDarkMode ? 'text-gray-400' : 'text-gray-500'}`}>Remember and repeat • 60s</p>
              </div>
            </div>
        ...
        <div className="grid grid-cols-7 gap-3 mb-4 h-[88px]">
          ...
          <StatCard icon={<Grid className="text-purple-600" />} value={`${gridSize}×${gridSize}`} label="Grid" isDark={isDarkMode} />
            ...
            {gameState === 'start' && (
              <div className={`absolute inset-0 flex items-center justify-center backdrop-blur-sm rounded-xl z-40 ${isBoxDarkMode ? 'bg-gray-900/95' : 'bg-white/95'}`}>
                <div className={`rounded-2xl p-8 text-center max-w-md mx-4 shadow-xl border ${isBoxDarkMode ? 'bg-gray-800 border-gray-700' : 'bg-white border-gray-200'}`}>
                  ...
                  <p className={`mb-6 ${isBoxDarkMode ? 'text-gray-300' : 'text-gray-600'}`}>60-second challenge • 3×3 → 7×7</p>
                  <button 
                    onClick={startGame}
            ...
            {(gameState === 'playing' || gameState === 'showing' || gameState === 'input' || gameState === 'success' || gameState === 'fail') && (
              <div className="w-full h-full flex flex-col items-center justify-center">
                ...
                <div className="absolute bottom-4 left-4 flex gap-3">
                  ...
                  <div className={`px-4 py-2 rounded-lg font-bold flex items-center gap-2 ${attemptsLeft <= 1 ? 'bg-red-500/20 text-red-500' : isBoxDarkMode ? 'bg-zinc-800 text-orange-400' : 'bg-gray-200 text-orange-600'}`}>
                    <RotateCcw className="w-4 h-4" />
                    {attemptsLeft}/3 Chances
                  </div>
                </div>
              </div>
            )}
              ...
              <div className={`absolute inset-0 flex items-center justify-center backdrop-blur-sm rounded-xl z-40 ${isBoxDarkMode ? 'bg-gray-900/95' : 'bg-white/95'}`}>
                <div className={`rounded-2xl p-8 shadow-xl border w-[480px] ${isBoxDarkMode ? 'bg-gray-800 border-gray-700' : 'bg-white border-gray-200'}`}>
                  ...
                  <div className="grid grid-cols-2 gap-4 mb-8">
                    ...
                    <ResultCard label="Max Grid" value={`${gridSize}×${gridSize}`} icon={<Grid className="w-4 h-4" />} color="text-blue-500" />
                    ...
                    <ResultCard label="Completed" value={totalSequencesCompleted} icon={<Award className="w-4 h-4" />} color="text-emerald-500" />
                    <ResultCard label="Memory Master" value={isMemoryMaster ? '✓ Yes!' : 'Not yet'} icon={<Trophy className="w-4 h-4" />} color={isMemoryMaster ? 'text-yellow-500' : 'text-gray-400'} />
                  </div>
            ...
            <div className={`rounded-xl border overflow-hidden ${isDarkMode ? 'bg-gray-800 border-gray-700' : 'bg-white border-gray-200'}`}>
              ...
              <div className="p-4">
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  <div className="space-y-2">
                    ...
                    <div className="flex items-start gap-2">
                      ...
                      <p className={`text-sm ${isDarkMode ? 'text-gray-300' : 'text-gray-600'}`}>Success: <span className="font-semibold text-green-500">+15 × length + 5 × streak</span></p>
                    </div>
                    <div className="flex items-start gap-2">
                      <div className="w-5 h-5 rounded-full bg-red-500 flex items-center justify-center text-white text-xs font-bold mt-0.5">3</div>
                      <p className={`text-sm ${isDarkMode ? 'text-gray-300' : 'text-gray-600'}`}>Miss: -5 • 3 failures = <span className="font-semibold text-red-500">-10 + level reset</span></p>
                    </div>
                  </div>
                  <div className="space-y-2">
                    <div className="flex items-start gap-2">
                      ...
                      <p className={`text-sm ${isDarkMode ? 'text-gray-300' : 'text-gray-600'}`}>Progress: 3×3 → 4×4 → 5×5 → 6×6 → <span className="font-semibold text-purple-500">7×7</span></p>
                    </div>
                    <div className="flex items-start gap-2">
                      ...
                      <p className={`text-sm ${isDarkMode ? 'text-gray-300' : 'text-gray-600'}`}>Complete 7×7 to become <span className="font-semibold text-orange-500">Memory Master</span></p>
                    </div>
                    <div className="flex items-start gap-2">
                ...
                <div className={`mt-3 pt-3 border-t text-xs ${isDarkMode ? 'border-gray-700 text-gray-400' : 'border-gray-200 text-gray-500'} flex items-center justify-between`}>
                  <span>🧠 3×3: 4→9 • 4×4: 8→16 • 5×5: 15→25 • 6×6: 25→36 • 7×7: 36→49</span>
