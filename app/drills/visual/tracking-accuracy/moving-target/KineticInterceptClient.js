...
export default function KineticInterceptClient() {
  ...
  const sharePage = async () => { if (navigator.share) { try { await navigator.share({ title: 'Free Kinetic Intercept Moving Target Drill | SkillDrills', text: 'Train hand-eye coordination by clicking moving targets. Free!', url: 'https://skilldrills.online/drills/visual/tracking-accuracy/moving-target' }); } catch (e) {} } else { navigator.clipboard.writeText('https://skilldrills.online/drills/visual/tracking-accuracy/moving-target'); alert('Link copied!'); } };
  const copyPageLink = () => { navigator.clipboard.writeText('https://skilldrills.online/drills/visual/tracking-accuracy/moving-target'); alert('Link copied!'); };
  ...
  return (
    <div className={`min-h-screen select-none ${isDarkMode ? 'bg-gray-900' : 'bg-gray-50'}`}>
      <script type="application/ld+json" dangerouslySetInnerHTML={{ __html: JSON.stringify({ "@context": "https://schema.org", "@type": "WebApplication", "name": "Kinetic Intercept Drill - Moving Target Tracking & Hand-Eye Coordination Training", "url": "https://skilldrills.online/drills/visual/tracking-accuracy/moving-target", "description": "Free hand-eye coordination drill with fast-moving white targets spawning from 4 screen edges at 12-22 speed. Cursor turns green when hovering over target. 3 lives system with penalty scoring after depletion. 60-second timed challenge with 5-streak bonuses and hit tracking.", "applicationCategory": "EducationalApplication", "operatingSystem": "All", "offers": { "@type": "Offer", "price": "0", "priceCurrency": "USD", "availability": "https://schema.org/OnlineOnly" }, "author": { "@type": "Organization", "name": "SkillDrills", "url": "https://skilldrills.online" }, "publisher": { "@type": "Organization", "name": "SkillDrills" }, "educationalUse": ["Hand-Eye Coordination Training", "Target Tracking Practice", "Aim Training", "Reflex Speed Development", "Visual Motor Skills"], "learningResourceType": ["Interactive Exercise", "Tracking Drill", "Aim Training"], "timeRequired": "PT60S", "interactivityType": "active", "inLanguage": "en-US", "teaches": ["Moving Target Tracking", "Mouse Accuracy", "Hand-Eye Coordination", "Reflex Training", "Visual Pursuit"], "educationalLevel": "All Levels", "typicalAgeRange": "8-80", "datePublished": "2026-05-14", "dateModified": new Date().toISOString().split('T')[0], "version": "1.0", "isAccessibleForFree": true, "accessMode": ["visual"], "accessModeSufficient": ["visual"] }) }} />
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        ...
        {!isFullscreen && (
          <section className="mt-8" aria-label="About this kinetic intercept moving target drill">
            <div className={`rounded-xl border overflow-hidden ${isDarkMode ? 'bg-gray-800 border-gray-700' : 'bg-white border-gray-200'}`}>
              <div className={`px-4 py-3 border-b ${isDarkMode ? 'border-gray-700 bg-gray-800/50' : 'border-gray-200 bg-gray-50'}`}>
                <div className="flex items-center gap-2">
                  <GraduationCap className={`w-5 h-5 ${isDarkMode ? 'text-red-400' : 'text-red-600'}`} aria-hidden="true" />
                  <h2 className={`font-semibold ${isDarkMode ? 'text-white' : 'text-gray-900'}`}>About This Free Kinetic Intercept Drill</h2>
                </div>
              </div>
              <div className="p-5">
                <p className={`text-sm leading-relaxed mb-5 ${isDarkMode ? 'text-gray-300' : 'text-gray-600'}`}>
                  This free Kinetic Intercept drill trains hand-eye coordination and moving target tracking by challenging you to click fast-moving white targets that spawn from all four screen edges. Targets travel at randomized speeds between 12-22 units with slightly randomized angles toward the center, creating unpredictable movement patterns. The cursor dynamically changes from red to green when hovering over a target, providing clear visual feedback for precision. With a 3 lives system and penalty scoring after depletion, the drill rewards accuracy while encouraging risk-taking. Perfect for gamers improving tracking aim, athletes developing hand-eye coordination, designers enhancing mouse precision, and anyone wanting faster visual-motor response to moving objects.
                </p>
                <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 mb-5">
                  <div className={`p-4 rounded-xl border ${isDarkMode ? 'bg-gray-700/50 border-gray-600' : 'bg-red-50 border-red-100'}`}>
                    <div className="flex items-center gap-2 mb-2"><div className="w-8 h-8 rounded-lg bg-red-500 flex items-center justify-center"><GraduationCap className="w-4 h-4 text-white" /></div><h3 className={`text-sm font-semibold ${isDarkMode ? 'text-white' : 'text-gray-900'}`}>Who It's For</h3></div>
                    <p className={`text-xs leading-relaxed ${isDarkMode ? 'text-gray-400' : 'text-gray-500'}`}>Gamers improving tracking aim for FPS and MOBA games, athletes developing hand-eye coordination, designers enhancing mouse precision, and anyone wanting faster visual-motor response to moving objects.</p>
                  </div>
                  <div className={`p-4 rounded-xl border ${isDarkMode ? 'bg-gray-700/50 border-gray-600' : 'bg-green-50 border-green-100'}`}>
                    ...
                    <p className={`text-xs leading-relaxed ${isDarkMode ? 'text-gray-400' : 'text-gray-500'}`}>Moving target tracking, hand-eye coordination, mouse accuracy, visual pursuit, reflex speed, and the ability to predict and intercept moving objects with precise click timing.</p>
                  </div>
                  <div className={`p-4 rounded-xl border ${isDarkMode ? 'bg-gray-700/50 border-gray-600' : 'bg-purple-50 border-purple-100'}`}>
                    ...
                    <p className={`text-xs leading-relaxed ${isDarkMode ? 'text-gray-400' : 'text-gray-500'}`}>Score, total hits, streak count, lives remaining, current target speed, best performance records saved locally, and green cursor feedback when hovering over targets.</p>
                  </div>
                </div>
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                  <div className={`p-4 rounded-xl border ${isDarkMode ? 'bg-gray-700/50 border-gray-600' : 'bg-yellow-50 border-yellow-100'}`}>
                    <div className="flex items-center gap-2 mb-3"><div className="w-8 h-8 rounded-lg bg-yellow-500 flex items-center justify-center"><Lightbulb className="w-4 h-4 text-white" /></div><h3 className={`text-sm font-semibold ${isDarkMode ? 'text-white' : 'text-gray-900'}`}>Why Practice Moving Target Tracking?</h3></div>
                    <ul className={`text-xs space-y-2 ${isDarkMode ? 'text-gray-400' : 'text-gray-600'}`}>
                      <li className="flex items-start gap-2"><CheckCircle2 className="w-3 h-3 text-yellow-500 mt-0.5 flex-shrink-0" />Essential skill for competitive gaming requiring precise tracking aim</li>
                      <li className="flex items-start gap-2"><CheckCircle2 className="w-3 h-3 text-yellow-500 mt-0.5 flex-shrink-0" />Builds visual pursuit ability critical for sports and real-world activities</li>
                      <li className="flex items-start gap-2"><CheckCircle2 className="w-3 h-3 text-yellow-500 mt-0.5 flex-shrink-0" />Green cursor feedback provides instant confirmation of target acquisition</li>
                    </ul>
                  </div>
                  <div className={`p-4 rounded-xl border ${isDarkMode ? 'bg-gray-700/50 border-gray-600' : 'bg-orange-50 border-orange-100'}`}>
                    ...
                    <ol className={`text-xs space-y-2 ${isDarkMode ? 'text-gray-400' : 'text-gray-600'}`}>
                      <li className="flex items-start gap-2"><span className="w-5 h-5 rounded-full bg-orange-500 text-white text-xs flex items-center justify-center flex-shrink-0 mt-0.5">1</span>Watch for the cursor to turn green before clicking for confirmed hits</li>
                      <li className="flex items-start gap-2"><span className="w-5 h-5 rounded-full bg-orange-500 text-white text-xs flex items-center justify-center flex-shrink-0 mt-0.5">2</span>Lead the target slightly to account for movement and click timing</li>
                      <li className="flex items-start gap-2"><span className="w-5 h-5 rounded-full bg-orange-500 text-white text-xs flex items-center justify-center flex-shrink-0 mt-0.5">3</span>Build streaks of 5+ consecutive hits for bonus notifications</li>
                      <li className="flex items-start gap-2"><span className="w-5 h-5 rounded-full bg-orange-500 text-white text-xs flex items-center justify-center flex-shrink-0 mt-0.5">4</span>Practice 10-15 minutes daily for best tracking improvement</li>
                    </ol>
                  </div>
                </div>
              </div>
          ...
          <footer className="mt-12 bg-gray-900 text-gray-400 rounded-xl py-10 px-6" role="contentinfo">
            <div className="max-w-7xl mx-auto">
              ...
              <div className="border-t border-gray-800 pt-8 text-center">
                ...
                <p className="text-xs max-w-2xl mx-auto leading-relaxed mb-6">Free online kinetic intercept drill for moving target tracking training. White targets spawn from 4 screen edges at 12-22 speed with green cursor feedback. 3 lives system with penalty scoring. Perfect for gamers athletes and anyone wanting to improve hand-eye coordination. No registration required. More free drills at skilldrills.online.</p>
                <div className="flex items-center justify-center gap-5 flex-wrap">
                  <button onClick={sharePage} className="text-gray-500 hover:text-white transition-colors" title="Share this drill" aria-label="Share this free kinetic intercept drill"><Share2 className="w-5 h-5" /></button>
