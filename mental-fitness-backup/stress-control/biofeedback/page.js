...
export default function CoherenceDrillPage() {
  ...
  useEffect(() => {
    if (gameState === 'gameOver' && totalBreaths > bestBreaths) {
      ...
      showFeedback(`ðŸ† New Record! ${totalBreaths} breaths`, 'success');
    }
  ...
  function runCycle() {
    ...
    inhaleTimeoutRef.current = setTimeout(() => {
      ...
      setTotalBreaths(prev => {
        ...
        if (newCount % 10 === 0) {
          showFeedback(`ðŸŽ¯ ${newCount} Breaths Complete!`, 'success');
        }
        return newCount;
      });
    ...
    timerIntervalRef.current = setInterval(() => {
      setTimeLeft(prev => {
        if (prev <= 1) {
          ...
          showFeedback(`ðŸŽ¯ Session Complete!`, 'success');
          return 0;
        }
        return prev - 1;
      });
          ...
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <p className={`${isDarkMode ? 'text-gray-400' : 'text-gray-500'}`}>Heart rate variability & breath coherence training</p>
              </div>
            </div>
          ...
          {gameState === 'start' && (
            <div className="absolute inset-0 flex items-center justify-center rounded-xl z-20 backdrop-blur-sm" style={{ background: isBoxDarkMode ? 'rgba(10,10,26,0.95)' : 'rgba(240,249,255,0.95)' }}>
              <div className={`rounded-2xl p-8 text-center max-w-md mx-4 shadow-xl border ${isBoxDarkMode ? 'bg-gray-800 border-gray-700' : 'bg-white border-gray-200'}`}>
                ...
                <p className={`mb-6 ${isBoxDarkMode ? 'text-gray-300' : 'text-gray-600'}`}>Heart rate variability & breath coherence training</p>
                <button
                  onClick={startDrill}
            ...
            <div className={`rounded-xl border overflow-hidden ${isDarkMode ? 'bg-gray-800 border-gray-700' : 'bg-white border-gray-200'}`}>
              ...
              <div className="p-4">
                ...
                <div className={`mt-3 pt-3 border-t text-xs ${isDarkMode ? 'border-gray-700 text-gray-400' : 'border-gray-200 text-gray-500'} flex items-center justify-between`}>
                  <span>ðŸ’“ Optimizes heart rate variability and vagal tone</span>
                  <span>ðŸŽµ 523Hz inhale tone â€¢ 392Hz exhale tone</span>
                </div>
              </div>
            </div>
          </div>
