...
import { 
  ...
  Eye, Timer, Crosshair, Move, Brain, X, Trophy, Info, Check
} from 'lucide-react';
...
export default function KineticInterceptPage() {
  ...
  const calculatePoints = () => {
    return 10 + Math.floor(streakRef.current / 5) * 5;
  };
      ...
      timerIntervalRef.current = setInterval(() => {
        setTimeLeft(prev => {
          if (prev <= 1) {
            ...
            const total = totalHits + mistakes;
            const finalAccuracy = total === 0 ? 100 : Math.round((totalHits / total) * 100);
  ...
  }, [gameState, timeLeft, totalHits, mistakes]);
  ...
  const fail = (reason) => {
    ...
    const penaltyPoints = calculatePoints();
    ...
    function update() {
      ...
      if (targetRef.current.x < -60 || targetRef.current.x > cvs.width + 60 || 
        ...
        spawnTimeoutRef.current = setTimeout(() => {
          if (cvs.width && cvs.height && isActiveRef.current) {
          ...
          {isFullscreen && gameState === 'playing' && (
            <>
              ...
              <div className="absolute top-4 left-4 z-20 bg-black/50 backdrop-blur-sm rounded-lg px-4 py-2 text-white text-sm">
                Score: <span className="text-yellow-400">{score}</span> | Speed: <span className="text-cyan-400">{targetSpeed}</span> | Streak: <span className="text-orange-400">{streak}</span>
              </div>
            </>
          )}
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  <div className="space-y-2">
                    ...
                    <div className="flex items-start gap-2">
                      ...
                      <p className={`text-sm ${isDarkMode ? 'text-gray-300' : 'text-gray-600'}`}>
                        <span className="font-semibold text-green-500">Hit: +10 + streak bonus</span> (+5 per 5 streak)
                      </p>
                    </div>
                    <div className="flex items-start gap-2">
                      ...
                      <p className={`text-sm ${isDarkMode ? 'text-gray-300' : 'text-gray-600'}`}>
                        <span className="font-semibold text-blue-500">Target speed: 8-15</span> • Varies each spawn
                      </p>
                    </div>
                  </div>
                  <div className="space-y-2">
                    <div className="flex items-start gap-2">
                      ...
                      <p className={`text-sm ${isDarkMode ? 'text-gray-300' : 'text-gray-600'}`}>
                        <span className="font-semibold text-yellow-500">Penalty = Potential Gain</span> • Miss loses what hit would earn
                      </p>
                    </div>
                    <div className="flex items-start gap-2">
                      ...
                      <p className={`text-sm ${isDarkMode ? 'text-gray-300' : 'text-gray-600'}`}>
                        <span className="font-semibold text-purple-500">5 streak bonus notification</span> • 200ms respawn
                      </p>
                    </div>
                    <div className="flex items-start gap-2">
