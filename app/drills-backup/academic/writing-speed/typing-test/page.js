...
export default function TypingTestDrill() {
  ...
  // Update best score
  useEffect(() => {
    if (gameState === 'gameOver' && wpm > bestScore) {
      setBestScore(wpm);
      localStorage.setItem('typingDrillBestScore', wpm.toString());
    }
  }, [gameState, wpm, bestScore]);
  ...
  const startTest = () => {
    ...
    playSound('start');
    showFeedback(`60s challenge • ${difficulty} mode`, 'success');
  ...
  const handleInputChange = (e) => {
    ...
    if (val === targetText) {
      ...
      playSound('complete');
      showFeedback(`Complete! Score: ${Math.round(finalScore)}`, 'success');
    }
  };
      ...
      timerRef.current = setInterval(() => {
        setTimeLeft(prev => {
          if (prev <= 1) {
            ...
            playSound('complete');
            showFeedback(`Time's up! Score: ${Math.round(finalScore)}`, 'success');
            return 0;
          }
          return prev - 1;
        });
  ...
  }, [gameState, timeLeft, wpm, accuracy]);
    ...
    <div className={`min-h-screen select-none ${isDarkMode ? 'bg-gray-900' : 'bg-gray-50'}`}>
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        ...
        <div className="grid grid-cols-7 gap-3 mb-4 h-[88px]">
          ...
          <StatCard icon={<Activity className="text-orange-600" />} value={rawKpm} label="KPM" isDark={isDarkMode} />
              ...
              <div className={`absolute inset-0 flex items-center justify-center backdrop-blur-sm rounded-xl z-40 ${isBoxDarkMode ? 'bg-gray-900/95' : 'bg-white/95'}`}>
                <div className={`rounded-2xl p-8 shadow-xl border w-[480px] ${isBoxDarkMode ? 'bg-gray-800 border-gray-700' : 'bg-white border-gray-200'}`}>
                  ...
                  <div className="grid grid-cols-2 gap-4 mb-8">
                    ...
                    <ResultCard label="KPM" value={rawKpm} icon={<Activity className="w-4 h-4" />} color="text-blue-500" />
            ...
            <div className={`rounded-xl border overflow-hidden ${isDarkMode ? 'bg-gray-800 border-gray-700' : 'bg-white border-gray-200'}`}>
              ...
              <div className="p-4">
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  <div className="space-y-2">
                    ...
                    <div className="flex items-start gap-2">
                      ...
                      <p className={`text-sm ${isDarkMode ? 'text-gray-300' : 'text-gray-600'}`}>Score = <span className="font-semibold text-green-500">WPM × (Accuracy ÷ 100)</span></p>
                    </div>
                    <div className="flex items-start gap-2">
                      <div className="w-5 h-5 rounded-full bg-red-500 flex items-center justify-center text-white text-xs font-bold mt-0.5">3</div>
                      <p className={`text-sm ${isDarkMode ? 'text-gray-300' : 'text-gray-600'}`}>Errors highlighted in <span className="font-semibold text-red-500">red with background</span></p>
                    </div>
                  </div>
                  <div className="space-y-2">
                    <div className="flex items-start gap-2">
                      ...
                      <p className={`text-sm ${isDarkMode ? 'text-gray-300' : 'text-gray-600'}`}>WPM calculated from <span className="font-semibold text-blue-500">correct characters only</span></p>
                    </div>
                    <div className="flex items-start gap-2">
                ...
                <div className={`mt-3 pt-3 border-t text-xs ${isDarkMode ? 'border-gray-700 text-gray-400' : 'border-gray-200 text-gray-500'} flex items-center justify-between`}>
                  <span>⌨️ EASY • MEDIUM • HARD difficulty levels</span>
