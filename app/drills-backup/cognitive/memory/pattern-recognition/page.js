...
import { 
  ArrowLeft, Brain, Award, Activity, Target,
  Volume2, VolumeX, Maximize2, Minimize2, Sun, Moon, 
  Eye, Timer, Trophy, Calculator, Hash, Zap, ArrowRight,
...
export default function MathPatternRecognitionPage() {
  ...
  const processAnswer = (answer) => {
    ...
    if (isCorrect) {
      ...
      playSound('incorrect');
      showFeedback(`✗ Wrong! Answer: ${currentPattern.answer}`, 'error');
    }
    ...
    return (
      <div className="min-h-screen flex items-center justify-center bg-gray-50">
        <div className="text-center">
          ...
          <p className="text-gray-600">Loading Math Patterns...</p>
        </div>
      </div>
    );
  }
            ...
            {gameState === 'start' && (
              <div className={`absolute inset-0 flex items-center justify-center backdrop-blur-sm rounded-xl z-40 ${isBoxDarkMode ? 'bg-gray-900/95' : 'bg-white/95'}`}>
                <div className={`rounded-2xl p-8 text-center max-w-md mx-4 shadow-xl border ${isBoxDarkMode ? 'bg-gray-800 border-gray-700' : 'bg-white border-gray-200'}`}>
                  ...
                  <p className={`mb-6 ${isBoxDarkMode ? 'text-gray-300' : 'text-gray-600'}`}>60-second challenge • Find the next number</p>
                  <button 
                    onClick={startGame}
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  ...
                  <div className="space-y-2">
                    ...
                    <div className="flex items-start gap-2">
                      ...
                      <p className={`text-sm ${isDarkMode ? 'text-gray-300' : 'text-gray-600'}`}>Each pattern is <span className="font-semibold text-orange-500">unique and never repeats</span></p>
                    </div>
                    <div className="flex items-start gap-2">
