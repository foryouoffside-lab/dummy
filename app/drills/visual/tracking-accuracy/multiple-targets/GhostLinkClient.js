...
export default function GhostLinkClient() {
  ...
  useEffect(() => { const h = (e) => { const c = canvasRef.current; if (!c) return; const r = c.getBoundingClientRect(); mousePositionRef.current = { x: (e.clientX - r.left) * (c.width / r.width), y: (e.clientY - r.top) * (c.height / r.height) }; }; const th = (e) => { if (e.touches && e.touches[0]) h(e.touches[0]); }; window.addEventListener('mousemove', h); window.addEventListener('touchmove', th, { passive: true }); return () => { window.removeEventListener('mousemove', h); window.removeEventListener('touchmove', th); }; }, []);
  useEffect(() => {
    ...
    const th = (e) => { if (e.touches && e.touches[0]) h(e.touches[0]); else h(e); };
    window.addEventListener('mousedown', h); window.addEventListener('touchstart', th, { passive: true }); return () => { window.removeEventListener('mousedown', h); window.removeEventListener('touchstart', th); };
    ...
    <div className={`min-h-screen select-none ${isDarkMode ? 'bg-gray-900' : 'bg-gray-50'}`}>
      ...
      {showRotateWarning && (
        <div className="absolute inset-0 z-50 flex flex-col items-center justify-center bg-gray-950/95 text-center p-6" aria-hidden="true">
          ...
          <canvas ref={canvasRef} style={{ display: 'block', position: 'absolute', cursor: gameState === 'playing' && !identificationPhase ? 'none' : 'default', touchAction: 'none' }} aria-hidden="true" />
