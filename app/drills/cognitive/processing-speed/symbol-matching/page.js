...
import { 
  ...
  BarChart3, Info, CheckCircle, Activity, RotateCcw, Clock, Compass
} from 'lucide-react';

export default function SymbolMatchingDrill() {
  const containerRef = useRef(null);
  
  // Professional Symbol Set - Greek letters
  const SYMBOLS = ['Δ', 'Φ', 'Ω', 'Σ', 'Ξ', 'Π', 'Ψ', 'Γ', 'Θ'];
        ...
        <div className="mb-6">
          ...
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-3">
              ...
              <div>
                <h1 className={`text-3xl font-bold ${isDarkMode ? 'text-white' : 'text-gray-900'}`}>Symbol Matching</h1>
                <p className={`${isDarkMode ? 'text-gray-400' : 'text-gray-500'}`}>Keys change every answer • +1/-1 • 3 lives</p>
              </div>
            </div>
              ...
              {gameState === 'start' && (
                <div className="absolute inset-0 flex items-center justify-center backdrop-blur-sm rounded-3xl z-40">
                  <div className={`rounded-2xl p-8 text-center max-w-md mx-4 shadow-xl border ${isBoxDarkMode ? 'bg-gray-800 border-gray-700' : 'bg-white border-gray-200'}`}>
                    ...
                    <h3 className={`text-2xl font-bold mb-2 ${isBoxDarkMode ? 'text-white' : 'text-gray-900'}`}>Symbol Matching</h3>
                    <p className={`mb-6 ${isBoxDarkMode ? 'text-gray-300' : 'text-gray-600'}`}>75-second challenge • +1 per correct • 3 lives system</p>
                    <button 
                      onClick={startDrill}
