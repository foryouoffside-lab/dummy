...
export default function StressInoculationPage() {
  ...
  const checkStressPhase = (currentTimeLeft) => {
    ...
    if (inStressPhase !== isStressPhase) {
      ...
      if (inStressPhase) {
        playSound('stress');
        showFeedback(`âš ï¸ STRESS PHASE ACTIVE - Stay Coherent!`, 'warning');
      } else if (elapsed >= STRESS_END) {
        showFeedback(`âœ… Stress Phase Complete - Great Resilience!`, 'success');
      }
    }
  };
  ...
  function runBreathingCycle() {
    ...
    inhaleTimeoutRef.current = setTimeout(() => {
      ...
      if (breathsRef.current % 5 === 0) {
        ...
        if (isStressPhase) {
          showFeedback(`ðŸ’ª +${pointsEarned} - Maintained Under Stress!`, 'success');
        } else {
            ...
            <div className={`rounded-xl border overflow-hidden ${isDarkMode ? 'bg-gray-800 border-gray-700' : 'bg-white border-gray-200'}`}>
              ...
              <div className="p-4">
                ...
                <div className={`mt-3 pt-3 border-t text-xs ${isDarkMode ? 'border-gray-700 text-gray-400' : 'border-gray-200 text-gray-500'} flex items-center justify-between`}>
                  <span>ðŸ§  Improves stress tolerance, focus under pressure, emotional regulation</span>
                  <span>ðŸŽµ 523Hz inhale â€¢ 392Hz exhale â€¢ 880Hz stress tone</span>
                </div>
              </div>
            </div>
          </div>
