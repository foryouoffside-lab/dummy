  ...
  return (
    <nav aria-label="Breadcrumb" className="max-w-7xl mx-auto px-4 sm:px-6 pt-6">
      <ol className="flex items-center gap-2 text-xs font-mono text-gray-400 uppercase tracking-wider min-w-0">
        ...
        <li className="shrink-0"><Link href="/drills" className="hover:text-white transition-colors">Drills</Link></li>
        <li className="shrink-0"><ChevronRight className="w-3 h-3 text-gray-700" /></li>
        <li className="shrink-0">
          <Link href={`/drills/${drill.category}`} className="hover:text-white transition-colors">
            {drill.categoryLabel}
          </Link>
        </li>
        <li className="shrink-0"><ChevronRight className="w-3 h-3 text-gray-700" /></li>
        <li className="min-w-0"><span className="block truncate text-white font-bold" aria-current="page">{drill.name}</span></li>
      </ol>
    </nav>
  );
}
