...
import { 
  ...
  Move, XCircle, Award, Keyboard, Heart, Gauge, Hourglass, Brain
} from 'lucide-react';
...
class AudioSynthesizer {
  ...
  playSound(type) {
    ...
    try {
      ...
      const freqMap = { 
        perfect: 1046.5, // C6 (For EXACT 0ms)
        good: 880,       // A5 (For standard hit)
        miss: 300,       // D#4
        streak: 1318.52, // E6
        ready: 660       // E5
      };
...
export default function TemporalPrecisionClient() {
  ...
  const engine = useRef({
    state: 'READY', // 'READY', 'HOLDING', 'RESULT'
    targetTime: 1.00,
    startTime: 0,
    resultTimer: 0,
  ...
  useEffect(() => {
    try {
      const savedBest = localStorage.getItem('temporalPrecision_bestScore');
  ...
  const endGame = useCallback(() => {
    ...
    setAnalytics({
      ...
      avgError: avgErr,
      bestError: e.bestError !== null ? e.bestError : '-'
    });

    setBestScore(prev => {
      if (e.score > prev) {
        setIsNewBest(true);
        try { localStorage.setItem('temporalPrecision_bestScore', e.score.toString()); } catch(err){}
        return e.score;
      }
      return prev;
    });
  ...
  const generateNewRound = useCallback(() => {
    const e = engine.current;
    e.targetTime = parseFloat((0.5 + Math.random() * 2.0).toFixed(2)); // Random 0.50s to 2.50s
    e.state = 'READY';
  }, []);
    ...
    const handleInputUp = (e) => {
      if (gameState === 'playing' && canvasRef.current) {
        ...
        if (eRef.state === 'HOLDING') {
          ...
          const elapsedMs = now - eRef.startTime;
          const targetMs = eRef.targetTime * 1000;
          const errorMs = elapsedMs - targetMs;
          const roundedError = Math.round(errorMs);
          const absError = Math.abs(roundedError);
          
          eRef.lastError = roundedError;
          eRef.elapsedTime = elapsedMs / 1000;
          eRef.totalAttempts++;

          if (absError <= 120) { // Hit Window
            eRef.hits++;
            eRef.totalErrorAbs += absError;
            eRef.streak++;
            if (eRef.streak > eRef.maxStreak) eRef.maxStreak = eRef.streak;

            if (eRef.bestError === null || absError < eRef.bestError) {
              eRef.bestError = absError;
            }

            // EXACT 0ms Difference (+20 PTS)
            if (absError === 0) {
              eRef.score += 20; 
              eRef.exactHits++;
              if (audioSynth) audioSynth.playSound('perfect');
              createExplosion(cx, cy, '#fbbf24', 30); // Golden Explosion
              setFlashBg('yellow');
            } else {
              eRef.score += 1; // Standard Hit (+1 PT)
              if (audioSynth) audioSynth.playSound('good');
              createExplosion(cx, cy, '#00ff88', 15); // Green Explosion
              setFlashBg('green');
            }
          ...
          eRef.state = 'RESULT';
          eRef.resultTimer = 1.0; // 1 second cooldown before next round
          
          // Update React State
          setScore(eRef.score);
          setStreak(eRef.streak);
          setExactHits(eRef.exactHits);
  ...
  useEffect(() => {
    ...
    const loop = (time) => {
      ...
      if (e.state === 'READY') {
        ...
        ctx.fillText(`${e.targetTime.toFixed(2)}s`, cx, cy + 20);
        
        ctx.fillStyle = "#06b6d4"; // Cyan
        ctx.font = "bold 16px sans-serif";
        ctx.letterSpacing = "4px";
        ctx.fillText("HOLD TO MATCH", cx, cy - 60);
      }

      if (e.state === 'HOLDING') {
        // Visual indicator that holding is active (no timing info given)
        const pulse = 80 + Math.sin(time / 100) * 8;
        ...
        ctx.letterSpacing = "4px";
        ctx.textAlign = "center";
        ctx.fillText("RELEASE AT TARGET", cx, cy + 120);
      }

      if (e.state === 'RESULT') {
        const absError = Math.abs(e.lastError);
        const isHit = absError <= 120;
        
        let rating = 'MISS';
        let color = '#ef4444'; // Red
        
        if (isHit) {
          if (absError === 0) { rating = 'EXACT'; color = '#fbbf24'; } // Gold
          else { rating = 'HIT'; color = '#00ff88'; } // Green
        }
        ...
        ctx.fillText(`${e.elapsedTime.toFixed(3)}s`, cx, cy + 10);
        
        ctx.fillStyle = color;
        ctx.font = "bold 24px monospace";
        ctx.fillText(`${e.lastError > 0 ? '+' : ''}${e.lastError}ms`, cx, cy + 50);
      }
      ...
      if (m.x > 0 && m.x < cvs.width && m.y > 0 && m.y < cvs.height && (gameState === 'playing' || gameState === 'start')) {
        ctx.strokeStyle = e.state === 'HOLDING' ? '#00ff88' : '#06b6d4';
        ctx.lineWidth = 2;
  ...
  const shareDrillLink = useCallback(() => {
    ...
    if (navigator.share) {
      navigator.share({ title: 'Temporal Precision Drill', url }).catch(() => {});
    } else {
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
          ...
          <div className="mb-6">
            <nav className="mb-4">
              <ol className="flex flex-wrap items-center gap-2 text-sm text-gray-500">
                ...
                <li><Link href="/drills/cognitive" className="hover:text-gray-300">Productivity</Link></li>
                <li><ChevronRight className="w-4 h-4 text-gray-600" /></li>
                <li className="text-cyan-400 font-medium">Temporal Precision</li>
              </ol>
            </nav>
            ...
            <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
              <div className="flex items-center gap-4">
                ...
                <div>
                  <h1 className="text-2xl sm:text-3xl font-bold tracking-tight">Temporal Precision</h1>
                  <p className="text-sm text-gray-400 mt-1 font-medium">Time Estimation • Internal Metronome</p>
                </div>
              </div>
        ...
        {!isFullscreen && (
          <div className="grid grid-cols-4 lg:grid-cols-7 gap-2 mb-2">
            ...
            <StatCard icon={<Award className="text-yellow-400" />} value={exactHits} label="Exact (0ms)" />
        ...
        <div 
          ...
          style={{ backgroundColor: flashBg === 'red' ? '#450a0a' : flashBg === 'green' ? '#064e3b' : flashBg === 'yellow' ? '#422006' : '#05060b', cursor: 'none' }}
        >
          {/* Progress Bar */}
          {gameState === 'playing' && (
          ...
          {gameState === 'start' && (
            <div className="absolute inset-0 flex items-center justify-center z-50 bg-black/70 backdrop-blur-md p-4 overflow-y-auto cursor-default">
              <div className="rounded-3xl p-8 text-center max-w-lg w-full border border-gray-700 bg-gray-900 shadow-2xl my-auto">
                ...
                <h2 className="text-3xl font-black mb-3 tracking-tight text-white uppercase">Temporal Precision</h2>
                <p className="text-sm mb-6 text-gray-400 leading-relaxed">
                  Hold down to match the target time (0.5s to 2.5s). Release exactly when you estimate the time has elapsed. Hit exactly 0ms for a massive 20-point bonus.
                </p>
                ...
                >
                  <Play className="w-6 h-6 fill-white" /> BEGIN ESTIMATION
                </button>
              </div>
            </div>
          )}
            ...
            <div className="absolute inset-0 flex items-center justify-center z-50 bg-black/90 backdrop-blur-md p-4 animate-in fade-in duration-300 overflow-y-auto cursor-default">
              <div className="rounded-3xl max-w-2xl w-full shadow-2xl border border-gray-800 bg-gray-950 overflow-hidden my-auto">
                <div className="bg-gradient-to-br from-cyan-900/40 to-blue-900/40 p-6 border-b border-gray-800 text-center relative">
                  ...
                  <p className="text-cyan-400 font-medium text-sm mt-1">Estimation Session Concluded</p>
                </div>
                  ...
                  <div className="flex flex-col sm:flex-row gap-4 mb-6">
                    <div className="flex-1 bg-gray-900 rounded-2xl p-4 border border-gray-800 flex justify-between items-center">
                      ...
                      <div className="text-right">
                        <span className="text-xs font-bold text-gray-500 uppercase tracking-widest mb-1 block">Avg Estimation Error</span>
  ...
  &plusmn;{analytics.avgError} <span className="text-sm">ms</span>
                      ...
                      <div className="space-y-3 sm:border-r border-gray-800 sm:pr-6">
                        ...
                        <ul className="space-y-2">
                          <li className="flex justify-between items-center bg-gray-900/50 p-2 rounded border border-gray-800">
                            <span className="text-gray-400">Estimation Accuracy:</span>
                            <span className="font-bold text-blue-400">{analytics.accuracy}%</span>
                          </li>
                          <li className="flex justify-between items-center bg-gray-900/50 p-2 rounded border border-gray-800">
                            <span className="text-gray-400">Exact 0ms Hits:</span>
                            <span className="font-bold text-yellow-400">{analytics.exactHits}</span>
                          </li>
                          <li className="flex justify-between items-center bg-gray-900/50 p-2 rounded border border-gray-800">
                            <span className="text-gray-400">Best Error Margin:</span>
                            <span className="font-bold text-emerald-400">{analytics.bestError}ms</span>
                          </li>
                          <li className="flex justify-between items-center bg-gray-900/50 p-2 rounded border border-gray-800">
                        ...
                        <div>
                          ...
                          <p className="text-gray-400 leading-relaxed font-sans">
                            {analytics.avgError > 100 ? (
                              <span className="text-red-300">You are consistently misjudging time duration. Do not rely entirely on visual rhythm; actively tap your foot or mentally subdivide the seconds to maintain a tighter temporal anchor.</span>
                            ) : analytics.exactHits > 0 ? (
                              <span className="text-yellow-300">Incredible temporal calibration! Achieving exactly 0ms error variance proves exceptional internal clock synchronization.</span>
                            ) : analytics.misses > 5 ? (
                              <span className="text-orange-300">Your internal clock drifts significantly under pressure. Stay focused on the immediate subdivision.</span>
                            ) : (
                              <span className="text-green-300">Solid internal metronome performance. Your average error shows reliable temporal awareness.</span>
                            )}
                          </p>
                        </div>
                      </div>
            ...
            <div className="rounded-2xl border border-gray-800 overflow-hidden bg-gray-900 shadow-2xl pointer-events-none">
              ...
              <div className="p-6 grid grid-cols-1 md:grid-cols-2 gap-6">
                <div className="space-y-5">
                  <RuleItem num="1" color="yellow" text="Exact 0ms Hit" highlight="+20 PTS" result="Zero Variance" />
                  <RuleItem num="2" color="green" text="Standard Hit" highlight="+1 PTS" result="< 120ms Error" />
                </div>
                <div className="space-y-5">
                  <RuleItem num="3" color="red" text="Miss / Reset" highlight="0 PTS" result="> 120ms Error" />
                  <RuleItem num="4" color="purple" text="Streak Bonus" highlight="Every 5 Hits" result="Visual/Audio Cue" />
                </div>
              </div>
            </div>
          </section>
          ...
          <section className="mt-12" aria-label="About this drill">
            <div className="rounded-2xl border border-gray-800 overflow-hidden bg-gray-900 shadow-xl">
              <div className="px-6 py-5 border-b border-gray-800 bg-black/40 flex items-center gap-3">
                ...
                <h2 className="font-bold text-white text-lg tracking-wide">About Temporal Precision</h2>
              </div>
              <div className="p-8">
                <p className="text-sm leading-relaxed mb-6 text-gray-300">
                  This free time estimation drill trains your internal clock through pure interval timing practice. A target time between 0.5 and 2.5 seconds is displayed. You hold down the mouse/screen and release precisely when you estimate the target duration has elapsed. There is no visual timer while holding—relying entirely on your internal sense of time passing.
                </p>
                ...
                <div className="grid grid-cols-1 sm:grid-cols-3 gap-5 mb-8">
                  <div className="p-5 rounded-xl border border-gray-800 bg-black/40">
                    ...
                    <p className="text-xs leading-relaxed text-gray-400">Project managers improving estimation, musicians calibrating rhythm, and professionals wanting a better intuitive sense of time passing.</p>
                  </div>
                  <div className="p-5 rounded-xl border border-gray-800 bg-black/40">
                    <div className="flex items-center gap-3 mb-3">
                    ...
                    <p className="text-xs leading-relaxed text-gray-400">Interval timing, internal clock calibration, temporal precision, time estimation accuracy, and motor timing.</p>
                  </div>
                  <div className="p-5 rounded-xl border border-gray-800 bg-black/40">
                    <div className="flex items-center gap-3 mb-3">
                    ...
                    <p className="text-xs leading-relaxed text-gray-400">Total Score, Average Error Margin (ms), Perfect 0ms Hits, Accuracy percentage, and Best Streak.</p>
                  </div>
                </div>
                ...
                <div className="mb-8 bg-[#0b0f19]/40 border border-gray-800 rounded-xl p-6">
                  ...
                  <div className="grid sm:grid-cols-2 gap-6 text-sm text-gray-300">
                    <ol className="space-y-3 list-decimal pl-5">
                      <li>Read the randomized target time on the screen.</li>
                      <li>Hold down the mouse button (or press screen). The timer begins invisibly.</li>
                      <li>Release exactly when you believe the target time has passed.</li>
                    </ol>
                    <ul className="space-y-3">
                      <li className="flex items-center gap-2"><CheckCircle2 className="w-4 h-4 text-green-500" /> <span className="text-white font-bold">Subdivisions:</span> Don't just count "1, 2". Count "1-and-2-and" to stay tighter on rhythm.</li>
                      <li className="flex items-center gap-2"><Star className="w-4 h-4 text-yellow-500" /> <span className="text-white font-bold">Perfection:</span> Releasing at exactly 0ms error grants a massive +20 score boost.</li>
                    </ul>
                  </div>
                </div>
              </div>
        ...
        {!isFullscreen && (
          <section className="mt-14" aria-label="Explore related productivity drills">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
              <RelatedCard href="/drills/cognitive/focus/focus-timer" title="Pomodoro Sync" desc="Focus cycles with canvas rings." color="blue" icon={<Timer className="w-4 h-4" />} />
              <RelatedCard href="/drills/cognitive/problem-solving/priority-sorting" title="Priority Sorting" desc="Sort items with dynamic rules." color="green" icon={<Star className="w-4 h-4" />} />
              <RelatedCard href="/drills/cognitive/attention/batch-processing" title="Batch Processing" desc="Process groups in time windows." color="purple" icon={<Activity className="w-4 h-4" />} />
              <RelatedCard href="/drills/memory/working-memory/n-back" title="3-Back Training" desc="Classic working memory task." color="orange" icon={<Brain className="w-4 h-4" />} />
              <RelatedCard href="/drills/academic" title="Mental Arithmetic" desc="Solve timed math problems." color="cyan" icon={<Calculator className="w-4 h-4" />} />
              <RelatedCard href="/drills/visual/visual-recognition/visual-search" title="Visual Search" desc="Find hidden targets in grids." color="red" icon={<Eye className="w-4 h-4" />} />
              <RelatedCard href="/drills/memory/spatial-memory/path-tracing" title="Path Tracing" desc="Retrace animated dot paths." color="teal" icon={<TrendingUp className="w-4 h-4" />} />
              <RelatedCard href="/drills/cognitive/memory/card-matching" title="Card Matching" desc="Improve visual concentration." color="indigo" icon={<Star className="w-4 h-4" />} />
            </div>
          </section>
        )}
          ...
          <footer className="mt-12 bg-slate-950/40 border border-slate-900 text-slate-500 rounded-xl py-10 px-6 font-mono text-[10px]" role="contentinfo">
            <div className="max-w-7xl mx-auto">
              <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-5 gap-8 mb-8">
                <div>
                  <h3 className="text-white font-bold mb-3 uppercase tracking-wider">Productivity</h3>
                  <ul className="space-y-2">
                    <li><Link href="/drills/motor/timing-accuracy/time-estimation" className="hover:text-cyan-400 transition-colors">Temporal Precision</Link></li>
                    <li><Link href="/drills/cognitive/focus/focus-timer" className="hover:text-cyan-400 transition-colors">Pomodoro Sync</Link></li>
                    <li><Link href="/drills/cognitive" className="text-cyan-500 hover:text-cyan-400 transition-colors font-bold">All Productivity →</Link></li>
                  </ul>
                </div>
                <div>
                ...
                <div>
                  ...
                  <ul className="space-y-2">
                    ...
                    <li><Link href="/drills/fps" className="hover:text-cyan-400 transition-colors">FPS Training (21)</Link></li>
              ...
              <div className="border-t border-slate-900 pt-8 text-center">
                ...
                <p className="text-[9px] max-w-2xl mx-auto leading-relaxed mb-6">
                  Free online temporal precision and time estimation training drill. Open-source telemetry training platform. Free forever. No downloads required.
                </p>
