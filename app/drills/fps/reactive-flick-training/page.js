...
import { 
  ...
  Info, Award, Activity, Check, RefreshCw
} from 'lucide-react';
  ...
  useEffect(() => {
    ...
    function render(now) {
      ...
      if (now - lastWarpRef.current > warpIntervalRef.current) {
        teleport(cvs);
        lastWarpRef.current = now;
        // No penalty for timeout - just teleport
      }
          ...
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <p className={`${isDarkMode ? 'text-gray-400' : 'text-gray-500'}`}>Click teleporting target • 5 lives system</p>
              </div>
            </div>
          ...
          {isFullscreen && gameState === 'playing' && (
            <>
              ...
              <div className="absolute top-4 left-4 z-20 bg-black/50 backdrop-blur-sm rounded-lg px-4 py-2 text-white text-sm">
                Score: <span className="text-yellow-400">{flickScore}</span> | Hits: <span className="text-green-400">{hitCount}</span> | Lives: <span className="text-red-400">{lives}</span>
              </div>
            </>
          )}
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  <div className="space-y-2">
                    <div className="flex items-start gap-2">
                      ...
                      <p className={`text-sm ${isDarkMode ? 'text-gray-300' : 'text-gray-600'}`}>
                        <span className="font-semibold text-green-500">Click the green target</span> • Teleports on hit
                      </p>
                    </div>
                    <div className="flex items-start gap-2">
                  ...
                  <div className="space-y-2">
                    ...
                    <div className="flex items-start gap-2">
                      ...
                      <p className={`text-sm ${isDarkMode ? 'text-gray-300' : 'text-gray-600'}`}>
                        <span className="font-semibold text-cyan-500">Speed increases over time</span> • 700ms → 350ms
                      </p>
                    </div>
                  </div>
                </div>
