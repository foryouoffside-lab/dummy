...
import { 
  ...
  Info, Award, Activity, Check, Crosshair, RefreshCw
} from 'lucide-react';
...
export default function FlickShotPage() {
  ...
  useEffect(() => {
    const handleMouseDown = (e) => {
      ...
      if (dist < targetObj.r + 10) {
        const reactionTime = performance.now() - reactionStartTimeRef.current;
        
        const timeSaved = currentLifespanRef.current - targetObj.timer;
          ...
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <p className={`${isDarkMode ? 'text-gray-400' : 'text-gray-500'}`}>Elite flick shots • 5 lives system • Window shrinks</p>
              </div>
            </div>
          ...
          {isFullscreen && gameState === 'playing' && (
            <>
              ...
              <div className="absolute top-4 left-4 z-20 bg-black/50 backdrop-blur-sm rounded-lg px-4 py-2 text-white text-sm">
                Score: <span className="text-yellow-400">{score}</span> | Hits: <span className="text-green-400">{flickCount}</span> | Lives: <span className="text-red-400">{lives}</span>
              </div>
            </>
          )}
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  ...
                  <div className="space-y-2">
                    <div className="flex items-start gap-2">
                      ...
                      <p className={`text-sm ${isDarkMode ? 'text-gray-300' : 'text-gray-600'}`}>
                        <span className="font-semibold text-blue-500">5 Lives system</span> • Each miss uses 1 life
                      </p>
                    </div>
                    <div className="flex items-start gap-2">
