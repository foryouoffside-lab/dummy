...
export default function AttenixShapePage() {
  ...
  useEffect(() => {
    const savedBestScore = localStorage.getItem('attenixShapeBestScore');
    if (savedBestScore) {
  ...
  const updateBestScore = (finalScore) => {
    const currentBestScore = parseInt(localStorage.getItem('attenixShapeBestScore') || '0', 10);
    if (finalScore > currentBestScore) {
      localStorage.setItem('attenixShapeBestScore', finalScore.toString());
  ...
  const handleSuccess = (selectedShape) => {
    ...
    let pointsEarned = 0;
    if (reaction < 200) pointsEarned = 50;
    else if (reaction < 300) pointsEarned = 30;
    else if (reaction < 450) pointsEarned = 20;
    else if (reaction < 650) pointsEarned = 10;
    else pointsEarned = 5;
  ...
  const handleLeftClick = () => {
    ...
    if (currentShape === "CIRCLE") {
      ...
      showFeedback('✗ No shape! -10', 'error');
      playSound('fail');
    }
  };
        ...
        <div className="mb-6">
          ...
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-3">
              ...
              <div>
                <h1 className={`text-3xl font-bold ${isDarkMode ? 'text-white' : 'text-gray-900'}`}>Attenix: Neural Shape ID</h1>
          ...
          {gameState === 'start' && (
            <div className={`absolute inset-0 flex items-center justify-center backdrop-blur-sm rounded-xl z-40 ${isBoxDarkMode ? 'bg-gray-900/95' : 'bg-white/95'}`}>
              <div className={`rounded-2xl p-8 text-center max-w-md mx-4 shadow-xl border ${isBoxDarkMode ? 'bg-gray-800 border-gray-700' : 'bg-white border-gray-200'}`}>
                ...
                <h3 className={`text-2xl font-bold mb-2 ${isBoxDarkMode ? 'text-white' : 'text-gray-900'}`}>Attenix: Neural Shape ID</h3>
            ...
            <div className={`absolute inset-0 flex items-center justify-center backdrop-blur-sm rounded-xl z-40 ${isBoxDarkMode ? 'bg-gray-900/95' : 'bg-white/95'}`}>
              <div className={`rounded-2xl p-8 shadow-xl border w-[480px] ${isBoxDarkMode ? 'bg-gray-800 border-gray-700' : 'bg-white border-gray-200'}`}>
                ...
                <div className="grid grid-cols-2 gap-4 mb-8">
                  ...
                  <ResultCard label="Final Shutter" value={`${flashDuration}ms`} icon={<Timer className="w-4 h-4" />} color="text-blue-500" />
                </div>
            ...
            <div className={`rounded-xl border overflow-hidden ${isDarkMode ? 'bg-gray-800 border-gray-700' : 'bg-white border-gray-200'}`}>
              ...
              <div className="p-4">
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  <div className="space-y-2">
                    ...
                    <div className="flex items-start gap-2">
                      ...
                      <p className={`text-sm ${isDarkMode ? 'text-gray-300' : 'text-gray-600'}`}>Correct: <span className="font-semibold text-green-500">5-50 points</span> based on speed</p>
                    </div>
                  </div>
                  <div className="space-y-2">
                    <div className="flex items-start gap-2">
                      ...
                      <p className={`text-sm ${isDarkMode ? 'text-gray-300' : 'text-gray-600'}`}>Wrong: <span className="font-semibold text-orange-500">-15 points</span></p>
                    </div>
                    <div className="flex items-start gap-2">
                ...
                <div className={`mt-3 pt-3 border-t text-xs ${isDarkMode ? 'border-gray-700 text-gray-400' : 'border-gray-200 text-gray-500'} flex items-center justify-between`}>
                  <span>🎯 &lt;200ms: 50pts • &lt;300ms: 30pts • &lt;450ms: 20pts • &lt;650ms: 10pts • 650ms+: 5pts</span>
