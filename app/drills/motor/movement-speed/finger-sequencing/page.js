...
export default function FractalLinkPage() {
  ...
  const updateAccuracy = useCallback(() => {
    ...
    if (total > 0) {
      const acc = (successfulHitsRef.current / total) * 100;
  ...
  const handleTimeout = useCallback(() => {
    ...
    if (livesRef.current > 0) {
      ...
      if (livesRef.current === 0) {
        ...
        showFeedback(`⏰ Timeout! No penalty • ${livesRef.current}  left`, 'error');
      }
    } else {
    ...
    const handleMouseDown = () => {
      ...
      if (dist < target.r + 10) {
        ...
        if (activeIndexRef.current >= chain.length) {
          ...
          // +1 point per correct chain completion
          ...
          if (newStreak > bestStreak) {
            ...
            localStorage.setItem('fractalLinkBestStreak', newStreak.toString());
          }
          ...
          if (newStreak % 5 === 0 && newStreak > 0) {
            ...
            showFeedback(`✓ Chain complete! +1 point`, 'success');
          }
        ...
        // Clicked wrong spot - count as miss
        totalAttemptsRef.current++;
        setMisses(prev => prev + 1);
        ...
        if (livesRef.current > 0) {
          ...
          if (livesRef.current === 0) {
            ...
            showFeedback(`✗ Wrong spot! No penalty • ${livesRef.current}  left`, 'error');
          }
        } else {
          // Out of lives - apply penalty
          scoreRef.current = Math.max(0, scoreRef.current - PENALTY);
  ...
  }, [bestStreak, PENALTY, updateAccuracy]);
  ...
  }, [gameState, isBoxDarkMode, handleTimeout]);
          ...
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <p className={`${isDarkMode ? 'text-gray-400' : 'text-gray-500'}`}>+1 per chain • -1 penalty after lives empty • 3  • 60s</p>
              </div>
            </div>
          ...
          {gameState === 'start' && (
            <div className={`absolute inset-0 flex items-center justify-center backdrop-blur-sm rounded-xl z-40 ${isBoxDarkMode ? 'bg-gray-900/95' : 'bg-white/95'}`}>
              <div className={`rounded-2xl p-8 text-center max-w-md mx-4 shadow-xl border ${isBoxDarkMode ? 'bg-gray-800 border-gray-700' : 'bg-white border-gray-200'}`}>
                ...
                <p className={`mb-6 ${isBoxDarkMode ? 'text-gray-300' : 'text-gray-600'}`}>60-second challenge • Click largest to smallest</p>
                <button 
                  onClick={startGame} 
            ...
            <div className={`absolute inset-0 flex items-center justify-center backdrop-blur-sm rounded-xl z-40 ${isBoxDarkMode ? 'bg-gray-900/95' : 'bg-white/95'}`}>
              <div className={`rounded-2xl p-8 shadow-xl border w-[480px] ${isBoxDarkMode ? 'bg-gray-800 border-gray-700' : 'bg-white border-gray-200'}`}>
                ...
                <div className="grid grid-cols-2 gap-4 mb-8">
                  ...
                  <ResultCard label="Chains Done" value={sequencesCompleted} icon={<GitBranch className="w-4 h-4" />} color="text-green-500" isDark={isBoxDarkMode} />
                  <ResultCard label="Misses" value={misses} icon={<Activity className="w-4 h-4" />} color="text-red-500" isDark={isBoxDarkMode} />
                </div>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  <div className="space-y-2">
                    <div className="flex items-start gap-2">
                      ...
                      <p className={`text-sm ${isDarkMode ? 'text-gray-300' : 'text-gray-600'}`}>
                        Click nodes from <span className="font-semibold text-emerald-500">largest to smallest</span>
                      </p>
                    </div>
                    <div className="flex items-start gap-2">
                      <div className="w-5 h-5 rounded-full bg-green-500 flex items-center justify-center text-white text-xs font-bold mt-0.5">2</div>
                      <p className={`text-sm ${isDarkMode ? 'text-gray-300' : 'text-gray-600'}`}>
                        <span className="font-semibold text-green-500">+1 point per completed chain</span>
                      </p>
                    </div>
                    <div className="flex items-start gap-2">
                  ...
                  <div className="space-y-2">
                    <div className="flex items-start gap-2">
                      ...
                      <p className={`text-sm ${isDarkMode ? 'text-gray-300' : 'text-gray-600'}`}>
                        Each chain has <span className="font-semibold text-cyan-500">2 seconds</span> to complete
                      </p>
                    </div>
                    <div className="flex items-start gap-2">
