...
const guide = {
  ...
  intro: [
    ...
    "本『三角形追従（Triangular Pursuit）』ドリルは、この幾何学的アジリティをブラウザ上で徹底的に鍛え上げます。正三角形の軌道上を周回するターゲットを目で追い続けることで、直線区間の等速追従と角での急減速・再加速ダイナミクスを同時に習得できます。『Hide Line（軌道線非表示）』をオンにすることで視覚的なガイドラインを排除した純粋な感覚運動追従をテストでき、『Random Speed』により機械的なタイミング学習を排した臨場感あふれる適応訓練が可能です。高駆動モニター（Woods et al., 2015）の低遅延環境で、角のブレをゼロにする極限の眼球制御を体感してください。"
  ],
  benchmarks: {
