...
export default function ReactiveStrafeClient() {
  ...
  useEffect(() => { try { const s = localStorage.getItem('strafeDrillBestScore'); if (s) { const p = parseInt(s, 10); if (!isNaN(p)) setBestScore(p); } } catch (e) {} }, []);

  const updateBestScore = useCallback((finalScore) => { try { const c = parseInt(localStorage.getItem('strafeDrillBestScore') || '0', 10); if (finalScore > c) { localStorage.setItem('strafeDrillBestScore', finalScore.toString()); setBestScore(finalScore); } } catch (e) {} }, []);
  ...
  useEffect(() => {
    ...
    trackingIntervalRef.current = setInterval(() => {
      ...
      isOnTargetRef.current = dist <= radius + 5;
      ...
      if (isOnTargetRef.current) {
        ...
        while (trackingAccumulatorRef.current >= SCORE_INTERVAL) { scoreRef.current += 1; setScore(scoreRef.current); trackingAccumulatorRef.current -= SCORE_INTERVAL; playSound('combo'); showFeedback('✓ +1', 'success'); if (comboRef.current % 5 === 0) { showFeedback(`🔥 ${comboRef.current} Combo!`, 'success'); } }
      ...
      } else { trackingAccumulatorRef.current = 0; if (comboRef.current > 0) { comboRef.current = 0; setCombo(0); } }
    }, 50);
  ...
  useEffect(() => {
    ...
    function updateTarget() { if (!isActiveRef.current) return; const t = targetRef.current, r = t.size / 2; t.y = fixedYRef.current; if (Math.random() < t.changeChance) t.direction *= -1; t.x += t.speed * t.direction; if (t.x - r < 0) { t.x = r; t.direction = 1; } else if (t.x + r > cvs.width) { t.x = cvs.width - r; t.direction = -1; } }
    function draw() {
  ...
  const sharePage = async () => { if (navigator.share) { try { await navigator.share({ title: 'Free Reactive Strafe Tracking | SkillDrills', text: 'Train FPS tracking aim with adaptive speed! Free!', url: 'https://skilldrills.online/drills/fps/reactive-strafe' }); } catch (e) {} } else { navigator.clipboard.writeText('https://skilldrills.online/drills/fps/reactive-strafe'); alert('Link copied!'); } };
  const copyPageLink = () => { navigator.clipboard.writeText('https://skilldrills.online/drills/fps/reactive-strafe'); alert('Link copied!'); };
