...
export default function ClickAccuracyClient() {
  ...
  const addPenalty = useCallback((reason) => {
    ...
    else{scoreRef.current=Math.max(0,scoreRef.current-5);setScore(scoreRef.current);playSound('miss');showFeedback(`✗ ${reason}! -5 points`,'error');}
  ...
  useEffect(() => {
    const h = (e) => {
      ...
      if(dist<t.r){successfulHitsRef.current++;setTargetsHit(successfulHitsRef.current);const ns=streakRef.current+1;streakRef.current=ns;setStreak(ns);if(ns>bestStreakRef.current){bestStreakRef.current=ns;setBestStreak(ns);}scoreRef.current+=5;setScore(scoreRef.current);if(ns%10===0){playSound(ns>=20?'highstreak':'streak');showFeedback(`🔥 ${ns} Streak! +5`,'success');}else{playSound('hit');showFeedback(`✓ ${rt}ms | +5`,'success');}teleportTarget();}
  ...
  useEffect(() => {
    ...
    timerIntervalRef.current=setInterval(()=>{setTimeLeft(prev=>{if(prev<=1){setGameState('gameOver');gameStateRef.current='gameOver';isActiveRef.current=false;if(timerIntervalRef.current){clearInterval(timerIntervalRef.current);timerIntervalRef.current=null;}if(teleportIntervalRef.current){clearInterval(teleportIntervalRef.current);teleportIntervalRef.current=null;}updateBestScore(scoreRef.current);return 0;}return prev-1;});},1000);
  ...
  useEffect(()=>()=>{if(timerIntervalRef.current)clearInterval(timerIntervalRef.current);if(teleportIntervalRef.current)clearInterval(teleportIntervalRef.current);if(animationRef.current)cancelAnimationFrame(animationRef.current);},[]);
