  ...
  useEffect(() => {
    ...
    const loop = (now) => {
      ...
      if (chRef.x > 0 && chRef.x < cw && chRef.y > 0 && chRef.y < ch) {
        const chColor = pointerLocked ? '#00ff88' : '#ffbb00';
        ...
        ctx.lineWidth = 2;
        const gap = 8 + chRef.expand;
        const len = 6;

        ctx.beginPath();
        ctx.moveTo(chRef.x - gap - len, chRef.y); ctx.lineTo(chRef.x - gap, chRef.y);
        ctx.moveTo(chRef.x + gap, chRef.y); ctx.lineTo(chRef.x + gap + len, chRef.y);
        ctx.moveTo(chRef.x, chRef.y - gap - len); ctx.lineTo(chRef.x, chRef.y - gap);
        ctx.moveTo(chRef.x, chRef.y + gap); ctx.lineTo(chRef.x, chRef.y + gap + len);
        ctx.stroke();
      }
                ...
                <div className="space-y-4">
                  ...
                  <div>
                    ...
                    <input 
                      ...
                      className="w-full bg-slate-950 border border-slate-800 rounded px-2.5 py-2 text-xs text-white focus:outline-none focus:border-purple-500/50"
                    />
                  </div>
                  <div>
