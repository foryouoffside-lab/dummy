...
export default function FlowInductionClient() {
  ...
  useEffect(() => {
    ...
    timerIntervalRef.current = setInterval(() => { setTimeLeft(prev => { if (prev <= 1) { setGameState('gameOver'); gameStateRef.current = 'gameOver'; isActiveRef.current = false; if (timerIntervalRef.current) { clearInterval(timerIntervalRef.current); timerIntervalRef.current = null; } updateBestScore(Math.floor(scoreRef.current)); document.exitPointerLock(); return 0; } return prev - 1; }); }, 1000);
