...
export default function useGameEngine({
  ...
  const playSound = useCallback((type) => {
    try {
      ...
      const fm = { correct: 880, wrong: 440, penalty: 220, combo: 1046.5, newBest: 1318.5 };
      const vol = { correct: 0.1, wrong: 0.12, penalty: 0.15, combo: 0.12, newBest: 0.15 };
  ...
  const recordCorrectAction = useCallback(({
    ...
    if (c % 5 === 0 && c > 0) {
      playSound('combo');
      showFeedback(`🔥 ${c}x Combo! (${pointsToAdd}pts)`, 'success');
    } else {
      playSound('correct');
      let msg = `✓ +${pointsToAdd}`;
  ...
  }, [drillName, drillId, score, bestScore, accuracy, bestCombo, rating, newBest, category, showFeedback, sharePath]);

  const downloadScore = useCallback(async () => {
    try {
      const playerName = getPlayerName();
      const canvas = generateShareCard({
        score, bestScore, accuracy, bestCombo, rating, newBest, drillName, playerName,
      });
  ...
  }, [drillName, drillId, score, bestScore, accuracy, bestCombo, rating, newBest, showFeedback]);

  const copyScore = useCallback(() => {
    const text = `🎯 ${drillName}: ${score} pts | Accuracy: ${accuracy}% | Best Combo: ${bestCombo}x | Play free at skilldrills.online 🎮`;
    navigator.clipboard.writeText(text);
    showFeedback('📋 Copied!', 'success');
  }, [drillName, score, accuracy, bestCombo, showFeedback]);
  ...
  return {
    ...
    setGameState, setScore, setCombo, setLives,
  };
}