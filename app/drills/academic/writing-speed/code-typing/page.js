...
export default function CodeTypingDrill() {
  ...
  // Progressive Code Snippets Database - Points removed, 1 point per completion
  ...
  const handleInputChange = (e) => {
    ...
    if (val === currentSnippet.code) {
      ...
      // 1 point per completion + combo bonus
      const basePoints = 1;
      const comboBonus = Math.floor(combo / 3) * 1; // +1 bonus every 3 combo
          ...
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <p className={`${isDarkMode ? 'text-gray-400' : 'text-gray-500'}`}>1 point per completion • Combo bonuses</p>
              </div>
            </div>
            ...
            {gameState === 'start' && (
              <div className={`absolute inset-0 flex items-center justify-center backdrop-blur-sm rounded-xl z-40 ${isBoxDarkMode ? 'bg-gray-900/95' : 'bg-[#1e1e1e]/95'}`}>
                <div className={`rounded-2xl p-8 text-center max-w-md mx-4 shadow-xl border ${isBoxDarkMode ? 'bg-gray-800 border-gray-700' : 'bg-gray-800 border-gray-700'}`}>
                  ...
                  <p className={`mb-6 text-gray-300`}>{language} • {availableSnippets.length} snippets • 1 point per completion</p>
                  <button 
                    onClick={startTest}
              ...
              <div className="w-full h-full flex flex-col">
                <div className="mb-4 flex justify-between items-center">
                  <div>
                    ...
                    <span className="text-gray-400 text-sm mx-2">•</span>
                    <span className="text-yellow-400 text-sm">+1 point per completion</span>
                  </div>
