...
export default function TriangularPursuitClient() {
  ...
  const sharePage = useCallback(async () => {
    const url = 'https://skilldrills.online/drills/visual-tracking/triangular-pursuit';
    try {
      const canvas = generateSessionCard({
        drillName: 'Triangular Pursuit',
      ...
      const text = 'Triangular Pursuit — Free Visual Tracking & Gaze Calibration Drill!';
      if (typeof navigator !== 'undefined' && navigator.share) {
        try {
          await navigator.share({ title: 'Triangular Pursuit Drill', text, url });
        } catch (e) {}
  ...
  // Canvas Render Loop (Triangular Pursuit Physics)
  useEffect(() => {
    ...
    const draw = (ts: number) => {
      ...
      const nodes = trackingState.current.nodes;
      if (nodes.length === 3) {
        // Render Triangular Path Guide Line ONLY if math is NOT invisible
        if (!mathInvisible) {
        ...
        // Triangular Vertex Interpolation Physics
        trackingState.current.t += 0.65 * scaledDt;
        ...
        const nCurr = nodes[trackingState.current.currentNode];
        const nNext = nodes[trackingState.current.nextNode];
        ...
        <header className="border-b border-white/5 bg-[#080811]/80 backdrop-blur-md sticky top-0 z-50">
          <div className="max-w-6xl mx-auto px-4 h-14 flex items-center justify-between">
            <div className="flex items-center gap-2 text-xs text-slate-400">
              ...
              <span>/</span>
              <span className="text-red-400 font-medium">Triangular Pursuit</span>
            </div>
        ...
        {!isFullscreen && (
          <div className="text-center">
            ...
            <p className="text-xs text-slate-400 mt-1">
              Geometric Vertex Tracking & Angular Direction Shifts
            </p>
          </div>
        )}
