...
import { 
  ArrowLeft, Grid3X3, Volume2, VolumeX, Maximize2, Minimize2, 
  Sun, Moon, Eye, Timer, Trophy, CheckCircle2, Target, RefreshCw,
  BarChart3, Info, Zap, Award
} from 'lucide-react';
    ...
    <div className={`min-h-screen select-none ${isDarkMode ? 'bg-gray-900' : 'bg-gray-50'}`}>
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
          ...
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <p className={`${isDarkMode ? 'text-gray-400' : 'text-gray-500'}`}>4×4 → 5×5 → 6×6 → 7×7 • 60s</p>
              </div>
            </div>
        ...
        <div className="grid grid-cols-7 gap-3 mb-4 h-[88px]">
          ...
          <StatCard icon={<CheckCircle2 className="text-emerald-500" />} value={`${completedGrids}/4`} label="Completed" isDark={isDarkMode} />
            ...
            {gameState === 'start' && (
              <div className={`absolute inset-0 flex items-center justify-center backdrop-blur-sm rounded-xl z-40 ${isBoxDarkMode ? 'bg-gray-900/95' : 'bg-white/95'}`}>
                <div className={`rounded-2xl p-8 text-center max-w-md mx-4 shadow-xl border ${isBoxDarkMode ? 'bg-gray-800 border-gray-700' : 'bg-white border-gray-200'}`}>
                  ...
                  <p className={`mb-6 ${isBoxDarkMode ? 'text-gray-300' : 'text-gray-600'}`}>60-second challenge • 4×4 → 7×7</p>
                  <button 
                    onClick={startGame}
              ...
              <div className={`absolute inset-0 flex items-center justify-center backdrop-blur-sm rounded-xl z-40 ${isBoxDarkMode ? 'bg-gray-900/95' : 'bg-white/95'}`}>
                <div className={`rounded-2xl p-8 shadow-xl border w-[480px] ${isBoxDarkMode ? 'bg-gray-800 border-gray-700' : 'bg-white border-gray-200'}`}>
                  ...
                  <div className="grid grid-cols-2 gap-4 mb-8">
                    ...
                    <ResultCard label="Mistakes" value={mistakes} icon={<Target className="w-4 h-4" />} color="text-red-500" />
                    <ResultCard label="Best Combo" value={`${bestCombo}x`} icon={<Zap className="w-4 h-4" />} color="text-orange-500" />
                    <ResultCard label="Final Grid" value={getGridLabel()} icon={<Award className="w-4 h-4" />} color="text-blue-500" />
                  </div>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  ...
                  <div className="space-y-2">
                    <div className="flex items-start gap-2">
                      ...
                      <p className={`text-sm ${isDarkMode ? 'text-gray-300' : 'text-gray-600'}`}>Complete grid: <span className="font-semibold text-amber-500">+100 × grid size bonus</span></p>
                    </div>
                    <div className="flex items-start gap-2">
