...
export default function DropCatchClient() {
  ...
  const endGame = useCallback(() => {
    ...
    const finalAccuracy = e.totalAttempts > 0 ? Math.round((e.catches / e.totalAttempts) * 100) : 0;
  ...
  const spawnBall = useCallback((width) => {
    ...
    const isFake = Math.random() < 0.25; // 25% chance of Red Decoy
    const speedVariation = (Math.random() - 0.5) * 60;
  ...
  const applyPenalty = useCallback((type) => {
    ...
    if (type === 'miss') {
      e.misses++;
      e.score = Math.max(0, e.score - 2); // -2 PTS
      e.timeLeft -= 1.0; // -1s Time
      if (audioSynth) audioSynth.playSound('miss');
    } else if (type === 'decoy') {
      e.decoyHits++;
      e.score = Math.max(0, e.score - 5); // -5 PTS
      e.timeLeft -= 2.0; // -2s Time (Heavy Penalty)
      e.screenShake = 15;
    ...
    e.baseSpeed = Math.max(400, e.baseSpeed - 25);
    e.spawnDelay = Math.min(0.8, e.spawnDelay + 0.05);
  ...
  const handleCatch = useCallback(() => {
    ...
    if (e.streak % 5 === 0) {
      ...
      if (audioSynth) audioSynth.playSound('catch');
    }

    // Adaptive Difficulty: Speed up
    if (e.streak % 2 === 0) {
      e.baseSpeed = Math.min(1200, e.baseSpeed + 25); // Max out at 1200px/s
      e.spawnDelay = Math.max(0.3, e.spawnDelay - 0.02); // Max spawn rate 0.3s
    }
    ...
    const handleMouseDown = (e) => {
      ...
      if (gameState === 'playing' && pointerLocked && isActiveRef.current) {
        ...
        if (hitIndex !== -1) {
          ...
          if (b.isFake) {
            applyPenalty('decoy');
            createExplosion(b.x, b.y, '#ef4444'); // Red explosion
          } else {
            handleCatch();
            createExplosion(b.x, b.y, '#10b981'); // Emerald explosion
          }
  ...
  }, [gameState, pointerLocked, universalSens, applyPenalty, handleCatch]);
  ...
  useEffect(() => {
    ...
    const loop = (time) => {
      ...
      if (gameState === 'playing' && pointerLocked && isActiveRef.current) {
        ...
        // Spawning Logic
        e.spawnTimer += dt;
        if (e.spawnTimer >= e.spawnDelay && e.balls.length < 10) {
          spawnBall(cvs.width);
          e.spawnTimer = 0;
        }

        // Falling & Cleanup Logic
      ...
      // Environment Grid
      ctx.strokeStyle = 'rgba(59, 130, 246, 0.04)'; // Blue tint
      ctx.lineWidth = 1; 
      ...
      if (gameState === 'playing' || gameState === 'start') {
        for (const b of e.balls) {
          ...
          ctx.fillStyle = b.isFake ? "#ef4444" : "#10b981"; // Red / Emerald
          ctx.shadowBlur = 10;
      ...
      if (ch.initialized && (gameState === 'playing' || gameState === 'start')) {
        const activeColor = pointerLocked ? '#06b6d4' : '#f59e0b';
  ...
  }, [gameState, pointerLocked, spawnBall, applyPenalty]);

  const shareDrillLink = useCallback(() => {
    const url = typeof window !== 'undefined' ? window.location.href : '';
    if (navigator.share) {
      navigator.share({ title: 'Reflex Drop Catch', url }).catch(() => {});
    } else {
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
          ...
          <div className="mb-6">
            <nav className="mb-4">
              <ol className="flex flex-wrap items-center gap-2 text-sm text-gray-500">
                ...
                <li><ChevronRight className="w-4 h-4 text-gray-600" /></li>
                <li className="text-cyan-400 font-medium">Reflex Drop Catch</li>
              </ol>
            </nav>
            ...
            <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
              <div className="flex items-center gap-4">
                ...
                <div>
                  ...
                  <p className="text-sm text-gray-400 mt-1 font-medium">Desktop Exclusive • Target Discrimination</p>
                </div>
              </div>
        ...
        {!isFullscreen && (
          <div className="grid grid-cols-4 lg:grid-cols-7 gap-2 mb-2">
            ...
            <StatCard icon={<TrendingUp className="text-orange-400" />} value={currentSpeed} label="Base Speed" unit="px/s" />
          ...
          {gameState === 'start' && (
            <div className="absolute inset-0 flex items-center justify-center z-50 bg-black/70 backdrop-blur-md p-4 overflow-y-auto">
              <div className="rounded-3xl p-8 text-center max-w-lg w-full border border-gray-700 bg-gray-900 shadow-2xl my-auto">
                ...
                <h2 className="text-3xl font-black mb-3 tracking-tight text-white uppercase">Reflex Drop Catch</h2>
                <p className="text-sm mb-6 text-gray-400 leading-relaxed">
                  Catch the green balls before they fall off the screen. Do not click the red decoy balls marked with an X. Speed and spawn rates accelerate aggressively as your streak builds.
                </p>
                ...
                >
                  <Play className="w-6 h-6 fill-white" /> BEGIN REFLEX DRILL
                </button>
              </div>
            </div>
          )}
                      ...
                      <div className="space-y-3 sm:border-r border-gray-800 sm:pr-6">
                        ...
                        <ul className="space-y-2">
                          <li className="flex justify-between items-center bg-gray-900/50 p-2 rounded border border-gray-800">
                            <span className="text-gray-400">Total Valid Catches:</span>
                            <span className="font-bold text-blue-400">{analytics.catches}</span>
                          </li>
                          <li className="flex justify-between items-center bg-gray-900/50 p-2 rounded border border-gray-800">
                            <span className="text-gray-400">Dropped Misses:</span>
                            <span className={`font-bold ${analytics.misses > 5 ? 'text-yellow-500' : 'text-green-500'}`}>{analytics.misses}</span>
                          </li>
                          <li className="flex justify-between items-center bg-gray-900/50 p-2 rounded border border-gray-800">
                            <span className="text-gray-400">Fatal Decoy Hits:</span>
                            <span className={`font-bold ${analytics.decoyHits > 3 ? 'text-red-500' : 'text-orange-400'}`}>{analytics.decoyHits}</span>
                          </li>
                          <li className="flex justify-between items-center bg-gray-900/50 p-2 rounded border border-gray-800">
                            <span className="text-gray-400">Max Survival Streak:</span>
                            <span className="font-bold text-cyan-400">{analytics.maxStreak}</span>
                          </li>
                        </ul>
                      </div>
                        ...
                        <div>
                          ...
                          <p className="text-gray-400 leading-relaxed font-sans">
                            {analytics.decoyHits > 3 ? (
                              <span className="text-red-300">You are relying on instinct over visual discrimination. You are clicking before fully identifying the target. Decoy hits severely drain your clock. Force yourself to look for the white 'X' before firing.</span>
                            ) : analytics.misses > 5 ? (
                              <span className="text-yellow-300">The drill dynamically speeds up falling velocities up to {analytics.peakSpeed}px/s. You are struggling to acquire targets at high speeds. Track targets from top to bottom.</span>
                            ) : (
                              <span className="text-green-300">Excellent target discrimination! You navigated the high-velocity drops flawlessly while avoiding decoys. Your impulse control is highly tuned.</span>
                            )}
                          </p>
                        </div>
                      </div>
            ...
            <div className="rounded-2xl border border-gray-800 overflow-hidden bg-gray-900 shadow-2xl pointer-events-none">
              ...
              <div className="p-6 grid grid-cols-1 md:grid-cols-2 gap-6">
                <div className="space-y-5">
                  <RuleItem num="1" color="green" text="Catch Green Targets" highlight="+5 PTS | +1.0s Time" result="Speed heavily scales up" />
                  <RuleItem num="2" color="yellow" text="Missed Targets" highlight="-2 PTS | -1.0s Time" result="Speed scales down slightly" />
                </div>
                <div className="space-y-5">
                  <RuleItem num="3" color="red" text="Hit Red Decoys" result="-5 PTS | -2.0s Heavy Time Penalty" />
          ...
          <section className="mt-12" aria-label="About this drill">
            <div className="rounded-2xl border border-gray-800 overflow-hidden bg-gray-900 shadow-xl">
              <div className="px-6 py-5 border-b border-gray-800 bg-black/40 flex items-center gap-3">
                ...
                <h2 className="font-bold text-white text-lg tracking-wide">About Reflex Drop Catch</h2>
              </div>
              <div className="p-8">
                <p className="text-sm leading-relaxed mb-6 text-gray-300">
                  This target discrimination drill trains impulse control and reflex acquisition. By forcing you to rapidly differentiate between valid green targets and heavily-penalized red decoys moving at high speeds, you build the cognitive override necessary to prevent misclicks and friendly-fire incidents in high-pressure scenarios.
                </p>
                ...
                <div className="grid grid-cols-1 sm:grid-cols-3 gap-5 mb-8">
                  <div className="p-5 rounded-xl border border-gray-800 bg-black/40">
                    ...
                    <p className="text-xs leading-relaxed text-gray-400">Tactical FPS players (Valorant, CS2, R6) looking to improve target discrimination speed and eliminate reflexive panic-firing.</p>
                  </div>
                  <div className="p-5 rounded-xl border border-gray-800 bg-black/40">
                    <div className="flex items-center gap-3 mb-3">
                    ...
                    <p className="text-xs leading-relaxed text-gray-400">Target discrimination, impulse control, vertical reflex acquisition, and visual filtering under stress.</p>
                  </div>
                  <div className="p-5 rounded-xl border border-gray-800 bg-black/40">
                    <div className="flex items-center gap-3 mb-3">
                    ...
                    <p className="text-xs leading-relaxed text-gray-400">Net Score, accuracy percentage, max survival streak, and total fatal decoy errors.</p>
                  </div>
                </div>
                ...
                <div className="mb-8 bg-[#0b0f19]/40 border border-gray-800 rounded-xl p-6">
                  ...
                  <div className="grid sm:grid-cols-2 gap-6 text-sm text-gray-300">
                    <ol className="space-y-3 list-decimal pl-5">
                      ...
                      <li>Catch (click) the green targets before they drop off the screen.</li>
                    </ol>
                    <ul className="space-y-3">
                      <li className="flex items-center gap-2"><CheckCircle2 className="w-4 h-4 text-green-500" /> <span className="text-white font-bold">Valid Hit:</span> +5 PTS and +1.0s to clock. Speeds up spawn rate.</li>
                      <li className="flex items-center gap-2"><XCircle className="w-4 h-4 text-red-500" /> <span className="text-white font-bold">Decoy Error:</span> Hitting a Red X deducts -5 PTS and -2.0s.</li>
                    </ul>
                  </div>
                </div>
                ...
                <div className="p-5 rounded-xl border border-gray-800 bg-black/40">
                  ...
                  <div className="space-y-4">
                    <div>
                      ...
                      <p className="text-xs text-gray-400 mt-1">Unlike standard aim trainers, this drill actively punishes bad accuracy and target mismanagement. Missing a green target deducts 1 full second, but hitting a red decoy violently drains 2 full seconds from your clock. You must rely on accuracy over spamming.</p>
                    </div>
                    <div>
                      <h4 className="text-sm font-bold text-gray-200">How should I approach this drill?</h4>
                      <p className="text-xs text-gray-400 mt-1">Look higher up the screen. If you wait for the targets to hit the middle before identifying them, you won't have time to react at higher difficulty scales. Identify the target color immediately upon spawning, then track it downward to fire.</p>
                    </div>
                  </div>
                </div>
              </div>
