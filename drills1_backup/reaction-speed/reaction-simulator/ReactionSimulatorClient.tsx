...
import { 
  ...
  Lightbulb, Crosshair, TrendingUp, AlertTriangle, Lock, Unlock
} from 'lucide-react';
...
const getLevelParameters = (lvl: number) => {
  ...
  let fallSpeedFactor = 0.22; // multiplier of height for fall speed
...
export default function ReactionSimulatorClient() {
  ...
  // Personal Bests loaded on startup
  ...
  const getTargetRadius = useCallback((W: number, H: number, currentLevel: number, isMicro = false) => {
    ...
    if (isMobileDevice) {
      ...
      return Math.max(8, Math.round(18 * screenFactor * sizeMult * deviceScale));
    } else {
      const baseRadius = 35;
      if (isFullscreen) {
        return Math.max(10, Math.round(baseRadius * sizeMult));
      } else {
        return Math.max(10, Math.round(baseRadius * (H / 1080) * sizeMult));
      }
    }
  ...
  useEffect(() => {
    ...
    if (typeof window !== 'undefined') {
      ...
      try {
        ...
        const storedAcc = localStorage.getItem('skilldrills_reaction-simulator_bestAccuracy');
        if (storedAcc) setBestAccuracy(parseInt(storedAcc, 10));

        const storedRt = localStorage.getItem('skilldrills_reaction-simulator_bestReaction');
        if (storedRt) setBestReaction(parseInt(storedRt, 10));

        const storedLvl = localStorage.getItem('skilldrills_reaction-simulator_bestLevel');
        ...
        if (timeLeftRef.current <= 0) {
          ...
          try {
            ...
            if (scoreRef.current > currentBest) {
              ...
              localStorage.setItem('skilldrills_reaction-simulator_best', scoreRef.current.toString());
            ...
            if (overallAccPercent > currentBestAcc && hitsRef.current >= 5) {
              localStorage.setItem('skilldrills_reaction-simulator_bestAccuracy', overallAccPercent.toString());
            ...
            if (currentBestRtVal < currentBestRt && currentBestRtVal > 0) {
              localStorage.setItem('skilldrills_reaction-simulator_bestReaction', Math.round(currentBestRtVal).toString());
            ...
            if (levelRef.current > currentBestLevel) {
              localStorage.setItem('skilldrills_reaction-simulator_bestLevel', levelRef.current.toString());
  ...
  useEffect(() => {
    ...
    const drawLoop = (ts: number) => {
      ...
      {
        // Update targets positions & trails
        ...
        for (let i = targets.length - 1; i >= 0; i--) {
          ...
          if (t.y >= H + radius + 15) {
            // Target Escaped
            handleMiss(t.x, t.y, true);
            targets.splice(i, 1);
          }
        }

        // Update fake warnings
        ...
        if (trackingState.current.spawnTimer > spawnInterval) {
          hasPenalizedRef.current = false; // Reset penalize flag on new spawn wave
          
          // Base vertical falling speed
          const baseFallSpeed = H * fallSpeedFactor;
          
          // Determine spawn size / wave type
          let dropCount = 1;
          ...
          // Mixed velocities target drops
          for (let d = 0; d < dropCount; d++) {
            // Lane horizontal distribution
            ...
            // Random mixed velocity variant (0.85 to 1.15)
          ...
          // Rare fake warning indicator spawning
      ...
      if (trailEffect) {
        ...
        for (let t of targets) {
          if (t.trail.length > 1) {
            const radius = getTargetRadius(W, H, currentLevel, t.isMicro);
            for (let tr = 0; tr < t.trail.length; tr++) {
              const alpha = (tr / t.trail.length) * 0.12;
              ctx.fillStyle = targetColor;
              ctx.globalAlpha = alpha;
              ctx.beginPath();
              ctx.arc(t.trail[tr].x, t.trail[tr].y, radius * (0.4 + (tr / t.trail.length) * 0.6), 0, Math.PI * 2);
              ctx.fill();
            }
            ctx.globalAlpha = 1.0;
          }
      ...
      for (let t of targets) {
        ...
        ctx.shadowColor = t.isSpeedBurst ? '#f59e0b' : targetColor; // Orange glow for speed bursts
        ctx.fillStyle = t.isSpeedBurst ? '#f59e0b' : targetColor;
        ctx.beginPath();
        ...
        // Speed burst indicators (outer fiery ring)
        if (t.isSpeedBurst) {
      ...
      for (let fw of fakeWarnings) {
        ...
        // Draw alert exclamation warning block at the top boundary
  ...
  const handlePointerDown = (e: React.PointerEvent<HTMLCanvasElement>) => {
    ...
    for (let i = targets.length - 1; i >= 0; i--) {
      const t = targets[i];
      const radius = getTargetRadius(cvs.width, cvs.height, currentLevel, t.isMicro);
      
      const dx = x - t.x;
      const dy = y - t.y;
      const dist = Math.hypot(dx, dy);
      
      const hitRadius = radius * (isMobileDevice ? 2.25 : 1.75); // generous margins
          ...
          <div className={`${isFullscreen ? 'col-span-4' : isMobileDevice ? 'col-span-4' : 'lg:col-span-3'} flex flex-col relative`}>
            ...
            {!isFullscreen && !isMobileLandscape && (
              <div className="mt-3 text-center text-[10px] text-slate-550 flex items-center justify-center gap-2 font-mono">
                ...
                <span>Drill is fully interactive. Hit targets to gain time (+0.5s). Misses and escapes deduct (-1s). One penalty per target.</span>
              </div>
            )}
          </div>
        </div>
              ...
              <div className="bg-[#0b0f19] border-t border-gray-800 p-8">
                <div className="p-5 rounded-xl border border-gray-800 bg-black/40">
                  ...
                  <div className="space-y-5">
                    ...
                    <div>
                      <h4 className="text-sm font-bold text-gray-200 tracking-tight">How often should I practice Reaction Simulator?</h4>
