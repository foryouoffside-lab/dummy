...
import { 
  ...
  Activity, Award, Info, Check, Gauge, RefreshCw
} from 'lucide-react';
    ...
    const handleMouseDown = () => {
      ...
      if (dist < target.r) {
        if (isLitRef.current) {
          ...
          // Only penalize when user actually clicks (early click)
          handleEarlyClick();
        }
      }
    };
    ...
    function render(now) {
      ...
      if (isActiveRef.current) {
        ...
        if (isLitRef.current && flashTimerRef.current > 0.20) {
          isLitRef.current = false;
          reactionStartTimeRef.current = 0;
          ...
          {isFullscreen && gameState === 'playing' && (
            <>
              ...
              <div className="absolute top-4 left-4 z-20 bg-black/50 backdrop-blur-sm rounded-lg px-4 py-2 text-white text-sm">
                Score: <span className="text-yellow-400">{score}</span> | Combo: <span className="text-purple-400">{combo}x</span> | Lives: <span className="text-red-400">{lives}</span>
              </div>
            </>
          )}
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  <div className="space-y-2">
                    ...
                    <div className="flex items-start gap-2">
                      ...
                      <p className={`text-sm ${isDarkMode ? 'text-gray-300' : 'text-gray-600'}`}>
                        <span className="font-semibold text-green-500">+1 point for correct click</span> • Simple scoring system
                      </p>
                    </div>
                    <div className="flex items-start gap-2">
                      ...
                      <p className={`text-sm ${isDarkMode ? 'text-gray-300' : 'text-gray-600'}`}>
                        <span className="font-semibold text-purple-500">Build combo streaks</span> • 5 combo streak bonus sound
                      </p>
                    </div>
                  </div>
                  <div className="space-y-2">
                    <div className="flex items-start gap-2">
                      ...
                      <p className={`text-sm ${isDarkMode ? 'text-gray-300' : 'text-gray-600'}`}>
                        <span className="font-semibold text-blue-500">5 Lives system</span> • Each wrong click uses 1 life
                      </p>
                    </div>
                    <div className="flex items-start gap-2">
                    ...
                    <div className="flex items-start gap-2">
                      ...
                      <p className={`text-sm ${isDarkMode ? 'text-gray-300' : 'text-gray-600'}`}>
                        <span className="font-semibold text-yellow-500">No penalty for missing</span> • Only clicks count against you
                      </p>
                    </div>
                  </div>
                </div>
