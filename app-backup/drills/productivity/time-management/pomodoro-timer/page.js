...
import { 
  ...
  Eye, Timer, Play, Pause, RotateCcw, Brain, X, Info
} from 'lucide-react';
    ...
    if (runningRef.current && secondsRef.current > 0) {
      ...
      if (!soundPlayedRef.current) {
        ...
        if (!isBreakRef.current) {
          ...
          // Bonus points for completing Pomodoro
          let bonusPoints = 25;
          if (newCount % 4 === 0) {
            bonusPoints += 25;
            playSound('complete');
            showFeedback(`🎉 4 Pomodoros! +${bonusPoints} Bonus!`, 'success');
          } else {
            playSound('focus');
            showFeedback(`✓ Pomodoro Complete! +${bonusPoints} Bonus!`, 'success');
          }
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  <div className="space-y-2">
                    ...
                    <div className="flex items-start gap-2">
                      ...
                      <p className={`text-sm ${isDarkMode ? 'text-gray-300' : 'text-gray-600'}`}>
                        <span className="font-semibold text-blue-500">Complete Pomodoro: +25 bonus points</span> • Every 4th: +50 bonus
                      </p>
                    </div>
                    <div className="flex items-start gap-2">
                      <div className="w-5 h-5 rounded-full bg-red-500 flex items-center justify-center text-white text-xs font-bold mt-0.5">3</div>
                      <p className={`text-sm ${isDarkMode ? 'text-gray-300' : 'text-gray-600'}`}>
                        <span className="font-semibold text-red-500">Reset timer: -5 points</span> • Use reset wisely
                      </p>
                    </div>
                  </div>
                  <div className="space-y-2">
                    <div className="flex items-start gap-2">
                      ...
                      <p className={`text-sm ${isDarkMode ? 'text-gray-300' : 'text-gray-600'}`}>
                        <span className="font-semibold text-purple-500">5 mistakes reset your score</span> • Stay consistent
                      </p>
                    </div>
                    <div className="flex items-start gap-2">
