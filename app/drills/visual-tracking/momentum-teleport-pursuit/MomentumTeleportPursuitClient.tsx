...
export default function MomentumTeleportPursuitClient() {
  ...
  const sharePage = useCallback(async () => {
    const url = 'https://skilldrills.online/drills/visual-tracking/momentum-teleport-pursuit';
    try {
      const canvas = generateSessionCard({
        drillName: 'Momentum Teleport Pursuit',
      ...
      const text = 'Momentum Teleport Pursuit — Free Visual Tracking & Gaze Calibration Drill!';
      if (typeof navigator !== 'undefined' && navigator.share) {
        try {
          await navigator.share({ title: 'Momentum Teleport Pursuit Drill', text, url });
        } catch (e) {}
  ...
  // Canvas Render Loop (Momentum Teleport Physics)
  useEffect(() => {
    ...
    const draw = (ts: number) => {
      ...
      // Teleportation Timer Logic (instant position jump while preserving velocity vector momentum)
      trackingState.current.timer += scaledDt * 1000;
      const teleportInterval = randomSpeed ? 800 + Math.random() * 600 : 1200;

      if (trackingState.current.timer > teleportInterval) {
        trackingState.current.px = targetSize * 2 + Math.random() * (W - targetSize * 4);
        trackingState.current.py = targetSize * 2 + Math.random() * (H - targetSize * 4);
      ...
      // Render Momentum Vector Direction Line ONLY if math is NOT invisible
      if (!mathInvisible) {
        ...
        <header className="border-b border-white/5 bg-[#080811]/80 backdrop-blur-md sticky top-0 z-50">
          <div className="max-w-6xl mx-auto px-4 h-14 flex items-center justify-between">
            <div className="flex items-center gap-2 text-xs text-slate-400">
              ...
              <span>/</span>
              <span className="text-red-400 font-medium">Momentum Teleport Pursuit</span>
            </div>
          ...
          <div className="[&>div]:!mt-0">
            ...
            >
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <RuleItem num="1" text="Teleport Pursuit" highlight="Pure Visual" result="Predict momentum trajectory" />
            ...
            <DrillAccordion
              id="about"
              title="About Momentum Teleport Pursuit"
              ...
              <div className="space-y-6 font-sans">
                <section>
                  <h4 className="text-base font-bold text-white mb-2">
                    What Is Momentum Teleport Pursuit Training?
                  </h4>
                  <p className="text-sm leading-relaxed mb-3 text-gray-300">
                    <strong>Momentum Teleport Pursuit Training</strong> conditions rapid gaze re-acquisition by forcing your eyes to snap to new target coordinates after instant teleportation while predicting its continuous momentum trajectory.
                  </p>
                  <p className="text-sm leading-relaxed text-gray-300">
                    By utilizing features like <strong>Hide Line</strong> and <strong>Random Speed Acceleration</strong>, your visual cortex learns to track unpredictable teleporting trajectories independently.
                  </p>
                </section>
                ...
                <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
                  <div className="p-4 rounded-xl border border-gray-800 bg-white/[0.02]">
                    ...
                    <p className="text-xs text-gray-300 leading-relaxed">Gamers, athletes, and vision training practitioners looking to build rapid target re-acquisition.</p>
                  </div>
                  <div className="p-4 rounded-xl border border-gray-800 bg-white/[0.02]">
                    <h5 className="text-xs font-bold text-white mb-1.5">Momentum Preservation</h5>
                    <p className="text-xs text-gray-300 leading-relaxed">Forces eye muscles to anticipate continuous motion direction immediately post-teleport.</p>
                  </div>
                  <div className="p-4 rounded-xl border border-gray-800 bg-white/[0.02]">
            ...
            >
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4 font-sans">
                <FAQItem q="What is the Momentum Teleport Pursuit drill?" a="The Momentum Teleport Pursuit drill conditions visual tracking agility by forcing your eyes to snap to new target coordinates after instant teleportation while predicting its continuous momentum trajectory." />
                ...
                <FAQItem q="What does the Random Speed feature do?" a="Random Speed introduces unpredictable acceleration bursts and random teleport intervals, strengthening eye muscle modulation under erratic speeds." />
