...
import { 
  ...
  Eye, Timer, Radio, Brain, X, Trophy, Info, Check
} from 'lucide-react';
...
export default function NeuroSwitchPage() {
  ...
  const [whiteHits, setWhiteHits] = useState(0);
  ...
  const whiteBallRef = useRef({ x: 0, y: 0, r: 35 });
  const redBallRef = useRef({ x: 0, y: 0, r: 35 });
  ...
  const calculatePoints = (reaction) => {
    if (reaction < 200) return 50;
    if (reaction < 400) return 30;
    if (reaction < 600) return 20;
    if (reaction < 800) return 10;
    return 5;
  };
  ...
  const spawn = () => {
    ...
    const pad = 100;
    whiteBallRef.current.x = pad + Math.random() * (cvs.width - pad * 2);
    whiteBallRef.current.y = pad + Math.random() * (cvs.height - pad * 2);
    
    do {
      redBallRef.current.x = pad + Math.random() * (cvs.width - pad * 2);
      redBallRef.current.y = pad + Math.random() * (cvs.height - pad * 2);
    } while (Math.hypot(whiteBallRef.current.x - redBallRef.current.x, 
                        whiteBallRef.current.y - redBallRef.current.y) < 120);

    // Randomly choose target
    if (Math.random() > 0.5) {
      activeCommandRef.current = "WHITE";
      setCurrentCommand("WHITE");
      playSound('white');
    } else {
  ...
  const startCycle = () => {
    ...
    cycleTimeoutRef.current = setTimeout(() => {
      if (stateRef.current === "WAITING" && isActiveRef.current) {
        spawn();
      }
    }, 600 + Math.random() * 600);
  };
  ...
  const fail = (reason) => {
    ...
    if (!canClickRef.current && reason !== "MISSED") return;
    
    clearAllTimeouts();
    canClickRef.current = false;
    
    const penaltyPoints = calculatePoints(400);
  ...
  const handleSuccess = () => {
    ...
    if (activeCommandRef.current === "WHITE") {
      setWhiteHits(prev => prev + 1);
    } else {
  ...
  // Click handler - check both balls
  const handleClick = useCallback((e) => {
    ...
    const distToWhite = Math.hypot(mouseX - whiteBall.x, mouseY - whiteBall.y);
    const distToRed = Math.hypot(mouseX - redBall.x, mouseY - redBall.y);
    
    // Check if clicked on white ball
    if (distToWhite < whiteBall.r + 10) {
      if (activeCommandRef.current === "WHITE") {
        handleSuccess();
      } else {
        fail("WRONG BALL");
    ...
    // Check if clicked on red ball
    if (distToRed < redBall.r + 10) {
  ...
  useEffect(() => {
    ...
    function draw() {
      ...
      if (stateRef.current === "ACTIVE" || stateRef.current === "FEEDBACK") {
        // White ball
        ctx.beginPath();
        ctx.arc(whiteBallRef.current.x, whiteBallRef.current.y, whiteBallRef.current.r, 0, Math.PI * 2);
        ...
        ctx.shadowBlur = 20;
        ctx.shadowColor = "rgba(229,57,53,0.4)";
        ctx.fill();
        ctx.shadowBlur = 0;
        ctx.strokeStyle = "#666666";
        ctx.lineWidth = 2;
        ...
        // Inner highlights
        ctx.beginPath();
        ctx.arc(whiteBallRef.current.x - 5, whiteBallRef.current.y - 5, 8, 0, Math.PI * 2);
        ctx.fillStyle = "rgba(255,255,255,0.2)";
        ctx.fill();
        
        ctx.beginPath();
        ctx.arc(redBallRef.current.x - 5, redBallRef.current.y - 5, 8, 0, Math.PI * 2);
      ...
      if (stateRef.current === "ACTIVE") {
        ...
        ctx.fillStyle = activeCommandRef.current === "WHITE" ? "#FFFFFF" : "#E53935";
        ctx.fillText(`CLICK ${activeCommandRef.current} BALL`, cvs.width / 2, 50);
      }
    ...
    <div className={`min-h-screen select-none ${isDarkMode ? 'bg-gray-900' : 'bg-gray-50'}`}>
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
          ...
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <p className={`${isDarkMode ? 'text-gray-400' : 'text-gray-500'}`}>Audio-cued • High pitch=White / Low pitch=Red</p>
              </div>
            </div>
        ...
        <div className="grid grid-cols-7 gap-3 mb-4 h-[88px]">
          ...
          <StatCard icon={<Radio className="text-gray-500" />} value={whiteHits} label="White" isDark={isDarkMode} />
          ...
          {isFullscreen && gameState === 'playing' && (
            <>
              ...
              <div className="absolute top-4 left-4 z-20 bg-black/50 backdrop-blur-sm rounded-lg px-4 py-2 text-white text-sm">
                Score: <span className="text-yellow-400">{score}</span> | Command: <span style={{color: currentCommand === 'WHITE' ? '#FFFFFF' : '#E53935'}}>{currentCommand || 'WAIT'}</span>
              </div>
            </>
          )}
          ...
          {gameState === 'start' && (
            <div className={`absolute inset-0 flex items-center justify-center backdrop-blur-sm rounded-xl z-40 ${isBoxDarkMode ? 'bg-gray-900/95' : 'bg-white/95'}`}>
              <div className={`rounded-2xl p-8 text-center max-w-md mx-4 shadow-xl border ${isBoxDarkMode ? 'bg-gray-800 border-gray-700' : 'bg-white border-gray-200'}`}>
                ...
                <p className={`mb-6 ${isBoxDarkMode ? 'text-gray-300' : 'text-gray-600'}`}>High pitch = WHITE ball • Low pitch = RED ball</p>
                <button 
                  onClick={startGame} 
            ...
            <div className={`absolute inset-0 flex items-center justify-center backdrop-blur-sm rounded-xl z-40 ${isBoxDarkMode ? 'bg-gray-900/95' : 'bg-white/95'}`}>
              <div className={`rounded-2xl p-8 shadow-xl border w-[480px] ${isBoxDarkMode ? 'bg-gray-800 border-gray-700' : 'bg-white border-gray-200'}`}>
                ...
                <div className="grid grid-cols-2 gap-4 mb-8">
                  ...
                  <ResultCard label="White Hits" value={whiteHits} icon={<Radio className="w-4 h-4" />} color="text-gray-500" />
            ...
            <div className={`rounded-xl border overflow-hidden ${isDarkMode ? 'bg-gray-800 border-gray-700' : 'bg-white border-gray-200'}`}>
              <div className={`px-4 py-3 border-b ${isDarkMode ? 'border-gray-700 bg-gray-800/50' : 'border-gray-200 bg-gray-50'}`}>
                <div className="flex items-center gap-2">
                  ...
                  <h3 className={`font-semibold ${isDarkMode ? 'text-white' : 'text-gray-900'}`}>Drill Rules</h3>
                </div>
              </div>
              <div className="p-4">
                <p className={`text-sm ${isDarkMode ? 'text-gray-300' : 'text-gray-600'}`}>
                  🔊 <span className="font-semibold">High pitch (1200Hz) = Click WHITE ball</span> • 
                  <span className="font-semibold text-red-500"> Low pitch (250Hz) = Click RED ball</span> • 
                  Faster reaction = More points • 5 mistakes reset score
                </p>
              </div>
            </div>
          </div>
...
function ResultCard({ label, value, unit = '', icon, color }) {
  const bgColor = color === 'text-blue-500' ? 'bg-blue-500/10' : 
                   ...
                   color === 'text-green-500' ? 'bg-green-500/10' : 'bg-purple-500/10';
