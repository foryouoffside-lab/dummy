...
import { 
  ...
  AlertTriangle, Crosshair, Calculator, Code2, LogOut, Radio, Check, Sparkles
} from 'lucide-react';
  ...
  playDrillSound(profile) { 
    ...
    try { 
      ...
      if (profile === 'success') {
        ...
        osc.frequency.exponentialRampToValueAtTime(880, now + 0.15); // A5 Sweep
      ...
      } else if (profile === 'cue-green') {
        osc.type = 'sine';
        osc.frequency.setValueAtTime(523.25, now); // C5 (Soothing High)
        gainNode.gain.setValueAtTime(0, now);
        gainNode.gain.linearRampToValueAtTime(0.15, now + 0.05); // Soft attack
        gainNode.gain.exponentialRampToValueAtTime(0.001, now + 0.4); // Smooth gentle release
        osc.start(now); osc.stop(now + 0.4);
      } else if (profile === 'cue-red') {
        osc.type = 'sine';
        osc.frequency.setValueAtTime(329.63, now); // E4 (Soothing Low)
        gainNode.gain.setValueAtTime(0, now);
        gainNode.gain.linearRampToValueAtTime(0.15, now + 0.05); // Soft attack
        gainNode.gain.exponentialRampToValueAtTime(0.001, now + 0.4); // Smooth gentle release
        osc.start(now); osc.stop(now + 0.4);
...
class GameErrorBoundary extends React.Component {
  ...
  componentDidCatch(error, errorInfo) { console.error('Neuro-Switch Error:', error, errorInfo); }
  render() {
    if (this.state.hasError) {
      return (
        <div className="absolute inset-0 flex items-center justify-center bg-black/95 rounded-xl z-50 border border-red-500/30">
          <div className="text-center p-6 max-w-sm">
            ...
            <p className="text-gray-400 text-sm mb-4">The cognitive engine encountered a boundary exception. Let's restart the runtime sync.</p>
            <button onClick={() => { this.setState({ hasError: false }); window.location.reload(); }} className="w-full py-2.5 bg-purple-600 hover:bg-purple-500 text-white font-bold rounded-xl transition-colors shadow-[0_0_15px_rgba(147,51,234,0.4)]">Reset Frame</button>
          </div>
        </div>
      );
    }
...
export default function NeuroSwitchClient() {
  ...
  const windowTimeRef = useRef(1300); 
  const minWindowTime = 350;
  const maxWindowTime = 1300;
  
  const streakRef = useRef(0);
  const bestStreakRef = useRef(0);
  const bestReactionRef = useRef(0);
  
  const greenBallRef = useRef({ x: 0, y: 0, r: 24 });
  const redBallRef = useRef({ x: 0, y: 0, r: 24 });
  ...
  const engine = useGameEngine({
    category: 'visual',
    drillId: 'neuro-switch',
    drillName: 'Neuro-Switch Lab',
    totalGameTime: 9999, // Intercepted by local economic timer
    lives: 9999,
    infiniteLives: true,
    sharePath: 'drills/visual/reaction-speed/neuro-switch',
  });
  ...
  const updateEconomy = useCallback((scoreDelta, timeDelta) => {
    setCustomScore(prev => {
      ...
      // Score-based difficulty scaling: shrink response window duration as score grows
      windowTimeRef.current = Math.max(minWindowTime, maxWindowTime - Math.floor(updated * 8));
      return updated;
    });
  ...
  const spawnTargets = useCallback(() => {
    ...
    // Laptop & Mobile Optimized Sizing (Scaled down to 3% for a smaller, distinct flat target)
    const r = Math.max(15, Math.min(cvs.width, cvs.height) * 0.03); 
    greenBallRef.current.r = r;
    redBallRef.current.r = r;

    const padding = r * 2 + 10;
    const minTargetDistance = r * 5; 

    greenBallRef.current.x = padding + Math.random() * (cvs.width - padding * 2);
    greenBallRef.current.y = padding + Math.random() * (cvs.height - padding * 2);

    let iterations = 0;
    let spatialValidation = false;
    while (!spatialValidation && iterations < 60) {
      redBallRef.current.x = padding + Math.random() * (cvs.width - padding * 2);
      redBallRef.current.y = padding + Math.random() * (cvs.height - padding * 2);

      const distance = Math.hypot(greenBallRef.current.x - redBallRef.current.x, greenBallRef.current.y - redBallRef.current.y);
      if (distance >= minTargetDistance) spatialValidation = true;
      iterations++;
    }

    if (Math.random() > 0.5) {
      activeCommandRef.current = "GREEN";
      if (audioSynth) audioSynth.playDrillSound('cue-green');
    } else {
      activeCommandRef.current = "RED";
      if (audioSynth) audioSynth.playDrillSound('cue-red');
    }
    ...
    timeoutRef.current = setTimeout(() => {
      if (stateRef.current === "ACTIVE" && isActiveRef.current && !isTimeUp) {
        ...
        triggerFeedback('⏰ TIMEOUT! -2s', 'error');
  ...
  useEffect(() => {
    if (engine.gameState === 'playing' && !isTimeUp) {
      if (!hasInitializedRoundRef.current) {
        ...
        stateRef.current = "IDLE";
        windowTimeRef.current = maxWindowTime; // Start at 1.3 Seconds
        streakRef.current = 0;
  ...
  const handleInputStrikes = useCallback((e) => {
    ...
    if (gameStateRef.current !== 'playing' || !isActiveRef.current || stateRef.current !== "ACTIVE" || isTimeUp) return;
    ...
    const physicalClickRadius = greenBallRef.current.r + 25; 

    const distanceToGreen = Math.hypot(clickX - greenBallRef.current.x, clickY - greenBallRef.current.y);
    const distanceToRed = Math.hypot(clickX - redBallRef.current.x, clickY - redBallRef.current.y);

    let interactionRegistered = false;
    let evaluatedTargetStatus = false;

    if (distanceToGreen <= physicalClickRadius) {
      interactionRegistered = true;
      if (activeCommandRef.current === "GREEN") evaluatedTargetStatus = true;
    } else if (distanceToRed <= physicalClickRadius) {
      interactionRegistered = true;
      if (activeCommandRef.current === "RED") evaluatedTargetStatus = true;
    }

    if (!interactionRegistered) return; // Strike did not intersect targeting bounds

    stateRef.current = "IDLE";
    if (timeoutRef.current) clearTimeout(timeoutRef.current);

    if (evaluatedTargetStatus) {
      ...
      triggerFeedback(`✓ HIT! +5 PTS | +2s (${currentLatency}ms)`, 'success');
    } else {
      streakRef.current = 0;
      setStreak(0);
      setFailedHits(prev => prev + 1);
      ...
      updateEconomy(0, -2); // No PTS Penalty, -2s
      if (audioSynth) audioSynth.playDrillSound('fail');
      triggerFeedback(`✗ MISMATCH! -2s`, 'error');
    }

    scheduleNextFlash();
  }, [isTimeUp, scheduleNextFlash, updateEconomy, triggerFeedback]);

  // === Native Structural Render Loops (Flat 2D targets) ===
  useEffect(() => {
    ...
    const scaleLayoutFrame = () => {
      ...
      let w = boundingBox.width, h = boundingBox.height;
  ...
  const shareScore = useCallback(() => {
    ...
    else if (customScoreRef.current >= 80 && finalAccuracy >= 82) finalRank = 'Master';
    else if (customScoreRef.current >= 50 && finalAccuracy >= 75) finalRank = 'Diamond';
    else if (customScoreRef.current >= 30 && finalAccuracy >= 65) finalRank = 'Platinum';
    else if (customScoreRef.current >= 15 && finalAccuracy >= 55) finalRank = 'Gold';
    else if (customScoreRef.current >= 5) finalRank = 'Silver';

    const text = `🎯 I scored ${customScoreRef.current} PTS with ${finalAccuracy}% accuracy on the Sound Reaction audio cue test! Rank: ${finalRank}. Try it here: https://skilldrills.online/drills/visual/reaction-speed/sound-reaction`;
  ...
  let gradeLetter = 'F';
  if (accuracy >= 90 && customScore >= 120) gradeLetter = 'S';
  else if (accuracy >= 80 && customScore >= 80) gradeLetter = 'A';
  else if (accuracy >= 70 && customScore >= 50) gradeLetter = 'B';
  else if (accuracy >= 60 && customScore >= 30) gradeLetter = 'C';
  else if (accuracy >= 50 && customScore >= 15) gradeLetter = 'D';

  let rankName = 'Bronze';
  let rankColor = 'text-slate-500';
  if (customScore >= 120 && accuracy >= 90) {
    rankName = 'Grandmaster';
    rankColor = 'text-fuchsia-400 font-extrabold';
  } else if (customScore >= 80 && accuracy >= 82) {
    rankName = 'Master';
    rankColor = 'text-red-400 font-extrabold';
  } else if (customScore >= 50 && accuracy >= 75) {
    rankName = 'Diamond';
    rankColor = 'text-cyan-400 font-extrabold';
  } else if (customScore >= 30 && accuracy >= 65) {
    rankName = 'Platinum';
  ...
  let diagnostics = "Outstanding auditory reflex speed! Your discrimination accuracy shows well-developed audio-motor pathways.";
  if (failedHits > 5) {
    diagnostics = "High rate of pitch choice errors or timeouts. Listen to the pitch attack envelope carefully before executing the click response.";
  } else if (accuracy < 60) {
    diagnostics = "Low pitch discrimination accuracy. Slow down to prioritize accuracy, then build speed.";
  } else if (customScore < 40) {
    diagnostics = "Keep the clock alive by maintaining high hit streaks to raise your score.";
  }
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        ...
        {!isFullscreen && (
          <nav aria-label="Breadcrumb" className="mb-4">
            <ol className="flex flex-wrap items-center gap-2 text-sm">
              ...
              <li className="text-gray-600">/</li>
              <li className="text-purple-400 font-medium" aria-current="page">Neuro-Switch Lab</li>
            </ol>
          </nav>
        )}
        ...
        {!isFullscreen && (
          <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 mb-6">
            <div className="flex items-center gap-4">
              ...
              <div>
                <h1 className="text-2xl sm:text-3xl font-bold tracking-tight">Neuro-Switch Lab</h1>
                <p className="text-sm text-gray-400 mt-1 font-medium">Auditory-Cued Reaction Drill • Cognitive Switch</p>
              </div>
            </div>
        ...
        {!isFullscreen && (
          <div className="grid grid-cols-4 sm:grid-cols-7 gap-1.5 sm:gap-3 mb-2 h-auto py-1">
            ...
            <StatCard icon={<Award className="text-green-400" />} value={perfectHits} label="Hits" />
            <StatCard icon={<XCircle className="text-red-400" />} value={failedHits} label="Miss/Timeout" />
            <StatCard icon={<Zap className="text-orange-400" />} value={streak} label="Streak" />
            <StatCard icon={<Clock className="text-blue-400" />} value={bestReaction ? `${bestReaction}` : '-'} label="Best RT" unit="ms" />
          </div>
        )}
        ...
        <GameErrorBoundary>
          <div ref={containerRef} className={`relative overflow-hidden flex flex-col z-10 transition-all duration-100 ${isFullscreen ? 'fixed inset-0 z-50 w-screen h-screen' : 'w-full rounded-2xl border shadow-[0_0_40px_rgba(0,0,0,0.5)] min-h-[60vh] md:min-h-[600px] lg:min-h-[650px]'}`} style={{ margin: '0 auto', borderColor: isDarkMode ? '#374151' : '#e5e7eb', backgroundColor: isBoxDarkMode ? '#050508' : '#f9fafb' }}>
            
            {/* Countdown Strip */}
            {engine.gameState === 'playing' && !isTimeUp && (
              <div className="absolute top-0 left-0 right-0 h-1.5 bg-gray-900 z-[60] pointer-events-none">
                <div className={`h-full transition-all duration-1000 ease-linear ${localTimeRemaining <= 10 ? 'bg-red-500 animate-pulse' : 'bg-purple-500'}`} style={{ width: `${Math.min(100, (localTimeRemaining / 60) * 100)}%` }} />
              </div>
            )}
            ...
            <canvas 
              ...
              className={`block absolute z-20 touch-none ${engine.gameState === 'playing' ? 'cursor-pointer' : 'cursor-default'}`} 
            />
              ...
              <div className="absolute inset-0 flex items-center justify-center z-40 bg-black/90 backdrop-blur-sm p-4 overflow-y-auto pointer-events-auto">
                <div className="rounded-3xl p-6 sm:p-8 text-center max-w-sm w-full border border-gray-700 bg-gray-900 shadow-2xl flex flex-col my-auto shrink-0">
                  <div className="flex-1 mb-8">
                    ...
                    <h2 className="text-xl sm:text-2xl font-black tracking-tight text-white">Neuro-Switch Lab</h2>
                  </div>
                  <button onPointerDown={(e)=>e.stopPropagation()} onClick={handleStartGame} className="w-full flex items-center justify-center gap-2 px-6 py-4 bg-gradient-to-r from-purple-600 to-pink-600 text-white rounded-xl font-black text-base sm:text-lg hover:brightness-110 transition-all transform hover:scale-[1.02] active:scale-[0.98] animate-pulse hover:animate-none shadow-[0_0_20px_rgba(168,85,247,0.3)] focus:outline-none shrink-0">
                    <Radio className="w-5 h-5 fill-white" /> START DRILL
                  </button>
                </div>
              </div>
            )}

            {/* Premium Custom Structural End Card Component Inline */}
                ...
                <div className="min-h-full flex flex-col justify-center items-center py-4 w-full">
                  <div className="max-w-md w-full text-center">
                    ...
                    <div className="grid grid-cols-3 gap-2.5 mb-6 text-left font-mono">
                      ...
                      <div className="bg-slate-900/60 border border-slate-800 p-2.5 rounded-xl">
                        <span className="text-[7.5px] text-slate-500 block uppercase font-bold">Miss/Timeout</span>
            ...
            <div className="rounded-2xl border border-gray-800 overflow-hidden bg-gray-900 shadow-2xl">
              ...
              <div className="p-6 grid grid-cols-1 md:grid-cols-2 gap-6">
                <div className="space-y-5">
                  <RuleItem num="1" color="purple" text="High Tone =" highlight="GREEN Sphere" result={`+5 PTS | +2s`} />
                  <RuleItem num="2" color="blue" text="Low Tone =" highlight="RED Sphere" result={`+5 PTS | +2s`} />
                </div>
                <div className="space-y-5">
                  <RuleItem num="3" color="red" text="Mismatch / Timeout" result="0 PTS | -2s" />
                  <RuleItem num="4" color="cyan" text="Zero Latency Input" highlight="Desktop & Mobile" result="Direct Touch Support" />
                </div>
              </div>
            </div>
          </section>
          ...
          <section className="mt-12" aria-label="About this drill">
            <div className="rounded-2xl border border-gray-800 overflow-hidden bg-gray-900 shadow-xl">
              <div className="px-6 py-5 border-b border-gray-800 bg-black/40 flex items-center gap-3">
                ...
                <h2 className="font-bold text-white text-lg tracking-wide">About This Neuro-Switch Drill</h2>
              </div>
              
              <div className="p-8">
                <p className="text-sm leading-relaxed mb-6 text-gray-300">
                  Neuro-Switch focuses strictly on the neurological pathways governing multi-sensory processing and auditory frequency discrimination. By forcing quick motor responses based on auditory cues, it sharpens selective attention and builds cognitive resilience.
                </p>
                ...
                <div className="grid grid-cols-1 sm:grid-cols-3 gap-5 mb-8">
                  <div className="p-5 rounded-xl border border-gray-800 bg-black/40">
                    ...
                    <p className="text-xs leading-relaxed text-gray-400">Esports professionals tracking multi-layered cues, audio engineers, competitive tactical athletes, and students optimizing attention shifting.</p>
                  </div>
                  <div className="p-5 rounded-xl border border-gray-800 bg-black/40">
                    <div className="flex items-center gap-3 mb-3">
                    ...
                    <p className="text-xs leading-relaxed text-gray-400">Auditory processing speed, pitch classification accuracy, visual response mapping, and conflict resolution latency bounds.</p>
                  </div>
                  <div className="p-5 rounded-xl border border-gray-800 bg-black/40">
                    <div className="flex items-center gap-3 mb-3">
                    ...
                    <p className="text-xs leading-relaxed text-gray-400">Net score indices, strike accuracy ratios, total perfect inputs, timeout failures, maximum sequential streak levels, and lowest latency reaction metrics.</p>
                  </div>
                </div>
                ...
                <div className="p-5 rounded-xl border border-gray-800 bg-black/40 mb-6">
                  ...
                  <ol className="list-decimal pl-5 space-y-2 text-xs text-gray-400 leading-relaxed">
                    <li><strong className="text-gray-200">Prepare for Sound:</strong> Keep your headset active. This drill requires immediate interpretation of tone frequencies.</li>
                    <li><strong className="text-gray-200">Decode the Pitch:</strong> A higher soothing tone dictates a strike on the green sphere. A lower soothing tone commands a strike on the red sphere.</li>
                    <li><strong className="text-gray-200">Execute the Selection:</strong> Tap or click directly on the selected color target before the internal countdown slips away.</li>
                    <li><strong className="text-gray-200">Maintain Precision:</strong> Clicking incorrect targets or timing out costs you time on the clock but does not deduct points. As your score increases, the response window shrinks.</li>
                  </ol>
                </div>
                ...
                <div className="p-5 rounded-xl border border-gray-800 bg-black/40">
                  ...
                  <div className="space-y-4">
                    <div>
                      ...
                      <p className="text-xs text-gray-400 mt-1">This represents our adaptive difficulty scaling. Earning higher scores demonstrates cognitive mastery over the audio cues, prompting the engine to trim the lifespan down from 1300ms toward 350ms to push your limits.</p>
                    </div>
                    <div>
                      <h4 className="text-sm font-bold text-gray-200">What constitutes a high-tier performance score?</h4>
                      <p className="text-xs text-gray-400 mt-1">Maintaining an accuracy ratio above 85% while driving scores past 80 points highlights excellent cognitive efficiency and low auditory processing lag.</p>
                    </div>
                  </div>
                </div>
