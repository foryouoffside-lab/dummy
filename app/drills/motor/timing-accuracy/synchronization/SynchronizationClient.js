...
export default function SynchronizationClient() {
  ...
  useEffect(() => {
    try {
      const savedSens = localStorage.getItem('syncDrill_sens');
      if (savedSens) setUniversalSens(parseFloat(savedSens));
      const savedBest = localStorage.getItem('syncDrill_bestScore');
  ...
  useEffect(() => {
    if (gameState !== 'playing') {
      try { localStorage.setItem('syncDrill_sens', universalSens.toString()); } catch (e) {}
    }
  ...
  const endGame = useCallback(() => {
    ...
    const finalAccuracy = totalAttemptsRef.current === 0 ? 100 : Math.round((hitsRef.current / totalAttemptsRef.current) * 100);
    ...
    setAnalytics({
      ...
      missedClicks: totalAttemptsRef.current - hitsRef.current,
    ...
    setBestScore(prev => {
      if (scoreRef.current > prev) {
        setIsNewBest(true);
        try { localStorage.setItem('syncDrill_bestScore', scoreRef.current.toString()); } catch(err){}
  ...
  const resetBars = useCallback((cvs) => {
    ...
    // Variable speeds 400-1200px/s
    barsRef.current.speed = 400 + Math.floor(Math.random() * 801);
    barsRef.current.active = true;
    if (audioSynth) audioSynth.playSound('reset');
  }, []);

  const startNextRound = useCallback(() => {
    if (!isActiveRef.current) return;
    if (canvasRef.current) resetBars(canvasRef.current);
  }, [resetBars]);

  const applyPenalty = useCallback(() => {
    if (!isActiveRef.current) return;
    
    scoreRef.current = Math.max(0, scoreRef.current - 2); // -2 Penalty
    setScore(scoreRef.current); 
    
    engine.current.timeLeft -= 0.5; // -0.5s Heavy Penalty
  ...
  const startGame = useCallback(async () => {
    ...
    engine.current.timeLeft = DRILL_DURATION;
  ...
  useEffect(() => {
    ...
    const handleMouseDown = (e) => {
      ...
      if (gameState === 'playing') {
        if (!pointerLocked && canvasRef.current) {
        ...
        } else if (pointerLocked && isActiveRef.current && barsRef.current.active) {
          ...
          if (absError <= 45) { // 45ms Hit Window
            hitsRef.current++;
            if (absError <= 15) { // 15ms Perfect Window
              perfectHitsRef.current++; 
              setPerfectHits(perfectHitsRef.current); 
            }
            
            scoreRef.current += 3; // +3 Points
            setScore(scoreRef.current);
            
            engine.current.timeLeft += 1; // +1s Time Bonus
            ...
            applyPenalty(); 
            lastResultRef.current.error = 999; // 999 is flag for complete MISS
          }
          ...
          resetTimeoutRef.current = setTimeout(() => { 
            if (isActiveRef.current) startNextRound(); 
          }, 400);
        }
      }
    };
  ...
  }, [gameState, pointerLocked, universalSens, applyPenalty, startNextRound]);
  ...
  useEffect(() => {
    ...
    const loop = (time) => {
      ...
      if (gameState === 'playing' && pointerLocked && isActiveRef.current) {
        ...
        engine.current.timeLeft -= dt;
        if (engine.current.timeLeft <= 0) {
          engine.current.timeLeft = 0;
          endGame();
        }
        ...
        if (barsRef.current.active) { 
          ...
          // Timeout condition (Bars crossed over completely)
          if (barsRef.current.pos > 80) { 
            applyPenalty(); 
            barsRef.current.active = false; 
      ...
      if (barsRef.current.active || (lastResultRef.current.show && lastResultRef.current.timer > 0.4)) {
        ...
        // Left Bar
        ctx.fillRect(cx + barsRef.current.pos - bw, by, bw, bh); 
        // Right Bar
        ctx.fillRect(cx - barsRef.current.pos, by, bw, bh);
      ...
      if (lastResultRef.current.show && lastResultRef.current.timer > 0) {
        ...
        ctx.fillStyle = isMiss ? "#ef4444" : (Math.abs(lastResultRef.current.error) <= 15 ? "#00ff88" : "#06b6d4");
  ...
  }, [gameState, pointerLocked, applyPenalty, startNextRound, endGame]);

  const shareDrillLink = useCallback(() => {
    const url = typeof window !== 'undefined' ? window.location.href : '';
    if (navigator.share) {
      navigator.share({ title: 'Synchronization Elite', url }).catch(() => {});
    } else {
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
          ...
          <div className="mb-6">
            <nav className="mb-4">
              <ol className="flex flex-wrap items-center gap-2 text-sm text-gray-500">
                ...
                <li><ChevronRight className="w-4 h-4 text-gray-600" /></li>
                <li className="text-emerald-400 font-medium">Synchronization Elite</li>
              </ol>
            </nav>
            ...
            <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
              <div className="flex items-center gap-4">
                ...
                <div>
                  <h1 className="text-2xl sm:text-3xl font-bold tracking-tight">Synchronization Elite</h1>
                  <p className="text-sm text-gray-400 mt-1 font-medium">Desktop Exclusive • Timing Convergence</p>
                </div>
              </div>
        ...
        {!isFullscreen && (
          <div className="grid grid-cols-4 lg:grid-cols-7 gap-2 mb-2">
            ...
            <StatCard icon={<Award className="text-blue-400" />} value={perfectHits} label="Perfect Syncs" />
          ...
          {gameState === 'start' && (
            <div className="absolute inset-0 flex items-center justify-center z-50 bg-black/70 backdrop-blur-md p-4 overflow-y-auto">
              <div className="rounded-3xl p-8 text-center max-w-lg w-full border border-gray-700 bg-gray-900 shadow-2xl my-auto">
                ...
                <h2 className="text-3xl font-black mb-3 tracking-tight text-white uppercase">Synchronization Elite</h2>
                <p className="text-sm mb-6 text-gray-400 leading-relaxed">
                  Two bars converge at random variable speeds. Click the exact moment they perfectly align over the center line. Correct syncs grant time and points, while errors instantly drain your clock.
                </p>
                ...
                >
                  <Play className="w-6 h-6 fill-white" /> BEGIN TIMING DRILL
                </button>
              </div>
            </div>
          )}
            ...
            <div className="absolute inset-0 flex items-center justify-center z-50 bg-black/90 backdrop-blur-md p-4 animate-in fade-in duration-300 overflow-y-auto">
              <div className="rounded-3xl max-w-2xl w-full shadow-2xl border border-gray-800 bg-gray-950 overflow-hidden my-auto">
                <div className="bg-gradient-to-br from-emerald-900/40 to-teal-900/40 p-6 border-b border-gray-800 text-center relative">
                  ...
                  <h2 className="text-2xl font-black text-white tracking-tight mt-4">Timing Analysis Complete</h2>
                  <p className="text-emerald-400 font-medium text-sm mt-1">Time-Attack Session Concluded</p>
                </div>
                  ...
                  <div className="flex flex-col sm:flex-row gap-4 mb-6">
                    <div className="flex-1 bg-gray-900 rounded-2xl p-4 border border-gray-800 flex justify-between items-center">
                      ...
                      <div className="text-right">
                        <span className="text-xs font-bold text-gray-500 uppercase tracking-widest mb-1 block">Timing Accuracy</span>
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-6 text-xs leading-relaxed text-gray-300">
                      ...
                      <div className="space-y-3 sm:border-r border-gray-800 sm:pr-6">
                        ...
                        <ul className="space-y-2">
                          ...
                          <li className="flex justify-between items-center bg-gray-900/50 p-2 rounded border border-gray-800">
                            <span className="text-gray-400">Perfect Syncs (&lt;15ms):</span>
                            <span className="font-bold text-green-400">{analytics.perfectHits}</span>
                          </li>
                          <li className="flex justify-between items-center bg-gray-900/50 p-2 rounded border border-gray-800">
                            <span className="text-gray-400">Misses/Timeouts:</span>
                        ...
                        <div>
                          ...
                          <p className="text-gray-400 leading-relaxed font-sans">
                            {analytics.missedClicks > 5 ? (
                              <span className="text-red-300">You are anticipating the alignment and clicking too early. A miss costs valuable points and severely chunks your survival time. Relax your hand and wait for the visual convergence.</span>
                            ) : analytics.perfectHits < 3 ? (
                              <span className="text-yellow-300">Your accuracy is decent, but your margin of error is too high. Aim to click strictly when the bars perfectly overlap the center line to score Perfect Syncs.</span>
                            ) : (
                              <span className="text-green-300">Excellent visual-motor synchronization! You are maintaining high accuracy despite the aggressive variable speed scaling. Keep pushing your limits.</span>
                            )}
                          </p>
                        </div>
                      </div>
            ...
            <div className="rounded-2xl border border-gray-800 overflow-hidden bg-gray-900 shadow-2xl pointer-events-none">
              ...
              <div className="p-6 grid grid-cols-1 md:grid-cols-2 gap-6">
                <div className="space-y-5">
                  <RuleItem num="1" color="green" text="Click when bars align" highlight="+3 PTS | +1.0s Time" result="< 45ms window" />
                  <RuleItem num="2" color="indigo" text="Perfect Syncs" highlight="Under 15ms error" result="Telemetry Tracked" />
                </div>
                <div className="space-y-5">
                  <RuleItem num="3" color="red" text="Miss / Timeout" result="-2 PTS | -0.5s Time" />
                  <RuleItem num="4" color="purple" text="Variable Speed" highlight="400 - 1200 px/s" result="Changes every spawn" />
                </div>
              </div>
            </div>
          </section>
          ...
          <section className="mt-12" aria-label="About this drill">
            <div className="rounded-2xl border border-gray-800 overflow-hidden bg-gray-900 shadow-xl">
              <div className="px-6 py-5 border-b border-gray-800 bg-black/40 flex items-center gap-3">
                ...
                <h2 className="font-bold text-white text-lg tracking-wide">About Synchronization Elite</h2>
              </div>
              <div className="p-8">
                <p className="text-sm leading-relaxed mb-6 text-gray-300">
                  This free synchronization drill trains your visual timing accuracy by challenging you to click precisely when two converging bars align at the center. Variable speeds from 400-1200px/s keep you engaged. Perfect for gamers, musicians, and anyone wanting better hand-eye coordination and timing precision.
                </p>
                ...
                <div className="grid grid-cols-1 sm:grid-cols-3 gap-5 mb-8">
                  <div className="p-5 rounded-xl border border-gray-800 bg-black/40">
                    ...
                    <p className="text-xs leading-relaxed text-gray-400">Gamers, musicians, athletes, and anyone wanting better timing, synchronization, and visual-motor coordination.</p>
                  </div>
                  <div className="p-5 rounded-xl border border-gray-800 bg-black/40">
                    <div className="flex items-center gap-3 mb-3">
                    ...
                    <p className="text-xs leading-relaxed text-gray-400">Timing accuracy, visual synchronization, hand-eye coordination, reaction precision, and convergence tracking.</p>
                  </div>
                  <div className="p-5 rounded-xl border border-gray-800 bg-black/40">
                    <div className="flex items-center gap-3 mb-3">
                    ...
                    <p className="text-xs leading-relaxed text-gray-400">Score, accuracy, streak, perfect syncs, total attempts, and best performance.</p>
                  </div>
                </div>
                ...
                <div className="mb-8 bg-[#0b0f19]/40 border border-gray-800 rounded-xl p-6">
                  ...
                  <div className="grid sm:grid-cols-2 gap-6 text-sm text-gray-300">
                    <ol className="space-y-3 list-decimal pl-5">
                      ...
                      <li>Wait for the left and right bars to speed toward the center line.</li>
                      <li>Click exactly when the two bars perfectly overlap the center point.</li>
                    </ol>
                    <ul className="space-y-3">
                      <li className="flex items-center gap-2"><CheckCircle2 className="w-4 h-4 text-green-500" /> <span className="text-white font-bold">Valid Hit:</span> Clicking within a 45ms window grants +3 PTS and +1.0s.</li>
                      <li className="flex items-center gap-2"><XCircle className="w-4 h-4 text-red-500" /> <span className="text-white font-bold">Miss / Timeout:</span> Deducts -2 PTS and brutally drains -0.5s Time.</li>
                    </ul>
                  </div>
                </div>
                ...
                <div className="p-5 rounded-xl border border-gray-800 bg-black/40">
                  ...
                  <div className="space-y-4">
                    <div>
                      <h4 className="text-sm font-bold text-gray-200">Why does it tell me my click was Early or Late?</h4>
                      <p className="text-xs text-gray-400 mt-1">The engine tracks the exact millisecond differential between your click and the perfect visual overlap. If you click too soon, it shows a negative ms score (e.g. -25ms). If you click too late, it shows positive. Use this feedback to adjust your internal rhythm.</p>
                    </div>
                    <div>
                      <h4 className="text-sm font-bold text-gray-200">How should I approach this drill?</h4>
                      <p className="text-xs text-gray-400 mt-1">Do not track the moving bars with your eyes. Keep your foveal focus directly locked on the center line and use your peripheral vision to sense the converging bars. Your brain calculates convergence much faster this way.</p>
                    </div>
                  </div>
                </div>
              </div>
