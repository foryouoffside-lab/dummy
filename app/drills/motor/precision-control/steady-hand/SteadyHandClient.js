...
export default function SteadyHandClient() {
  ...
  const updateBestScore = useCallback((finalScore) => {
    try {
      ...
      if (finalScore > currentBestScore) { localStorage.setItem('sustainedCircuitBestScore', finalScore.toString()); setBestScore(finalScore); }
    } catch (e) { /* localStorage not available */ }
  }, []);
  ...
  const playSound = useCallback((type) => {
    ...
    try {
      ...
      const freqMap = { lap: 880, fail: 300, streak: 1046.5, start: 660, penalty: 200, bestLap: 1318.5 };
      osc.frequency.setValueAtTime(freqMap[type] || 660, now);
      gain.gain.setValueAtTime(type === 'penalty' || type === 'fail' ? 0.12 : 0.1, now);
  ...
  const generatePath = useCallback((cvs) => {
    if (!cvs) return;
    const path = []; let curY = cvs.height / 2;
    const segments = 45; const startX = stripOffset + stripW; const endX = cvs.width - (stripOffset + stripW); const step = (endX - startX) / segments;
    for (let i = 0; i <= segments; i++) {
      if (i === 0 || i === segments) curY = cvs.height / 2;
      else { curY += (Math.random() - 0.5) * 300; curY = Math.max(150, Math.min(cvs.height - 150, curY)); }
  ...
  const applyPenalty = useCallback(() => {
    ...
    playSound('penalty'); showFeedback(`✗ Mistake! -${penaltyPoints} points`, 'error');
  }, [playSound, showFeedback]);

  const triggerFail = useCallback((msg, cvs) => {
    stateRef.current = 'FAIL'; failMsgRef.current = msg;
    streakRef.current = 0; setStreak(0);
    corridorWidthRef.current = 50; setCorridorWidth(50);
    playSound('fail'); applyPenalty();
    setTimeout(() => { if (cvs && isActiveRef.current) { stateRef.current = 'WAITING'; failMsgRef.current = ""; generatePath(cvs); } }, 1000);
  ...
  const checkCollision = useCallback((cvs) => {
    ...
    const startX = stripOffset + stripW; const endX = cvs.width - (stripOffset + stripW);
    let onPath = false;
    if (mouse.x <= startX || mouse.x >= endX) onPath = true;
    else {
      for (let i = 0; i < pathRef.current.length - 1; i++) {
        const p1 = pathRef.current[i]; const p2 = pathRef.current[i + 1];
        if (mouse.x >= p1.x && mouse.x <= p2.x) { const t = (mouse.x - p1.x) / (p2.x - p1.x); if (Math.abs(mouse.y - (p1.y + t * (p2.y - p1.y))) < corridorWidthRef.current / 2) onPath = true; break; }
      }
    }
    ...
    if (mouse.x > cvs.width - (stripOffset + stripW)) {
      ...
      if (bestLapTimeRef.current === 0 || lapCompletionTime < bestLapTimeRef.current) { bestLapTimeRef.current = lapCompletionTime; setBestLapTime(lapCompletionTime); playSound('bestLap'); showFeedback(`🏆 Best Lap! ${lapCompletionTime.toFixed(1)}s`, 'success'); }
      const newStreak = streakRef.current + 1; streakRef.current = newStreak; setStreak(newStreak);
      if (newStreak > bestLapStreakRef.current) { bestLapStreakRef.current = newStreak; setBestLapStreak(newStreak); }
      const pointsEarned = 15; scoreRef.current += pointsEarned; setScore(scoreRef.current);
      if (scoreRef.current > bestScore) setBestScore(scoreRef.current);
      if (newStreak % 5 === 0 && newStreak > 0) { playSound('streak'); showFeedback(`🔥 ${newStreak} Lap Streak! +${pointsEarned}`, 'success'); }
      else { playSound('lap'); showFeedback(`✓ Lap ${lapsRef.current} Complete! +${pointsEarned}`, 'success'); }
      corridorWidthRef.current = Math.max(12, 50 - (newStreak * 2)); setCorridorWidth(corridorWidthRef.current);
  ...
  useEffect(() => {
    const handleMouseDown = () => {
      ...
      const mouse = mousePositionRef.current;
      if (stateRef.current === 'WAITING' && mouse.x >= stripOffset && mouse.x <= stripOffset + stripW && Math.abs(mouse.y - cvs.height / 2) < stripH / 2) {
  ...
  useEffect(() => {
    ...
    function update(dt) {
      if (!isActiveRef.current) return;
      if (stateRef.current === 'ACTIVE') { lapTimeRef.current -= dt; setLapTime(Math.max(0, lapTimeRef.current)); }
      checkCollision(cvs);
    }
    ...
    function draw() {
      ...
      for (let i = 0; i < cvs.width; i += 50) { ctx.beginPath(); ctx.moveTo(i, 0); ctx.lineTo(i, cvs.height); ctx.stroke(); ctx.beginPath(); ctx.moveTo(0, i); ctx.lineTo(cvs.width, i); ctx.stroke(); }
      
      if (stateRef.current === 'ACTIVE' || stateRef.current === 'FAIL') {
        ctx.beginPath();
        ctx.strokeStyle = stateRef.current === 'FAIL' ? (isBoxDarkMode ? "#400" : "#fee") : (isBoxDarkMode ? "#800" : "#fca5a5");
        ctx.lineWidth = corridorWidthRef.current; ctx.lineJoin = 'round'; ctx.lineCap = 'round';
        ...
        ctx.stroke();
        ctx.beginPath();
        ctx.strokeStyle = stateRef.current === 'ACTIVE' ? (isBoxDarkMode ? "#ff4444" : "#dc2626") : (isBoxDarkMode ? "#200" : "#fecaca");
        ctx.lineWidth = 2; ctx.moveTo(pathRef.current[0]?.x, pathRef.current[0]?.y);
        for (let i = 1; i < pathRef.current.length; i++) ctx.lineTo(pathRef.current[i].x, pathRef.current[i].y);
        ctx.stroke();
        
        const mouse = mousePositionRef.current;
        ctx.beginPath(); ctx.arc(mouse.x, mouse.y, 10, 0, Math.PI * 2);
        ctx.fillStyle = stateRef.current === 'FAIL' ? "#ff4444" : "#00ff88"; ctx.fill();
        ctx.beginPath(); ctx.arc(mouse.x, mouse.y, 6, 0, Math.PI * 2);
        ctx.fillStyle = isBoxDarkMode ? "#020202" : "#ffffff"; ctx.fill();
        ctx.strokeStyle = stateRef.current === 'FAIL' ? "#ff4444" : "#00ff88"; ctx.lineWidth = 2;
        ctx.beginPath(); ctx.moveTo(mouse.x - 5, mouse.y); ctx.lineTo(mouse.x + 5, mouse.y); ctx.moveTo(mouse.x, mouse.y - 5); ctx.lineTo(mouse.x, mouse.y + 5); ctx.stroke();
      }
      
      const stripY = cvs.height / 2 - stripH / 2;
      ctx.fillStyle = isBoxDarkMode ? "#ffffff" : "#000000";
      ctx.fillRect(stripOffset, stripY, stripW, stripH); ctx.fillRect(cvs.width - (stripOffset + stripW), stripY, stripW, stripH);
      ...
      if (stateRef.current !== 'ACTIVE' && mouse.x > 0 && mouse.x < cvs.width && mouse.y > 0 && mouse.y < cvs.height) {
        ctx.beginPath(); ctx.arc(mouse.x, mouse.y, 10, 0, Math.PI * 2); ctx.fillStyle = "rgba(0, 255, 136, 0.3)"; ctx.fill();
        ctx.beginPath(); ctx.arc(mouse.x, mouse.y, 6, 0, Math.PI * 2); ctx.fillStyle = isBoxDarkMode ? "#020202" : "#ffffff"; ctx.fill();
  ...
  }, [gameState, isBoxDarkMode, checkCollision, generatePath]);

  const startGame = useCallback(() => {
    setGameState('playing'); gameStateRef.current = 'playing';
    setScore(0); setLaps(0); setStreak(0); setBestLapStreak(0); setCorridorWidth(50); setLapTime(30); setBestLapTime(0); setFeedback('');
    isActiveRef.current = true; scoreRef.current = 0; lapsRef.current = 0; streakRef.current = 0; bestLapStreakRef.current = 0;
    corridorWidthRef.current = 50; lapTimeRef.current = 30; bestLapTimeRef.current = 0; stateRef.current = 'WAITING'; onPathFramesRef.current = 0;
    if (canvasRef.current) generatePath(canvasRef.current);
  }, [generatePath]);

  const resetGame = useCallback(() => {
    isActiveRef.current = false; if (feedbackTimeoutRef.current) clearTimeout(feedbackTimeoutRef.current);
  ...
  if (loading || !isClient) {
    return <div className="min-h-screen flex items-center justify-center bg-gray-50"><div className="text-center"><div className="w-16 h-16 border-4 border-red-600 border-t-transparent rounded-full animate-spin mx-auto mb-4"></div><p className="text-gray-600">Loading steady hand drill...</p></div></div>;
  }
  ...
  return (
    <div className={`min-h-screen select-none ${isDarkMode ? 'bg-gray-900' : 'bg-gray-50'}`}>
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        ...
        <section className="sr-only"><h2>Sustained Circuit - Steady Hand Path Tracking</h2><p>Steady hand precision drill. Click left strip to start tracing a winding path corridor. Complete laps for 15 points within 30-second time limits. Corridor shrinks from 50px to 12px with each successful lap streak. Off-path or timeout costs -15 points. Best lap time tracked. No lives - pure endurance challenge.</p></section>
        ...
        <div ref={containerRef} className={`relative ${isFullscreen ? 'fixed inset-0 z-50' : 'rounded-xl border-2'}`} style={{ background: isBoxDarkMode ? "#020202" : "#ffffff", aspectRatio: isFullscreen ? 'auto' : '16/9', maxWidth: '100%', margin: '0 auto', borderColor: isDarkMode ? '#374151' : '#e5e7eb', overflow: 'hidden' }}>
          ...
          <canvas ref={canvasRef} style={{ display: 'block', position: 'absolute', cursor: 'none' }} aria-label="Steady hand circuit canvas. Click left strip to start tracing the path." />
          
          {gameState === 'start' && (<div className={`absolute inset-0 flex items-center justify-center backdrop-blur-sm rounded-xl z-40 ${isBoxDarkMode ? 'bg-gray-900/95' : 'bg-white/95'}`}><div className={`rounded-2xl p-6 sm:p-8 text-center max-w-md mx-4 shadow-xl border ${isBoxDarkMode ? 'bg-gray-800 border-gray-700' : 'bg-white border-gray-200'}`}><div className="mb-4"><Route className="w-16 h-16 text-red-500 mx-auto" aria-hidden="true" /></div><h2 className={`text-2xl font-bold mb-2 ${isBoxDarkMode ? 'text-white' : 'text-gray-900'}`}>Sustained Circuit</h2><p className={`mb-2 ${isBoxDarkMode ? 'text-gray-300' : 'text-gray-600'}`}>15 points per lap • 30s lap limit • Corridor shrinks with streak</p><p className={`mb-6 text-sm ${isBoxDarkMode ? 'text-gray-400' : 'text-gray-500'}`}>Click left strip to start each lap. Follow the red corridor path to the right strip. Off-path or timeout costs -15 points.</p><button onClick={startGame} className="px-8 py-3 bg-gradient-to-r from-red-500 to-orange-600 text-white rounded-xl font-semibold hover:shadow-lg w-full transition-all transform hover:scale-[1.02] active:scale-[0.98] focus:outline-none focus:ring-2 focus:ring-red-500 focus:ring-offset-2" aria-label="Start steady hand circuit">Start Circuit</button></div></div>)}
          
          {gameState === 'gameOver' && (<div className={`absolute inset-0 flex items-center justify-center backdrop-blur-sm rounded-xl z-40 ${isBoxDarkMode ? 'bg-gray-900/95' : 'bg-white/95'}`}><div className={`rounded-2xl p-6 sm:p-8 shadow-xl border w-full max-w-[480px] mx-4 ${isBoxDarkMode ? 'bg-gray-800 border-gray-700' : 'bg-white border-gray-200'}`}><div className="flex items-center justify-center gap-3 mb-4"><Award className="w-10 h-10 text-yellow-500" aria-hidden="true" /><h2 className={`text-2xl font-bold ${isBoxDarkMode ? 'text-white' : 'text-gray-900'}`}>Circuit Complete!</h2></div><p className={`text-center text-sm mb-6 ${isBoxDarkMode ? 'text-gray-400' : 'text-gray-500'}`}>Keep practicing to improve your path tracing precision and lap speed.</p><div className="grid grid-cols-2 gap-3 mb-6"><ResultCard label="Final Score" value={score} icon={<Target className="w-4 h-4" />} color="yellow" isDark={isBoxDarkMode} /><ResultCard label="Best Score" value={bestScore} icon={<Trophy className="w-4 h-4" />} color="yellow" isDark={isBoxDarkMode} /><ResultCard label="Total Laps" value={laps} icon={<Repeat className="w-4 h-4" />} color="green" isDark={isBoxDarkMode} /><ResultCard label="Best Lap" value={bestLapTime > 0 ? bestLapTime.toFixed(1) : '-'} unit="s" icon={<Clock className="w-4 h-4" />} color="purple" isDark={isBoxDarkMode} /><ResultCard label="Best Streak" value={bestLapStreak} icon={<Zap className="w-4 h-4" />} color="orange" isDark={isBoxDarkMode} /><ResultCard label="Min Corridor" value={`${corridorWidth}px`} icon={<Target className="w-4 h-4" />} color="cyan" isDark={isBoxDarkMode} /></div><div className="flex gap-3"><Link href="/drills/motor" className="flex-1"><button className={`w-full px-4 py-2.5 rounded-lg font-semibold transition-all ${isDarkMode ? 'bg-gray-700 text-gray-300 hover:bg-gray-600' : 'bg-gray-200 text-gray-700 hover:bg-gray-300'}`}>← Back to Motor</button></Link><button onClick={startGame} className="flex-1 px-4 py-2.5 bg-gradient-to-r from-red-500 to-orange-600 text-white rounded-lg font-semibold hover:shadow-lg transition-all transform hover:scale-[1.02] active:scale-[0.98] focus:outline-none focus:ring-2 focus:ring-red-500 focus:ring-offset-2">Play Again →</button></div></div></div>)}
        </div>

        {!isFullscreen && (<footer className="mt-6" aria-label="Drill rules"><div className={`rounded-xl border overflow-hidden ${isDarkMode ? 'bg-gray-800 border-gray-700' : 'bg-white border-gray-200'}`}><div className={`px-4 py-3 border-b ${isDarkMode ? 'border-gray-700 bg-gray-800/50' : 'border-gray-200 bg-gray-50'}`}><div className="flex items-center gap-2"><Info className={`w-4 h-4 ${isDarkMode ? 'text-red-400' : 'text-red-600'}`} aria-hidden="true" /><h2 className={`font-semibold ${isDarkMode ? 'text-white' : 'text-gray-900'}`}>Circuit Rules & Scoring</h2></div></div><div className="p-4"><div className="grid grid-cols-1 md:grid-cols-2 gap-4"><div className="space-y-3"><div className="flex items-start gap-2"><div className="w-5 h-5 rounded-full bg-red-500 flex items-center justify-center text-white text-xs font-bold flex-shrink-0 mt-0.5">1</div><p className={`text-sm ${isDarkMode ? 'text-gray-300' : 'text-gray-600'}`}>Click left strip to start • Follow the <span className="font-semibold text-red-500">red path</span></p></div><div className="flex items-start gap-2"><div className="w-5 h-5 rounded-full bg-green-500 flex items-center justify-center text-white text-xs font-bold flex-shrink-0 mt-0.5">2</div><p className={`text-sm ${isDarkMode ? 'text-gray-300' : 'text-gray-600'}`}>Complete lap: <span className="font-semibold text-green-500">+15 points</span></p></div><div className="flex items-start gap-2"><div className="w-5 h-5 rounded-full bg-orange-500 flex items-center justify-center text-white text-xs font-bold flex-shrink-0 mt-0.5">3</div><p className={`text-sm ${isDarkMode ? 'text-gray-300' : 'text-gray-600'}`}>Corridor <span className="font-semibold text-orange-500">shrinks with streak</span> (50px → 12px)</p></div></div><div className="space-y-3"><div className="flex items-start gap-2"><div className="w-5 h-5 rounded-full bg-blue-500 flex items-center justify-center text-white text-xs font-bold flex-shrink-0 mt-0.5">4</div><p className={`text-sm ${isDarkMode ? 'text-gray-300' : 'text-gray-600'}`}>Off path/Timeout: <span className="font-semibold text-blue-500">-15 points</span></p></div><div className="flex items-start gap-2"><div className="w-5 h-5 rounded-full bg-purple-500 flex items-center justify-center text-white text-xs font-bold flex-shrink-0 mt-0.5">5</div><p className={`text-sm ${isDarkMode ? 'text-gray-300' : 'text-gray-600'}`}><span className="font-semibold text-purple-500">30 seconds</span> per lap time limit</p></div><div className="flex items-start gap-2"><div className="w-5 h-5 rounded-full bg-yellow-500 flex items-center justify-center text-white text-xs font-bold flex-shrink-0 mt-0.5">6</div><p className={`text-sm ${isDarkMode ? 'text-gray-300' : 'text-gray-600'}`}>5x lap streak <span className="font-semibold text-yellow-500">bonus notification</span></p></div></div></div><div className={`mt-4 pt-3 border-t flex flex-col sm:flex-row items-start sm:items-center justify-between gap-2 text-xs ${isDarkMode ? 'border-gray-700 text-gray-400' : 'border-gray-200 text-gray-500'}`}><span>🎯 Stay within corridor • Timer bar at top • Best lap tracked</span><span>💰 15 points per lap • -15 penalty • Best Score saves locally</span></div></div></div></footer>)}
      </div>
    </div>
  );
}
...
function ResultCard({ label, value, unit = '', icon, color, isDark }) { const colorMap = { yellow: { bg: 'bg-yellow-500/10', border: 'border-yellow-500/30', text: 'text-yellow-500' }, purple: { bg: 'bg-purple-500/10', border: 'border-purple-500/30', text: 'text-purple-500' }, green: { bg: 'bg-green-500/10', border: 'border-green-500/30', text: 'text-green-500' }, orange: { bg: 'bg-orange-500/10', border: 'border-orange-500/30', text: 'text-orange-500' }, red: { bg: 'bg-red-500/10', border: 'border-red-500/30', text: 'text-red-500' }, blue: { bg: 'bg-blue-500/10', border: 'border-blue-500/30', text: 'text-blue-500' }, cyan: { bg: 'bg-cyan-500/10', border: 'border-cyan-500/30', text: 'text-cyan-500' } }; const c = colorMap[color] || colorMap.yellow; return <div className={`flex items-center justify-between p-3 rounded-lg border ${c.bg} ${c.border}`}><div className="flex items-center gap-2 min-w-0"><div className={c.text} aria-hidden="true">{icon}</div><span className={`text-xs sm:text-sm truncate ${isDark ? 'text-gray-300' : 'text-gray-600'}`}>{label}</span></div><span className={`font-bold text-base sm:text-lg flex-shrink-0 ml-2 ${c.text}`}>{value}{unit}</span></div>; }